...
//
// Generate Lumyn config.yaml and .env from Vyrex build-arg env vars.
//
// Called at Docker image build time. Reads VYREX_* env vars and writes:
//   ~/.lumyn/config.yaml  — Lumyn configuration (immutable at runtime)
//   ~/.lumyn/.env         — Messaging token placeholders (immutable at runtime)
//
// Sets what's required for Lumyn to run inside OpenShell:
...
function main(): void {
  const model = process.env.VYREX_MODEL!;
  const baseUrl = process.env.VYREX_INFERENCE_BASE_URL!;

  const channelsB64 = process.env.VYREX_MESSAGING_CHANNELS_B64 || "W10=";
  const allowedIdsB64 = process.env.VYREX_MESSAGING_ALLOWED_IDS_B64 || "e30=";
  ...
  const configPath = join(homedir(), ".lumyn", "config.yaml");
  ...
  const envPath = join(homedir(), ".lumyn", ".env");
