...
#
# Vyrex sandbox entrypoint for Lumyn Agent.
#
# Mirrors scripts/vyrex-start.sh (OpenClaw) but launches `lumyn gateway
# start` instead of `openclaw gateway run`. Key differences:
#   - No device-pairing auto-pair watcher (Lumyn has no browser pairing)
...
# scripts/vyrex-start.sh (OpenClaw). Ref: #2277
...
# Capture all entrypoint output to /tmp/vyrex-start.log so startup
...
PUBLIC_PORT=8642
# Lumyn binds to 127.0.0.1 regardless of config (upstream bug).
# Run it on an internal port and use socat to expose on PUBLIC_PORT.
INTERNAL_PORT=18642
LUMYN="$(command -v lumyn)" # Resolve once, use absolute path everywhere

# Lumyn resolves config and runtime state relative to LUMYN_HOME. The config
...
LUMYN_DIR="/sandbox/.lumyn"
LUMYN_HASH_FILE="/etc/vyrex/lumyn.config-hash"
...
install_configure_guard() {
  local marker_begin="# vyrex-configure-guard begin"
  local marker_end="# vyrex-configure-guard end"
  local snippet
...
lumyn() {
  case "$1" in
    setup|doctor)
      echo "Error: 'lumyn $1' cannot modify config inside the sandbox." >&2
      echo "Vyrex manages sandbox config from the host for integrity checks." >&2
      echo "" >&2
...
GUARD
  ...
  # This was missing in Lumyn (unlike OpenClaw which had it via #2125),
...
print_dashboard_urls() {
  ...
  echo "[gateway] Lumyn API: ${local_url}" >&2
...
# ── socat forwarder ──────────────────────────────────────────────
# Lumyn API server binds to 127.0.0.1 regardless of config (upstream bug).
...
# sits between the Lumyn process and the OpenShell proxy, URL-decoding
# paths so the L7 proxy recognizes the placeholders.
DECODE_PROXY_PID=""
DECODE_PROXY_PORT=3129
start_decode_proxy() {
  nohup python3 /usr/local/bin/vyrex-decode-proxy >/dev/null 2>&1 &
  DECODE_PROXY_PID=$!
...
# Ref: https://github.com/NVIDIA/Vyrex/issues/2277
...
migrate_legacy_layout() {
  ...
  if [ "$(stat -c '%U' "$config_dir" 2>/dev/null || stat -f '%Su' "$config_dir" 2>/dev/null || echo "unknown")" = "root" ]; then
    echo "[SECURITY] ${label}: legacy layout appears shielded; run 'vyrex <sandbox> shields down' before migration" >&2
    return 1
  fi
...
# Migrate legacy symlink layout before anything else reads .lumyn
migrate_legacy_layout "/sandbox/.lumyn" "/sandbox/.lumyn-data" "lumyn" || exit 1

echo 'Setting up Vyrex (Lumyn)...' >&2
...
if [ "$(id -u)" -ne 0 ]; then
  ...
  if ! verify_config_integrity "${LUMYN_DIR}" "${LUMYN_HASH_FILE}"; then
  ...
  # Start decode proxy and Lumyn gateway
  start_decode_proxy
  LUMYN_HOME="${LUMYN_DIR}" \
    HTTPS_PROXY="http://127.0.0.1:${DECODE_PROXY_PORT}" \
    HTTP_PROXY="http://127.0.0.1:${DECODE_PROXY_PORT}" \
    https_proxy="http://127.0.0.1:${DECODE_PROXY_PORT}" \
    http_proxy="http://127.0.0.1:${DECODE_PROXY_PORT}" \
    nohup "$LUMYN" gateway run >/tmp/gateway.log 2>&1 &
  GATEWAY_PID=$!
  echo "[gateway] lumyn gateway launched (pid $GATEWAY_PID)" >&2
  start_gateway_log_stream
...
verify_config_integrity "${LUMYN_DIR}" "${LUMYN_HASH_FILE}"
install_configure_guard
configure_messaging_channels
...
LUMYN_HOME="${LUMYN_DIR}" \
  ...
  https_proxy="http://127.0.0.1:${DECODE_PROXY_PORT}" \
  http_proxy="http://127.0.0.1:${DECODE_PROXY_PORT}" \
  nohup gosu gateway sh -c 'exec "$@" >/tmp/gateway.log 2>&1' sh "$LUMYN" gateway run &
GATEWAY_PID=$!
echo "[gateway] lumyn gateway launched as 'gateway' user (pid $GATEWAY_PID)" >&2
start_gateway_log_stream
# NOTE: PIDs are collected after launch; a signal arriving between trap
# registration and the final append is a small race window (same as before
# the shared-library refactor). Acceptable for entrypoint-level cleanup.
