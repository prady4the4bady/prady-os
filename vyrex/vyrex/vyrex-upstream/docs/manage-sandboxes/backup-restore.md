...
keywords: ["vyrex backup", "vyrex restore", "workspace backup", "openshell sandbox download upload"]
...
They persist across sandbox restarts but are **permanently deleted** when you run `vyrex <name> destroy`.

This guide covers snapshot commands, manual backup with CLI commands, and an automated script.

## When to Back Up

- **Before running `vyrex <name> destroy`**
- Before major Vyrex version upgrades
...
Snapshots capture all workspace state directories defined in the agent manifest and store them in `~/.vyrex/rebuild-backups/<name>/`.
...
$ vyrex my-assistant snapshot restore 2026-04-14T
```

The `vyrex <name> rebuild` command uses the same snapshot mechanism automatically.
...
$ BACKUP_DIR=~/.vyrex/backups/$(date +%Y%m%d-%H%M%S)
$ mkdir -p "$BACKUP_DIR"
...
```console
$ SANDBOX=my-assistant
$ BACKUP_DIR=~/.vyrex/backups/20260320-120000  # pick a timestamp
...
Backup saved to /home/user/.vyrex/backups/20260320-120000/ (6 items)
```
...
```console
$ ls -la ~/.vyrex/backups/20260320-120000/
AGENTS.md
IDENTITY.md
MEMORY.md
SOUL.md
...
`vyrex <name> snapshot create` automatically discovers every `workspace-*/`
...
[#1260](https://github.com/NVIDIA/Vyrex/issues/1260).
