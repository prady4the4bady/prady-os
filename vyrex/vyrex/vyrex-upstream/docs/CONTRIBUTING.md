# Contributing to Vyrex Documentation

This guide covers how to write, edit, and review documentation for Vyrex. If you change code that affects user-facing behavior, update the relevant docs in the same PR.
...
If you use an AI coding agent (Cursor, Claude Code, Codex, etc.), the repo includes the `vyrex-contributor-update-docs` skill that automates doc work.
...
The skill lives in `.agents/skills/vyrex-contributor-update-docs/` and follows the style guide below automatically.
...
These generated skills let AI agents walk users through Vyrex tasks (installation, inference configuration, policy management, monitoring, and more) without reading raw doc pages.

Always edit pages in `docs/`.
Never edit generated skill files under `.agents/skills/vyrex-user-*/`. Your changes will be overwritten on the next run.
...
| Skill | Source docs |
|---|---|
| `vyrex-user-overview` | `docs/about/overview.md`, `docs/about/ecosystem.md`, `docs/about/how-it-works.md`, `docs/about/release-notes.md` |
| `vyrex-user-agent-skills` | `docs/resources/agent-skills.md` |
| `vyrex-user-deploy-remote` | `docs/deployment/deploy-to-remote-gpu.md`, `docs/deployment/install-openclaw-plugins.md`, `docs/deployment/sandbox-hardening.md` |
| `vyrex-user-get-started` | `docs/get-started/prerequisites.md`, `docs/get-started/quickstart.md`, `docs/get-started/quickstart-lumyn.md`, `docs/get-started/windows-preparation.md` |
| `vyrex-user-configure-inference` | `docs/inference/inference-options.md`, `docs/inference/use-local-inference.md`, `docs/inference/switch-inference-providers.md`, `docs/inference/set-up-sub-agent.md` |
| `vyrex-user-manage-sandboxes` | `docs/manage-sandboxes/lifecycle.md`, `docs/manage-sandboxes/messaging-channels.md`, `docs/manage-sandboxes/workspace-files.md`, `docs/manage-sandboxes/backup-restore.md` |
| `vyrex-user-monitor-sandbox` | `docs/monitoring/monitor-sandbox-activity.md` |
| `vyrex-user-manage-policy` | `docs/network-policy/customize-network-policy.md`, `docs/network-policy/approve-network-requests.md` |
| `vyrex-user-reference` | `docs/reference/architecture.md`, `docs/reference/commands.md`, `docs/reference/cli-selection-guide.md`, `docs/reference/network-policies.md`, `docs/reference/troubleshooting.md` |
| `vyrex-user-configure-security` | `docs/security/best-practices.md`, `docs/security/credential-storage.md`, `docs/security/openclaw-controls.md` |

### Regenerating skills after doc changes

Pull requests that change docs should normally include only the source pages under `docs/`, not the generated `.agents/skills/vyrex-user-*` output.
...
After a docs change merges to `main`, the `Docs to Skills` workflow regenerates `.agents/skills/vyrex-user-*` from `docs/` and publishes the generated update.
...
```bash
python scripts/docs-to-skills.py docs/ .agents/skills/ --prefix vyrex-user
```

Always use this exact output path (`.agents/skills/`) and prefix (`vyrex-user`) so skill names and locations stay consistent.

Preview what would change before writing files:

```bash
python scripts/docs-to-skills.py docs/ .agents/skills/ --prefix vyrex-user --dry-run
```
...
---
title:
  page: "Vyrex Page Title: Subtitle with Context"
  nav: "Short Nav Title"
description:
...
Lower numbers win.
For example, set the OpenClaw quickstart to `10` and the Lumyn quickstart to `20` so `vyrex-user-get-started/SKILL.md` leads with the OpenClaw procedure and folds Lumyn into `references/`.
