---
title:
  page: "Vyrex Architecture: Plugin, Blueprint, and Sandbox Structure"
  nav: "Architecture"
description:
  main: "Learn how Vyrex combines a lightweight CLI plugin with a versioned blueprint to move OpenClaw into a controlled sandbox."
  agent: "Describes the Vyrex plugin and blueprint architecture and how they orchestrate the OpenClaw sandbox. Use when looking up architecture, plugin structure, or blueprint design."
keywords: ["vyrex architecture", "vyrex plugin blueprint structure"]
...
Vyrex has two main components: a TypeScript plugin that integrates with the OpenClaw CLI, and a Python blueprint that orchestrates OpenShell resources.

## System Overview

NVIDIA OpenShell is a general-purpose agent runtime. It provides sandbox containers, a credential-storing gateway, inference proxying, and policy enforcement, but has no opinions about what runs inside. Vyrex is an opinionated reference stack built on OpenShell that handles what goes in the sandbox and makes the setup accessible.

```{mermaid}
graph LR
    classDef vyrex fill:#76b900,stroke:#5a8f00,color:#fff,stroke-width:2px,font-weight:bold
    ...
    subgraph HOST["Host Machine"]
        ...
        subgraph VYREX["Vyrex"]
            direction TB
            NCLI["CLI + Onboarding<br/><small>Guided setup · provider selection<br/>credential validation · deploy</small>"]:::vyrex
            BP["Blueprint<br/><small>Hardened Dockerfile<br/>Network policies · Presets<br/>Security configuration</small>"]:::vyrex
            MIGRATE["State Management<br/><small>Migration snapshots<br/>Credential stripping<br/>Integrity verification</small>"]:::vyrex
        end
        ...
        subgraph OPENSHELL["OpenShell"]
            ...
            subgraph SANDBOX["Sandbox Container 🔒"]
                ...
                PLUG["Vyrex Plugin<br/><small>Extends agent with<br/>managed configuration</small>"]:::sandbox
            end
        end
    end

    USER -->|"vyrex onboard<br/>vyrex connect"| NCLI
...
graph TB
    ...
    subgraph HOST["Host machine · Linux / macOS / WSL2 / DGX Spark"]
        direction TB
        CLI["vyrex CLI<br/><small>bin/vyrex.js → dist/<br/>onboard · connect · status · logs</small>"]:::cli
            ...
            subgraph GWCON["OpenShell gateway container"]
                ...
                subgraph K3S["Embedded k3s cluster"]
                    ...
                    subgraph POD["Sandbox pod 🔒<br/><small>Landlock + seccomp + netns</small>"]
                        direction TB
                        AGENT["OpenClaw agent<br/>+ Vyrex plugin"]:::pod
                    end
                end
            end
        end
...
| Layer | Runs as | Role |
|---|---|---|
| Host CLI | Host process (`vyrex` on Node.js) | Orchestrates OpenShell via `openshell` CLI calls. |
...
| Sandbox pod | Pod in the embedded k3s cluster | Runs the OpenClaw agent and the Vyrex plugin under Landlock + seccomp + netns. |
...
Vyrex never gives the sandbox a raw provider key.
...
For the DGX Spark-specific variant of this topology (cgroup v2, aarch64, unified memory), refer to the [NVIDIA Spark playbook](https://build.nvidia.com/spark/vyrex).

## Vyrex Plugin

The plugin is a thin TypeScript package that registers an inference provider and the `/vyrex` slash command.
...
│   │   └── slash.ts                /vyrex chat command handler
│   └── blueprint/
...
- OpenClaw runs with the Vyrex plugin pre-installed.
...
- The Vyrex plugin injects sandbox and policy context into agent turns so the agent can report policy blocks accurately.
...
Vyrex never writes them to host disk; the OpenShell L7 proxy injects values at egress.
...
Vyrex keeps non-secret operator-facing state on the host rather than inside the sandbox.

| Path | Purpose |
|---|---|
| `~/.vyrex/sandboxes.json` | Registered sandbox metadata, including the default sandbox selection. |
| `~/.openclaw/openclaw.json` | Host OpenClaw configuration that Vyrex snapshots or restores during migration flows. |

The following environment variables configure optional services and local access.

| Variable | Purpose |
|---|---|
| `TELEGRAM_BOT_TOKEN` | Telegram bot token you provide before `vyrex onboard`. OpenShell stores it in a provider; the sandbox receives placeholders, not the raw secret. |
...
| `VYREX_DISABLE_DEVICE_AUTH` | Build-time-only toggle that disables gateway device pairing when set to `1` before the sandbox image is created. |

For normal setup and reconfiguration, prefer `vyrex onboard` over editing these files by hand.
Do not treat `VYREX_DISABLE_DEVICE_AUTH` as a runtime setting for an already-created sandbox.
