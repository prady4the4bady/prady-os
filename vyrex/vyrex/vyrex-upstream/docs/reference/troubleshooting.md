---
title:
  page: "Vyrex Troubleshooting Guide"
  nav: "Troubleshooting"
description:
  main: "Diagnose and resolve common Vyrex installation, onboarding, and runtime issues."
  agent: "Lists fixes for common installation, onboarding, and runtime issues. Use when diagnosing a reported Vyrex error, a failed onboard, or unexpected sandbox behavior."
keywords: ["vyrex troubleshooting", "vyrex debug sandbox issues"]
topics: ["generative_ai", "ai_agents"]
tags: ["openclaw", "openshell", "troubleshooting", "vyrex"]
content:
  type: reference
...
This page covers common issues you may encounter when installing, onboarding, or running Vyrex, along with their resolution steps.

:::{admonition} Get Help
:class: tip

If your issue is not listed here, join the [Vyrex Discord channel](https://discord.gg/XFpfPv9Uvx) to ask questions and get help from the community. You can also [file an issue on GitHub](https://github.com/NVIDIA/Vyrex/issues/new).
:::

## Installation

### `vyrex` not found after install

If you use nvm or fnm to manage Node.js, the installer may not update your current shell's PATH.
The `vyrex` binary is installed but the shell session does not know where to find it.

Run `source ~/.bashrc` (or `source ~/.zshrc` for zsh), or open a new terminal window.

When installing from a source checkout with `npm install`, Vyrex first tries `npm link`.
If the global npm prefix is not writable, it writes a managed shim to `~/.local/bin/vyrex` instead.
...
Vyrex requires Node.js 22.16 or later.
...
To avoid these issues, install the prerequisites in the following order before running the Vyrex installer:
...
The Vyrex installer does not require `sudo` or root.
It installs Node.js via nvm and Vyrex via npm, both into user-local directories.
The installer also handles OpenShell installation automatically using a pinned release.

If you see permission errors during installation, they typically come from Docker, not the Vyrex installer itself.
...
Vyrex's preflight runs a short `docker run --rm busybox nslookup registry.npmjs.org` probe before starting the sandbox build. When the probe confirms a DNS failure, onboarding stops with platform-specific remediation instead of hanging for ~15 minutes and printing a cryptic `Exit handler never called`.

The fix depends on your platform and runtime. Pick the matching path from the preflight output, apply it, then re-run `vyrex onboard`.
...
The Vyrex dashboard uses port `18789` by default and the gateway uses port `8080`.
...
If a non-Vyrex process is already bound to the dashboard port or the gateway port, identify the conflicting process, verify it is safe to stop, and terminate it:
...
```console
$ vyrex onboard                                                   # first sandbox uses default 18789
$ vyrex onboard                                                   # second sandbox uses the next free port
$ vyrex onboard --control-ui-port 19000                          # explicit port override
```
...
`vyrex list` prints the recorded dashboard URL for each sandbox.

## Onboarding

### Cgroup v2 errors during onboard

Older Vyrex releases relied on a Docker cgroup workaround on Ubuntu 24.04, DGX Spark, and WSL2.
Current OpenShell releases handle that behavior themselves, so Vyrex no longer requires a Spark-specific setup step.
...
This is a Docker default-driver change, not a Vyrex or OpenShell regression.
...
Vyrex detects the Docker 26+ containerd-snapshotter overlayfs configuration during onboarding and transparently builds a small drop-in replacement for the cluster image on the local Docker engine.
...
- `VYREX_DISABLE_OVERLAY_FIX=1` — skip the auto-fix and run against the unmodified upstream cluster image.
  Useful for diagnosis or when you have already applied the manual workaround below.
- `VYREX_OVERLAY_SNAPSHOTTER=native` — build the patched image with k3s's `native` snapshotter instead of `fuse-overlayfs`.
...
Then restart Docker (`sudo systemctl restart docker`) and re-run `vyrex onboard`.
...
Each Vyrex release validates against a range of tested OpenShell versions.
If the installed OpenShell version exceeds the configured maximum, `vyrex onboard` exits with an error:

```text
✗ openshell <version> is above the maximum supported by this Vyrex release.
  blueprint.yaml max_openshell_version: <max>
```

Upgrade Vyrex to a version that supports your OpenShell release, or install a supported OpenShell version from the [OpenShell releases page](https://github.com/NVIDIA/OpenShell/releases).
...
Current Vyrex detects this case and kills the orphaned SSH process
...
Then re-run `vyrex onboard`.

### Updated messaging token is not picked up

Re-running `vyrex onboard --non-interactive` with a new
...
Current Vyrex stores SHA-256 hashes of messaging credentials in the
sandbox registry at creation time and detects when a token has changed.
When rotation is detected, Vyrex automatically backs up workspace state,
...
Vyrex automatically detects low memory during onboarding and prompts to create a 4 GB swap file.
If this automatic step fails or you are using a custom setup flow, create swap manually before running `vyrex onboard`:
...
If a previous `vyrex onboard` attempt fails partway through (for example, a provider or inference-setup step reporting an error), Vyrex records the failure in `~/.vyrex/onboard-session.json`.
...
- In non-interactive mode (piped `curl | bash` with `VYREX_NON_INTERACTIVE=1`, CI, scripts), the installer refuses and exits with a non-zero status so a scripted re-run cannot loop.
  ...
  ```console
  $ curl -fsSL https://www.nvidia.com/vyrex.sh | bash -s -- --fresh
  ```
  ...
  ```console
  $ curl -fsSL https://www.nvidia.com/vyrex.sh | VYREX_FRESH=1 bash
  ```
...
1. Restart the gateway (if needed).
   ...
   ```console
   $ openshell gateway start --name vyrex
   ```
...
1. Reconnect.
   ...
   You do not need to edit `known_hosts` by hand or re-run `vyrex onboard` in this case.

1. Start host auxiliary services (if needed).

   If you use the cloudflared tunnel started by `vyrex tunnel start`, start it again:

   ```console
   $ vyrex tunnel start
   ```

   Telegram, Discord, and Slack are handled by OpenShell-managed channel messaging configured at onboarding, not by a separate bridge process from `vyrex tunnel start`. To pause a single bridge without destroying the sandbox, use `vyrex <name> channels stop <channel>`.

:::{admonition} If the sandbox does not recover
:class: warning

If the sandbox remains missing after restarting the gateway, run `vyrex onboard` to recreate it.
...
After upgrading Vyrex, `vyrex <name> connect` and `vyrex <name> status` warn if the sandbox is running an older agent version than the current image.
...
Run `vyrex onboard` to recreate the sandbox from the same blueprint and policy definitions.
...
Current Vyrex exports that bundle as `GIT_SSL_CAINFO` during sandbox startup and persists it for `vyrex <name> connect` sessions, so Git can trust the proxy CA.
...
If they are missing on an older sandbox, upgrade Vyrex and run:
...
```console
$ openshell gateway trust -g vyrex
$ vyrex onboard --resume
```

### `openclaw update` hangs or times out inside the sandbox

This is expected for the current Vyrex deployment model.
Vyrex installs `openclaw` into the sandbox image at build time, so the CLI is image-pinned rather than updated in place inside a running sandbox.

Do not run `openclaw update` inside the sandbox.
Instead:

1. Upgrade to a Vyrex release that includes the newer `openclaw` version.
2. If you build Vyrex from source, bump the pinned `openclaw` version in `Dockerfile.base` and rebuild the sandbox base image.
3. Run `vyrex <name> rebuild` to recreate the sandbox with the updated image. The rebuild command automatically backs up workspace state before destroying the old sandbox and restores it afterward.
...
For local Ollama and local vLLM, `vyrex <name> status` also prints an `Inference` line that probes the host-side health endpoint directly.
...
If large prompts still cause timeouts, increase it with `VYREX_LOCAL_INFERENCE_TIMEOUT` before re-running onboard:
...
For the compatible-endpoint provider, Vyrex now defaults to
...
If you previously set `VYREX_PREFERRED_API=openai-responses` to force the
...
Do not rely on `VYREX_INFERENCE_API_OVERRIDE` alone — it patches the config
at container startup but does not update the Dockerfile ARG baked into the
image.
A fresh `vyrex onboard` is the reliable fix.

### `VYREX_DISABLE_DEVICE_AUTH=1` does not change an existing sandbox

This is expected behavior.
`VYREX_DISABLE_DEVICE_AUTH` is a build-time setting used when Vyrex creates the sandbox image.
Changing or exporting it later does not rewrite the baked `openclaw.json` inside an existing sandbox.

If you need a different device-auth setting, rerun onboarding so Vyrex rebuilds the sandbox image with the desired configuration.
...
This is expected.
The messaging channel list is frozen into the sandbox's container image when the image is built during `vyrex onboard` or `vyrex rebuild` (the selected channel names are passed to the `docker build` as `VYREX_MESSAGING_CHANNELS_B64` and written into `/sandbox/.openclaw/openclaw.json` as part of the image).
...
Vyrex's sandbox entrypoint installs a guard that intercepts `openclaw channels <add|remove>` and prints an actionable error pointing at the host-side commands below, instead of letting the call fail deep in the binary with a raw `EACCES` trace.

Run the equivalent host-side command instead:

```console
$ vyrex <sandbox> channels list
$ vyrex <sandbox> channels add <telegram|discord|slack>
$ vyrex <sandbox> channels remove <telegram|discord|slack>
```
...
In non-interactive mode (`VYREX_NON_INTERACTIVE=1`), the commands stage the change and leave the rebuild to a follow-up `vyrex <sandbox> rebuild`.
...
Vyrex's sandbox entrypoint installs a guard that intercepts `openclaw config set` and `openclaw config unset` and prints an actionable error, because changes made inside the running sandbox do not persist across rebuilds.
...
If Vyrex reports a resumable failed onboarding session, run `vyrex onboard --resume` instead.
...
This is expected in Vyrex-managed sandboxes.
Vyrex bakes channel entries into `/sandbox/.openclaw/openclaw.json` at image build time.
...
If your Discord channel config is wrong, rerun onboarding so Vyrex rebuilds the sandbox image with the correct messaging selection.
...
Current Vyrex rebuilds bake Telegram in the account-based layout and set Telegram group chats to `groupPolicy: open`, which avoids the empty `groupAllowFrom` warning path for default group-chat access.
...
Bot tokens for Telegram (`getUpdates`), Discord (gateway), and Slack (Socket Mode) only allow one active consumer per token. If two Vyrex sandboxes are configured with the same bot token, each one kicks the other off its polling connection and neither delivers messages. `vyrex status` still reports the bridge as running because the gateway process itself is alive.

To diagnose, open a shell in the sandbox and inspect the gateway log:

```console
$ vyrex <sandbox-name> connect
...
To fix, run `vyrex <other-sandbox> destroy` on whichever sandbox should stop polling, or rerun onboarding on it with the channel disabled. Current Vyrex warns at `vyrex onboard` time when another sandbox already has the same channel enabled, but sandboxes created before that check was added may still be in a conflict loop.

### Landlock filesystem restrictions silently degraded

After sandbox creation, Vyrex checks whether the host kernel supports Landlock (Linux 5.13+).
...
Running `vyrex onboard` automatically upgrades OpenShell to 0.0.24 or later during the preflight check.
After the upgrade, recreate the sandbox with `vyrex onboard`.

### Agent cannot reach external hosts through a proxy

Vyrex uses a default proxy address of `10.200.0.1:3128` (the OpenShell-injected gateway).
If your environment uses a different proxy, set `VYREX_PROXY_HOST` and `VYREX_PROXY_PORT` before onboarding:
...
Changing them after onboarding requires re-running `vyrex onboard` to rebuild the image.

When `HTTP_PROXY` or `HTTPS_PROXY` is set on the host, Vyrex adds `localhost` and `127.0.0.1` to `NO_PROXY` for managed subprocesses.
...
Expose the service on a host IP address that the OpenShell gateway can reach, create a custom Vyrex policy preset for that IP and port, and apply it with `vyrex <sandbox> policy-add --from-file`.
The sandbox request then flows through the OpenShell proxy while Vyrex preserves the existing live policy entries.
...
Then create a custom Vyrex preset for the host-side service.
...
Apply the preset to the running sandbox with the Vyrex CLI:

```console
$ vyrex my-assistant policy-add --from-file ./host-memory-api.yaml
```
...
If you ran `vyrex onboard` with a custom dashboard port and onboarding completed
but the dashboard URL is unreachable (browser shows connection refused or the page fails
to load), the sandbox was most likely created with an older Vyrex version that did not
...
Re-run onboarding on the current Vyrex release with the desired port. Current versions
...
For an end-to-end Ollama walkthrough on DGX Spark, refer to the [NVIDIA Spark playbook](https://build.nvidia.com/spark/vyrex).
...
```console
$ python3 -m venv ~/.venvs/vyrex
$ source ~/.venvs/vyrex/bin/activate
$ pip install ...
```
