---
title:
  page: "Vyrex Network Policies: Baseline Rules and Operator Approval"
  nav: "Network Policies"
description:
  main: "Baseline network policy, filesystem rules, and operator approval flow."
  agent: "Covers the baseline network policy, filesystem rules, and operator approval flow. Use when looking up a specific default endpoint, filesystem path, or the runtime approval sequence Vyrex applies on blocked requests."
keywords: ["vyrex network policy", "sandbox egress control operator approval"]
...
Vyrex runs with a deny-by-default network policy.
...
The baseline policy is defined in `vyrex-blueprint/policies/openclaw-sandbox.yaml`.
...
Tier definitions are stored in `vyrex-blueprint/policies/tiers.yaml`.

In non-interactive mode, set the tier with `VYREX_POLICY_TIER`:

```console
$ VYREX_POLICY_TIER=open vyrex onboard --non-interactive --yes-i-accept-third-party-software
```
...
Edit `vyrex-blueprint/policies/openclaw-sandbox.yaml` and re-run the onboard wizard:
