---
title:
  page: "Monitor Vyrex Sandbox Activity and Debug Issues"
...
keywords: ["monitor vyrex sandbox", "debug vyrex agent issues"]
topics: ["generative_ai", "ai_agents"]
tags: ["openclaw", "openshell", "monitoring", "troubleshooting", "vyrex"]
content:
  type: how_to
...
Use the Vyrex status, logs, and TUI tools together to inspect sandbox health, trace agent behavior, and diagnose problems.
...
For local Ollama and local vLLM routes, `vyrex <name> status` also probes the host-side health endpoint directly.
...
- Gateway and process health, which show whether Vyrex can still reach the OpenShell gateway and whether the in-sandbox agent process is running.
...
Run `vyrex <name> status` on the host to check sandbox state.
...
1. Run `vyrex <name> status` to confirm the active provider and endpoint.
...
2. Run `vyrex <name> logs --follow` to view error messages from the blueprint runner.
