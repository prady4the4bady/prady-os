---
title:
  page: "How Vyrex Works: Plugin, Blueprint, and Sandbox Lifecycle"
  nav: "How It Works"
description:
  ...
  agent: "Describes how Vyrex works internally: CLI, plugin, blueprint runner, OpenShell orchestration, inference routing, and protection layers. Use for sandbox lifecycle and architecture mechanics; not for product definition (Overview) or multi-project placement (Ecosystem)."
keywords: ["how vyrex works", "vyrex sandbox lifecycle blueprint"]
...
This page explains how Vyrex operates, which parts run where, how the blueprint drives OpenShell, and how inference and policy attach to the sandbox.

## How the Pieces Connect

The `vyrex` CLI is the primary entrypoint for setting up and managing sandboxed OpenClaw agents.
...
Between your shell and the running sandbox, Vyrex contributes these integration layers:

| Layer | Role in the flow |
|-------|------------------|
| Onboarding | `vyrex onboard` validates credentials, selects providers, and drives blueprint execution until the sandbox is ready. |
...
| Channel messaging | OpenShell-managed processes connect Telegram, Discord, Slack, and similar platforms to the agent. Vyrex enables this through onboarding and blueprint wiring; delivery is not a separate Vyrex host daemon. |
...
Vyrex architecture follows the following principles.
...
: The `vyrex` CLI is the primary interface for sandbox management.

Supply chain safety
: Blueprint artifacts are immutable, versioned, and digest-verified before execution.

OpenShell-backed lifecycle
: Vyrex orchestrates OpenShell resources under the hood, but `vyrex onboard`
  is the supported operator entry point for creating or recreating Vyrex-managed sandboxes.
...
Vyrex is split into two parts:

- The *plugin* is a TypeScript package that registers an inference provider and the `/vyrex` slash command inside the sandbox.
...
When you run `vyrex onboard`, Vyrex creates an OpenShell sandbox that runs OpenClaw in an isolated container.
...
During onboarding, Vyrex validates the selected provider and model, configures the OpenShell route, and bakes the matching model reference into the sandbox image.
...
- Read [Ecosystem](ecosystem.md) for stack-level relationships and Vyrex versus OpenShell-only paths.
