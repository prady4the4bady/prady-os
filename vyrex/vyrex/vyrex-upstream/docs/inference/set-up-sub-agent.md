...
description:
  main: "Where Vyrex stores OpenClaw sub-agent model configuration, credentials, and workspace files inside the sandbox."
  agent: "Shows the Vyrex-specific file paths and update flow for adding an auxiliary OpenClaw sub-agent model. Use when users ask how to add a second model, configure a sub-agent model, use Omni for vision tasks, configure agents.list, or use sessions_spawn in Vyrex."
keywords: ["vyrex additional model", "vyrex sub-agent model", "openclaw sub-agent", "agents.list", "sessions_spawn", "vlm-demo"]
topics: ["generative_ai", "ai_agents"]
tags: ["vyrex", "openclaw", "inference", "sub-agent", "models"]
content:
  type: how_to
...
This Vyrex page covers the sandbox-specific pieces: where the OpenClaw config lives, where to put per-agent credentials, which writable workspace path agents should use, and how the Omni VLM demo maps onto those paths.

## Vyrex Sandbox Paths

Vyrex runs OpenClaw inside an OpenShell sandbox.
...
The [`vlm-demo`](https://github.com/brevdev/vyrex-demos/tree/main/vlm-demo) applies the OpenClaw sub-agent pattern to a vision task.
It keeps the primary `main` agent on the normal Vyrex inference route and adds a `vision-operator` sub-agent backed by an Omni vision model.
...
$ export DOCKER_CTR=openshell-cluster-vyrex
...
In the default mutable state, this keeps the local hash consistent but does not make it tamper-proof; run `vyrex <name> shields up` afterward if the sandbox should enforce config integrity at startup.
...
If the auxiliary model uses a provider key outside the normal Vyrex inference route, put that key in the sub-agent auth profile.
...
Use the [`vlm-demo`](https://github.com/brevdev/vyrex-demos/tree/main/vlm-demo) repository for runnable Omni example assets:
