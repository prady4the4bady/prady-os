---
title:
  page: "Vyrex Security Best Practices: Controls, Risks, and Posture Profiles"
  nav: "Security Best Practices"
description:
  main: "A risk framework for every configurable security control in Vyrex: defaults, what you can change, and what happens if you do."
  agent: "Presents a risk framework for every configurable security control in Vyrex. Use when evaluating security posture, reviewing sandbox security defaults, or assessing control trade-offs."
keywords: ["vyrex security best practices", "sandbox security controls risk framework"]
topics: ["generative_ai", "ai_agents"]
tags: ["openclaw", "openshell", "sandboxing", "security", "network_policy", "vyrex"]
content:
  type: concept
...
Vyrex ships with deny-by-default security controls across four layers: network, filesystem, process, and inference.
...
:::{seealso}
OpenShell enforces the platform-level mechanisms that Vyrex configures, including network namespace isolation, seccomp filters, SSRF protection, TLS termination, and gateway authentication.
...
Vyrex enforces security at four layers.
Vyrex locks some when it creates the sandbox and requires a restart to change them.
You can hot-reload others while the sandbox runs.

The following diagram shows the default posture immediately after `vyrex onboard`, before you approve any endpoints or apply any presets.

```{mermaid}
flowchart TB
    subgraph HOST["Your Machine: default posture after vyrex onboard"]
        direction TB

        YOU["👤 Operator"]

        subgraph NC["Vyrex + OpenShell"]
            direction TB
...
Vyrex controls which hosts, ports, and HTTP methods the sandbox can reach, and lets operators approve or deny requests in real time.
...
The sandbox blocks all outbound connections unless you explicitly list the endpoint in the policy file `vyrex-blueprint/policies/openclaw-sandbox.yaml`.
...
| What you can change | The system merges approved endpoints into the sandbox's policy as a new durable revision. They persist across sandbox restarts within the same sandbox instance. However, when you destroy and recreate the sandbox (for example, by running `vyrex onboard`), the policy resets to the baseline defined in the blueprint. |
...
Vyrex ships preset policy files in `vyrex-blueprint/policies/presets/` for common integrations.
...
Vyrex restricts which paths the agent can read and write, protecting system binaries, configuration files, and gateway credentials.
...
The current entrypoint reads the gateway auth token from OpenClaw config when present, exports it as `OPENCLAW_GATEWAY_TOKEN`, and writes it to `/tmp/vyrex-proxy-env.sh` so interactive sandbox sessions can reach the gateway through the static `/sandbox/.bashrc` and `/sandbox/.profile` source shims.
...
Operators can opt into immutability by running `vyrex <name> shields up`, which locks the config and writable state entry points until `shields down` restores the default writable state.
...
- **Config integrity hash.** The image includes a SHA256 hash of `openclaw.json`. In the default mutable state, `.config-hash` is sandbox-owned and is not a tamper-proof trust anchor, so startup does not fail closed on that hash. After `vyrex <name> shields up` locks the hash root-owned and read-only, startup enforces it and refuses to start if the hash does not match.
- **Gateway token environment.** The gateway exports `OPENCLAW_GATEWAY_TOKEN` and writes it to `/tmp/vyrex-proxy-env.sh` for interactive sandbox sessions. Keep this in mind when deciding whether a workload should run with mutable config or with Shields UP.
- **Shields UP (opt-in).** `vyrex <name> shields up` applies root-owned read-only permissions and best-effort immutable bits to sensitive config files, and locks writable state directories such as workspace, memory, skills, hooks, cron, agents, and extensions.

| Aspect | Detail |
|---|---|
| Default | The sandbox keeps `/sandbox/.openclaw` writable (`700 sandbox:sandbox`), sets `openclaw.json` to `600 sandbox:sandbox`, lets the agent manage state directly, and has the gateway place `OPENCLAW_GATEWAY_TOKEN` in `/tmp/vyrex-proxy-env.sh` for interactive shells. |
| What you can change | Run `vyrex <name> shields up` to lock config and state directories with DAC permissions and the immutable flag where available. Run `shields down` to return to the writable default. |
...
| What you can change | This is a Vyrex default, not a user-facing knob. |
...
Vyrex limits the capabilities, user privileges, and resource quotas available to processes inside the sandbox.
...
| What you can change | When launching with `docker run` directly, pass `--cap-drop=ALL --cap-add=NET_BIND_SERVICE` for stricter enforcement. In the standard Vyrex flow (with `vyrex onboard`), the entrypoint handles capability dropping automatically. |
...
| Recommendation | Run on an image that includes `capsh` (the Vyrex image includes it through `libcap2-bin`). For defense-in-depth, also pass `--cap-drop=ALL` at the container runtime level. |
...
| Aspect | Detail |
|---|---|
| Default | OpenShell sets `PR_SET_NO_NEW_PRIVS` using `prctl()` inside the sandbox process as part of the seccomp filter setup. The Vyrex Compose example also shows the equivalent `security_opt: no-new-privileges:true` setting. |
...
| Recommendation | No action needed. OpenShell enforces this automatically when the sandbox network policy is active. This flag prevents `gosu` from switching users, so non-root mode disables gateway process isolation in the Vyrex entrypoint. |
...
| Aspect | Detail |
|---|---|
| Default | `vyrex-blueprint/blueprint.yaml` pins the sandbox image by digest. A CI regression test blocks any mutable-tag reference from merging. |
...
| Recommendation | Always reference the sandbox image by digest. If you build a custom image with `vyrex onboard --from`, the digest constraint does not apply to your local build. |
...
Vyrex hardens these defaults at image build time.
...
| What you can change | Set `VYREX_DISABLE_DEVICE_AUTH=1` as a Docker build argument to disable device authentication. This is a build-time setting baked into `openclaw.json` and verified by hash at startup. |
...
| Recommendation | No action needed. If you share `vyrex debug` output, verify that no secrets appear in the collected diagnostics. |

### Memory Secret Scanner

The Vyrex plugin blocks the agent from writing likely secrets (API keys, tokens, private keys) into persistent memory files.
...
| What it covers | Examples include `.openclaw/memory/`, `.openclaw/workspace/`, `.openclaw/agents/`, `.openclaw/skills/`, `.openclaw/hooks/`, `.openclaw/credentials/`, `.openclaw/openclaw.json`, `.vyrex/`, and `MEMORY.md`; the exact coverage is defined by `MEMORY_PATH_SEGMENTS` and enforced through `isMemoryPath()`. |
...
The `VYREX_EXPERIMENTAL=1` environment variable gates local NVIDIA NIM and local vLLM.

| Aspect | Detail |
|---|---|
| Default | Disabled. The onboarding wizard does not show these providers. |
| What you can change | Set `VYREX_EXPERIMENTAL=1` before running `vyrex onboard`. |
| Risk if relaxed | Vyrex has not fully validated these providers. NIM requires a NIM-capable GPU. vLLM must already be running on `localhost:8000`. Misconfiguration can cause failed inference or unexpected behavior. |
| Recommendation | Use experimental providers only for evaluation. Do not rely on them for always-on assistants. |

## Posture Profiles

The following profiles describe how to configure Vyrex for different use cases.
...
| Leaving `/sandbox/.openclaw` writable on sensitive workloads | This directory contains the OpenClaw gateway configuration. A writable `.openclaw` lets the agent disable CORS, redirect inference routing, or weaken gateway protections. | Run `vyrex <name> shields up` to lock config for always-on assistants handling sensitive data. |
...
| Disabling device auth for remote deployments | Without device auth, any device on the network can connect to the gateway without pairing. Combined with a cloudflared tunnel, this makes the dashboard publicly accessible and unauthenticated. | Keep `VYREX_DISABLE_DEVICE_AUTH` at its default (`0`). Only set it to `1` for local headless or development environments. |
