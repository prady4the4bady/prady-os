---
title:
  page: "Customize the Vyrex Sandbox Network Policy"
...
keywords: ["customize vyrex network policy", "sandbox egress policy configuration"]
topics: ["generative_ai", "ai_agents"]
tags: ["openclaw", "openshell", "network_policy", "security", "vyrex"]
content:
  type: how_to
...
The sandbox policy is defined in a declarative YAML file in the Vyrex repository and enforced at runtime by [NVIDIA OpenShell](https://github.com/NVIDIA/OpenShell).
Vyrex supports both static policy changes that persist across restarts and dynamic updates applied to a running sandbox through the OpenShell CLI.
...
Apply a custom Vyrex preset with `vyrex <sandbox> policy-add --from-file`.
...
- A running Vyrex sandbox for dynamic changes, or the Vyrex source repository for static changes.
...
Open `vyrex-blueprint/policies/openclaw-sandbox.yaml` and add or modify endpoint entries.

If you only need one of the built-in presets, use `vyrex <name> policy-add` instead of editing YAML by hand:

```console
$ vyrex my-assistant policy-add
```

To remove a previously applied preset, use `vyrex <name> policy-remove`:
...
This is the non-destructive path and the only flow Vyrex supports out of the box for merging new entries into a running policy.

1. Create a preset-format YAML file under `vyrex-blueprint/policies/presets/`, for example `vyrex-blueprint/policies/presets/influxdb.yaml`:
...
2. Apply it to the running sandbox:
   ...
   Vyrex reads the live policy via `openshell policy get --full`, structurally merges your preset's `network_policies` into it, and writes the merged result back.
...
Use this path only when you cannot add a file under the Vyrex source tree.
...
To make a custom policy survive a sandbox recreation, ship the preset file in the repository (Option 1 above — the file under `presets/` persists) or edit `openclaw-sandbox.yaml` and re-run `vyrex onboard`.
...
Vyrex ships preset policy files for common integrations in `vyrex-blueprint/policies/presets/`.
...
To include a preset in the baseline, merge its entries into `openclaw-sandbox.yaml` and re-run `vyrex onboard`.

:::{note}
The `openshell policy set --policy <file> <sandbox-name>` command operates on raw policy files and does not
accept the `preset:` metadata block used in preset YAML files. Use `vyrex <name> policy-add` for
presets.
:::
...
Set `VYREX_NON_INTERACTIVE=1` instead of `--yes` to drive the same flow from an environment variable.
See [Commands](../reference/commands.md#vyrex-name-policy-add) for the full flag reference.

`vyrex <name> rebuild` reapplies every policy preset to the recreated sandbox, so presets survive an agent-version upgrade without manual reapplication.
...
A custom preset follows the same shape as the built-in ones under `vyrex-blueprint/policies/presets/`:
...
Rename `preset.name` if Vyrex refuses to apply the file because of a collision.
...
Preview the endpoints without applying with `--dry-run`, and skip the confirmation prompt with `--yes` or by exporting `VYREX_NON_INTERACTIVE=1`.
...
:::{warning}
Custom preset hosts bypass Vyrex's review process and can widen sandbox egress to arbitrary destinations.
Review every host in a custom preset before applying it, especially when the file originates outside your team.
:::

### Remove a Custom Preset

Custom presets applied with `--from-file` or `--from-dir` are recorded in the Vyrex sandbox registry alongside their full YAML content, so they can be removed by name — the original file does not need to be kept on disk:

```console
$ vyrex my-assistant policy-remove my-internal-api --yes
```

`policy-remove` accepts both built-in and custom preset names. Run `vyrex <name> policy-list` to see every preset currently applied to the sandbox.
