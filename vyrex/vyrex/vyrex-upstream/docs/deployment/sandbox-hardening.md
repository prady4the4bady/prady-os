...
description:
  main: "Security hardening measures applied to the Vyrex sandbox container image."
...
keywords: ["vyrex sandbox hardening", "container security", "docker capabilities", "process limits"]
topics: ["generative_ai", "ai_agents"]
tags: ["vyrex", "sandboxing", "security"]
content:
  type: reference
...
The Vyrex sandbox image applies several security measures to reduce attack
...
(`vyrex-start.sh`) applies the same limit.
...
| `/sandbox/.vyrex` | read-write | Plugin state and config; blueprints within are DAC-protected (root-owned) |
...
These files source runtime proxy configuration from `/tmp/vyrex-proxy-env.sh`.

### Landlock Kernel Requirements

Landlock LSM requires Linux kernel 5.13 or later with `CONFIG_SECURITY_LANDLOCK=y`.
The Vyrex sandbox policy uses `compatibility: best_effort`, which means Landlock enforcement is silently skipped on kernels that do not support it.
...
- [#804](https://github.com/NVIDIA/Vyrex/issues/804): Filesystem layout and Landlock policy
- [#807](https://github.com/NVIDIA/Vyrex/issues/807): gcc in sandbox image
- [#808](https://github.com/NVIDIA/Vyrex/issues/808): netcat in sandbox image
- [#809](https://github.com/NVIDIA/Vyrex/issues/809): No process limit
- [#797](https://github.com/NVIDIA/Vyrex/issues/797): Drop Linux capabilities
