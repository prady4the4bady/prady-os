...
description:
  main: "How to install OpenClaw plugins in a Vyrex-managed sandbox today."
  agent: "Explains the difference between OpenClaw plugins and agent skills, and shows the current Dockerfile-based workflow for baking a plugin into a Vyrex sandbox. Use when users ask how to install, build, or configure OpenClaw plugins under Vyrex."
keywords: ["vyrex plugins", "openclaw plugins", "install openclaw plugin", "vyrex onboard from dockerfile"]
topics: ["generative_ai", "ai_agents"]
tags: ["vyrex", "openclaw", "plugins", "sandbox"]
content:
  type: how_to
...
provider integrations. They are different from Vyrex-managed agent skills:
...
Today, the supported Vyrex path for OpenClaw plugins is to bake the plugin
...
`vyrex onboard --from <Dockerfile>` uses the Dockerfile's parent directory as
...
and let OpenClaw refresh its config before Vyrex starts the sandbox.

```dockerfile
ARG SANDBOX_BASE=ghcr.io/nvidia/vyrex/sandbox-base:latest
FROM ${SANDBOX_BASE}
...
Point `vyrex onboard --from` at the Dockerfile in the build directory.

```console
$ vyrex onboard --from ./my-plugin-sandbox/Dockerfile
```
...
other Vyrex extension paths.

- Do not use `vyrex <sandbox> skill install` for OpenClaw plugins. That
