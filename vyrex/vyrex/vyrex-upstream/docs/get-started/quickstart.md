---
title:
  page: "Vyrex Quickstart with OpenClaw"
  nav: "Quickstart with OpenClaw"
description:
  main: "Install Vyrex, launch a sandbox, and run your first agent prompt."
  agent: "Installs Vyrex, launches a sandbox, and runs the first agent prompt. Use when onboarding, installing, or launching a Vyrex sandbox for the first time."
keywords: ["vyrex quickstart", "install vyrex openclaw sandbox"]
topics: ["generative_ai", "ai_agents"]
tags: ["openclaw", "openshell", "sandboxing", "inference_routing", "vyrex"]
content:
  type: get_started
...
# Vyrex Quickstart with OpenClaw

Follow these steps to get started with Vyrex and your first sandboxed OpenClaw agent.

:::{note}
Make sure you have completed reviewing the [Prerequisites](prerequisites.md) before following this guide.
:::

## Install Vyrex and Onboard OpenClaw Agent

Download and run the installer script.
The script installs Node.js if it is not already present, then runs the guided onboard wizard to create a sandbox, configure inference, and apply security policies.

:::{note}
Vyrex creates a fresh OpenClaw instance inside the sandbox during the onboarding process.
:::
...
If `vyrex` is not found after install, run `source ~/.bashrc` (or `source ~/.zshrc` for zsh) or open a new terminal.

:::{note}
The onboard flow builds the sandbox image with `VYREX_DISABLE_DEVICE_AUTH=1` so the dashboard is immediately usable during setup.
This is a build-time setting baked into the sandbox image, not a runtime knob.
If you export `VYREX_DISABLE_DEVICE_AUTH` after onboarding finishes, it has no effect on an existing sandbox.
:::

### Respond to the Onboard Wizard

After the installer launches `vyrex onboard`, the wizard runs preflight checks, starts or reuses the OpenShell gateway, and asks for an inference provider, sandbox name, optional web search, optional messaging channels, and network policy presets.
...
Local Ollama appears only when Vyrex detects Ollama on the host.
...
Vyrex validates the model against the catalog API before creating the sandbox.
...
2. At the `OpenAI-compatible base URL` prompt, enter the provider's base URL. Find the exact value in your provider's API documentation. Vyrex appends `/v1` automatically, so leave that suffix off.
...
Vyrex sends a real inference request to validate the endpoint and model.
If the endpoint does not return the streaming events OpenClaw needs from the Responses API, Vyrex falls back to the chat completions API and configures OpenClaw to use `openai-completions`.
...
No API key is required. Vyrex generates a token and starts an authenticated proxy so containers can reach Ollama without exposing it to your network.
...
These options appear when `VYREX_EXPERIMENTAL=1` is set and the prerequisites are met.

- **Local NVIDIA NIM** requires a NIM-capable GPU. Vyrex pulls and manages a NIM container.
- **Local vLLM** requires a vLLM server already running on `localhost:8000`. Vyrex auto-detects the loaded model.
...
The default is `Y`, so you can press Enter once to continue. Answer `n` to abort cleanly, fix the entries, and re-run `vyrex onboard`.

Non-interactive runs (`VYREX_NON_INTERACTIVE=1`) print the summary for log clarity but skip the prompt.

### Configure Web Search and Messaging

After you confirm the summary, Vyrex registers the selected provider with the OpenShell gateway and sets the `inference.local` route.
...
If you select a channel, Vyrex validates the token format before it bakes the channel configuration into the sandbox.
For example, Slack bot tokens must start with `xoxb-`.

### Choose Network Policy Presets

After the sandbox image builds and OpenClaw starts inside the sandbox, Vyrex asks which network policy tier to apply.
...
──────────────────────────────────────────────────
Run:         vyrex my-gpt-claw connect
Status:      vyrex my-gpt-claw status
Logs:        vyrex my-gpt-claw logs --follow
──────────────────────────────────────────────────
...
If that port is already taken, Vyrex uses the next free dashboard port, such as `18790`, and prints that port in the final URL.
...
Vyrex prints the token only once.
...
- [VyrexLumyn Quickstart](quickstart-lumyn.md) to launch Lumyn instead of the default OpenClaw agent.
- [Manage Vyrex sandboxes](../manage-sandboxes/lifecycle.md) for port forwards, rebuilds, upgrades, and uninstall.
