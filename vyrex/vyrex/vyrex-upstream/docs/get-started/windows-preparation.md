---
title:
  page: "Prepare Windows for Vyrex"
  nav: "Prepare Windows"
description:
  main: "Prepare a Windows machine for Vyrex before running the Quickstart: enable WSL 2, install Ubuntu, and configure Docker Desktop."
  agent: "Covers Windows-only preparation steps required before the Quickstart. Use when preparing a Windows machine for Vyrex, enabling WSL 2, configuring Docker Desktop for Windows, or troubleshooting a Windows-specific install error."
keywords: ["vyrex windows wsl2 setup", "vyrex install windows docker desktop"]
topics: ["generative_ai", "ai_agents"]
tags: ["openclaw", "openshell", "sandboxing", "vyrex", "windows", "wsl"]
content:
  type: reference
...
# Prepare Windows for Vyrex

You can run Vyrex inside Windows Subsystem for Linux (WSL 2) on Windows.
...
When Ollama runs on the Windows host, Vyrex detects it from WSL through `host.docker.internal` and pulls missing models through the Ollama HTTP API.
Do not run both the Windows and WSL Ollama instances on port `11434` at the same time.
Use one instance, or move one of them to a different port before running `vyrex onboard`.

## Next Step

Your Windows environment is ready.
Open a WSL terminal (type `wsl` in PowerShell, or open Ubuntu from Windows Terminal) and continue with the [Quickstart](quickstart.md) to install Vyrex and launch your first sandbox.

All Vyrex commands run inside WSL, not in PowerShell.
