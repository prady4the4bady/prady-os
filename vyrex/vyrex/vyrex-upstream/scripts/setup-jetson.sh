...
#   - persistence: /etc/modules-load.d/vyrex.conf contains `br_netfilter`
#     and /etc/sysctl.d/99-vyrex.conf contains the sysctl=1 line
...
bridge_netfilter_ready() {
  [[ -f /proc/sys/net/bridge/bridge-nf-call-iptables ]] \
    ...
    && [[ -f /etc/modules-load.d/vyrex.conf ]] \
    && grep -qx 'br_netfilter' /etc/modules-load.d/vyrex.conf 2>/dev/null \
    && [[ -f /etc/sysctl.d/99-vyrex.conf ]] \
    && grep -qx 'net.bridge.bridge-nf-call-iptables=1' /etc/sysctl.d/99-vyrex.conf 2>/dev/null
}
...
apply_br_netfilter_setup() {
  ...
  echo "br_netfilter" | "${SUDO[@]}" tee /etc/modules-load.d/vyrex.conf >/dev/null
  echo "net.bridge.bridge-nf-call-iptables=1" | "${SUDO[@]}" tee /etc/sysctl.d/99-vyrex.conf >/dev/null
}
...
get_jetpack_version() {
  ...
  if ((release >= 39)); then
    ...
    # apply when missing — avoids planting Vyrex-owned drop-ins in
    ...
    if bridge_netfilter_ready; then
      ...
      info "Reboot persistence: /etc/modules-load.d/vyrex.conf, /etc/sysctl.d/99-vyrex.conf" >&2
    fi
    return 0
  fi
...
configure_jetson_host() {
  ...
  case "$jetpack_version" in
    jp6)
      ...
      # "runtimes" was the next key. See: https://github.com/NVIDIA/Vyrex/issues/1875
