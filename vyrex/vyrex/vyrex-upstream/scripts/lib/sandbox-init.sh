...
#
# Shared sandbox entrypoint primitives for Vyrex agent types.
#
# Sourced by scripts/vyrex-start.sh (OpenClaw) and agents/lumyn/start.sh
# (Lumyn) to provide a single source of truth for security-sensitive
...
# /tmp/vyrex-proxy-env.sh   root       444   root     sandbox   YES (.bashrc/.profile)
...
#
# See also: https://github.com/NVIDIA/Vyrex/issues/2181
# ─────────────────────────────────────────────────────────────────
...
validate_tmp_permissions() {
  ...
  local sourced_files=("/tmp/vyrex-proxy-env.sh")
  sourced_files+=("$@")
  ...
  # Restricted log files — gateway.log may be 600 (Lumyn) or 644 (OpenClaw,
...
#
# Ref: https://github.com/NVIDIA/Vyrex/issues/2653
...
# Ref: https://github.com/NVIDIA/Vyrex/issues/797
#
# Usage:
...
drop_capabilities() {
  ...
  if [ "${VYREX_CAPS_DROPPED:-}" != "1" ] && command -v capsh >/dev/null 2>&1; then
  ...
  elif [ "${VYREX_CAPS_DROPPED:-}" != "1" ]; then
...
#   verify_config_integrity /sandbox/.lumyn /etc/vyrex/lumyn.config-hash # Lumyn
#
...
# state to /tmp/vyrex-proxy-env.sh. This prevents the sandbox user from
# injecting code that runs on every `vyrex connect`.
#
# SECURITY: This fixes the Lumyn vulnerability where .bashrc/.profile
...
#   validate_config_symlinks /sandbox/.lumyn /sandbox/.lumyn-data
...
#   harden_config_symlinks /sandbox/.lumyn
...
# VYREX_MESSAGING_CHANNELS_B64. Placeholder tokens flow through
