...
socket_exists() {
  ...
  if [ -n "${VYREX_TEST_SOCKET_PATHS:-}" ]; then
    case ":$VYREX_TEST_SOCKET_PATHS:" in
...
get_local_provider_base_url() {
  ...
  local vllm_port="${VYREX_VLLM_PORT:-8000}"
  local ollama_port="${VYREX_OLLAMA_PORT:-11434}"
  _validate_port VYREX_VLLM_PORT "$vllm_port" || return 1
  _validate_port VYREX_OLLAMA_PORT "$ollama_port" || return 1
  case "$provider" in
...
# See: https://github.com/NVIDIA/Vyrex/issues/431
