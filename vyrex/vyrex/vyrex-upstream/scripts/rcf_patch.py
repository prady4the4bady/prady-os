...
replacement = (
    ...
    + ' console.error("[vyrex] Config is read-only in sandbox \\u2014 plugin metadata not persisted (plugins auto-load from extensions/)"); }'
    + " else { throw _rcfErr; } }"
)
...
open(p, "w").write(out)
print(f"[vyrex] rcf_patch applied to {p}")
