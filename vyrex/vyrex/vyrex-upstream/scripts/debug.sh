...
#
# Collect Vyrex diagnostic information for bug reports.
#
...
#   vyrex debug [--quick] [--output path]    # via CLI wrapper
#
# Can also be run without cloning:
#   curl -fsSL https://raw.githubusercontent.com/NVIDIA/Vyrex/main/scripts/debug.sh | bash -s -- --quick
...
SANDBOX_NAME="${VYREX_SANDBOX:-${SANDBOX_NAME:-}}"
QUICK=false
OUTPUT=""
...
Options:
  --sandbox NAME    Target sandbox (default: $VYREX_SANDBOX or auto-detect)
  --quick           Collect minimal diagnostics only
  --output PATH     Write tarball to PATH (e.g. /tmp/vyrex-debug.tar.gz)
  --help            Show this help

Examples:
  vyrex debug
  vyrex debug --quick
  vyrex debug --output /tmp/diag.tar.gz
  curl -fsSL https://raw.githubusercontent.com/NVIDIA/Vyrex/main/scripts/debug.sh | bash -s -- --quick
USAGE
      exit 0
      ;;
...
COLLECT_DIR=$(mktemp -d "${TMPDIR_BASE}/vyrex-debug-XXXXXX")
SANDBOX_SSH_CONFIG=""
SANDBOX_SSH_KNOWN=""
cleanup() {
  rm -rf "$COLLECT_DIR"
...
# Ref: https://github.com/NVIDIA/Vyrex/issues/2381
redact() {
...
# Collect logs for Vyrex-related containers
for cid in $(docker ps -a --filter "label=com.nvidia.vyrex" --format '{{.Names}}' 2>/dev/null || true); do
...
if [ "$QUICK" = false ]; then
  ...
  _dp="$(printf '%s' "${VYREX_DASHBOARD_PORT:-18789}" | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')"
...
info "  vyrex debug --output /tmp/vyrex-debug.tar.gz"
