...
#
# Pre-bakes a VM with everything needed for Vyrex E2E tests so that
# CI runs only need to: rsync branch code → npm ci → vyrex onboard → test.
#
# What this installs:
#   1. Docker (docker.io) — enabled and running
#   2. Node.js 22 (nodesource)
#   3. OpenShell CLI binary (pinned release)
#   4. Vyrex repo cloned with npm deps installed and TS plugin built
...
#
# Readiness detection:
#   Writes /var/run/vyrex-launchable-ready when complete.
...
#   curl -fsSL https://raw.githubusercontent.com/NVIDIA/Vyrex/<ref>/scripts/brev-launchable-ci-cpu.sh | bash
#
# Environment overrides:
#   OPENSHELL_VERSION     — OpenShell CLI release tag (default: v0.0.36)
#   VYREX_REF          — Vyrex git ref to clone (default: main)
#   VYREX_CLONE_DIR    — Where to clone Vyrex (default: ~/Vyrex)
#   SKIP_DOCKER_PULL      — Set to 1 to skip Docker image pre-pulls
#
# Related:
#   - Epic: https://github.com/NVIDIA/Vyrex/issues/1326
#   - Issue: https://github.com/NVIDIA/Vyrex/issues/1327
...
VYREX_CLONE_DIR="${VYREX_CLONE_DIR:-${TARGET_HOME}/Vyrex}"
...
DOCKER_IMAGES=(
  "ghcr.io/nvidia/vyrex/sandbox-base:latest"
  "node:22-slim"
)
...
# ══════════════════════════════════════════════════════════════════════
# 5. Clone Vyrex and install deps
# ══════════════════════════════════════════════════════════════════════
if [[ -d "$VYREX_CLONE_DIR/.git" ]]; then
  info "Vyrex repo exists at $VYREX_CLONE_DIR — refreshing"
  git -C "$VYREX_CLONE_DIR" fetch origin "$VYREX_REF"
  git -C "$VYREX_CLONE_DIR" checkout "$VYREX_REF"
  git -C "$VYREX_CLONE_DIR" pull --ff-only origin "$VYREX_REF" || true
else
  info "Cloning Vyrex (ref: $VYREX_REF)..."
  git clone --branch "$VYREX_REF" --depth 1 \
    "https://github.com/NVIDIA/Vyrex.git" "$VYREX_CLONE_DIR"
fi
...
# builds dist/ (via `build:cli`). Build it explicitly — bin/vyrex.js
# does `require("../dist/vyrex")` and needs the compiled output.
info "Building CLI (dist/)..."
npm run build:cli 2>&1 | tail -3
info "CLI built"

info "Building TypeScript plugin..."
cd "$VYREX_CLONE_DIR/vyrex"
npm install --ignore-scripts 2>&1 | tail -3
npm run build 2>&1 | tail -3
cd "$VYREX_CLONE_DIR"
info "Plugin built"

# Expose the vyrex CLI on PATH. Earlier this was `sudo npm link`, but
...
info "Linking vyrex CLI (direct symlink)..."
sudo ln -sf "$VYREX_CLONE_DIR/bin/vyrex.js" /usr/local/bin/vyrex
sudo chmod +x "$VYREX_CLONE_DIR/bin/vyrex.js"
info "vyrex CLI linked at /usr/local/bin/vyrex"
...
info "  Vyrex:   $VYREX_CLONE_DIR (ref: $VYREX_REF)"
