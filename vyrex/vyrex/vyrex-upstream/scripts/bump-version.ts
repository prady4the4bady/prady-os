...
const PLUGIN_PACKAGE_JSON = path.join(REPO_ROOT, "vyrex", "package.json");
const BLUEPRINT_YAML = path.join(REPO_ROOT, "vyrex-blueprint", "blueprint.yaml");
...
function main(): void {
  ...
  const nextDocsPublicUrl = `https://docs.nvidia.com/vyrex/${docsSegment}`;
...
function ensureOriginIsCanonicalRepo(): void {
  ...
  const allowed = new Set([
    "git@github.com:NVIDIA/Vyrex.git",
    "https://github.com/NVIDIA/Vyrex.git",
    "https://github.com/NVIDIA/Vyrex",
  ]);

  if (!allowed.has(originUrl)) {
    throw new Error(
      `origin must point to the canonical NVIDIA/Vyrex repository. Found: ${originUrl}`,
    );
  }
}
...
function updateInstallScriptDefaultVersion(previousVersion: string, nextVersion: string): void {
  replaceExact(
    INSTALL_SH,
    `DEFAULT_VYREX_VERSION="${previousVersion}"`,
    `DEFAULT_VYREX_VERSION="${nextVersion}"`,
  );
}
...
function buildDocsVersionUrl(version: string): string {
  return `https://docs.nvidia.com/vyrex/${version}/`;
}
...
function updateDocsVersionLinks(nextDocsPublicUrl: string): void {
  for (const filePath of VERSIONED_DOC_LINK_FILES) {
    ...
    const updated = current.replaceAll(
      /https:\/\/docs\.nvidia\.com\/vyrex\/(?:latest|[0-9]+\.[0-9]+\.[0-9]+)\//g,
      `${nextDocsPublicUrl}/`,
    );
    if (updated === current) {
      throw new Error(`No docs.nvidia.com/vyrex links found in ${relative(filePath)}`);
    }
...
function updateInstallAndUninstallDocs(nextDocsVersion: string): void {
  const installReplacement = `curl -fsSL https://www.nvidia.com/vyrex.sh | bash # ${nextDocsVersion}`;
  const uninstallReplacement = `curl -fsSL https://raw.githubusercontent.com/NVIDIA/Vyrex/refs/heads/main/uninstall.sh | bash # ${nextDocsVersion}`;

  replaceCodeBlockLine(
    README_MD,
    /^curl -fsSL https:\/\/www\.nvidia\.com\/vyrex\.sh \| bash(?: # v[0-9]+\.[0-9]+\.[0-9]+(?:[-+][0-9A-Za-z.-]+)?)?$/m,
    installReplacement,
  );
  replaceCodeBlockLine(
    QUICKSTART_MD,
  ...
  replaceCodeBlockLine(
    README_MD,
    /^curl -fsSL https:\/\/raw\.githubusercontent\.com\/NVIDIA\/Vyrex\/refs\/heads\/main\/uninstall\.sh \| bash(?: # v[0-9]+\.[0-9]+\.[0-9]+(?:[-+][0-9A-Za-z.-]+)?)?$/m,
...
function verifyVersionState(
  ...
  requireContains(INSTALL_SH, `DEFAULT_VYREX_VERSION="${version}"`);
...
function buildPrBody(previousVersion: string, nextVersion: string, options: PrBodyOptions): string {
  ...
  return [
    "## Summary",
    `Bump Vyrex from ${previousVersion} to ${nextVersion} across the CLI package, plugin package,`,
    ...
    "### General",
    "",
    "- [x] I have read and followed the [contributing guide](https://github.com/NVIDIA/Vyrex/blob/main/CONTRIBUTING.md).",
    "- [ ] I have read and followed the [style guide](https://github.com/NVIDIA/Vyrex/blob/main/docs/CONTRIBUTING.md). (for doc-only changes)",
    "",
    "### Code Changes",
    ...
    "",
    "### Doc Changes",
    '- [ ] Follows the [style guide](https://github.com/NVIDIA/Vyrex/blob/main/docs/CONTRIBUTING.md). Try running the `update-docs` agent skill to draft changes while complying with the style guide. For example, prompt your agent with "`/update-docs` catch up the docs for the new changes I made in this PR."',
...
function verifyDocsLinks(filePath: string, expectedDocsPublicUrl: string): void {
  ...
  const matches = Array.from(
    content.matchAll(/https:\/\/docs\.nvidia\.com\/vyrex\/([^/]+)\//g),
    (match) => match[1],
  );

  if (matches.length === 0) {
    throw new Error(`Expected at least one docs.nvidia.com/vyrex link in ${relative(filePath)}`);
  }

  const expectedSegment = expectedDocsPublicUrl.replace("https://docs.nvidia.com/vyrex/", "");
