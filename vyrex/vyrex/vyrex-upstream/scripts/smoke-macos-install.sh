...
#
# Run the primary Vyrex install flow on a local machine, capture logs,
...
LOG_DIR="${TMPDIR:-/tmp}/vyrex-smoke"
RUNTIME=""
...
Options:
  ...
  --allow-existing-state      Allow running even if Vyrex/OpenShell state already exists
...
ensure_clean_start() {
  ...
  if [ -d "$HOME/.vyrex" ] || [ -d "$HOME/.config/vyrex" ] || [ -d "$HOME/.config/openshell" ]; then
    fail "Existing Vyrex/OpenShell state detected. Re-run with --allow-existing-state if you really want to test on this machine."
  fi
...
verify_cleanup() {
  ...
  if [ -d "$HOME/.vyrex" ] || [ -d "$HOME/.config/vyrex" ]; then
    warn "Vyrex state directories still exist under HOME."
    leftovers=1
  fi
  ...
  if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1; then
    ...
    related_containers="$(
      docker ps -a --format '{{.Image}} {{.Names}}' 2>/dev/null \
        | awk 'BEGIN { IGNORECASE=1 } /openshell-cluster|openshell|openclaw|vyrex/ { print }'
    )"
...
main() {
  ...
  ANSWERS_PIPE="$(mktemp -u "${TMPDIR:-/tmp}/vyrex-smoke-answers-XXXXXX")"
  mkfifo "$ANSWERS_PIPE"
  trap cleanup EXIT
