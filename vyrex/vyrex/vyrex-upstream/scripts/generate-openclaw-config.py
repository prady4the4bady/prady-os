...
Environment variables:
    ...
    VYREX_MODEL                      Model identifier
    VYREX_PROVIDER_KEY               Provider key for model config
    VYREX_PRIMARY_MODEL_REF          Primary model reference
    VYREX_INFERENCE_BASE_URL         Inference endpoint
    VYREX_INFERENCE_API              Inference API type
    VYREX_INFERENCE_INPUTS           Comma-separated model inputs (default: text)
    VYREX_CONTEXT_WINDOW             Context window size (default: 131072)
    VYREX_MAX_TOKENS                 Max tokens (default: 4096)
    VYREX_REASONING                  Enable reasoning (default: false)
    VYREX_AGENT_TIMEOUT              Per-request timeout seconds (default: 600)
    VYREX_INFERENCE_COMPAT_B64       Base64-encoded inference compat JSON
    VYREX_MESSAGING_CHANNELS_B64     Base64-encoded channel list
    VYREX_MESSAGING_ALLOWED_IDS_B64  Base64-encoded allowed IDs map
    VYREX_DISCORD_GUILDS_B64         Base64-encoded Discord guild config
    VYREX_TELEGRAM_CONFIG_B64        Base64-encoded Telegram config (e.g. {"requireMention": true})
    VYREX_DISABLE_DEVICE_AUTH        Set to "1" to force-disable device auth
    VYREX_PROXY_HOST                 Egress proxy host (default: 10.200.0.1)
    VYREX_PROXY_PORT                 Egress proxy port (default: 3128)
    VYREX_WEB_SEARCH_ENABLED         Set to "1" to enable web search tools
"""
...
def build_config(env: dict | None = None) -> dict:
    ...
    # CHAT_UI_URL=`).
    proxy_host = env.get("VYREX_PROXY_HOST") or "10.200.0.1"
    proxy_port = env.get("VYREX_PROXY_PORT") or "3128"
    proxy_url = f"http://{proxy_host}:{proxy_port}"
    model = env["VYREX_MODEL"]
    chat_ui_url = env.get("CHAT_UI_URL") or "http://127.0.0.1:18789"
    provider_key = env["VYREX_PROVIDER_KEY"]
    primary_model_ref = env["VYREX_PRIMARY_MODEL_REF"]
    inference_base_url = env["VYREX_INFERENCE_BASE_URL"]
    inference_api = env["VYREX_INFERENCE_API"]
    context_window = int(env.get("VYREX_CONTEXT_WINDOW", "131072"))
    max_tokens = int(env.get("VYREX_MAX_TOKENS", "4096"))
    reasoning = env.get("VYREX_REASONING", "false") == "true"
    inference_inputs = [
        v.strip()
        for v in env.get("VYREX_INFERENCE_INPUTS", "text").split(",")
        if v.strip()
    ] or ["text"]

    _raw_agent_timeout = env.get("VYREX_AGENT_TIMEOUT", "600")
    if not _raw_agent_timeout.isdigit() or int(_raw_agent_timeout) <= 0:
        raise ValueError("VYREX_AGENT_TIMEOUT must be a positive integer")
    agent_timeout = int(_raw_agent_timeout)

    inference_compat = json.loads(
        base64.b64decode(env["VYREX_INFERENCE_COMPAT_B64"]).decode("utf-8")
    )

    msg_channels = json.loads(
        base64.b64decode(
            env.get("VYREX_MESSAGING_CHANNELS_B64", "W10=") or "W10="
        ).decode("utf-8")
    )
    _allowed_ids = json.loads(
        base64.b64decode(
            env.get("VYREX_MESSAGING_ALLOWED_IDS_B64", "e30=") or "e30="
        ).decode("utf-8")
    )
    _discord_guilds = json.loads(
        base64.b64decode(
            env.get("VYREX_DISCORD_GUILDS_B64", "e30=") or "e30="
        ).decode("utf-8")
    )
    _telegram_config = json.loads(
        base64.b64decode(
            env.get("VYREX_TELEGRAM_CONFIG_B64", "e30=") or "e30="
        ).decode("utf-8")
    )
    ...
    disable_device_auth = (
        env.get("VYREX_DISABLE_DEVICE_AUTH", "") == "1"
        or _is_remote
    )
    ...
    # provider plugin. Vyrex bakes one model provider into openclaw.json, so
    ...
    config = {
        "agents": {
            "defaults": {
                ...
                # Vyrex sandboxes are provisioned non-interactively and the
                ...
                # Vyrex's sandbox contract is a direct CLI answer, not an
        ...
        #     Introduced in OpenClaw 2026.4.15. See Vyrex#2484.
        #
        ...
        # also reaches npm during gateway startup. Vyrex's primary CLI path
        # invokes openclaw-agent directly, not ACPx.
        #
        # Provider plugins with staged runtime dependencies are disabled above
        # unless they match VYREX_PROVIDER_KEY. That keeps the baked image
    ...
    if env.get("VYREX_WEB_SEARCH_ENABLED", "") == "1":
        config["tools"] = {
            "web": {
                "search": {
                    "enabled": True,
