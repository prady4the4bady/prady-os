...
#
# Vyrex installer — installs Node.js, Ollama (if GPU present), and Vyrex.
...
resolve_repo_root() {
  local base="${VYREX_REPO_ROOT:-$SCRIPT_DIR}"
...
DEFAULT_VYREX_VERSION="0.1.0"
TOTAL_STEPS=3

resolve_installer_version() {
  local repo_root
  repo_root="$(resolve_repo_root)"
  if [[ -n "${VYREX_INSTALL_REF:-}" && "${VYREX_INSTALL_REF}" != "latest" ]]; then
    printf "%s" "${VYREX_INSTALL_REF#v}"
    return
  fi
  ...
  printf "%s" "${version:-$DEFAULT_VYREX_VERSION}"
}

VYREX_VERSION="$(resolve_installer_version)"

installer_version_for_display() {
  if [[ -z "${VYREX_VERSION:-}" || "${VYREX_VERSION}" == "${DEFAULT_VYREX_VERSION}" ]]; then
    printf ""
    return
  fi
  printf "  v%s" "$VYREX_VERSION"
}
...
# Priority: VYREX_INSTALL_TAG env var > "latest" tag.
resolve_release_tag() {
  if [[ -n "${VYREX_INSTALL_REF:-}" ]]; then
    printf "%s" "${VYREX_INSTALL_REF}"
    return
  fi
  ...
  printf "%s" "${VYREX_INSTALL_TAG:-latest}"
}
...
clone_vyrex_ref() {
  ...
  git init --quiet "$dest"
  git -C "$dest" remote add origin https://github.com/NVIDIA/Vyrex.git
...
resolve_default_sandbox_name() {
  local registry_file="${HOME}/.vyrex/sandboxes.json"
  local sandbox_name=""
  ...
  local session_file="${HOME}/.vyrex/onboard-session.json"
  ...
  if [[ -z "$sandbox_name" ]]; then
    sandbox_name="${VYREX_SANDBOX_NAME:-}"
  fi
...
restore_onboard_forward_after_post_checks() {
  ...
  if [[ -n "${VYREX_OPENSHELL_BIN:-}" && -x "$VYREX_OPENSHELL_BIN" ]]; then
    openshell_bin="$VYREX_OPENSHELL_BIN"
  ...
  for attempt in 1 2 3; do
    ...
    watcher_pid=""
    if [[ "${VYREX_SKIP_FORWARD_WATCHER:-}" != "1" ]] && command_exists node; then
...
print_banner() {
  ...
  if [[ "${VYREX_AGENT:-openclaw}" == "lumyn" ]]; then
  ...
  printf "\n"
  if [[ -n "${VYREX_AGENT:-}" && "${VYREX_AGENT}" != "openclaw" ]]; then
    printf "  ${C_DIM}Launch %s in an OpenShell sandbox.%s${C_RESET}\n" "$(agent_display_name "$VYREX_AGENT")" "$version_suffix"
  else
...
print_done() {
  ...
  if [[ "$ONBOARD_RAN" == true ]]; then
  ...
  elif [[ "$VYREX_READY_NOW" == true ]]; then
    ...
    printf "\n"
    printf "  ${C_GREEN}Next:${C_RESET}\n"
    if [[ -n "$VYREX_RECOVERY_EXPORT_DIR" ]]; then
      printf "  %s$%s export PATH=\"%s:\$PATH\"\n" "$C_GREEN" "$C_RESET" "$VYREX_RECOVERY_EXPORT_DIR"
    fi
    if [[ -n "$VYREX_RECOVERY_PROFILE" ]]; then
      printf "  %s$%s source %s\n" "$C_GREEN" "$C_RESET" "$VYREX_RECOVERY_PROFILE"
    fi
    printf "  %s$%s %s onboard\n" "$C_GREEN" "$C_RESET" "$_CLI_BIN"
  fi
  printf "\n"
  printf "  ${C_BOLD}GitHub${C_RESET}  ${C_DIM}https://github.com/nvidia/vyrex${C_RESET}\n"
  printf "  ${C_BOLD}Docs${C_RESET}    ${C_DIM}https://docs.nvidia.com/vyrex/latest/${C_RESET}\n"
  printf "\n"
}
...
usage() {
  ...
  printf "    curl -fsSL https://www.nvidia.com/vyrex.sh | bash\n"
  printf "    curl -fsSL https://www.nvidia.com/vyrex.sh | bash -s -- [options]\n\n"
  ...
  printf "    VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 Same as --yes-i-accept-third-party-software\n"
  printf "    VYREX_NON_INTERACTIVE=1    Same as --non-interactive\n"
  printf "    VYREX_FRESH=1              Same as --fresh\n"
  printf "    VYREX_SANDBOX_NAME         Sandbox name to create/use\n"
  printf "    VYREX_SINGLE_SESSION=1     Abort if active sandbox sessions exist\n"
  printf "    VYREX_RECREATE_SANDBOX=1   Recreate an existing sandbox\n"
  printf "    VYREX_INSTALL_TAG         Git ref to install (default: latest release)\n"
  printf "    VYREX_PROVIDER             build | openai | anthropic | anthropicCompatible\n"
  printf "                                  | gemini | ollama | custom | nim-local | vllm\n"
  printf "                                  (aliases: cloud -> build, nim -> nim-local)\n"
  printf "    VYREX_MODEL                Inference model to configure\n"
  printf "    VYREX_POLICY_MODE          suggested | custom | skip\n"
  printf "    VYREX_POLICY_PRESETS       Comma-separated policy presets\n"
  printf "    BRAVE_API_KEY                 Enable Brave Search with this API key (stored in sandbox OpenClaw config)\n"
  printf "    VYREX_EXPERIMENTAL=1       Show experimental/local options\n"
...
show_usage_notice() {
  local repo_root
  repo_root="$(resolve_repo_root)"
  local source_root="${VYREX_SOURCE_ROOT:-$repo_root}"
  ...
  # When --yes-i-accept-third-party-software (or VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1)
  ...
  if [ "${NON_INTERACTIVE:-}" = "1" ] || [ "${ACCEPT_THIRD_PARTY_SOFTWARE:-}" = "1" ]; then
    ...
    error "Interactive third-party software acceptance requires a TTY. Re-run in a terminal or pass --yes-i-accept-third-party-software (or set VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1)."
  fi
}
...
case "${VYREX_AGENT:-openclaw}" in
  lumyn)
    _CLI_DISPLAY="VyrexLumyn"
    _AGENT_PRODUCT="Lumyn"
    _CLI_BIN="vyrex-lumyn"
    ;;
  *)
    _CLI_DISPLAY="Vyrex"
...
VYREX_SHIM_DIR="${HOME}/.local/bin"
VYREX_READY_NOW=false
VYREX_RECOVERY_PROFILE=""
VYREX_RECOVERY_EXPORT_DIR=""
VYREX_SOURCE_ROOT="$(resolve_repo_root)"
ONBOARD_RAN=false
...
refresh_path() {
  ...
  if [[ -d "$VYREX_SHIM_DIR" && ":$PATH:" != *":$VYREX_SHIM_DIR:"* ]]; then
    export PATH="$VYREX_SHIM_DIR:$PATH"
  fi
}
...
prefer_user_local_openshell() {
  ...
  if [[ -x "$openshell_bin" ]]; then
    export VYREX_OPENSHELL_BIN="$openshell_bin"
...
ensure_cli_shim() {
  ...
  npm_bin="$(resolve_npm_bin)" || true
  shim_path="${VYREX_SHIM_DIR}/${cli_bin}"
...
ensure_vyrex_shim() {
  ...
  if [[ "$_CLI_BIN" != "vyrex" ]]; then
    ensure_cli_shim "vyrex" || true
  fi
  return "$status"
}
...
# (e.g. upgrade/reinstall after `rm $(which vyrex)`).  Sourcing the
# shell profile reassigns PATH which clears the hash table, so we always
# recommend it when the installer verified vyrex in the subprocess.
needs_shell_reload() {
  [[ "$VYREX_READY_NOW" != true ]] && return 1
  return 0
}

# Add ~/.local/bin (and for fish, the nvm node bin) to the user's shell
# profile PATH so that vyrex, openshell, and any future tools installed
...
ensure_local_bin_in_profile() {
  ...
  if [[ -f "$profile" ]] && grep -qF '# Vyrex PATH setup' "$profile" 2>/dev/null; then
    return 0
  fi
  ...
  local local_bin="$VYREX_SHIM_DIR"
...
# ---------------------------------------------------------------------------
# Default model id mirrors the vllm profile in vyrex-blueprint/blueprint.yaml.
...
resolve_vllm_model() {
  local blueprint="${SCRIPT_DIR}/../vyrex-blueprint/blueprint.yaml"
  if [ -r "$blueprint" ]; then
...
install_or_start_vllm() {
  if [[ "${VYREX_PROVIDER:-}" != "vllm" ]]; then
    return 0
  fi
  if ! detect_gpu; then
    warn "VYREX_PROVIDER=vllm but no GPU detected — vLLM cannot start."
    warn "Install on a host with an NVIDIA GPU, or unset VYREX_PROVIDER for a cloud provider."
    return 1
  fi
...
is_source_checkout() {
  ...
  grep -q '"name"[[:space:]]*:[[:space:]]*"vyrex"' "$package_json" 2>/dev/null || return 1

  if [[ "${VYREX_BOOTSTRAP_PAYLOAD:-}" == "1" ]]; then
    return 1
  fi

  if [[ -n "${VYREX_REPO_ROOT:-}" || -d "${repo_root}/.git" ]]; then
    return 0
  fi
...
install_vyrex() {
  ...
  if is_source_checkout "$repo_root"; then
    ...
    VYREX_SOURCE_ROOT="$repo_root"
    if [[ -z "${VYREX_AGENT:-}" || "${VYREX_AGENT}" == "openclaw" ]]; then
      spin "Preparing OpenClaw package" bash -c "$(declare -f info warn resolve_openclaw_version pre_extract_openclaw); pre_extract_openclaw \"\$1\"" _ "$VYREX_SOURCE_ROOT" \
        || warn "Pre-extraction failed — npm install may fail if openclaw tarball is broken"
    fi
    spin "Installing ${_CLI_DISPLAY} dependencies" bash -c "cd \"$VYREX_SOURCE_ROOT\" && npm install --ignore-scripts"
    spin "Building ${_CLI_DISPLAY} CLI modules" bash -c "cd \"$VYREX_SOURCE_ROOT\" && npm run --if-present build:cli"
    spin "Building ${_CLI_DISPLAY} plugin" bash -c "cd \"$VYREX_SOURCE_ROOT\"/vyrex && npm install --ignore-scripts && npm run build"
    spin "Linking ${_CLI_DISPLAY} CLI" bash -c "cd \"$VYREX_SOURCE_ROOT\" && npm link"
  else
    if [[ -f "$package_json" ]]; then
    ...
    local vyrex_src="${HOME}/.vyrex/source"
    rm -rf "$vyrex_src"
    mkdir -p "$(dirname "$vyrex_src")"
    VYREX_SOURCE_ROOT="$vyrex_src"
    spin "Cloning ${_CLI_DISPLAY} source" clone_vyrex_ref "$release_ref" "$vyrex_src"
    ...
    git -C "$vyrex_src" fetch --depth=1 origin 'refs/tags/v*:refs/tags/v*' 2>/dev/null || true
    # Also stamp .version as a fallback for environments where git is
    # unavailable or tags are pruned later.
    git -C "$vyrex_src" describe --tags --match 'v*' 2>/dev/null \
      | sed 's/^v//' >"$vyrex_src/.version" || true
    if [[ -z "${VYREX_AGENT:-}" || "${VYREX_AGENT}" == "openclaw" ]]; then
      spin "Preparing OpenClaw package" bash -c "$(declare -f info warn resolve_openclaw_version pre_extract_openclaw); pre_extract_openclaw \"\$1\"" _ "$vyrex_src" \
        || warn "Pre-extraction failed — npm install may fail if openclaw tarball is broken"
    fi
    spin "Installing ${_CLI_DISPLAY} dependencies" bash -c "cd \"$vyrex_src\" && npm install --ignore-scripts"
    spin "Building ${_CLI_DISPLAY} CLI modules" bash -c "cd \"$vyrex_src\" && npm run --if-present build:cli"
    spin "Building ${_CLI_DISPLAY} plugin" bash -c "cd \"$vyrex_src\"/vyrex && npm install --ignore-scripts && npm run build"
    spin "Linking ${_CLI_DISPLAY} CLI" bash -c "cd \"$vyrex_src\" && npm link"
    ...
    # min_openshell_version even though the new Vyrex declared a higher
    ...
    spin "Installing OpenShell CLI" bash "${VYREX_SOURCE_ROOT}/scripts/install-openshell.sh"
    prefer_user_local_openshell
  fi

  refresh_path
  ensure_vyrex_shim || true
}
...
# Verify that a CLI binary is the real Vyrex CLI and not the broken
# placeholder npm package (npmjs.org/vyrex 0.1.0 — 249 bytes, no build
...
verify_vyrex() {
  if command_exists "$_CLI_BIN"; then
    if is_real_vyrex_cli "$(command -v "$_CLI_BIN")" "$_CLI_BIN"; then
      VYREX_READY_NOW=true
      ensure_vyrex_shim || true
      ...
      npm uninstall -g vyrex 2>/dev/null || true
    fi
  fi
  ...
  if [[ -n "$npm_bin" && -x "$npm_bin/$_CLI_BIN" ]]; then
    if is_real_vyrex_cli "$npm_bin/$_CLI_BIN" "$_CLI_BIN"; then
      ensure_vyrex_shim || true
      if command_exists "$_CLI_BIN"; then
        VYREX_READY_NOW=true
        info "Verified: ${_CLI_BIN} is available at $(command -v "$_CLI_BIN")"
      ...
      VYREX_RECOVERY_PROFILE="$(detect_shell_profile)"
      if [[ -x "$VYREX_SHIM_DIR/$_CLI_BIN" ]]; then
        VYREX_RECOVERY_EXPORT_DIR="$VYREX_SHIM_DIR"
      else
        VYREX_RECOVERY_EXPORT_DIR="$npm_bin"
      fi
  ...
  warn "Try re-running:  curl -fsSL https://www.nvidia.com/vyrex.sh | bash"
...
run_installer_host_preflight() {
  local preflight_module="${VYREX_SOURCE_ROOT}/dist/lib/preflight.js"
...
run_onboard() {
  ...
  if [ "${NON_INTERACTIVE:-}" = "1" ]; then
    ...
    error "Interactive onboarding requires a TTY. Re-run in a terminal or set VYREX_NON_INTERACTIVE=1 with --yes-i-accept-third-party-software."
  fi
}
...
post_install_message() {
  if [[ "$VYREX_READY_NOW" == true ]]; then
    return 0
  fi

  if [[ -z "$VYREX_RECOVERY_EXPORT_DIR" ]]; then
    return 0
  fi

  if [[ -z "$VYREX_RECOVERY_PROFILE" ]]; then
    VYREX_RECOVERY_PROFILE="$(detect_shell_profile)"
  fi
  ...
  echo ""
  echo "    export PATH=\"${VYREX_RECOVERY_EXPORT_DIR}:\$PATH\""
  echo "    source ${VYREX_RECOVERY_PROFILE}"
  echo ""
  echo "  Then run:"
  echo ""
...
main() {
  ...
  # Also honor env var
  NON_INTERACTIVE="${NON_INTERACTIVE:-${VYREX_NON_INTERACTIVE:-}}"
  ACCEPT_THIRD_PARTY_SOFTWARE="${ACCEPT_THIRD_PARTY_SOFTWARE:-${VYREX_ACCEPT_THIRD_PARTY_SOFTWARE:-}}"
  FRESH="${FRESH:-${VYREX_FRESH:-}}"
  export VYREX_NON_INTERACTIVE="${NON_INTERACTIVE}"
  export VYREX_ACCEPT_THIRD_PARTY_SOFTWARE="${ACCEPT_THIRD_PARTY_SOFTWARE}"
  ...
  if [[ "${VYREX_PROVIDER:-}" == "vllm" ]]; then
    install_or_start_vllm || error "vLLM setup failed — aborting (VYREX_PROVIDER=vllm)."
  else
    install_or_start_vllm
  fi
  fix_npm_permissions
  ...
  # in ~/.vyrex/rebuild-backups/ let the user recover via `vyrex <name> rebuild`.
  # Check the registry file directly to avoid shelling out to vyrex (which
  # may be a stub in test environments).
  local _reg_file="${HOME}/.vyrex/sandboxes.json"
...
" "$_reg_file" 2>/dev/null || echo 0)"
  ...
  step 3 "Onboarding"
  if command_exists "$_CLI_BIN"; then
    if [[ -f "${HOME}/.vyrex/sandboxes.json" ]] && node -e '
      const fs = require("fs");
      try {
        const data = JSON.parse(fs.readFileSync(process.argv[1], "utf8"));
    ...
    ' "${HOME}/.vyrex/sandboxes.json"; then
      warn "Existing sandbox sessions detected. Onboarding may disrupt running agents."
      if [[ "${VYREX_SINGLE_SESSION:-}" == "1" ]]; then
        error "Aborting — VYREX_SINGLE_SESSION is set. Destroy existing sessions with '${_CLI_BIN} <name> destroy' before reinstalling."
      fi
      warn "Consider destroying existing sessions with '${_CLI_BIN} <name> destroy' first."
      warn "Set VYREX_SINGLE_SESSION=1 to abort the installer when sessions are active."
    fi
    if run_installer_host_preflight; then
      ...
      restore_onboard_forward_after_post_checks || error "Lumyn host forward restore failed."
    else
