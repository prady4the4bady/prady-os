...
#
# Vyrex sandbox entrypoint. Runs as root (via ENTRYPOINT) to start the
...
#   VYREX_DISABLE_DEVICE_AUTH  Build-time only. Set to "1" to skip device-pairing auth.
...
#   VYREX_MODEL_OVERRIDE       Override the primary model at startup without rebuilding
#                                 the sandbox image. Must match the model configured on
#                                 the gateway via `openshell inference set`.
#   VYREX_INFERENCE_API_OVERRIDE  Override the inference API type when switching between
#                                 provider families (e.g., "anthropic-messages" or
#                                 "openai-completions"). Only needed for cross-provider switches.
#   VYREX_CONTEXT_WINDOW        Override the model's context window size (e.g., "32768").
#   VYREX_MAX_TOKENS            Override the model's max output tokens (e.g., "8192").
#   VYREX_REASONING             Set to "true" to enable reasoning mode for the model.
#                                 Required for reasoning models (o1, Claude with thinking).
#   VYREX_CORS_ORIGIN           Add a browser origin to allowedOrigins at startup without
...
# Capture all entrypoint output to /tmp/vyrex-start.log so that if
...
_START_LOG="/tmp/vyrex-start.log"
if [ "$(id -u)" -eq 0 ]; then
  : >"$_START_LOG"
...
# agents/lumyn/start.sh. Ref: https://github.com/NVIDIA/Vyrex/issues/2277
# Installed location (container): /usr/local/lib/vyrex/sandbox-init.sh
# Dev fallback: scripts/lib/sandbox-init.sh relative to this script.
_SANDBOX_INIT="/usr/local/lib/vyrex/sandbox-init.sh"
...
# The same entries are emitted into /tmp/vyrex-proxy-env.sh (see below) so
...
drop_capabilities /usr/local/bin/vyrex-start "$@"

# Normalize the sandbox-create bootstrap wrapper. Onboard launches the
# container as `env CHAT_UI_URL=... vyrex-start`, but this script is already
# the ENTRYPOINT. If we treat that wrapper as a real command, the root path will
# try `gosu sandbox env ... vyrex-start`, which fails on Spark/arm64 when
...
if [ "${1:-}" = "env" ]; then
  ...
  for ((i = 1; i < ${#_raw_args[@]}; i += 1)); do
    case "${_raw_args[$i]}" in
      *=*) ;;
      vyrex-start | /usr/local/bin/vyrex-start)
...
# receiving our own name as $1 would otherwise recurse via the VYREX_CMD
# exec path. Only strip from $1 — later args with this name are legitimate.
case "${1:-}" in
  vyrex-start | /usr/local/bin/vyrex-start) shift ;;
esac
VYREX_CMD=("$@")
# Validate VYREX_DASHBOARD_PORT if set (same behavior as ports.js: fail fast).
_DASHBOARD_PORT_RAW="${VYREX_DASHBOARD_PORT:-}"
if [ -z "$_DASHBOARD_PORT_RAW" ]; then
  ...
  case "$_DASHBOARD_PORT" in
    *[!0-9]* | '')
      echo "[SECURITY] Invalid VYREX_DASHBOARD_PORT='${VYREX_DASHBOARD_PORT}' — must be an integer between 1024 and 65535" >&2
      exit 1
      ;;
  esac
...
# When VYREX_DASHBOARD_PORT is explicitly set (injected at sandbox create time
...
if [ -n "${VYREX_DASHBOARD_PORT:-}" ]; then
...
prepare_openclaw_config_for_write() {
  ...
  _VYREX_CONFIG_WRITE_MODE="locked"
  if [ "$(openclaw_config_dir_owner "$config_dir")" != "root" ]; then
    _VYREX_CONFIG_WRITE_MODE="mutable"
    if [ "$(id -u)" -eq 0 ]; then
...
restore_openclaw_config_after_write() {
  ...
  if [ "${_VYREX_CONFIG_WRITE_MODE:-locked}" = "mutable" ]; then
    if [ "$(id -u)" -eq 0 ]; then
...
# Patches openclaw.json at startup when VYREX_MODEL_OVERRIDE is set,
...
# Ref: https://github.com/NVIDIA/Vyrex/issues/759

apply_model_override() {
  # Only explicit override env vars trigger a config patch. VYREX_CONTEXT_WINDOW,
  # VYREX_MAX_TOKENS, and VYREX_REASONING are promoted from Dockerfile build
  ...
  [ -n "${VYREX_MODEL_OVERRIDE:-}" ] \
    || [ -n "${VYREX_INFERENCE_API_OVERRIDE:-}" ] \
    || return 0
  ...
  local model_override="${VYREX_MODEL_OVERRIDE:-}"
  local api_override="${VYREX_INFERENCE_API_OVERRIDE:-}"
  ...
  if printf '%s' "$model_override" | grep -qP '[\x00-\x1f\x7f]'; then
    printf '[SECURITY] VYREX_MODEL_OVERRIDE contains control characters — refusing\n' >&2
    return 1
  fi
  if [ "${#model_override}" -gt 256 ]; then
    printf '[SECURITY] VYREX_MODEL_OVERRIDE exceeds 256 characters — refusing\n' >&2
    return 1
  fi
  ...
  if [ -n "$api_override" ]; then
    case "$api_override" in
      ...
      *)
        printf '[SECURITY] VYREX_INFERENCE_API_OVERRIDE must be "openai-completions" or "anthropic-messages", got "%s" — skipping override\n' "$api_override" >&2
        return 0
        ;;
    esac
  fi

  local context_window="${VYREX_CONTEXT_WINDOW:-}"
  local max_tokens="${VYREX_MAX_TOKENS:-}"
  local reasoning="${VYREX_REASONING:-}"
  ...
  if [ -n "$context_window" ] && ! printf '%s' "$context_window" | grep -qE '^[1-9][0-9]*$'; then
    printf '[SECURITY] VYREX_CONTEXT_WINDOW must be a positive integer, got "%s" — skipping override\n' "$context_window" >&2
    return 0
  fi
  if [ -n "$max_tokens" ] && ! printf '%s' "$max_tokens" | grep -qE '^[1-9][0-9]*$'; then
    printf '[SECURITY] VYREX_MAX_TOKENS must be a positive integer, got "%s" — skipping override\n' "$max_tokens" >&2
    return 0
  fi
  if [ -n "$reasoning" ]; then
    case "$reasoning" in
      true | false) ;;
      *)
        printf '[SECURITY] VYREX_REASONING must be "true" or "false", got "%s" — skipping override\n' "$reasoning" >&2
        return 0
        ;;
    esac
  fi
  ...
  VYREX_CONTEXT_WINDOW="$context_window" \
    VYREX_MAX_TOKENS="$max_tokens" \
    VYREX_REASONING="$reasoning" \
...
context_window = os.environ.get("VYREX_CONTEXT_WINDOW", "")
max_tokens = os.environ.get("VYREX_MAX_TOKENS", "")
reasoning = os.environ.get("VYREX_REASONING", "")
...
# Ref: https://github.com/NVIDIA/Vyrex/issues/719

apply_cors_override() {
  [ -n "${VYREX_CORS_ORIGIN:-}" ] || return 0

  if [ "$(id -u)" -ne 0 ]; then
    printf '[SECURITY] VYREX_CORS_ORIGIN ignored — requires root (non-root mode cannot write to config)\n' >&2
    return 0
  fi
  ...
  local cors_origin="$VYREX_CORS_ORIGIN"

  if printf '%s' "$cors_origin" | grep -qP '[\x00-\x1f\x7f]'; then
    printf '[SECURITY] VYREX_CORS_ORIGIN contains control characters — refusing\n' >&2
    return 1
  fi
  if [ "${#cors_origin}" -gt 256 ]; then
    printf '[SECURITY] VYREX_CORS_ORIGIN exceeds 256 characters — refusing\n' >&2
    return 1
  fi
  if ! printf '%s' "$cors_origin" | grep -qE '^https?://'; then
    printf '[SECURITY] VYREX_CORS_ORIGIN must start with http:// or https://, got "%s" — skipping override\n' "$cors_origin" >&2
    return 0
  fi
...
#
# Ref: https://github.com/NVIDIA/Vyrex/issues/2085

_SLACK_REWRITER_SCRIPT="/tmp/vyrex-slack-token-rewriter.js"
...
// vyrex-start.sh writes a byte-identical copy to /tmp and loads it via
...
// /opt/vyrex-blueprint/scripts/ in the optimized sandbox build.
//
// Ref: https://github.com/NVIDIA/Vyrex/issues/2085
...
(function () {
  ...
  function wrapClientRequest(req) {
    ...
    if (req.__vyrexSlackTokenRewriter) return req;
    try {
      Object.defineProperty(req, '__vyrexSlackTokenRewriter', { value: true });
    } catch (_e) {
      req.__vyrexSlackTokenRewriter = true;
    }
...
#
# Ref: https://github.com/NVIDIA/Vyrex/issues/2340
_SLACK_GUARD_SCRIPT="/tmp/vyrex-slack-channel-guard.js"
...
(function () {
  ...
  if (process.__vyrexSlackChannelGuardInstalled) return;
  try {
    Object.defineProperty(process, '__vyrexSlackChannelGuardInstalled', { value: true });
  } catch (_e) {
    process.__vyrexSlackChannelGuardInstalled = true;
  }
...
_TELEGRAM_DIAGNOSTICS_SCRIPT="/tmp/vyrex-telegram-diagnostics.js"
...
// Vyrex#2766 is that "[telegram] [default] starting provider" means the
...
(function () {
  ...
  if (process.__vyrexTelegramDiagnosticsInstalled) return;
  try {
    Object.defineProperty(process, '__vyrexTelegramDiagnosticsInstalled', { value: true });
  } catch (_e) {
    process.__vyrexTelegramDiagnosticsInstalled = true;
  }
...
while time.time() < DEADLINE:
    ...
    # `openclaw agent` connects (Vyrex#2484: WS handshake-timeout).
    if QUIET_POLLS >= 4:
        if has_browser:
...
#
# VYREX_PROXY_HOST / VYREX_PROXY_PORT can be overridden at sandbox
# creation time if the gateway IP or port changes in a future OpenShell release.
# Ref: https://github.com/NVIDIA/Vyrex/issues/626
PROXY_HOST="${VYREX_PROXY_HOST:-10.200.0.1}"
PROXY_PORT="${VYREX_PROXY_PORT:-3128}"
...
# Git TLS CA bundle fix (Vyrex#2270).
...
# HTTP library + NODE_USE_ENV_PROXY double-proxy fix (Vyrex#2109).
...
# vyrex-blueprint/scripts/http-proxy-fix.js because the blueprint
# scripts/ directory is intentionally excluded from the optimized sandbox
# build context — adding it cache-busts the `COPY vyrex-blueprint/`
...
_SANDBOX_SAFETY_NET="/tmp/vyrex-sandbox-safety-net.js"
...
_PROXY_FIX_SCRIPT="/tmp/vyrex-http-proxy-fix.js"
...
//
// This file is the canonical source for review and tests. At sandbox boot
// vyrex-start.sh writes an identical copy to /tmp/vyrex-http-proxy-fix.js
// and loads it via NODE_OPTIONS=--require. A sync test enforces byte-for-byte
// equality. The content cannot be baked into /opt/vyrex-blueprint/scripts/
// because adding files to the optimized sandbox build context cache-busts the
// `COPY vyrex-blueprint/` Dockerfile layer and hangs npm ci in k3s
...
# disabled for Vyrex's OpenAI-compatible chat-completions path.
#
# The preload wraps http.request() — the lowest common denominator every
...
_NEMOTRON_FIX_SCRIPT="/tmp/vyrex-nemotron-inference-fix.js"
...
# Ref: https://github.com/NVIDIA/Vyrex/issues/2340
_CIAO_GUARD_SCRIPT="/tmp/vyrex-ciao-network-guard.js"
...
# WebSocket CONNECT tunnel fix (Vyrex#1570).
...
_WS_FIX_SOURCE="/usr/local/lib/vyrex/ws-proxy-fix.js"
_WS_FIX_SCRIPT="/tmp/vyrex-ws-proxy-fix.js"
...
_SECCOMP_GUARD_SCRIPT="/tmp/vyrex-seccomp-guard.js"
...
#
# We write dynamic connect-session config to /tmp/vyrex-proxy-env.sh. The
...
# Ref: https://github.com/NVIDIA/Vyrex/issues/2181
#
...
_RUNTIME_SHELL_ENV_FILE="/tmp/vyrex-proxy-env.sh"
_RUNTIME_SHELL_ENV_SHIM="[ -f ${_RUNTIME_SHELL_ENV_FILE} ] && . ${_RUNTIME_SHELL_ENV_FILE}"

write_runtime_shell_env() {
  _PROXY_ENV_FILE="/tmp/vyrex-proxy-env.sh"
  {
    cat <<PROXYEOF
...
openclaw() {
  case "$1" in
    ...
    channels)
      case "$2" in
        ...
        *)
          ...
          echo "  vyrex <sandbox> channels add <telegram|discord|slack>" >&2
          echo "  vyrex <sandbox> channels remove <telegram|discord|slack>" >&2
          echo "" >&2
    ...
    agent)
      ...
      for _arg in "$@"; do
        if [ "$_arg" = "--local" ]; then
          echo "Error: 'openclaw agent --local' is not supported inside Vyrex sandboxes." >&2
...
GUARDENVEOF
    ...
    # `openshell sandbox connect` benefit from the preload. (Vyrex#2109)
    if [ "${NODE_USE_ENV_PROXY:-}" = "1" ]; then
      echo "export NODE_OPTIONS=\"\${NODE_OPTIONS:+\$NODE_OPTIONS }--require $_PROXY_FIX_SCRIPT\""
    fi
    # WebSocket CONNECT tunnel fix for connect sessions. (Vyrex#1570)
    if [ -f "$_WS_FIX_SCRIPT" ]; then
      echo "export NODE_OPTIONS=\"\${NODE_OPTIONS:+\$NODE_OPTIONS }--require $_WS_FIX_SCRIPT\""
    fi
    # Git TLS CA bundle for connect sessions (Vyrex#2270)
    if [ -n "${GIT_SSL_CAINFO:-}" ]; then
      printf 'export GIT_SSL_CAINFO=%q\n' "$GIT_SSL_CAINFO"
    fi
    # Nemotron inference fix for connect sessions. (Vyrex#1193, #2051)
...
# gateway auth, and command guards through /tmp/vyrex-proxy-env.sh.
...
ensure_mutable_for_migration() {
  ...
  echo "[SECURITY] ${label}: ${target} is immutable; run 'vyrex <sandbox> shields down' before migration" >&2
  return 1
}
...
if [ "$(id -u)" -ne 0 ]; then
  ...
  # below for rationale (NVIDIA/Vyrex#2484).
...
#
# Ref: https://github.com/NVIDIA/Vyrex/issues/1260
...
# sandbox via `vyrex <sandbox> logs` and is not captured by the e2e test
...
# Ref: NVIDIA/Vyrex#2484 (TC-SBX-02 hang investigation)
{ tail -n +1 -F /tmp/gateway.log 2>/dev/null | sed -u 's/^/[gateway-log:] /' >&2; } &
GATEWAY_LOG_TAIL_PID=$!
