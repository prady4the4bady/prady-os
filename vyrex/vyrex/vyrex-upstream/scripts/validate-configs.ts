...
//
// Validates Vyrex configuration files against JSON Schemas.
  ...
  const targets: ConfigTarget[] = [
    {
      ...
      files: ["vyrex-blueprint/blueprint.yaml"],
    },
    {
      schema: "schemas/sandbox-policy.schema.json",
      files: ["vyrex-blueprint/policies/openclaw-sandbox.yaml"],
    },
    {
      schema: "schemas/openclaw-plugin.schema.json",
      files: ["vyrex/openclaw.plugin.json"],
    },
  ];

  // Discover all preset YAML files dynamically.
  const presetsDir = join(REPO_ROOT, "vyrex-blueprint/policies/presets");
  try {
    const presetFiles = readdirSync(presetsDir)
      .filter((f) => f.endsWith(".yaml") || f.endsWith(".yml"))
      .map((f) => `vyrex-blueprint/policies/presets/${f}`);
