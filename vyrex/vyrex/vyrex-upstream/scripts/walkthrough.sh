...
#
# Vyrex walkthrough — sandboxed agent approval flow.
#
...
#
# Prerequisites:
#   - Vyrex setup complete (vyrex onboard)
...
#
#   Terminal 2 (Agent):
#     openshell sandbox connect vyrex
...
if ! command -v tmux >/dev/null 2>&1; then
  ...
  echo "    openshell sandbox connect vyrex"
  echo "    vyrex-start openclaw agent --agent main --local --session-id live"
  exit 0
fi

SESSION="vyrex-walkthrough"
...
tmux split-window -h -t "$SESSION" \
  "openshell sandbox connect vyrex -- bash -c 'vyrex-start openclaw agent --agent main --local --session-id live'"
