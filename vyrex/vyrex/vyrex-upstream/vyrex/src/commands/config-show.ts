...
/**
 * Slash command handler for `/vyrex config`.
 *
...
export function slashConfigShow(): PluginCommandResult {
  ...
  if (!config) {
    return {
      text: [
        "**Vyrex Config**",
        "",
        "No configuration found. Run `vyrex onboard` from the host to configure.",
      ].join("\n"),
    };
  }
  ...
  const lines = [
    ...
    "Use `vyrex config get <sandbox>` for the full sandbox config.",
  ];
