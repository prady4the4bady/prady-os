...
import type { VyrexOnboardConfig } from "../onboard/config.js";
...
vi.mock("./config-show.js", () => ({
  slashConfigShow: vi.fn(() => ({ text: "**Vyrex Config**" })),
}));
...
describe("commands/slash", () => {
  ...
  describe("help", () => {
    it("returns help text for empty args", () => {
      ...
      expect(result.text).toContain("Vyrex");
  ...
  describe("eject", () => {
    it("reports nothing to eject when state is blank", () => {
      ...
      expect(result.text).toContain("No Vyrex deployment found");
    });
    ...
    it("shows eject instructions when migration snapshot exists", () => {
      ...
      const result = handleSlashCommand(makeCtx("eject"), makeApi());
      expect(result.text).toContain("Eject from Vyrex");
      expect(result.text).toContain("vyrex <name> destroy");
  ...
  describe("onboard", () => {
    it("shows setup instructions when no config exists", () => {
      ...
      expect(result.text).toContain("vyrex onboard");
    });
    ...
    it("shows onboard status when config exists", () => {
      ...
      const result = handleSlashCommand(makeCtx("onboard"), makeApi());
      expect(result.text).toContain("Vyrex Onboard Status");
    ...
    it("includes NCP partner when set", () => {
      const config: VyrexOnboardConfig = {
        endpointType: "ncp",
