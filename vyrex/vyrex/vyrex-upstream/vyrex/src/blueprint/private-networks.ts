...
// CIDR set from vyrex-blueprint/private-networks.yaml and builds a
...
// VYREX_BLUEPRINT_PATH when set, otherwise try the dev-checkout
...
// exist, so VYREX_BLUEPRINT_PATH (set by the CLI launcher) or a
...
function resolveBlueprintPath(): string {
  const fromEnv = process.env.VYREX_BLUEPRINT_PATH;
  if (fromEnv) return fromEnv;
  const here = dirname(fileURLToPath(import.meta.url));
  const devGuess = join(here, "..", "..", "..", "vyrex-blueprint");
  ...
  if (!existsSync(source)) {
    throw new Error(
      `private-networks.yaml not found at ${source}. ` +
        `Set VYREX_BLUEPRINT_PATH to the directory containing the blueprint, ` +
        `or run from a checkout that includes vyrex-blueprint/.`,
    );
  }
 ...
 *
 * Intended for user-input boundaries (e.g. `vyrex config set`) where
