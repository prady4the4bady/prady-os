...
const STATE_DIR = join(homedir(), ".vyrex", "state");

export interface VyrexState {
...
function readStatePatch(value: unknown): Partial<VyrexState> {
  if (!isRecord(value)) {
    return {};
  }

  const patch: Partial<VyrexState> = {};
...
function statePath(): string {
  return join(STATE_DIR, "vyrex.json");
}
...
export function loadState(): VyrexState {
  ensureStateDir();
  ...
  try {
    ...
    // were added still return valid VyrexState with sensible defaults.
...
export function saveState(state: VyrexState): void {
  ensureStateDir();
