...
describe("private-networks loader", () => {
  beforeEach(() => {
    store.clear();
    existsCalls = [];
    delete process.env.VYREX_BLUEPRINT_PATH;
    resetCache();
  });

  describe("resolveBlueprintPath", () => {
    it("honours VYREX_BLUEPRINT_PATH when set", () => {
      process.env.VYREX_BLUEPRINT_PATH = "/custom/path";
    ...
    it("throws a descriptive error when the YAML is missing", () => {
      process.env.VYREX_BLUEPRINT_PATH = "/missing/path";
      expect(() => getNetworkEntries()).toThrow(
        /private-networks\.yaml not found at \/missing\/path\/private-networks\.yaml.*VYREX_BLUEPRINT_PATH/s,
      );
    });
  ...
  describe("schema validation", () => {
    beforeEach(() => {
      process.env.VYREX_BLUEPRINT_PATH = "/blueprint";
    });
