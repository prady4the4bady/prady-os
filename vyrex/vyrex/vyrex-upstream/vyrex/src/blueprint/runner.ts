...
export function loadBlueprint(): Blueprint {
  const blueprintPath = process.env.VYREX_BLUEPRINT_PATH ?? ".";
...
export async function actionApply(
  ...
  const stateDir = join(homedir(), ".vyrex", "state", "runs", rid);
...
export function actionStatus(rid?: string): void {
  emitRunId();
  const runsDir = join(homedir(), ".vyrex", "state", "runs");
