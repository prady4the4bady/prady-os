...
const VYREX_DIR = join(HOME, ".vyrex");
const SNAPSHOTS_DIR = join(VYREX_DIR, "snapshots");
...
export function cutoverHost(): boolean {
  ...
  const archivePath = join(HOME, `.openclaw.pre-vyrex.${compactTimestamp()}`);
  try {
...
export function rollbackFromSnapshot(snapshotDir: string): boolean {
  ...
  const archivePath = existsSync(OPENCLAW_DIR)
    ? join(HOME, `.openclaw.vyrex-archived.${compactTimestamp()}`)
    : null;
