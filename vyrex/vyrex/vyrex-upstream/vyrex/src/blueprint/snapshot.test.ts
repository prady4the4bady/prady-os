...
const SNAPSHOTS_DIR = `${FAKE_HOME}/.vyrex/snapshots`;
...
describe("snapshot", () => {
  ...
  describe("createSnapshot", () => {
    ...
    it("rejects when an ancestor of ~/.vyrex is a symlink", () => {
      addDir(OPENCLAW_DIR);
      addSymlink(`${FAKE_HOME}/.vyrex`, "/attacker-controlled");
  ...
  describe("cutoverHost", () => {
    ...
    it("renames ~/.openclaw to archive path", () => {
      ...
      // Archived under a .openclaw.pre-vyrex.* name
      const archived = [...store.keys()].find((k) => k.includes(".openclaw.pre-vyrex."));
  ...
  describe("rollbackFromSnapshot", () => {
    ...
    it("archives existing ~/.openclaw before restoring", () => {
      ...
      const archived = [...store.keys()].find((k) => k.includes(".openclaw.vyrex-archived."));
      expect(archived).toBeDefined();
    });
