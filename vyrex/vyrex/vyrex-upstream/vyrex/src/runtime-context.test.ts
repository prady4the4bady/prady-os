...
import type { VyrexState } from "./blueprint/state.js";
...
const { loadState } = (await import("./blueprint/state.js")) as { loadState: () => VyrexState };
...
const defaultConfig: VyrexConfig = {
  blueprintVersion: "latest",
  blueprintRegistry: "ghcr.io/nvidia/vyrex-blueprint",
  sandboxName: "openclaw",
  inferenceProvider: "nvidia",
};

function blankState(): VyrexState {
  return {
    lastRunId: null,
    lastAction: null,
...
describe("registerRuntimeContext", () => {
  ...
  describe("first call — full context injection", () => {
    ...
    it("prependContext opens and closes vyrex-runtime tags", async () => {
      const { api } = makeMockApi();
      registerRuntimeContext(api, defaultConfig);
      ...
      expect(result.prependContext).toContain("<vyrex-runtime>");
      expect(result.prependContext).toContain("</vyrex-runtime>");
    });
  ...
  describe("delta update — fingerprint changes between calls", () => {
    it("sends a vyrex-runtime-update block when phase changes", async () => {
      const { api } = makeMockApi();
      registerRuntimeContext(api, defaultConfig);
      const key = { sessionKey: nextSessionKey() };
      ...
      expect(result.prependContext).toContain("<vyrex-runtime-update>");
  ...
  describe("error fallback", () => {
    it("returns minimal static context and logs a warning when getCachedRuntimeInjection throws", async () => {
      ...
      expect(
        warnMessages.some((m) => m.includes("vyrex runtime context injection failed")),
      ).toBe(true);
    });
