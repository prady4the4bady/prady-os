...
import type { VyrexConfig, OpenClawPluginApi } from "./index.js";
 ...
 */
function getSandboxName(pluginConfig: VyrexConfig): string {
 ...
 */
async function getRuntimeFingerprint(pluginConfig: VyrexConfig): Promise<RuntimeFingerprint> {
...
/**
 * Assembles the full `<vyrex-runtime>` context block for injection at the
 * start of an agent session.
 */
function buildRuntimeContextText(summary: RuntimeSummary): string {
  const lines = [
    "<vyrex-runtime>",
    `You are running inside OpenShell sandbox "${summary.sandboxName}" via Vyrex.`,
...
/**
 * Assembles a `<vyrex-runtime-update>` delta block when the sandbox state
...
function buildRuntimeDeltaText(
  ...
  const lines = [
    "<vyrex-runtime-update>",
    "OpenShell sandbox state changed since your earlier Vyrex context.",
  ];
  ...
  lines.push("</vyrex-runtime-update>");
  return lines.join("\n");
}
 ...
 */
export async function getRuntimeSummary(pluginConfig: VyrexConfig): Promise<RuntimeSummary> {
  try {
    const fingerprint = await getRuntimeFingerprint(pluginConfig);
...
/**
 * Registers a `before_agent_start` hook that prepends a `<vyrex-runtime>`
 * context block (or a `<vyrex-runtime-update>` delta) to each agent turn.
 *
 ...
 */
export function registerRuntimeContext(api: OpenClawPluginApi, pluginConfig: VyrexConfig): void {
  api.on("before_agent_start", async (_event: unknown, hookContext: unknown) => {
    ...
    try {
      ...
      api.logger.warn(`vyrex runtime context injection failed: ${String(err)}`);
      return {
        prependContext: [
          "<vyrex-runtime>",
          `You are running inside OpenShell sandbox "${activeSandbox}" via Vyrex.`,
