...
// ---------------------------------------------------------------------------
// Agent branding — derived from VYREX_AGENT when an alias launcher sets it;
...
const SANDBOX_EXEC_STARTED_MARKER = "__VYREX_SANDBOX_EXEC_STARTED__";
...
const VYREX_GATEWAY_NAME = "vyrex";
const DASHBOARD_FORWARD_PORT = String(DASHBOARD_PORT);
const DEFAULT_LOGS_PROBE_TIMEOUT_MS = 5000;
const LOGS_PROBE_TIMEOUT_ENV = "VYREX_LOGS_PROBE_TIMEOUT_MS";
...
function cleanupGatewayAfterLastSandbox() {
  ...
  runOpenshell(["gateway", "destroy", "-g", VYREX_GATEWAY_NAME], { ignoreError: true });
  dockerRemoveVolumesByPrefix(`openshell-cluster-${VYREX_GATEWAY_NAME}`, {
    ignoreError: true,
  });
}
...
function executeSandboxCommand(sandboxName: string, command: string): SandboxCommandResult | null {
  ...
  const tmpFile = path.join(os.tmpdir(), `vyrex-ssh-${process.pid}-${Date.now()}.conf`);
...
function executeSandboxExecCommand(
  ...
  const timeoutOverride = Number(process.env.VYREX_SANDBOX_EXEC_TIMEOUT_MS || "");
...
function waitForRecoveredSandboxGateway(sandboxName: string): boolean {
  const timeoutSeconds = readNonNegativeNumberEnv(
    "VYREX_GATEWAY_RECOVERY_WAIT_SECONDS",
    30,
  );
  const intervalSeconds = readNonNegativeNumberEnv(
    "VYREX_GATEWAY_RECOVERY_POLL_INTERVAL_SECONDS",
    3,
  );
  const attempts =
    intervalSeconds > 0
...
function hasNamedGateway(output = ""): boolean {
  return stripAnsi(output).includes("Gateway: vyrex");
}
...
function getNamedGatewayLifecycleState() {
  ...
  const gatewayInfo = captureOpenshell(["gateway", "info", "-g", "vyrex"], {
    timeout: OPENSHELL_PROBE_TIMEOUT_MS,
  });
  ...
  if (connected && activeGateway === "vyrex" && named) {
    return {
      state: "healthy_named",
  ...
  if (activeGateway === "vyrex" && named && refusing) {
    return {
...
/** Attempt to recover the named Vyrex gateway after a restart or connectivity loss. */
async function recoverNamedGatewayRuntime() {
  ...
  runOpenshell(["gateway", "select", "vyrex"], {
    ignoreError: true,
...
/**
 * Reconcile a NotFound sandbox lookup against the named Vyrex gateway state.
 * When the active OpenShell gateway has drifted off vyrex, a NotFound is
 * ambiguous: the sandbox may actually be registered against the vyrex
 * gateway but invisible because some other gateway is currently active. This
 * helper self-heals by attempting `openshell gateway select vyrex` and
...
/**
 * Print actionable guidance when the vyrex gateway exists but another
...
function printWrongGatewayActiveGuidance(
  ...
  const other = activeGateway && activeGateway !== "vyrex" ? activeGateway : "another gateway";
  writer(
  ...
  writer("      openshell gateway select vyrex");
...
function printGatewayLifecycleHint(output = "", sandboxName = "", writer = console.error) {
  ...
  if (/No gateway configured/i.test(cleanOutput)) {
    ...
    writer(
      "  Start the gateway again with `openshell gateway start --name vyrex` before expecting existing sandboxes to reconnect.",
    );
    writer(
  ...
  if (
    ...
    /Gateway:\s+vyrex/i.test(cleanOutput)
  ) {
    writer(
      "  The selected Vyrex gateway exists in metadata, but its API is refusing connections after restart.",
    );
    writer("  This usually means the gateway runtime did not come back cleanly after the restart.");
    writer(
      "  Retry `openshell gateway start --name vyrex`; if it stays in this state, rebuild the gateway before expecting existing sandboxes to reconnect.",
    );
    return;
  }
...
async function getReconciledSandboxGatewayState(
  ...
  if (lookup.state === "gateway_error") {
    ...
    if (
      ...
      /Gateway:\s+vyrex/i.test(latestStatus)
    ) {
      return {
...
async function ensureLiveSandboxOrExit(
  ...
  if (lookup.state === "missing") {
    // Belt-and-suspenders: only destroy registry state if the vyrex gateway
  ...
  if (lookup.state === "gateway_unreachable_after_restart") {
    ...
    console.error(
      "  Retry `openshell gateway start --name vyrex` and verify `openshell status` is healthy before reconnecting.",
    );
    console.error(
  ...
  if (lookup.state === "gateway_missing_after_restart") {
    ...
    console.error(
      "  Start the gateway again with `openshell gateway start --name vyrex` before retrying.",
    );
    console.error(
...
async function sandboxConnect(
  ...
  const rawTimeout = process.env.VYREX_CONNECT_TIMEOUT;
  let timeout = 120;
  if (rawTimeout !== undefined) {
    const parsed = parseInt(rawTimeout, 10);
    if (Number.isNaN(parsed) || parsed <= 0) {
      console.warn(
        `  Warning: invalid VYREX_CONNECT_TIMEOUT="${rawTimeout}", using default 120s`,
      );
    } else {
      timeout = parsed;
    }
  ...
  if (!isSandboxReady(list, sandboxName)) {
    ...
    if (!ready) {
      ...
      console.error(`  Check: openshell sandbox list`);
      console.error(
        `  Override timeout: VYREX_CONNECT_TIMEOUT=300 ${CLI_NAME} ${sandboxName} connect`,
      );
      process.exit(1);
    }
...
function dockerInspectGateway(containerName: string): DoctorCheck[] {
  ...
  if (inspect.status !== 0) {
    checks.push({
      ...
      hint: "run `docker ps --filter name=openshell-cluster-vyrex`",
    });
    return checks;
  }
  ...
  checks.push({
    ...
    hint: running ? undefined : "restart the gateway with `openshell gateway start --name vyrex`",
  });
  ...
  if (port.status === 0 && port.stdout.trim()) {
    ...
    checks.push({
      ...
      hint: mapping.includes(`:${GATEWAY_PORT}`)
        ? undefined
        : `expected host port ${GATEWAY_PORT} from VYREX_GATEWAY_PORT`,
    });
  } else {
    checks.push({
      group: "Gateway",
...
function cloudflaredDoctorCheck(sandboxName: string): DoctorCheck {
  const pidFile = path.join(`/tmp/vyrex-services-${sandboxName}`, "cloudflared.pid");
...
async function sandboxDoctor(sandboxName: string, args: string[] = []): Promise<void> {
  ...
  checks.push({
    group: "Host",
    label: "CLI build",
    status: fs.existsSync(path.join(ROOT, "dist", "vyrex.js")) ? "ok" : "fail",
    detail: fs.existsSync(path.join(ROOT, "dist", "vyrex.js"))
      ? "dist/vyrex.js present"
      : "dist/vyrex.js missing",
    hint: fs.existsSync(path.join(ROOT, "dist", "vyrex.js")) ? undefined : "run `npm run build:cli`",
  });
  ...
  checks.push(...dockerInspectGateway(`openshell-cluster-${VYREX_GATEWAY_NAME}`));
  ...
  if (openshellBin) {
    ...
    checks.push({
      ...
      detail: openshellConnected
        ? "connected to vyrex"
        : oneLine(cleanStatus || lifecycle?.gatewayInfo || "not connected to vyrex"),
      hint: openshellConnected ? undefined : "run `openshell gateway select vyrex` and retry",
    });
  }
  ...
  if (openshellBin && openshellConnected) {
    ...
    checks.push({
      ...
      label: "Live sandbox",
      status: "fail",
      detail: "skipped because the vyrex gateway is not connected",
...
/**
 * Handle `vyrex <sandbox> policy-add [flags]`. Supports three mutually
 ...
 * `--yes`/`-y`/`--force` (or `VYREX_NON_INTERACTIVE=1`) skips the
...
async function sandboxPolicyAdd(sandboxName: string, args: string[] = []): Promise<void> {
  ...
  const skipConfirm =
    ...
    args.includes("-y") ||
    args.includes("--force") ||
    process.env.VYREX_NON_INTERACTIVE === "1";
  ...
  if (presetArg) {
    ...
    if (process.env.VYREX_NON_INTERACTIVE === "1") {
...
async function applyChannelAddToGatewayAndRegistry(
  ...
  if (!recovery.recovered) {
    ...
    console.error("  'openshell gateway start --name vyrex' manually.");
    process.exit(1);
  }
  ...
  // `vyrex <sandbox> rebuild` knows the channel set without needing
  // tokens on disk.
...
async function applyChannelRemoveToGatewayAndRegistry(
  ...
  if (!recovery.recovered) {
    ...
    console.error(
      "  Re-run after starting the gateway, or run 'openshell gateway start --name vyrex'.",
    );
    process.exit(1);
  }
  // Capture each delete's outcome. If any non-NotFound failure surfaces
  // we must NOT update the registry — otherwise Vyrex would record
...
async function sandboxChannelsRemove(sandboxName: string, args: string[] = []): Promise<void> {
  ...
  // not leave a stale bridge running against a token Vyrex has
...
function printPluginInstallHint(): void {
  ...
  console.error(
    "  To use an OpenClaw plugin today, bake it into a custom sandbox image with `vyrex onboard --from <Dockerfile>`.",
  );
}
...
async function sandboxSkillInstall(sandboxName: string, args: string[] = []): Promise<void> {
  ...
  const tmpSshConfig = path.join(
    os.tmpdir(),
    `vyrex-ssh-skill-${process.pid}-${Date.now()}.conf`,
  );
...
function cleanupSandboxServices(
  ...
  try {
    fs.rmSync(`/tmp/vyrex-services-${sandboxName}`, { recursive: true, force: true });
  } catch {
...
async function sandboxRebuild(
  ...
  const verbose =
    ...
    args.includes("-v") ||
    process.env.VYREX_REBUILD_VERBOSE === "1";
  ...
  if (session && session.sandboxName && session.sandboxName !== sandboxName) {
    ...
    // wrong (e.g. lumyn session while rebuilding openclaw).  Skip the
  ...
  process.env.VYREX_SANDBOX_NAME = sandboxName;
  ...
  log(
    `Env: VYREX_SANDBOX_NAME=${process.env.VYREX_SANDBOX_NAME}, VYREX_RECREATE_SANDBOX=${process.env.VYREX_RECREATE_SANDBOX}`,
  );
  ...
  // Lumyn: no explicit post-restore step needed. Lumyn's SessionDB._init_schema()
  // auto-migrates state.db (SQLite) on first connection via sequential ALTER TABLE
  // migrations (idempotent, schema_version tracked). ensure_lumyn_home() repairs
  // missing directories implicitly. The Vyrex plugin's skill cache refreshes on
...
function resolveSrcPodImage(srcName: string): string | null {
  const gatewayContainer = `openshell-cluster-${VYREX_GATEWAY_NAME}`;
  try {
...
async function autoCreateSandboxFromSource(
  ...
  const basePolicy = path.join(ROOT, "vyrex-blueprint", "policies", "openclaw-sandbox.yaml");
  ...
  if (fs.existsSync(dnsScript)) {
    run(["bash", dnsScript, VYREX_GATEWAY_NAME, dstName], { ignoreError: true });
  }

  // Register dst in the Vyrex registry, cloning most fields from src.
...
function probeGatewayRunning(): boolean {
  const container = `openshell-cluster-${VYREX_GATEWAY_NAME}`;
 ...
 * Vyrex or OpenShell so sandbox state is recoverable if the upgrade
...
function backupAll() {
  ...
  if (backed > 0) {
    console.log(`  Backups stored in: ~/.vyrex/rebuild-backups/`);
  }
...
const mainPromise = (async () => {
  ...
  // Sandbox-scoped commands: vyrex <name> <action>
