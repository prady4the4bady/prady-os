...
// agent runs in each sandbox (openclaw, lumyn, etc.), and agent-defs.ts
...
//   - lumyn:   /sandbox/.lumyn/config.yaml      (YAML)
//
...
function resolveAgentConfig(sandboxName: string): AgentConfigTarget {
  try {
    ...
    // Lumyn stores credentials in .env alongside the config
    if (entry.agent === "lumyn") sensitiveFiles.push(`${dir}/.env`);
...
function getOpenshellBinary(): string {
  return process.env.VYREX_OPENSHELL_BIN || "openshell";
}
...
function parseConfigGetArgs(args: string[], cliName = "vyrex"): ConfigGetParseResult {
