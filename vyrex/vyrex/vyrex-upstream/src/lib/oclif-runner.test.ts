...
function makeConfig() {
  const rootPlugin = {
    root: "/repo",
    pjson: { oclif: { bin: "vyrex" } },
    options: { pjson: { oclif: { bin: "vyrex" } } },
  };
  return {
    root: "/repo",
    bin: "vyrex",
...
describe("runRegisteredOclifCommand", () => {
  ...
  it("loads the oclif config, applies branded bin metadata, and runs the command", async () => {
    ...
    expect(config.bin).toBe("vyrex");
    expect(config.pjson.oclif.bin).toBe("vyrex");
    expect(config.options.pjson.oclif.bin).toBe("vyrex");
    expect(config.plugins.get("root")?.pjson.oclif.bin).toBe("vyrex");
  });
