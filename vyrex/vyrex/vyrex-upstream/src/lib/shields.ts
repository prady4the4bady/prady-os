...
const STATE_DIR = path.join(process.env.HOME ?? "/tmp", ".vyrex", "state");
...
const K3S_CONTAINER = "openshell-cluster-vyrex";
...
const HIGH_RISK_STATE_DIRS = [
  ...
  "agents",
  "extensions",
  "plugins", // Lumyn equivalent of extensions
  "workspace",
  "memory",
  "credentials",
  "identity",
...
// user and the gateway UID can write the mutable config tree. Lumyn keeps its
...
function unlockAgentConfig(
  ...
  // Lumyn is unchanged — its sandbox does not run a separate gateway UID,
  // so the shared-group contract does not apply.
  const fileMode = target.agentName === "lumyn" ? "640" : "660";
  const dirMode = target.agentName === "lumyn" ? "750" : "2770";
...
function shieldsDown(sandboxName: string, opts: ShieldsDownOpts = {}): void {
  ...
  if (state.shieldsDown) {
    ...
    console.error("  Run `vyrex shields up` first, or use --extend (not yet implemented).");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(`  Re-run \`vyrex ${sandboxName} shields down\` after correcting file ownership.`);
    process.exit(1);
  }
  ...
  try {
    ...
    if (rollbackLocked) {
      ...
      console.error(
        `  Re-lock manually via kubectl exec, then run: vyrex ${sandboxName} shields up`,
      );
    }
    process.exit(1);
  }
  ...
  console.log(`  Run \`vyrex ${sandboxName} shields up\` to opt into lockdown.`);
}
...
function shieldsStatus(sandboxName: string): void {
  ...
  switch (mode) {
    case "mutable_default":
      ...
      console.log("  Config is mutable. Run `vyrex <sandbox> shields up` to opt into lockdown.");
      return;
