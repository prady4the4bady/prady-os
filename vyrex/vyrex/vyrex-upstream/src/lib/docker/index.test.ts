...
describe("docker helpers", () => {
  ...
  it("filters docker volume names by exact prefix", () => {
    runCaptureMock.mockReturnValue(
      [
        ...
        "openshell-cluster-vyrex-cache",
        "openshell-cluster-vyrex-2",
        "not-a-match",
        "",
      ].join("\n"),
    );

    const names = dockerListVolumesByPrefix("openshell-cluster-vyrex");
  ...
  it("removes only volumes returned by the prefix probe", () => {
    runCaptureMock.mockReturnValue("openshell-cluster-vyrex\nopenshell-cluster-vyrex-cache\n");

    const removed = dockerRemoveVolumesByPrefix("  openshell-cluster-vyrex  ", {
      ignoreError: true,
    });
  ...
  it("treats failed volume probes as empty when ignoreError is set", () => {
    ...
    expect(dockerRemoveVolumesByPrefix("openshell-cluster-vyrex", { ignoreError: true })).toEqual(
      [],
    );
