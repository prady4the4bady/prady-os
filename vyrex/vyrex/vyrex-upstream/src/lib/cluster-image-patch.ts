...
// roadmap migration off k3s lands. Vyrex downstream cannot wait. This
...
const TAG_PREFIX = "vyrex-cluster";
 ...
 * would let the same Vyrex tag produce different image bytes whenever
...
export function buildPatchDockerfile(snapshotter: SnapshotterChoice): string {
  ...
  if (snapshotter === "native") {
    ...
    return [
      "# Generated by Vyrex — see src/lib/cluster-image-patch.ts.",
      "ARG UPSTREAM",
      "FROM ${UPSTREAM}",
...
export function ensurePatchedClusterImage(opts: EnsurePatchedClusterImageOpts): string {
  ...
  const tmpDir = fsApi.mkdtempSync(path.join(tmpdirImpl(), "vyrex-cluster-patch-"));
  try {
