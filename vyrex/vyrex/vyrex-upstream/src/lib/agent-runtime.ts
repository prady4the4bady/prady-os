...
//
// Agent-specific runtime logic — called from vyrex.ts when the active
...
export function buildOpenClawRecoveryScript(port: number): string {
  ...
  return [
    "if [ -r /tmp/vyrex-proxy-env.sh ]; then . /tmp/vyrex-proxy-env.sh; _PE_MISSING=0; else _PE_MISSING=1; fi;",
    "[ -f ~/.bashrc ] && . ~/.bashrc;",
    'if [ "$_PE_MISSING" = "0" ]; then case "${NODE_OPTIONS:-}" in *vyrex-sandbox-safety-net*) _SN_MISSING=0 ;; *) _SN_MISSING=1 ;; esac; case "${NODE_OPTIONS:-}" in *vyrex-ciao-network-guard*) _CIAO_MISSING=0 ;; *) _CIAO_MISSING=1 ;; esac; if [ "$_SN_MISSING" = "0" ] && [ "$_CIAO_MISSING" = "0" ]; then _GUARDS_MISSING=0; else _GUARDS_MISSING=1; fi; else _GUARDS_MISSING=0; fi;',
    ...
    '[ "$_PE_MISSING" = "1" ] && { _W="[gateway-recovery] WARNING: /tmp/vyrex-proxy-env.sh missing - gateway launching without library guards (#2478)"; echo "$_W" >&2; echo "$_W" >> "$_GATEWAY_LOG"; };',
    '[ "$_PE_MISSING" = "0" ] && [ "$_GUARDS_MISSING" = "1" ] && { _E="[gateway-recovery] ERROR: /tmp/vyrex-proxy-env.sh present but NODE_OPTIONS missing safety-net preload or ciao preload - refusing unguarded gateway relaunch (#2478)"; echo "$_E" >&2; echo "$_E" >> "$_GATEWAY_LOG"; exit 1; };',
...
export function buildRecoveryScript(agent: AgentDefinition | null, port: number): string | null {
  ...
  const isLumyn = agent.name === "lumyn";
  const lumynHome = isLumyn ? "export LUMYN_HOME=/sandbox/.lumyn; " : "";

  // Source /tmp/vyrex-proxy-env.sh immediately before launching. That file
