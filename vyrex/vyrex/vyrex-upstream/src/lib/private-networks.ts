...
// equivalent module at vyrex/src/blueprint/private-networks.ts; the
...
const NETWORKS_FILE = path.join(ROOT, "vyrex-blueprint", "private-networks.yaml");
  ...
  if (!fs.existsSync(NETWORKS_FILE)) {
    throw new Error(
      `private-networks.yaml not found at ${NETWORKS_FILE}. ` +
        ...
        `so the checkout must include vyrex-blueprint/private-networks.yaml ` +
        `(the plugin has a separate VYREX_BLUEPRINT_PATH override).`,
    );
  }
