...
function getNonInteractiveProvider() {
  const providerKey = (process.env.VYREX_PROVIDER || "").trim().toLowerCase();
  if (!providerKey) return null;
  const aliases = {
    cloud: "build",
    nim: "nim-local",
  ...
  if (!validProviders.has(normalized)) {
    console.error(`  Unsupported VYREX_PROVIDER: ${providerKey}`);
    console.error(
...
function getNonInteractiveModel(providerKey) {
  const model = (process.env.VYREX_MODEL || "").trim();
  if (!model) return null;
  if (!isSafeModelId(model)) {
    console.error(`  Invalid VYREX_MODEL for provider '${providerKey}': ${model}`);
