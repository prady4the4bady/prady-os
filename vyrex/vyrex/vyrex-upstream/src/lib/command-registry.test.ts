...
describe("command-registry", () => {
  ...
  describe("hidden commands", () => {
    it("exactly 9 hidden commands: help/version aliases + shields + config", () => {
      ...
      expect(usages).toEqual([
        ...
        "vyrex <name> shields down",
        "vyrex <name> shields status",
        "vyrex <name> shields up",
        "vyrex help",
      ]);
    });
  });
  ...
  describe("deprecated commands", () => {
    it("should include setup, setup-spark, deploy, start, stop", () => {
      ...
      expect(usages).toContain("vyrex setup");
      expect(usages).toContain("vyrex setup-spark");
      expect(usages).toContain("vyrex deploy");
      expect(usages).toContain("vyrex start");
      expect(usages).toContain("vyrex stop");
    });
  });
  ...
  describe("canonicalUsageList()", () => {
    ...
    it("every entry starts with vyrex", () => {
      for (const entry of canonicalUsageList()) {
        expect(entry).toMatch(/^vyrex /);
      }
    });
    ...
    it("excludes hidden commands", () => {
      const list = canonicalUsageList();
      expect(list).not.toContain("vyrex <name> shields down");
      expect(list).not.toContain("vyrex <name> config get");
    });
  });
