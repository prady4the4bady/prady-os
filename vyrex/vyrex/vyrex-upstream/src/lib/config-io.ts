...
function buildRemediation(): string {
  ...
  const backupDir = `${vyrexDir}.backup.${String(process.pid)}`;
  const recoveryHome = path.join(
    os.tmpdir(),
    `vyrex-home-${String(process.getuid?.() ?? "user")}`,
  );
  ...
  return [
    ...
    `    sudo chown -R $(whoami) ${shellQuote(vyrexDir)}`,
    "    # or recreate it if it was created by another user:",
    `    sudo rm -rf ${shellQuote(vyrexDir)} && vyrex onboard`,
    "",
    "    # If sudo is unavailable, move the bad config aside from a writable HOME:",
    `    mv ${shellQuote(vyrexDir)} ${shellQuote(backupDir)} && vyrex onboard`,
    "    # or, if you already own the directory, remove it without sudo:",
    `    rm -rf ${shellQuote(vyrexDir)} && vyrex onboard`,
    "",
    "    # If HOME itself is not writable, start Vyrex with a writable HOME:",
    `    mkdir -p ${shellQuote(recoveryHome)} && HOME=${shellQuote(recoveryHome)} vyrex onboard`,
    "",
    "  This usually happens when Vyrex was first run with sudo",
 ...
 * This prevents an attacker from planting e.g. ~/.vyrex as a symlink to an
