...
describe("buildDeployEnvLines", () => {
  it("includes standard non-interactive deploy env plus passthrough values", () => {
    ...
    expect(envLines).toContain("VYREX_NON_INTERACTIVE=1");
    expect(envLines).toContain("VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1");
    expect(envLines).toContain("VYREX_SANDBOX_NAME='my-assistant'");
    expect(envLines).toContain("VYREX_PROVIDER='build'");
    expect(envLines).toContain("CHAT_UI_URL='https://chat.example.com'");
    expect(envLines).toContain("VYREX_POLICY_MODE='suggested'");
