...
const REBUILD_BACKUPS_DIR = path.join(HOME_DIR, ".vyrex", "rebuild-backups");
 ...
 * vyrex/src/blueprint/snapshot.ts.
 */
    ...
    for (const entry of readdirSync(current, { withFileTypes: true })) {
      ...
      try {
        ...
        if (stat.isSymbolicLink()) {
          ...
          // directory (/sandbox/.openclaw-data/** or /sandbox/.lumyn-data/**),
          ...
          const SANDBOX_DATA_PREFIXES = ["/sandbox/.openclaw-data/", "/sandbox/.lumyn-data/"];
...
function writeTempSshConfig(sshConfig: string): string {
  const tmpFile = path.join(os.tmpdir(), `vyrex-state-${process.pid}-${Date.now()}.conf`);
...
function computeBlueprintDigest(): string | null {
  ...
  const candidates = [
    path.join(process.env.HOME || "/tmp", ".vyrex", "blueprints", "0.1.0", "blueprint.yaml"),
    path.join(__dirname, "..", "..", "vyrex-blueprint", "blueprint.yaml"),
  ];
    ...
    for (const entry of readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        if (isSensitiveFile(entry.name)) {
          ...
          // Lumyn stores API keys in .env alongside config.yaml.
          try {
...
const _verbose = () => process.env.VYREX_REBUILD_VERBOSE === "1";
...
export function backupSandboxState(sandboxName: string, options: BackupOptions = {}): BackupResult {
  ...
  // Without this check, an attacker who plants ~/.vyrex/rebuild-backups
...
// ── CLI argv parser ────────────────────────────────────────────────
//
// Argument parser for `vyrex <name> snapshot restore [selector] [--to <dst>]`.
