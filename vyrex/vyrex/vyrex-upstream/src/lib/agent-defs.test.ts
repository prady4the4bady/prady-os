...
afterEach(() => {
  vi.restoreAllMocks();
  delete process.env.VYREX_AGENT;
...
describe("agent definitions", () => {
  it("loads computed OpenClaw manifest properties", () => {
    ...
    expect(openclaw.legacyPaths?.startScript).toContain("scripts/vyrex-start.sh");
  });

  it("loads Lumyn manifest properties without falling back to OpenClaw defaults", () => {
    const lumyn = loadAgent("lumyn");

    expect(lumyn.name).toBe("lumyn");
    expect(lumyn.displayName).toBe("Lumyn Agent");
    expect(lumyn.hasDevicePairing).toBe(false);
    expect(lumyn.configPaths).toEqual({
      dir: "/sandbox/.lumyn",
      configFile: "config.yaml",
      envFile: ".env",
      format: "yaml",
    });
    expect(lumyn.healthProbe.url).toBe("http://localhost:8642/health");
    expect(lumyn.messagingPlatforms).toEqual(["telegram", "discord", "slack"]);
  });

  it("orders OpenClaw first in interactive choices", () => {
    const choices = getAgentChoices();
    expect(choices[0]?.name).toBe("openclaw");
    expect(choices.map((choice) => choice.name)).toContain("lumyn");
  });
