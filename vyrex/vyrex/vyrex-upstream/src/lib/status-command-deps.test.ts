...
describe("buildStatusCommandDeps", () => {
  ...
  beforeEach(() => {
    previousOverride = process.env.VYREX_OPENSHELL_BIN;
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-status-deps-"));
    callsFile = path.join(tmp, "openshell.calls");
    openshell = path.join(tmp, "openshell");
    process.env.VYREX_OPENSHELL_BIN = openshell;
  });
  ...
  afterEach(() => {
    if (previousOverride === undefined) {
      ...
      process.env.VYREX_OPENSHELL_BIN = previousOverride;
    }
