...
/**
 * `vyrex <name> share mount|unmount|status` — SSHFS-based sandbox file sharing.
 *
...
export function defaultShareMountDir(sandboxName: string): string {
  return path.join(process.env.HOME || os.homedir(), ".vyrex", "mounts", sandboxName);
}
...
export default class ShareCommand extends Command {
  ...
  private async mount(
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-sshfs-"));
