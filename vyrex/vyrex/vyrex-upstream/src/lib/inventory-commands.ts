...
/**
 * Render the `vyrex list` output. For the default sandbox (the one the
...
/**
 * Render the `vyrex status` output (no sandbox name): a compact per-row
  ...
  if (deps.checkMessagingBridgeHealth && defaultSandbox) {
    ...
    if (Array.isArray(channels) && channels.length > 0) {
      ...
      if (degraded.length > 0) {
        ...
        // Surface gateway log tail for Lumyn sandboxes when messaging is degraded.
        if (deps.readGatewayLog && defaultEntry?.agent === "lumyn") {
