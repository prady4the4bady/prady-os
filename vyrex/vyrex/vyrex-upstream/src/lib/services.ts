...
export interface ServiceOptions {
  ...
  repoDir?: string;
  /** Override PID directory (default: /tmp/vyrex-services-{sandbox}). */
  pidDir?: string;
}
...
function resolvePidDir(opts: ServiceOptions): string {
  const sandbox = validateSandboxName(
    opts.sandboxName ?? process.env.VYREX_SANDBOX ?? process.env.SANDBOX_NAME ?? "default",
  );
  return opts.pidDir ?? `/tmp/vyrex-services-${sandbox}`;
}
...
const GATEWAY_CLUSTER_CONTAINER = "openshell-cluster-vyrex";
...
export function stopAll(opts: ServiceOptions = {}): void {
  ...
  if (sandboxName) {
    ...
    warn("Hint: run 'vyrex stop' with a registered sandbox or set VYREX_SANDBOX_NAME.");
  }
