...
function collectDocker(collectDir: string, quick: boolean): void {
  ...
  // Vyrex-labelled containers
  if (commandExists("docker")) {
    try {
      const output = dockerExecFileSync(
        ["ps", "-a", "--filter", "label=com.nvidia.vyrex", "--format", "{{.Names}}"],
...
function collectSandboxInternals(
  ...
  const sshConfigPath = join(tmpdir(), `vyrex-ssh-${String(Date.now())}`);
  try {
...
export function getDebugCompletionMessages(output?: string): string[] {
  ...
  return [
    ...
    "  vyrex debug --output /tmp/vyrex-debug.tar.gz",
  ];
}
...
/**
 * Collect local and sandbox diagnostics for a Vyrex environment and
...
export function runDebug(opts: DebugOptions = {}): void {
  ...
  let sandboxName =
    opts.sandboxName ?? process.env.VYREX_SANDBOX ?? process.env.SANDBOX_NAME ?? "";
  if (!sandboxName) {
    sandboxName = detectSandboxName();
  }

  // Create temp collection directory
  const collectDir = mkdtempSync(join(tmpdir(), "vyrex-debug-"));
