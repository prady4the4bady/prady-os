...
/**
 * Preflight checks for Vyrex onboarding: port availability, memory
...
export function planHostRemediation(assessment: HostAssessment): RemediationAction[] {
  ...
  if (!assessment.dockerInstalled) {
    const installCommands: Record<PackageManager, string> = {
      apt: "Install Docker Engine, then rerun `vyrex onboard`.",
      dnf: "Install Docker Engine with your package manager, then rerun `vyrex onboard`.",
      yum: "Install Docker Engine with your package manager, then rerun `vyrex onboard`.",
      brew: "Install Docker Desktop or Colima, then rerun `vyrex onboard`.",
      pacman: "Install Docker Engine with your package manager, then rerun `vyrex onboard`.",
      unknown: "Install Docker, then rerun `vyrex onboard`.",
    };
    actions.push({
      id: "install_docker",
      ...
      commands:
        assessment.platform === "darwin"
          ? ["Install Docker Desktop or Colima, then rerun `vyrex onboard`."]
    ...
    if (likelyGroupIssue) {
      ...
      actions.push({
        ...
        title: "Start Docker",
        kind: "manual",
        reason: "Docker is installed but Vyrex could not talk to the Docker daemon.",
        commands:
          assessment.platform === "darwin"
            ? ["Start Docker Desktop or Colima, then rerun `vyrex onboard`."]
            : assessment.systemctlAvailable
              ? ["sudo systemctl start docker", "vyrex onboard"]
              : ["Start the Docker daemon, then rerun `vyrex onboard`."],
        blocking: true,
      });
    }
  }
  ...
  if (!assessment.nodeInstalled) {
    actions.push({
      ...
      kind: "manual",
      reason: "Vyrex requires Node.js for its CLI and plugin build steps.",
      commands: ["Run the Vyrex installer to install Node.js automatically."],
      blocking: false,
    });
  }
  ...
  if (!assessment.openshellInstalled) {
    actions.push({
      ...
      commands: ["Run the Vyrex installer or `scripts/install-openshell.sh`."],
      blocking: false,
    });
  }
...
function writeManagedSwapMarker(): void {
  const vyrexDir = path.join(os.homedir(), ".vyrex");
  if (!fs.existsSync(vyrexDir)) {
    runCapture(["mkdir", "-p", vyrexDir], { ignoreError: true });
  }

  try {
    fs.writeFileSync(path.join(vyrexDir, "managed_swap"), "/swapfile");
  } catch {
...
export function ensureSwap(minTotalMB?: number, opts: EnsureSwapOpts = {}): SwapResult {
  const o: {
    ...
    dryRun: false,
    interactive: process.stdout.isTTY && !process.env.VYREX_NON_INTERACTIVE,
