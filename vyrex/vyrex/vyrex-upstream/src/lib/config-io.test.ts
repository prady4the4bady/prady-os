...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-config-io-"));
  tmpDirs.push(dir);
  return dir;
}
...
describe("config-io", () => {
  ...
  it("rejects a symlink in place of the config directory", () => {
    ...
    const tmp = fs.mkdtempSync(path.join(home, ".vyrex-test-"));
    tmpDirs.push(tmp);
    const attackerDir = path.join(tmp, "attacker");
    fs.mkdirSync(attackerDir);
    const symlinkPath = path.join(tmp, ".vyrex");
