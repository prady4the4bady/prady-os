...
vi.mock("./agent-defs.js", () => ({
  loadAgent: vi.fn((name: string) => ({
    name,
    displayName: name === "openclaw" ? "OpenClaw" : "Lumyn Agent",
    versionCommand: name === "openclaw" ? "openclaw --version" : "lumyn --version",
...
describe("checkAgentVersion", () => {
  ...
  beforeEach(() => {
    ...
    mkdirSync(join(tmpDir, ".vyrex"), { recursive: true });
    writeFileSync(
      join(tmpDir, ".vyrex", "sandboxes.json"),
