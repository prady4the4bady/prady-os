...
// Credential env keys Vyrex knows how to round-trip. listCredentialKeys()
// projects the in-process env through this set; entries not in the set are
// invisible to `vyrex credentials list` even if exported.
...
// read into memory. The largest realistic credential set Vyrex has ever
// shipped is well under 1 KiB; the cap exists purely so an attacker who
// can write to ~/.vyrex/ cannot OOM the next onboard by planting a
...
/** Return `~/.vyrex`, resolving and validating `HOME` once per process. */
export function getCredsDir(): string {
  const home = resolveHomeDir();
  if (_cachedHome !== home) {
    _cachedHome = home;
    _credsDir = path.join(home, ".vyrex");
    _legacyCredsFile = null;
  }
  return _credsDir || path.join(home, ".vyrex");
}
 ...
 * to a one-time on-demand stage of any pre-fix `~/.vyrex/credentials.json`,
...
export function stageLegacyCredentialsToEnv(): string[] {
  ...
  // ancestor between HOME and ~/.vyrex and refuse if any of them is
  // a symlink — otherwise a planted directory symlink at ~/.vyrex/
 ...
 * directories between HOME and ~/.vyrex first so a planted directory
...
export async function ensureGithubToken(): Promise<void> {
  ...
  console.log("  │                                                                │");
  console.log("  │  Recommended: run 'gh auth login'. Vyrex picks up whatever  │");
