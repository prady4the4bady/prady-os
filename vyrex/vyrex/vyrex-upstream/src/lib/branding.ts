 ...
 *
 * `vyrex-lumyn` is a thin alias launcher that sets `VYREX_AGENT` before
...
export interface AgentBranding {
  /** Binary name shown in usage strings, e.g. "vyrex" or "vyrex-lumyn". */
  cli: string;
  /** Title-case display name, e.g. "Vyrex" or "VyrexLumyn". */
  display: string;
  /** The agent product name shown in messages, e.g. "OpenClaw" or "Lumyn". */
  product: string;
}
...
const AGENT_BRANDING: Record<string, AgentBranding> = {
  ...
  lumyn: { cli: "vyrex-lumyn", display: "VyrexLumyn", product: "Lumyn" },
};

const DEFAULT_AGENT = "openclaw";

export function getAgentBranding(
  agentName: string | null | undefined = process.env.VYREX_AGENT,
): AgentBranding {
...
/** CLI binary name for usage strings — "vyrex" or "vyrex-lumyn". */
export const CLI_NAME: string = branding.cli;

/** Title-case display name for headers — "Vyrex" or "VyrexLumyn". */
export const CLI_DISPLAY_NAME: string = branding.display;

/** Agent product name for user-facing messages — "OpenClaw" or "Lumyn". */
