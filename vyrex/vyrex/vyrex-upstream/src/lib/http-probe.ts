...
export function runCurlProbe(argv: string[], opts: CurlProbeOptions = {}): CurlProbeResult {
  const bodyFile = secureTempFile("vyrex-curl-probe", ".json");
  try {
    const args = [...argv];
    const url = args.pop();
    ...
    cleanupTempDir(bodyFile, "vyrex-curl-probe");
  }
}
...
export function runChatCompletionsStreamingProbe(
  ...
  const bodyFile = secureTempFile("vyrex-chat-streaming-probe", ".sse");
  try {
    const args = [...argv];
    const url = args.pop();
    const spawnSyncImpl = opts.spawnSyncImpl ?? spawnSync;
    ...
    cleanupTempDir(bodyFile, "vyrex-chat-streaming-probe");
  }
}
...
export function runStreamingEventProbe(
  ...
  const bodyFile = secureTempFile("vyrex-streaming-probe", ".sse");
  try {
    const args = [...argv];
    const url = args.pop();
    const spawnSyncImpl = opts.spawnSyncImpl ?? spawnSync;
    ...
    cleanupTempDir(bodyFile, "vyrex-streaming-probe");
  }
}
