...
// non-default agent (e.g. Lumyn) is selected via --agent flag or
// VYREX_AGENT env var. The OpenClaw path never touches this module.
...
export function createAgentSandbox(agent: AgentDefinition): {
  ...
  if (baseDockerfile) {
    const baseImageTag = `ghcr.io/nvidia/vyrex/${agent.name}-sandbox-base:latest`;
  ...
  const buildCtx = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-build-"));
...
const AGENT_BINARY_CHECK_PREFIX = "VYREX_AGENT_BINARY_CHECK:";
