  ...
  it("rejects non-WSL Ollama when the backend and proxy ports collide", () => {
    const output = execFileSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          VYREX_OLLAMA_PORT: "11435",
          VYREX_OLLAMA_PROXY_PORT: "11435",
        },
      },
    );
    ...
    expect(result.message).toContain("VYREX_OLLAMA_PORT");
    expect(result.message).toContain("VYREX_OLLAMA_PROXY_PORT");
