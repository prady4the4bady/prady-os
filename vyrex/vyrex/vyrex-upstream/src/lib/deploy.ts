...
export function buildDeployEnvLines(opts: {
  ...
  const envLines = [
    ...
    `VYREX_SANDBOX_NAME=${shellQuote(sandboxName)}`,
    `VYREX_PROVIDER=${shellQuote(provider)}`,
  ];
...
export async function executeDeploy(opts: DeployExecutionOptions): Promise<void> {
  ...
  log("");
  log("  ⚠  `vyrex deploy` is deprecated and will be removed in a future release.");
  log(
    "  Prefer provisioning the remote host separately, then run the standard installer and `vyrex onboard` on that host.",
  );
  log("");
  if (!instanceName) {
    return fail(
      [
        "  Usage: vyrex deploy <instance-name>",
        "",
        "  Examples:",
        "    vyrex deploy my-gpu-box",
        "    vyrex deploy vyrex-prod",
        "    vyrex deploy vyrex-test",
      ],
      error,
      exit,
    );
  ...
  const gpu = env.VYREX_GPU || "a2-highgpu-1g:nvidia-tesla-a100:1";
  const brevProvider = String(env.VYREX_BREV_PROVIDER || "gcp")
    .trim()
    .toLowerCase();
  const skipConnect = ["1", "true"].includes(
    String(env.VYREX_DEPLOY_NO_CONNECT || "").toLowerCase(),
  );
  const skipStartServices = ["1", "true"].includes(
    String(env.VYREX_DEPLOY_NO_START_SERVICES || "").toLowerCase(),
  );
  const sandboxName = validateName(env.VYREX_SANDBOX_NAME || "my-assistant", "sandbox name");
  ...
  const provider = inferDeployProvider(env.VYREX_PROVIDER, credentials);
  if (!provider) {
    return fail(
      [
        "  Could not determine which inference provider to configure for remote onboarding.",
        "  Set `VYREX_PROVIDER` explicitly or provide exactly one matching provider credential.",
  ...
  log("");
  log(`  Deploying Vyrex to Brev instance: ${name}`);
  log("");
  ...
  // Ref: https://github.com/NVIDIA/Vyrex/issues/691
  const khDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-ssh-"));
  ...
  try {
    ...
    const remoteDir = `${remoteHome}/vyrex`;

    log("  Syncing Vyrex to VM...");
    ...
    const envDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-env-"));
    ...
    if (skipStartServices) {
      log("  Skipping service startup (VYREX_DEPLOY_NO_START_SERVICES=1).");
    }

    if (skipConnect) {
      log("");
      log("  Skipping interactive sandbox connect (VYREX_DEPLOY_NO_CONNECT=1).");
