...
describe("stopAll with sandbox channels", () => {
  ...
  beforeEach(() => {
    pidDir = mkdtempSync(join(tmpdir(), "vyrex-svc-sandbox-test-"));
    originalResolve = resolveOpenshellModule.resolveOpenshell;
    resolveOpenshellModule.resolveOpenshell = vi.fn(() => "/usr/local/bin/openshell");
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const cp = require("node:child_process");
  ...
  it("warns when no sandbox name is available", () => {
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const savedVyrex = process.env.VYREX_SANDBOX;
    const savedVyrexName = process.env.VYREX_SANDBOX_NAME;
    const savedSandbox = process.env.SANDBOX_NAME;
    delete process.env.VYREX_SANDBOX;
    ...
    try {
      ...
      if (savedVyrex !== undefined) process.env.VYREX_SANDBOX = savedVyrex;
      if (savedVyrexName !== undefined) process.env.VYREX_SANDBOX_NAME = savedVyrexName;
  ...
  it("reads sandbox name from VYREX_SANDBOX env when not in opts", () => {
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const saved = process.env.VYREX_SANDBOX;
    process.env.VYREX_SANDBOX = "env-sandbox";

    try {
      stopAll({ pidDir });
    } finally {
      if (saved !== undefined) {
        process.env.VYREX_SANDBOX = saved;
      } else {
        delete process.env.VYREX_SANDBOX;
      }
    }
  ...
  it("reads sandbox name from VYREX_SANDBOX_NAME when VYREX_SANDBOX is unset", () => {
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const savedVyrex = process.env.VYREX_SANDBOX;
    const savedVyrexName = process.env.VYREX_SANDBOX_NAME;
    delete process.env.VYREX_SANDBOX;
    process.env.VYREX_SANDBOX_NAME = "named-sandbox";

    try {
      stopAll({ pidDir });
    } finally {
      if (savedVyrex !== undefined) {
        process.env.VYREX_SANDBOX = savedVyrex;
      } else {
        delete process.env.VYREX_SANDBOX;
      }
      if (savedVyrexName !== undefined) {
        process.env.VYREX_SANDBOX_NAME = savedVyrexName;
      } else {
        delete process.env.VYREX_SANDBOX_NAME;
      }
    }
  ...
  it("prefers VYREX_SANDBOX_NAME over VYREX_SANDBOX (consistent with resolveDefaultSandboxName)", () => {
    const logSpy = vi.spyOn(console, "log").mockImplementation(() => {});
    const savedVyrex = process.env.VYREX_SANDBOX;
    const savedVyrexName = process.env.VYREX_SANDBOX_NAME;
    const savedSandbox = process.env.SANDBOX_NAME;
    process.env.VYREX_SANDBOX_NAME = "name-sandbox";
    process.env.VYREX_SANDBOX = "other-sandbox";
    delete process.env.SANDBOX_NAME;
    ...
    try {
      ...
      if (savedVyrex !== undefined) process.env.VYREX_SANDBOX = savedVyrex;
      else delete process.env.VYREX_SANDBOX;
      if (savedVyrexName !== undefined) process.env.VYREX_SANDBOX_NAME = savedVyrexName;
      else delete process.env.VYREX_SANDBOX_NAME;
      if (savedSandbox !== undefined) process.env.SANDBOX_NAME = savedSandbox;
  ...
  it("uses the effective env-selected sandbox for host-side pid cleanup", () => {
    ...
    const savedVyrexName = process.env.VYREX_SANDBOX_NAME;
    const savedSandbox = process.env.SANDBOX_NAME;
    const effectivePidDir = "/tmp/vyrex-services-name-sandbox";
    const lowerPriorityPidDir = "/tmp/vyrex-services-other-sandbox";
  ...
  it("rejects malformed env var sandbox names before calling stopSandboxChannels", () => {
    ...
    const savedSandbox = process.env.SANDBOX_NAME;
    delete process.env.VYREX_SANDBOX;
    process.env.VYREX_SANDBOX_NAME = "bad name";
    delete process.env.SANDBOX_NAME;
  ...
  it("rejects path-traversal sandbox names from env vars", () => {
    ...
    const savedSandbox = process.env.SANDBOX_NAME;
    delete process.env.VYREX_SANDBOX;
    process.env.VYREX_SANDBOX_NAME = "../../etc/passwd";
    delete process.env.SANDBOX_NAME;
