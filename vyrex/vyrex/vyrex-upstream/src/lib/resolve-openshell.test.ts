...
describe("lib/resolve-openshell", () => {
  ...
  it("prefers explicit installer override over command -v", () => {
    const previous = process.env.VYREX_OPENSHELL_BIN;
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-openshell-bin-"));
    const override = path.join(tmp, "openshell");
    fs.writeFileSync(override, "#!/usr/bin/env bash\nexit 0\n", { mode: 0o755 });

    try {
      process.env.VYREX_OPENSHELL_BIN = override;
      ...
      if (previous === undefined) {
        ...
        process.env.VYREX_OPENSHELL_BIN = previous;
      }
    }
  });
