...
const PRESETS_DIR = path.join(ROOT, "vyrex-blueprint", "policies", "presets");
...
/**
 * Enumerate every preset YAML under `vyrex-blueprint/policies/presets/`
...
function buildPolicySetCommand(policyFile: string, sandboxName: string): string[] {
  const binary = process.env.VYREX_OPENSHELL_BIN || "openshell";
...
function removePreset(sandboxName: string, presetName: string): boolean {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-policy-"));
 ...
 * from `vyrex-blueprint/policies/presets/<name>.yaml` and delegates to
