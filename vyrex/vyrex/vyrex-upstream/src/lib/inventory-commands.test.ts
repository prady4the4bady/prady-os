...
describe("inventory commands", () => {
  ...
  it("prints the per-sandbox agent type in list output", async () => {
    ...
    expect(lines).toContain(
      "      agent: lumyn  model: nvidia/nemotron-3-super-120b-a12b  provider: nvidia-prod  CPU  policies: none",
    );
  });
  ...
  it("surfaces Lumyn gateway log when messaging is degraded", () => {
    const lines: string[] = [];
    const checkMessagingBridgeHealth = vi
      .fn()
  ...
  it("does not show gateway log for non-Lumyn sandboxes", () => {
    const lines: string[] = [];
    const checkMessagingBridgeHealth = vi
      .fn()
      .mockReturnValue([{ channel: "telegram", conflicts: 3 }]);
