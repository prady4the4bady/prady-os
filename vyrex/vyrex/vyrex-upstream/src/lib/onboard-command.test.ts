...
describe("onboard command", () => {
  ...
  it("accepts the env-based third-party notice acknowledgement", () => {
    expect(
      parseOnboardArgs(
        ...
        "VYREX_ACCEPT_THIRD_PARTY_SOFTWARE",
        {
          env: { VYREX_ACCEPT_THIRD_PARTY_SOFTWARE: "1" },
          error: () => {},
          exit: exitWithCode,
        },
      ),
  ...
  it("runs onboard with parsed options", async () => {
    ...
    await runOnboardCommand({
      ...
      noticeAcceptEnv: "VYREX_ACCEPT_THIRD_PARTY_SOFTWARE",
      env: {},
      runOnboard,
      error: () => {},
      exit: exitWithCode,
  ...
  it("parses --from <Dockerfile>", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-from-parse-"));
  ...
  it("parses --name <sandbox>", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-name-parse-"));
    const dockerfilePath = path.join(tmpDir, "Custom.Dockerfile");
    fs.writeFileSync(dockerfilePath, "FROM scratch\n");
  ...
  it("prints the setup-spark deprecation text before delegating", async () => {
    ...
    expect(lines.join("\n")).toContain("Use `vyrex onboard` instead");
  ...
  it("prints the setup deprecation text before delegating", async () => {
    ...
    expect(lines.join("\n")).toContain("`vyrex setup` is deprecated");
    expect(lines.join("\n")).toContain("Use `vyrex onboard` instead");
    expect(runOnboard).toHaveBeenCalledTimes(1);
  });
});
