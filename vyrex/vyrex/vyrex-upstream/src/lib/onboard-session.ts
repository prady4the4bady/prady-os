 ...
 * onboarding session file (~/.vyrex/onboard-session.json) with
...
export const SESSION_DIR = path.join(process.env.HOME || "/tmp", ".vyrex");
...
export interface Session {
  ...
  // earlier and can safely remove ~/.vyrex/credentials.json on the
...
function parseSessionMetadata(value: SessionJsonValue | undefined): SessionMetadata | undefined {
  ...
  return {
    gatewayName: readString(value.gatewayName) ?? "vyrex",
...
export function createSession(overrides: Partial<Session> = {}): Session {
  ...
  return {
    ...
    metadata: {
      gatewayName: overrides.metadata?.gatewayName ?? "vyrex",
