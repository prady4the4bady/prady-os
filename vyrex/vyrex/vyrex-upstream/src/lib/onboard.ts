...
// VYREX_NON_INTERACTIVE=1 env var for CI/CD pipelines.
...
const LOCAL_INFERENCE_TIMEOUT_SECS = envInt("VYREX_LOCAL_INFERENCE_TIMEOUT", 180);
/** Sandbox Ready wait after OpenShell create returns but k3s is still converging. */
const SANDBOX_READY_TIMEOUT_SECS = envInt("VYREX_SANDBOX_READY_TIMEOUT", 180);
...
function onboardBranding(): import("./branding").AgentBranding {
  return getAgentBranding(onboardBrandingAgent || process.env.VYREX_AGENT || null);
}
 ...
 * Ref: https://github.com/NVIDIA/Vyrex/issues/1093
 */
...
const EXPERIMENTAL = process.env.VYREX_EXPERIMENTAL === "1";
const USE_COLOR = !process.env.NO_COLOR && !!process.stdout.isTTY;
const DIM = USE_COLOR ? "\x1b[2m" : "";
const RESET = USE_COLOR ? "\x1b[0m" : "";
let OPENSHELL_BIN: string | null = null;
const GATEWAY_NAME = "vyrex";
...
const BACK_TO_SELECTION = "__VYREX_BACK_TO_SELECTION__";
...
function isNonInteractive(): boolean {
  return NON_INTERACTIVE || process.env.VYREX_NON_INTERACTIVE === "1";
}

function isRecreateSandbox(): boolean {
  return RECREATE_SANDBOX || process.env.VYREX_RECREATE_SANDBOX === "1";
}
...
function streamGatewayStart(
  ...
  const GATEWAY_START_TIMEOUT = envInt("VYREX_GATEWAY_START_TIMEOUT", 600) * 1000;
  let killedByTimeout = false;
  const killTimer = setTimeout(() => {
    killedByTimeout = true;
    lines.push("[Vyrex] Gateway start timed out — killing process.");
    child.kill("SIGTERM");
...
/**
 * Read a semver field from vyrex-blueprint/blueprint.yaml. Returns null if
...
function getBlueprintVersionField(field: string, rootDir = ROOT): string | null {
  try {
    ...
    // pulling it at module load would slow down `vyrex --help` for users
    // who never reach the preflight path.
    const YAML = require("yaml");
    const blueprintPath = path.join(rootDir, "vyrex-blueprint", "blueprint.yaml");
...
const SANDBOX_BASE_IMAGE = "ghcr.io/nvidia/vyrex/sandbox-base";
...
function pullAndResolveBaseImageDigest(): { digest: string; ref: string } | null {
  ...
  // RepoDigests is a JSON array like ["ghcr.io/nvidia/vyrex/sandbox-base@sha256:abc..."].
...
// imported from ~/.vyrex/credentials.json at the start of this run.
...
function readSandboxSelectionConfig(sandboxName: string): ProviderSelectionConfig | null {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-selection-"));
  try {
...
function writeSandboxConfigSyncFile(script: string): string {
  const scriptFile = secureTempFile("vyrex-sync", ".sh");
...
/**
 * Check whether the agent's Dockerfile declares ARG VYREX_WEB_SEARCH_ENABLED.
...
function agentSupportsWebSearch(
  ...
  // Lumyn has native web tools, but the Vyrex onboarding wizard wires the
  // OpenClaw Brave provider path. Do not offer a Brave prompt for Lumyn until
  ...
  for (const dockerfilePath of candidates) {
    try {
      ...
      return /^\s*ARG\s+VYREX_WEB_SEARCH_ENABLED=/m.test(content);
    } catch {
 ...
 * sandbox. Lumyn silently ignores unknown web.backend values, so checking
 * the config file alone is insufficient — we need to ask the runtime.
 *
 * For Lumyn: runs `lumyn dump` and checks for an active web backend.
...
function verifyWebSearchInsideSandbox(
  ...
  try {
    if (agentName === "lumyn") {
      // `lumyn dump` outputs config_overrides and active toolsets.
      // Look for the web backend in its output.
      const dump = runCaptureOpenshell(
        ["sandbox", "exec", "-n", sandboxName, "--", "lumyn", "dump"],
        {
          ignoreError: true,
          timeout: 10_000,
        },
      ...
      if (!dump) {
        console.warn("  ⚠ Could not verify web search config inside sandbox (lumyn dump failed).");
        return;
      }
      ...
      if (!hasWebBackend) {
        console.warn(
          "  ⚠ Web search was configured but Lumyn does not report an active web backend.",
        );
        console.warn("    The agent may not have accepted the web search configuration.");
        console.warn(`    Check: ${cliName()} ${sandboxName} exec lumyn dump`);
      } else {
...
// Shared validators for VYREX_PROXY_HOST / VYREX_PROXY_PORT.
...
function patchStagedDockerfile(
  ...
  dockerfile = dockerfile.replace(/^ARG VYREX_MODEL=.*$/m, `ARG VYREX_MODEL=${model}`);
  dockerfile = dockerfile.replace(
    /^ARG VYREX_PROVIDER_KEY=.*$/m,
    `ARG VYREX_PROVIDER_KEY=${providerKey}`,
  );
  dockerfile = dockerfile.replace(
    /^ARG VYREX_PRIMARY_MODEL_REF=.*$/m,
    `ARG VYREX_PRIMARY_MODEL_REF=${primaryModelRef}`,
  );
  dockerfile = dockerfile.replace(/^ARG CHAT_UI_URL=.*$/m, `ARG CHAT_UI_URL=${chatUiUrl}`);
  dockerfile = dockerfile.replace(
    /^ARG VYREX_INFERENCE_BASE_URL=.*$/m,
    `ARG VYREX_INFERENCE_BASE_URL=${inferenceBaseUrl}`,
  );
  dockerfile = dockerfile.replace(
    /^ARG VYREX_INFERENCE_API=.*$/m,
    `ARG VYREX_INFERENCE_API=${inferenceApi}`,
  );
  dockerfile = dockerfile.replace(
    /^ARG VYREX_INFERENCE_COMPAT_B64=.*$/m,
    `ARG VYREX_INFERENCE_COMPAT_B64=${encodeDockerJsonArg(inferenceCompat)}`,
  );
  dockerfile = dockerfile.replace(
    /^ARG VYREX_BUILD_ID=.*$/m,
    `ARG VYREX_BUILD_ID=${buildId}`,
  );
  // Honor VYREX_CONTEXT_WINDOW / VYREX_MAX_TOKENS / VYREX_REASONING
  // so the user can tune model metadata without editing the Dockerfile.
  const POSITIVE_INT_RE = /^[1-9][0-9]*$/;
  const contextWindow = process.env.VYREX_CONTEXT_WINDOW;
  if (contextWindow && POSITIVE_INT_RE.test(contextWindow)) {
    dockerfile = dockerfile.replace(
      /^ARG VYREX_CONTEXT_WINDOW=.*$/m,
      `ARG VYREX_CONTEXT_WINDOW=${contextWindow}`,
    );
  }
  const maxTokens = process.env.VYREX_MAX_TOKENS;
  if (maxTokens && POSITIVE_INT_RE.test(maxTokens)) {
    dockerfile = dockerfile.replace(
      /^ARG VYREX_MAX_TOKENS=.*$/m,
      `ARG VYREX_MAX_TOKENS=${maxTokens}`,
    );
  }
  const reasoning = process.env.VYREX_REASONING;
  if (reasoning === "true" || reasoning === "false") {
    dockerfile = dockerfile.replace(
      /^ARG VYREX_REASONING=.*$/m,
      `ARG VYREX_REASONING=${reasoning}`,
    );
  }
  // Honor VYREX_INFERENCE_INPUTS for vision-capable models. OpenClaw's
  ...
  const inferenceInputs = process.env.VYREX_INFERENCE_INPUTS;
  if (inferenceInputs && /^(text|image)(,(text|image))*$/.test(inferenceInputs)) {
    dockerfile = dockerfile.replace(
      /^ARG VYREX_INFERENCE_INPUTS=.*$/m,
      `ARG VYREX_INFERENCE_INPUTS=${inferenceInputs}`,
    );
  }
  // VYREX_AGENT_TIMEOUT — override agents.defaults.timeoutSeconds at build
  // time. Lets users increase the per-request inference timeout without
  // editing the Dockerfile. Ref: issue #2281
  const agentTimeout = process.env.VYREX_AGENT_TIMEOUT;
  if (agentTimeout && POSITIVE_INT_RE.test(agentTimeout)) {
    dockerfile = dockerfile.replace(
      /^ARG VYREX_AGENT_TIMEOUT=.*$/m,
      `ARG VYREX_AGENT_TIMEOUT=${agentTimeout}`,
    );
  }
  // Honor VYREX_PROXY_HOST / VYREX_PROXY_PORT exported in the host
  // shell so the sandbox-side vyrex-start.sh sees them via $ENV at runtime.
  ...
  const proxyHostEnv = process.env.VYREX_PROXY_HOST;
  if (proxyHostEnv && isValidProxyHost(proxyHostEnv)) {
    dockerfile = dockerfile.replace(
      /^ARG VYREX_PROXY_HOST=.*$/m,
      `ARG VYREX_PROXY_HOST=${proxyHostEnv}`,
    );
  }
  const proxyPortEnv = process.env.VYREX_PROXY_PORT;
  if (proxyPortEnv && isValidProxyPort(proxyPortEnv)) {
    dockerfile = dockerfile.replace(
      /^ARG VYREX_PROXY_PORT=.*$/m,
      `ARG VYREX_PROXY_PORT=${proxyPortEnv}`,
    );
  }
  dockerfile = dockerfile.replace(
    /^ARG VYREX_WEB_SEARCH_ENABLED=.*$/m,
    `ARG VYREX_WEB_SEARCH_ENABLED=${webSearchConfig ? "1" : "0"}`,
  );
  ...
  dockerfile = dockerfile.replace(
    /^ARG VYREX_DISABLE_DEVICE_AUTH=.*$/m,
    `ARG VYREX_DISABLE_DEVICE_AUTH=1`,
  );
  if (messagingChannels.length > 0) {
    dockerfile = dockerfile.replace(
      /^ARG VYREX_MESSAGING_CHANNELS_B64=.*$/m,
      `ARG VYREX_MESSAGING_CHANNELS_B64=${encodeDockerJsonArg(messagingChannels)}`,
    );
  }
  if (Object.keys(messagingAllowedIds).length > 0) {
    dockerfile = dockerfile.replace(
      /^ARG VYREX_MESSAGING_ALLOWED_IDS_B64=.*$/m,
      `ARG VYREX_MESSAGING_ALLOWED_IDS_B64=${encodeDockerJsonArg(messagingAllowedIds)}`,
    );
  }
  if (Object.keys(discordGuilds).length > 0) {
    dockerfile = dockerfile.replace(
      /^ARG VYREX_DISCORD_GUILDS_B64=.*$/m,
      `ARG VYREX_DISCORD_GUILDS_B64=${encodeDockerJsonArg(discordGuilds)}`,
    );
  }
  if (telegramConfig && Object.keys(telegramConfig).length > 0) {
    dockerfile = dockerfile.replace(
      /^ARG VYREX_TELEGRAM_CONFIG_B64=.*$/m,
      `ARG VYREX_TELEGRAM_CONFIG_B64=${encodeDockerJsonArg(telegramConfig)}`,
    );
  }
...
async function validateCustomOpenAiLikeSelection(
  ...
  const probe = probeOpenAiLikeEndpoint(endpointUrl, model, apiKey, {
    ...
    skipResponsesProbe: shouldForceCompletionsApi(process.env.VYREX_PREFERRED_API),
    probeStreaming: true,
  });
  if (probe.ok) {
    console.log(`  ${probe.label} available — ${agentProductName()} will use ${probe.api}.`);
...
function getRequestedSandboxNameHint(opts: { sandboxName?: string | null } = {}): string | null {
  const raw =
    typeof opts.sandboxName === "string" && opts.sandboxName.length > 0
      ? opts.sandboxName
      : process.env.VYREX_SANDBOX_NAME;
...
function getResumeSandboxConflict(
  ...
  // resolved it (--name first, VYREX_SANDBOX_NAME only when prompting
  ...
  // VYREX_SANDBOX_NAME but which never actually consult it.
...
function getResumeConfigConflicts(
  ...
  const requestedAgent = opts.agent || process.env.VYREX_AGENT || null;
...
function installOpenshell(): {
  ...
  if (OPENSHELL_BIN) {
    process.env.VYREX_OPENSHELL_BIN = OPENSHELL_BIN;
  }
  return {
...
function destroyGateway() {
  ...
  // Clear the local registry so `vyrex list` stays consistent with OpenShell state. (#532)
...
function getGatewayHealthWaitConfig(_startStatus = 0, containerState = "") {
  ...
  const standardCount = envInt("VYREX_HEALTH_POLL_COUNT", isArm64 ? 30 : 12);
  const standardInterval = envInt("VYREX_HEALTH_POLL_INTERVAL", isArm64 ? 10 : 5);
  const extendedCount = envInt("VYREX_GATEWAY_START_POLL_COUNT", standardCount);
  const extendedInterval = envInt("VYREX_GATEWAY_START_POLL_INTERVAL", standardInterval);
...
async function preflight(): Promise<ReturnType<typeof nim.detectGpu>> {
  ...
  if (!isOpenshellInstalled()) {
    ...
    if (!currentVersion) {
      ...
      // Source of truth: min_openshell_version in vyrex-blueprint/blueprint.yaml.
  ...
  // Enforce vyrex-blueprint/blueprint.yaml's min_openshell_version. Without
  ...
  if (
    ...
    console.error("");
    console.error(
      `  ✗ openshell ${installedOpenshellVersion} is below the minimum required by this Vyrex release.`,
    );
  ...
  // Enforce vyrex-blueprint/blueprint.yaml's max_openshell_version. Newer
  // OpenShell releases may change sandbox semantics that this Vyrex version
  // has not been validated against. Blocking early avoids silent runtime
  // breakage. Users should upgrade Vyrex to pick up support for newer
  // OpenShell releases.
  ...
  if (
    ...
    console.error("");
    console.error(
      `  ✗ openshell ${installedOpenshellVersion} is above the maximum supported by this Vyrex release.`,
    );
  ...
  // Clean up stale or unnamed Vyrex gateway state before checking ports.
  ...
  // as Vyrex state; let the port checks surface the conflict instead.
  ...
  if (gatewayReuseState === "stale" || gatewayReuseState === "active-unnamed") {
    ...
    // clear the local registry so `vyrex list` stays consistent. (#532)
    if (destroyResult.status === 0) {
      registry.clearAll();
    }
  ...
  for (const { port, label } of requiredPorts) {
    ...
    if (!portCheck.ok) {
      ...
      // Auto-cleanup orphaned SSH port-forward from a previous Vyrex session
...
async function startGatewayWithOptions(
  ...
  // allocate GPUs. See: https://build.nvidia.com/spark/vyrex/instructions
  ...
  try {
    ...
    if (exitOnFailure) {
      ...
      console.error("    openshell doctor logs --name vyrex");
 ...
 * (or `native` via VYREX_OVERLAY_SNAPSHOTTER) and return its tag so the
...
function applyOverlayfsAutoFix(upstreamImage: string): string | null {
  if (process.env.VYREX_DISABLE_OVERLAY_FIX === "1") {
    return null;
  }
  ...
  const requestedSnapshotter = (process.env.VYREX_OVERLAY_SNAPSHOTTER || "")
    .trim()
    .toLowerCase();
  ...
  if (requestedSnapshotter === "native" || requestedSnapshotter === "fuse-overlayfs") {
    ...
    console.warn(
      `  VYREX_OVERLAY_SNAPSHOTTER='${requestedSnapshotter}' is not recognized. ` +
  ...
  console.log(
    "  Set VYREX_DISABLE_OVERLAY_FIX=1 to disable this auto-fix; see docs for the manual daemon.json workaround.",
  );
...
// makes 'vyrex status connect' route to the global status command
// instead of the sandbox, so reject these wherever a sandbox name enters
// the system (interactive prompt, --name flag, VYREX_SANDBOX_NAME).
...
function formatSandboxAgentName(agentName: string | null | undefined): string {
  ...
  if (normalized === "lumyn") return "Lumyn";
  return normalized;
}

function getDefaultSandboxNameForAgent(agent: AgentDefinition | null | undefined): string {
  return getRequestedSandboxAgentName(agent) === "lumyn" ? "lumyn" : "my-assistant";
}
...
async function promptValidatedSandboxName(agent: AgentDefinition | null = null) {
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    // VYREX_SANDBOX_NAME env var and retry.
    if (isNonInteractive()) {
      process.exit(1);
    }
...
async function createSandbox(
  ...
  // Drop channels the operator disabled via `vyrex <sandbox> channels stop`.
  ...
  if (liveExists) {
    ...
    if (agentDrift.changed && !isRecreateSandbox()) {
      ...
      if (isNonInteractive()) {
        console.error(
          `  Aborting: choose a different name or set VYREX_RECREATE_SANDBOX=1 to recreate '${sandboxName}'.`,
        );
        console.error(
        ...
        if (existingSandboxState === "ready") {
          if (confirmedSelectionDrift) {
            ...
            if (selectionDrift.unknown) {
              ...
              note(
                "  [non-interactive] Set VYREX_RECREATE_SANDBOX=1 (or --recreate-sandbox) to force recreation.",
              );
            } else {
              note(`  [non-interactive] Sandbox '${sandboxName}' exists and is ready — reusing it`);
              note(
                "  Pass --recreate-sandbox or set VYREX_RECREATE_SANDBOX=1 to force recreation.",
              );
            }
          ...
          console.error(
            "  Pass --recreate-sandbox or set VYREX_RECREATE_SANDBOX=1 to overwrite.",
          );
          process.exit(1);
        }
  ...
  if (fromDockerfile) {
    ...
    buildCtx = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-build-"));
  ...
  // silently reusing a cached old image after Vyrex upgrades (#1904).
  ...
  // vyrex-start.sh starts the gateway on the correct port. When the
  ...
  envArgs.push(formatEnvAssignment("VYREX_DASHBOARD_PORT", effectiveDashboardPort));
  // Propagate VYREX_PROXY_HOST / VYREX_PROXY_PORT to the runtime
  ...
  // -- env … vyrex-start` only forwards the explicitly listed env vars
  // — image-baked ENV does not propagate into the running pod. Without
  // this, vyrex-start.sh:898 falls back to the default 10.200.0.1:3128
  ...
  const sandboxProxyHost = process.env.VYREX_PROXY_HOST;
  if (sandboxProxyHost && isValidProxyHost(sandboxProxyHost)) {
    envArgs.push(formatEnvAssignment("VYREX_PROXY_HOST", sandboxProxyHost));
  }
  const sandboxProxyPort = process.env.VYREX_PROXY_PORT;
  if (sandboxProxyPort && isValidProxyPort(sandboxProxyPort)) {
    envArgs.push(formatEnvAssignment("VYREX_PROXY_PORT", sandboxProxyPort));
  }
  ...
  // without this gate, Vyrex registers a phantom sandbox that
  ...
  // Lumyn silently ignores unknown web.backend values (e.g. "brave" before
  ...
  if (!builtImageMatch) {
    console.warn(
      "  Warning: could not parse image tag from build output; imageTag may be stale. Run 'vyrex gc' if destroy fails.",
    );
  }
  ...
  if (options.length > 1) {
    selectionLoop: while (true) {
      ...
      if (isNonInteractive()) {
        ...
        if (!providerKey) {
          ...
          if (recoveredKey) {
            ...
            if (!options.some((o) => o.key === recoveredKey)) {
              ...
              console.error(
                "  Set VYREX_PROVIDER explicitly, or restore the missing local-inference dependency.",
              );
              process.exit(1);
            }
            ...
            console.error("  Set VYREX_PROVIDER explicitly (vllm or nim-local) and re-run.");
            process.exit(1);
          } else {
            providerKey = "build";
          }
        ...
        const envProviderHint = (process.env.VYREX_PROVIDER || "").trim().toLowerCase();
      ...
      if (REMOTE_PROVIDER_CONFIG[selected.key]) {
        ...
        if (selected.key === "custom") {
          const _envUrl = (process.env.VYREX_ENDPOINT_URL || "").trim();
        ...
        if (selected.key === "build") {
          // Allow VYREX_PROVIDER_KEY as a fallback for NVIDIA_API_KEY.
          // Check raw process.env first — VYREX_PROVIDER_KEY is a user-facing
          // override that should take precedence before resolving from credentials.json.
          const _nvProviderKey = (process.env.VYREX_PROVIDER_KEY || "").trim();
          // check-direct-credential-env-ignore -- intentional: checking if env is already set before applying VYREX_PROVIDER_KEY override
          ...
          if (isNonInteractive()) {
            ...
            if (!resolvedNvidiaKey) {
              console.error(
                "  NVIDIA_API_KEY (or VYREX_PROVIDER_KEY) is required for NVIDIA Endpoints in non-interactive mode.",
              );
              process.exit(1);
            }
          ...
          const _envModel = (process.env.VYREX_MODEL || "").trim();
          model =
            requestedModel ||
          ...
          // VYREX_PROVIDER_KEY is a universal alias: if the specific credential env
          // isn't already set, use VYREX_PROVIDER_KEY as the API key for this provider.
          // Check raw process.env — the override must apply before resolving from credentials.json.
          const _providerKeyHint = (process.env.VYREX_PROVIDER_KEY || "").trim();
          ...
          if (isNonInteractive()) {
            if (!resolveProviderCredential(credentialEnv)) {
              console.error(
                `  ${credentialEnv} (or VYREX_PROVIDER_KEY) is required for ${remoteConfig.label} in non-interactive mode.`,
              );
              process.exit(1);
            }
          } else {
          ...
          const _envModelRemote = (process.env.VYREX_MODEL || "").trim();
          const defaultModel =
            requestedModel ||
            _envModelRemote ||
            (recoveredFromSandbox && recoveredModel) ||
          ...
          while (true) {
            ...
            if (selected.key === "custom") {
              ...
              if (validation.ok) {
                ...
                // See: https://github.com/NVIDIA/Vyrex/issues/1932
                const explicitApi = (process.env.VYREX_PREFERRED_API || "").trim().toLowerCase();
                if (
                  explicitApi &&
        ...
        if (models.length === 0) {
          ...
          if (isNonInteractive()) {
            ...
            if (targetModel) {
              ...
              if (!sel) {
                const label = requestedModel ? "VYREX_MODEL for NIM" : "Recorded NIM model";
          ...
          if (!nim.waitForNimHealth()) {
            ...
            // credential env (VYREX_VLLM_LOCAL_TOKEN). See GH #2519.
            credentialEnv = null;
        ...
        // an internal token (VYREX_OLLAMA_PROXY_TOKEN, set in setupInference).
        ...
        // See: https://github.com/NVIDIA/Vyrex/issues/976
        if (preferredInferenceApi !== "openai-completions") {
          console.log(
            "  ℹ Using chat completions API (tool-call-parser requires /v1/chat/completions)",
          );
...
async function setupInference(
  ...
  if (
    ...
    // Do not mutate ~/.vyrex/credentials.json here: local vLLM now uses
    ...
    // Use a dedicated internal credential env (VYREX_OLLAMA_PROXY_TOKEN)
    ...
    // Do not mutate ~/.vyrex/credentials.json here: local Ollama now uses
...
async function checkTelegramReachability(token: string) {
  if (process.env.VYREX_SKIP_TELEGRAM_REACHABILITY === "1") {
...
async function setupMessagingChannels(): Promise<string[]> {
  ...
  if (isNonInteractive() || process.env.VYREX_NON_INTERACTIVE === "1") {
...
async function setupOpenclaw(sandboxName: string, model: string, provider: string): Promise<void> {
  ...
  if (selectionConfig) {
    ...
    try {
      ...
      cleanupTempDir(scriptFile, "vyrex-sync");
    }
  }
...
async function _setupPolicies(
  ...
  if (isNonInteractive()) {
    const policyMode = (process.env.VYREX_POLICY_MODE || "suggested").trim().toLowerCase();
    ...
    if (policyMode === "custom" || policyMode === "list") {
      selectedPresets = parsePolicyPresetEnv(process.env.VYREX_POLICY_PRESETS || "");
      if (selectedPresets.length === 0) {
        console.error("  VYREX_POLICY_PRESETS is required when VYREX_POLICY_MODE=custom.");
        process.exit(1);
      }
    } else if (policyMode === "suggested" || policyMode === "default" || policyMode === "auto") {
      const envPresets = parsePolicyPresetEnv(process.env.VYREX_POLICY_PRESETS || "");
      if (envPresets.length > 0) {
        selectedPresets = envPresets;
      }
    } else {
      console.error(`  Unsupported VYREX_POLICY_MODE: ${policyMode}`);
 ...
 * In non-interactive mode reads VYREX_POLICY_TIER (default: balanced).
...
async function selectPolicyTier(): Promise<string> {
  ...
  if (isNonInteractive()) {
    const name = (process.env.VYREX_POLICY_TIER || "balanced").trim().toLowerCase();
...
async function setupPoliciesWithSelection(
  ...
  if (isNonInteractive()) {
    ...
    if (policyMode === "custom" || policyMode === "list") {
      chosen = parsePolicyPresetEnv(process.env.VYREX_POLICY_PRESETS || "");
      if (chosen.length === 0) {
        console.error("  VYREX_POLICY_PRESETS is required when VYREX_POLICY_MODE=custom.");
        process.exit(1);
      }
      ...
      console.warn(`  Unsupported VYREX_POLICY_MODE: ${policyMode}`);
      console.warn(
        "  Valid values: suggested, custom, skip (aliases: default/auto, list, none/no).",
      );
      if (tiers.getTier(policyMode)) {
        console.warn(
          `  '${policyMode}' is a policy tier — did you mean VYREX_POLICY_TIER=${policyMode}?`,
        );
      }
 ...
 * `openshell sandbox list` and the Vyrex registry from drifting when the
 ...
 * so onboard can print copy-paste Control UI URLs with #token= (same idea as vyrex-start.sh).
 */
function fetchGatewayAuthTokenFromSandbox(sandboxName: string): string | null {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-token-"));
  try {
...
function printDashboard(
  ...
  console.log(
    `    Model:       openshell inference set -g vyrex --model <model> --provider <provider>`,
  );
...
async function onboard(opts: OnboardOptions = {}): Promise<void> {
  setOnboardBrandingAgent(opts.agent || process.env.VYREX_AGENT || null);
  NON_INTERACTIVE = opts.nonInteractive || process.env.VYREX_NON_INTERACTIVE === "1";
  RECREATE_SANDBOX = opts.recreateSandbox || process.env.VYREX_RECREATE_SANDBOX === "1";
  ...
  const requestedFromDockerfile =
    opts.fromDockerfile ||
    (isNonInteractive() ? process.env.VYREX_FROM_DOCKERFILE || null : null);
  ...
  let requestedSandboxSource: "--name" | "VYREX_SANDBOX_NAME" | null = requestedSandboxName
    ? "--name"
    : null;
  if (!requestedSandboxName && cannotPrompt) {
    const envName = process.env.VYREX_SANDBOX_NAME;
    if (typeof envName === "string" && envName.trim().length > 0) {
      requestedSandboxName = envName.trim();
      requestedSandboxSource = "VYREX_SANDBOX_NAME";
    }
  }
  ...
  if (cannotPrompt && !resume && requestedFromDockerfile && !requestedSandboxName) {
    console.error(
      "  --from <Dockerfile> requires --name <sandbox> (or VYREX_SANDBOX_NAME) when running without a TTY or with --non-interactive.",
    );
  ...
  // Validate VYREX_PROVIDER early so invalid values fail before
  ...
  const lockResult = onboardSession.acquireOnboardLock(
    `vyrex onboard${resume ? " --resume" : ""}${fresh ? " --fresh" : ""}${isNonInteractive() ? " --non-interactive" : ""}${requestedFromDockerfile ? ` --from ${requestedFromDockerfile}` : ""}`,
  );
  ...
  try {
    ...
    if (resume) {
      ...
      setOnboardBrandingAgent(opts.agent || session?.agent || process.env.VYREX_AGENT || null);
      ...
      session = onboardSession.saveSession(
        onboardSession.createSession({
          ...
          metadata: { gatewayName: "vyrex", fromDockerfile: fromDockerfile || null },
        }),
      );
    }
    ...
    if (resumeGateway) {
      ...
      note("  Reusing healthy Vyrex gateway.");
    } else {
      if (resume && session?.steps?.gateway?.status === "complete") {
        if (gatewayReuseState === "active-unnamed") {
          ...
          note("  [resume] A different OpenShell gateway is active; Vyrex will not reuse it.");
    ...
    while (true) {
      ...
      process.env.VYREX_OPENSHELL_BIN = getOpenshellBinary();
    ...
    if (allStagedMigrated) {
      ...
      console.error(
        `  Kept ~/.vyrex/credentials.json: ${String(unmigrated.length)} ` +
