...
beforeEach(() => {
  ...
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-session-"));
...
describe("onboard session", () => {
  ...
  it("persists safe provider metadata without persisting secrets", () => {
    ...
    expect(loaded.metadata.gatewayName).toBe("vyrex");
  ...
  it("does not clear existing metadata when updates omit whitelisted metadata fields", () => {
    session.saveSession(
      session.createSession({ metadata: { gatewayName: "vyrex", fromDockerfile: null } }),
    );
  ...
  it("acquires and releases the onboard lock", () => {
    const acquired = session.acquireOnboardLock("vyrex onboard");
    expect(acquired.acquired).toBe(true);
    expect(fs.existsSync(session.LOCK_FILE)).toBe(true);

    const secondAttempt = session.acquireOnboardLock("vyrex onboard --resume");
  ...
  it("replaces a stale onboard lock", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        command: "vyrex onboard",
      }),
      { mode: 0o600 },
    );

    const acquired = session.acquireOnboardLock("vyrex onboard --resume");
    expect(acquired.acquired).toBe(true);
  ...
  it("regression #1281: stale-cleanup race does not unlink a fresh lock claimed by another process", () => {
      ...
      if (statCallCount === 2) {
        ...
        fs.writeFileSync(
          ...
          JSON.stringify({
            ...
            command: "vyrex onboard (fresh claim from concurrent process)",
          }),
          { mode: 0o600 },
        );
    ...
    try {
      ...
      const result = session.acquireOnboardLock("vyrex onboard --resume");
