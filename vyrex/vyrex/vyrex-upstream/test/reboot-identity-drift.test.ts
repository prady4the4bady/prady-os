...
// Covers: https://github.com/NVIDIA/Vyrex/issues/2056
//
...
//   1. The registry recovery gate triggers for bare `vyrex <name>` (no action)
...
/**
 * Build a temp HOME with a Vyrex registry and a fake openshell binary.
 *
...
function setupFixture(sandboxName: string, mode: "healthy" | "identity_drift" | "gateway_down") {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-reboot-"));
  ...
  switch (mode) {
    case "healthy":
      statusBehavior = `process.stdout.write("Gateway: vyrex\\nStatus: Connected\\n"); process.exit(0);`;
      gatewayInfoBehavior = `process.stdout.write("Gateway: vyrex\\nGateway endpoint: https://127.0.0.1:8080\\n"); process.exit(0);`;
...
function setupEmptyRegistry(sandboxName: string) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-reboot-noreg-"));
  tmpFixtures.push(tmpDir);
  const homeLocalBin = path.join(tmpDir, ".local", "bin");
  const registryDir = path.join(tmpDir, ".vyrex");
  const openshellPath = path.join(homeLocalBin, "openshell");
...
if (args[0] === "status") {
  process.stdout.write("Gateway: vyrex\\nStatus: Connected\\n");
  process.exit(0);
}
...
function runCli(tmpDir: string, args: string[]) {
  ...
  return spawnSync(process.execPath, [path.join(repoRoot, "bin", "vyrex.js"), ...args], {
    cwd: repoRoot,
    encoding: "utf-8",
    env: {
      ...process.env,
...
describe("post-reboot SSH identity drift (#2056)", () => {
  it(
    "bare `vyrex <name>` (no action) resolves to connect and finds registry entry",
  ...
  it(
    "explicit `vyrex <name> connect` works for healthy sandbox",
    testTimeoutOptions(20_000),
    () => {
  ...
  it(
    "bare `vyrex <name>` also detects identity drift (not silent failure)",
    testTimeoutOptions(20_000),
    () => {
...
describe("post-reboot registry recovery gate (#2056)", () => {
  it(
    "explicit `vyrex <name> connect` recovers registry from live gateway",
    testTimeoutOptions(20_000),
    () => {
  ...
  it(
    "bare `vyrex <name>` recovers registry from live gateway",
    testTimeoutOptions(20_000),
    () => {
