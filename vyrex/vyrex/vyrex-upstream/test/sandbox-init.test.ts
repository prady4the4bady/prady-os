...
EOF
  ...
  describe("validate_tmp_permissions", () => {
    ...
    const TMP_ARTIFACTS = ["/tmp/vyrex-proxy-env.sh", "/tmp/gateway.log", "/tmp/auto-pair.log"];
  ...
  describe("drop_capabilities", () => {
    it("function is defined and callable", () => {
      ...
      const { stdout } = runWithLib(
        ...
        echo "FALLTHROUGH_OK"
      `,
        { env: { PATH: "/usr/bin:/bin", VYREX_CAPS_DROPPED: "" } },
      );
      expect(stdout).toContain("capsh not available");
      expect(stdout).toContain("FALLTHROUGH_OK");
    });

    it("skips when VYREX_CAPS_DROPPED=1", () => {
      const { stdout } = runWithLib(
        `
        VYREX_CAPS_DROPPED=1
  ...
  describe("both entrypoints source the shared library", () => {
    it("vyrex-start.sh sources sandbox-init.sh", () => {
      const src = readFileSync(join(import.meta.dirname, "../scripts/vyrex-start.sh"), "utf-8");
      const start = src.indexOf("_SANDBOX_INIT=");
      const end = src.indexOf("# Harden: limit process count", start);
      if (start === -1 || end === -1 || end <= start) {
        throw new Error("Expected sandbox-init source block in scripts/vyrex-start.sh");
      }

      const workDir = mkdtempSync(join(tmpdir(), "vyrex-start-source-init-"));
      const scriptDir = join(workDir, "scripts");
      const libDir = join(scriptDir, "lib");
      mkdirSync(libDir, { recursive: true });
      writeFileSync(
        join(libDir, "sandbox-init.sh"),
        "export VYREX_TEST_SANDBOX_INIT_LOADED=1\n",
      );
      const wrapperPath = join(scriptDir, "vyrex-start.sh");
      writeFileSync(
        wrapperPath,
        [
          "#!/usr/bin/env bash",
          "set -euo pipefail",
          src.slice(start, end),
          'printf "LOADED=%s\\n" "${VYREX_TEST_SANDBOX_INIT_LOADED:-0}"',
        ].join("\n"),
        { mode: 0o700 },
      );
    ...
    it("lumyn start.sh sources sandbox-init.sh", () => {
      const src = readFileSync(join(import.meta.dirname, "../agents/lumyn/start.sh"), "utf-8");
      expect(src).toContain("source");
      expect(src).toContain("sandbox-init.sh");
    });

    it("lumyn start.sh calls lock_rc_files (vulnerability fix)", () => {
      const src = readFileSync(join(import.meta.dirname, "../agents/lumyn/start.sh"), "utf-8");
      expect(src).toContain("lock_rc_files");
    });

    it("lumyn start.sh rewrites configure guard rc blocks through the symlink-safe helper", () => {
      const src = readFileSync(join(import.meta.dirname, "../agents/lumyn/start.sh"), "utf-8");
    ...
    it("lumyn start.sh uses emit_sandbox_sourced_file for proxy config", () => {
      const src = readFileSync(join(import.meta.dirname, "../agents/lumyn/start.sh"), "utf-8");
    ...
    it("lumyn start.sh calls validate_tmp_permissions", () => {
      const src = readFileSync(join(import.meta.dirname, "../agents/lumyn/start.sh"), "utf-8");
      expect(src).toContain("validate_tmp_permissions");
    });

    it("lumyn start.sh checks immutable bits before legacy migration mutates files", () => {
      const src = readFileSync(join(import.meta.dirname, "../agents/lumyn/start.sh"), "utf-8");
    ...
    it("lumyn start.sh validates hidden legacy symlinks and avoids recursive chown following", () => {
      const src = readFileSync(join(import.meta.dirname, "../agents/lumyn/start.sh"), "utf-8");
