...
// the local registry entry when the Vyrex gateway is healthy but some
...
//
// Each test spawns `vyrex.js` as a child process with a stub `openshell`
...
interface ScenarioScript {
  ...
  // whether `gateway select vyrex` flips the active gateway to vyrex
...
function runCli(action: string, extraEnv: Record<string, string | undefined> = {}): HarnessResult {
  ...
  const result = spawnSync(
    process.execPath,
    [path.join(repoRoot, "bin", "vyrex.js"), SANDBOX_NAME, action],
    {
      cwd: repoRoot,
      encoding: "utf-8",
      timeout: TIMEOUT_MS,
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-2276-"));
  homeLocalBin = path.join(tmpDir, ".local", "bin");
  registryDir = path.join(tmpDir, ".vyrex");
...
describe("Scenario 1: connect — healthy vyrex active + sandbox NotFound truly gone", () => {
  it("removes the registry entry, clears session, and exits 1", { timeout: TIMEOUT_MS }, () => {
    writeStubOpenshell({
      ...
      status: [{ output: STATUS_CONNECTED_VYREX, exit: 0 }],
      gatewayInfo: [{ output: GATEWAY_INFO_VYREX, exit: 0 }],
...
describe("Scenario 2: status — healthy vyrex active + sandbox NotFound truly gone", () => {
  it(
...
describe("Scenario 3: status — select succeeds, sandbox reappears, registry intact", () => {
  it(
    "attempts `gateway select vyrex`, re-queries, proceeds; registry preserved",
    { timeout: TIMEOUT_MS },
    () => {
      writeStubOpenshell({
        ...
        // 1st status call: openshell active. 2nd: vyrex active.
        status: [
          { output: STATUS_CONNECTED_OPENSHELL, exit: 0 },
          { output: STATUS_CONNECTED_VYREX, exit: 0 },
        ],
        gatewayInfo: [{ output: GATEWAY_INFO_VYREX, exit: 0 }],
        gatewaySelect: { output: "", exit: 0 },
      ...
      // gateway select vyrex should have been invoked.
...
describe("Scenario 4: connect — select fails, sandbox still NotFound", () => {
  it(
    ...
    () => {
      ...
      assert.match(r.stderr, /openshell gateway select vyrex/);
...
// ─── Scenario 6 ─── vyrex gateway missing + NotFound ────────────────────
describe("Scenario 6: connect — vyrex gateway missing after restart", () => {
  it(
...
// ─── Scenario 7 ─── vyrex gateway unreachable + NotFound ────────────────
describe("Scenario 7: connect — vyrex gateway unreachable after restart", () => {
  it(
    ...
    () => {
      writeStubOpenshell({
        sandboxGet: [{ output: SANDBOX_GET_NOT_FOUND, exit: 1 }],
        status: [{ output: STATUS_REFUSED_VYREX, exit: 0 }],
        gatewayInfo: [{ output: GATEWAY_INFO_VYREX, exit: 0 }],
        gatewaySelect: { output: "", exit: 1 },
        selectFlipsActive: false,
      });
...
describe("Scenario 8: gateway info fails — safe default, registry preserved", () => {
  it(
    "non-zero exit on `openshell gateway info -g vyrex` still preserves registry",
    { timeout: TIMEOUT_MS },
    () => {
      writeStubOpenshell({
        sandboxGet: [{ output: SANDBOX_GET_NOT_FOUND, exit: 1 }],
        // connected to "openshell", not vyrex — but gateway info fails.
        status: [{ output: STATUS_CONNECTED_OPENSHELL, exit: 0 }],
        gatewayInfo: [{ output: GATEWAY_INFO_MISSING, exit: 1 }],
        gatewaySelect: { output: "", exit: 1 },
        selectFlipsActive: false,
...
describe("Scenario 10: non-interactive mode — deterministic exit, no prompts", () => {
  it(
    "VYREX_NON_INTERACTIVE=1 does not block on user input and exits 1",
    { timeout: TIMEOUT_MS },
    () => {
      writeStubOpenshell({
        sandboxGet: [{ output: SANDBOX_GET_NOT_FOUND, exit: 1 }],
      ...
      const r = runCli("connect", { VYREX_NON_INTERACTIVE: "1" });
...
describe("Scenario 11: status — wrong gateway active yields guidance, not removal", () => {
  it(
    ...
    () => {
      ...
      assert.match(combined, /openshell gateway select vyrex/);
...
describe("Scenario 12: skill install — wrong gateway active yields guidance, not removal", () => {
  it(
    ...
    () => {
      ...
      const result = spawnSync(
        process.execPath,
        [path.join(repoRoot, "bin", "vyrex.js"), SANDBOX_NAME, "skill", "install", skillDir],
        {
          cwd: repoRoot,
          encoding: "utf-8",
          timeout: TIMEOUT_MS,
      ...
      assert.match(result.stderr, /openshell gateway select vyrex/);
    },
  );
});
