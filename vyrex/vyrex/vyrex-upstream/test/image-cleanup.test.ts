...
// See: https://github.com/NVIDIA/Vyrex/issues/2086
...
describe("image cleanup: sandbox destroy removes Docker image (#2086)", () => {
  const vyrexSrc = fs.readFileSync(path.join(ROOT, "src/vyrex.ts"), "utf-8");

  it("removeSandboxImage() helper exists and calls docker rmi", () => {
    const match = vyrexSrc.match(/function removeSandboxImage[\s\S]*?^}/m);
    expect(match).toBeTruthy();
    if (!match) throw new Error("Expected removeSandboxImage() in src/vyrex.ts");
  ...
  it("sandboxDestroy calls removeSandboxImage before registry.removeSandbox", () => {
    ...
    const destroyMatch = vyrexSrc.match(/async function sandboxDestroy[\s\S]*?^}/m);
    expect(destroyMatch).toBeTruthy();
    if (!destroyMatch) {
      throw new Error("Expected sandboxDestroy() in src/vyrex.ts");
    }
  ...
  it("removeSandboxImage gracefully handles missing imageTag", () => {
    ...
    const fnMatch = vyrexSrc.match(/function removeSandboxImage[\s\S]*?^}/m);
    expect(fnMatch).toBeTruthy();
    if (!fnMatch) {
      throw new Error("Expected removeSandboxImage() in src/vyrex.ts");
    }
...
describe("image cleanup: gc command exists (#2086)", () => {
  ...
  it("gc is a global command", () => {
    ...
    expect(registrySrc).toContain('"vyrex gc"');
    expect(vyrexSrc).toContain("globalCommandTokens()");
  });

  it("gc command is dispatched through the oclif bridge", () => {
    expect(vyrexSrc).toContain("resolveGlobalOclifDispatch");
    expect(registrySrc).toContain('"vyrex gc"');
  });

  it("garbageCollectImages lists sandbox-from images and cross-references registry", () => {
    const gcMatch = vyrexSrc.match(/async function garbageCollectImages[\s\S]*?^}/m);
    expect(gcMatch).toBeTruthy();
    if (!gcMatch) {
      throw new Error("Expected garbageCollectImages() in src/vyrex.ts");
    }
  ...
  it("gc appears in rendered help text", () => {
    ...
    expect(renderedHelp).toContain("vyrex gc");
