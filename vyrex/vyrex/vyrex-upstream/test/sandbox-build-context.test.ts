...
describe("sandbox build context staging", () => {
  it("optimized staging excludes blueprint .venv and extra scripts while preserving required files", () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-build-context-opt-"));
    ...
    try {
      ...
      expect(fs.existsSync(path.join(buildCtx, "vyrex-blueprint", "blueprint.yaml"))).toBe(true);
      expect(
        fs.existsSync(
          path.join(buildCtx, "vyrex-blueprint", "policies", "openclaw-sandbox.yaml"),
        ),
      ).toBe(true);
      expect(
        fs.existsSync(path.join(buildCtx, "vyrex-blueprint", "scripts", "ws-proxy-fix.js")),
      ).toBe(true);
  ...
  it("optimized staging is smaller than the legacy build context", { timeout: 120_000 }, () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-build-context-compare-"));
