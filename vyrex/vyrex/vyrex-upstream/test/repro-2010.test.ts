...
const CLI_PATH = path.join(REPO_ROOT, "bin", "vyrex.js");
...
describe("issue #2010 — policy state inconsistency", () => {
  ...
  describe("sandboxPolicyList — CLI output via subprocess", () => {
    function runPolicyList(opts: {
      ...
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-repro-2010-"));
      const gw =
...
process.argv = ["node", "vyrex.js", "test-sandbox", "policy-list"];
