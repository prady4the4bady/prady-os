...
describe("stale gateway detection", () => {
  it("detects active vyrex gateway from real output", () => {
    // Actual output from `openshell gateway info -g vyrex` (ANSI stripped)
    const output = [
      "Gateway Info",
      "",
      "  Gateway: vyrex",
  ...
  it("returns false for a different gateway name", () => {
    // If someone ran a non-vyrex gateway, we should not touch it
    const output = [
      "Gateway Info",
      "",
