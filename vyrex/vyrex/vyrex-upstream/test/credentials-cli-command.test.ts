...
const RUNTIME_PATH = require.resolve(path.join(REPO_ROOT, "dist", "vyrex.js"));
...
describe("credentials oclif commands", () => {
  it("prints top-level credentials usage", async () => {
    ...
    expect(output.stdout).toContain("Usage: vyrex credentials <subcommand>");
  ...
  it("exits when provider list cannot query the gateway", async () => {
    ...
    expect(output.stderr).toContain("openshell gateway start --name vyrex");
  });
  ...
  it("deletes a provider credential with --yes", async () => {
    ...
    expect(output.stdout).toContain("Re-run 'vyrex onboard'");
  });
