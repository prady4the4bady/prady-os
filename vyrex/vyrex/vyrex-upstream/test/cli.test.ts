...
const CLI = path.join(import.meta.dirname, "..", "bin", "vyrex.js");
...
function runWithEnv(
  ...
  try {
    const out = execSync(`node "${CLI}" ${args}`, {
      ...
      env: {
        ...process.env,
        HOME: "/tmp/vyrex-cli-test-" + Date.now(),
...
function writeSandboxRegistry(
  ...
  const registryDir = path.join(home, ".vyrex");
...
function createCloudflaredServiceDir(prefix: string): { sandboxName: string; serviceDir: string } {
  ...
  const serviceDir = path.join("/tmp", `vyrex-services-${sandboxName}`);
...
describe("CLI dispatch", () => {
  ...
  it("no args exits 0 (shows help)", () => {
    const r = run("");
    expect(r.code).toBe(0);
    expect(r.out.includes("vyrex")).toBeTruthy();
  });
  ...
  it("suggests list for a mistyped list command", () => {
    ...
    expect(r.out).toContain("Did you mean: vyrex list?");
  });

  it("recovers a live sandbox before suggesting a bare command typo", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-recover-typo-"));
    const localBin = path.join(home, "bin");
    fs.mkdirSync(localBin, { recursive: true });
    fs.writeFileSync(
      path.join(localBin, "openshell"),
      [
        ...
        '  "gateway info -g vyrex") printf "Gateway: vyrex\\n"; exit 0 ;;',
    ...
    const r = runWithEnv("liost", {
      HOME: home,
      PATH: `${localBin}:${process.env.PATH || ""}`,
      VYREX_CONNECT_TIMEOUT: "1",
      VYREX_NO_CONNECT_HINT: "1",
    });
    expect(r.code).toBe(0);
  ...
  it("explains sandbox connect command order when the sandbox name is last", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-connect-order-"));
    const localBin = path.join(home, "bin");
    fs.mkdirSync(localBin, { recursive: true });
    writeSandboxRegistry(home);
    fs.writeFileSync(
    ...
    const r = runWithEnv("lumyn connect alpha", {
      HOME: home,
      PATH: `${localBin}:${process.env.PATH || ""}`,
    });

    expect(r.code).toBe(1);
    expect(r.out).toContain("Sandbox 'lumyn' does not exist");
    expect(r.out).toContain("Command order is: vyrex <sandbox-name> connect");
    expect(r.out).toContain("Did you mean: vyrex alpha connect?");
  });
  ...
  it("vyrex-lumyn list --help uses alias branding", () => {
    const out = execSync(`node "${LUMYN_CLI}" list --help`, {
      encoding: "utf-8",
      stdio: "pipe",
      timeout: execTimeout(),
      env: {
    ...
    expect(out).toContain("$ vyrex-lumyn list [--json]");
    expect(out).not.toContain("$ vyrex list [--json]");
  });
  ...
  it("list --json emits structured sandbox details", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-list-json-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    fs.mkdirSync(localBin, { recursive: true });
    fs.mkdirSync(registryDir, { recursive: true });
  ...
  it("status --json emits parseable structured status without credentials", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-status-json-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
  ...
  it("credentials help exits 0 and shows credential subcommands", () => {
    ...
    expect(r.code).toBe(0);
    expect(r.out).toContain("Usage: vyrex credentials <subcommand>");
  ...
  it("credentials reset without provider keeps provider-specific usage", () => {
    ...
    expect(r.code).toBe(1);
    expect(r.out).toContain("Usage: vyrex credentials reset <PROVIDER> [--yes]");
  ...
  it("maintenance commands dispatch through oclif", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-maintenance-"));
    const localBin = path.join(home, "bin");
    fs.mkdirSync(localBin, { recursive: true });
    fs.writeFileSync(
      path.join(localBin, "docker"),
  ...
  it("shows skill install help when --help follows install", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-skill-help-"));
    writeSandboxRegistry(home);

    const r = runWithEnv("alpha skill install --help", { HOME: home });

    expect(r.code).toBe(0);
    expect(r.out).toContain("Usage: vyrex <sandbox> skill install <path>");
  ...
  it("points plugin-shaped directories away from skill install", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-plugin-hint-"));
    ...
    expect(r.out).toContain("vyrex onboard --from <Dockerfile>");
  });

  it("detects openclaw.plugin.json as a plugin marker", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-plugin-marker-"));
    const pluginDir = path.join(home, "openclaw-plugin");
    fs.mkdirSync(pluginDir, { recursive: true });
    writeSandboxRegistry(home);
    fs.writeFileSync(
  ...
  it(
    ...
    () => {
      const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-start-no-key-"));
      const localBin = path.join(home, "bin");
      const registryDir = path.join(home, ".vyrex");
      ...
      expect(r.out).toContain("Vyrex Services");
    },
  );
  ...
  it("onboard --help exits 0 and shows usage", () => {
    ...
    expect(r.code).toBe(0);
    expect(r.out.includes("Usage: vyrex onboard")).toBeTruthy();
  ...
  it("resume rejection clarifies --resume semantics and points to onboard (#2281)", () => {
    ...
    expect(r.out.includes("vyrex onboard")).toBeTruthy();
  });
  ...
  it("setup-spark --help exits 0 and shows onboard usage", () => {
    ...
    expect(r.out.includes("Use `vyrex onboard` instead")).toBeTruthy();
    expect(r.out.includes("Usage: vyrex onboard")).toBeTruthy();
    expect(r.out.includes("Unknown onboard option")).toBeFalsy();
  });
  ...
  it("debug --help exits 0 and shows usage", () => {
    ...
    expect(r.code).toBe(0);
    expect(r.out.includes("Collect Vyrex diagnostic information")).toBeTruthy();
  ...
  it("debug --quick exits 0 and produces diagnostic output", testTimeoutOptions(30_000), () => {
    const r = runWithEnv(
      "debug --quick",
      createDebugCommandTestEnv("vyrex-cli-debug-quick-"),
      30000,
    );
    expect(r.code).toBe(0);
  ...
  it("debug warns when default sandbox is stale", testTimeoutOptions(), () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-stale-"));
    fs.mkdirSync(path.join(home, ".vyrex"), { recursive: true });
    fs.writeFileSync(
      path.join(home, ".vyrex", "sandboxes.json"),
  ...
  it("gateway-token help keeps the public sandbox-scoped usage", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-token-help-"));
    writeSandboxRegistry(home);

    const r = runWithEnv("alpha gateway-token --help", { HOME: home });

    expect(r.code).toBe(0);
    expect(r.out).toContain("Usage: vyrex <name> gateway-token [--quiet|-q]");
    expect(r.out).not.toContain("sandbox:gateway-token");
  });

  it("doctor fails a present sandbox that is not Ready", () => {
    const setup = createDoctorTestSetup("vyrex-cli-doctor-not-ready-", [
      'case "$*" in',
  ...
  it("doctor does not query sandbox state from a different active gateway", () => {
    const setup = createDoctorTestSetup("vyrex-cli-doctor-wrong-gateway-", [
      'case "$*" in',
      '  "status") printf "Server Status\\n\\n  Gateway: other\\n  Status: Connected\\n"; exit 0 ;;',
      '  "gateway info -g vyrex") printf "Gateway: vyrex\\n"; exit 0 ;;',
      '  "gateway select vyrex") exit 1 ;;',
      '  "gateway start --name vyrex --port 8080") exit 1 ;;',
  ...
  it("doctor treats a live non-cloudflared PID as stale", () => {
    ...
    const setup = createDoctorTestSetup(
      "vyrex-cli-doctor-wrong-cloudflared-pid-",
      [
        'case "$*" in',
        '  "status") printf "Server Status\\n\\n  Gateway: vyrex\\n  Status: Connected\\n"; exit 0 ;;',
        '  "gateway info -g vyrex") printf "Gateway: vyrex\\n"; exit 0 ;;',
  ...
  it("doctor accepts a live cloudflared PID", () => {
    ...
    const setup = createDoctorTestSetup(
      "vyrex-cli-doctor-cloudflared-pid-",
      [
        'case "$*" in',
        '  "status") printf "Server Status\\n\\n  Gateway: vyrex\\n  Status: Connected\\n"; exit 0 ;;',
        '  "gateway info -g vyrex") printf "Gateway: vyrex\\n"; exit 0 ;;',
    ...
    const shimDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cloudflared-shim-"));
  ...
  it("sandbox inspection help keeps public sandbox-scoped usage", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-inspection-help-"));
    writeSandboxRegistry(home);

    const connect = runWithEnv("alpha connect --help", { HOME: home });
    expect(connect.code).toBe(0);
    expect(connect.out).toContain("Usage: vyrex alpha connect");
  ...
  it("policy mutation dry-run paths dispatch through oclif", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-policy-dry-run-"));
    writeSandboxRegistry(home);
    ...
    const registryPath = path.join(home, ".vyrex", "sandboxes.json");
  ...
  it("channels mutation dry-run paths dispatch through oclif", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-channels-dry-run-"));
    writeSandboxRegistry(home);
  ...
  it("shields help keeps public sandbox-scoped usage", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-shields-help-"));
    writeSandboxRegistry(home);
  ...
  it("snapshot subcommand help keeps public sandbox-scoped usage", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-snapshot-help-"));
    writeSandboxRegistry(home);
  ...
  it("snapshot list dispatches through oclif", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-snapshot-list-"));
    writeSandboxRegistry(home);
  ...
  it("routes logs to OpenClaw and OpenShell log sources", () => {
    const setup = createLogsTestSetup("vyrex-cli-logs-routing-");
  ...
  it("enables OpenShell audit events before reading logs", () => {
    const setup = createLogsTestSetup("vyrex-cli-logs-audit-");
    const r = setup.runLogs();
  ...
  it("warns when OpenShell audit events cannot be enabled", () => {
    const setup = createLogsTestSetup("vyrex-cli-logs-audit-failed-", [
      'if [ "$1" = "settings" ]; then',
  ...
  it("continues log collection when audit enable times out", () => {
    const setup = createLogsTestSetup("vyrex-cli-logs-audit-timeout-", [
      'if [ "$1" = "settings" ]; then',
      "  sleep 5",
      "  exit 0",
      "fi",
    ...
    const r = setup.runLogs("alpha logs 2>&1", { VYREX_LOGS_PROBE_TIMEOUT_MS: "1500" });
  ...
  it("continues to OpenShell logs when the OpenClaw gateway log probe times out", () => {
    const setup = createLogsTestSetup("vyrex-cli-logs-openclaw-timeout-", [
      'if [ "$1" = "sandbox" ]; then',
      "  sleep 2",
      "  exit 0",
      "fi",
    ...
    const r = setup.runLogs("alpha logs 2>&1", { VYREX_LOGS_PROBE_TIMEOUT_MS: "500" });
  ...
  it("maps --follow to OpenShell live log streaming", () => {
    const setup = createLogsTestSetup("vyrex-cli-logs-follow-");
  ...
  it("starts OpenClaw logs before enabling audit logs for logs --follow", () => {
    const setup = createLogsTestSetup("vyrex-cli-logs-follow-audit-slow-", [
      'if [ "$1" = "settings" ]; then',
      "  sleep 1",
      "  exit 0",
      "fi",
    ...
    const r = setup.runLogs("alpha logs --follow", { VYREX_LOGS_PROBE_TIMEOUT_MS: "2000" });
    const calls = setup.readCalls();
  ...
  it("keeps logs --follow running when one log source exits", testTimeoutOptions(), async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-logs-follow-source-exit-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
  ...
  it("waits for logs --follow children to stop after SIGTERM", testTimeoutOptions(), async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-logs-follow-sigterm-wait-"));
    const localBin = path.join(home, "bin");
  ...
  it("uses named sandbox exec for bridge status helpers", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-status-messaging-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
  ...
  it("destroys the gateway runtime when the last sandbox is removed", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-destroy-last-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    ...
    expect(fs.readFileSync(openshellLog, "utf8")).toContain("gateway destroy -g vyrex");
  ...
  it("keeps the gateway runtime when other sandboxes still exist", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-destroy-shared-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    const openshellLog = path.join(home, "openshell.log");
    const bashLog = path.join(home, "docker.log");
    ...
    expect(fs.readFileSync(openshellLog, "utf8")).not.toContain("gateway destroy -g vyrex");
  ...
  it("keeps the gateway runtime when the live gateway still reports sandboxes", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-destroy-live-shared-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    const openshellLog = path.join(home, "openshell.log");
    const bashLog = path.join(home, "docker.log");
  ...
  it("fails destroy when openshell sandbox delete returns a real error", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-destroy-failure-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    const openshellLog = path.join(home, "openshell.log");
    fs.mkdirSync(localBin, { recursive: true });
  ...
  it("treats an already-missing sandbox as destroyed and clears the stale registry entry", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-destroy-missing-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    const openshellLog = path.join(home, "openshell.log");
    const bashLog = path.join(home, "docker.log");
  ...
  it("deletes messaging providers when destroying a sandbox", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-destroy-providers-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    const openshellLog = path.join(home, "openshell.log");
    const bashLog = path.join(home, "docker.log");
  ...
  it("passes plain logs through without the tail flag", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-logs-plain-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
  ...
  it("connect does not pre-start a duplicate port forward", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-connect-forward-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
  ...
  it("shows connect help without opening an interactive session", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-connect-help-"));
    const localBin = path.join(home, "bin");
    const markerFile = path.join(home, "openshell-calls");
    ...
    expect(r.code).toBe(0);
    expect(r.out).toContain("Usage: vyrex alpha connect");
    expect(r.out).toContain("--probe-only");
    expect(implicit.code).toBe(0);
    expect(implicit.out).toContain("Usage: vyrex alpha connect");
    expect(fs.existsSync(markerFile)).toBe(false);
    expect(fs.existsSync(sshMarkerFile)).toBe(false);
  });

  it("rejects the removed skip-permissions connect flag", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-connect-probe-flags-"));
    const localBin = path.join(home, "bin");
    const markerFile = path.join(home, "openshell-calls");
    const sshMarkerFile = path.join(home, "ssh-calls");
    fs.mkdirSync(localBin, { recursive: true });
  ...
  it("connect --probe-only recovers the gateway without opening SSH", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-connect-probe-"));
    const localBin = path.join(home, "bin");
    const markerFile = path.join(home, "openshell-calls");
    const sshMarkerFile = path.join(home, "ssh-calls");
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "      echo '__VYREX_SANDBOX_EXEC_STARTED__'",
  ...
  it("waits for recovered gateway health before failing probe-only", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-connect-probe-wait-"));
    const localBin = path.join(home, "bin");
    const markerFile = path.join(home, "openshell-calls");
    const stateFile = path.join(home, "probe-state");
    ...
    const r = runWithEnv("alpha connect --probe-only", {
      HOME: home,
      PATH: `${localBin}:${process.env.PATH || ""}`,
      VYREX_GATEWAY_RECOVERY_WAIT_SECONDS: "3",
      VYREX_GATEWAY_RECOVERY_POLL_INTERVAL_SECONDS: "0",
    });
  ...
  it("treats leading --probe-only as an implicit connect probe", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-connect-probe-leading-"));
    const localBin = path.join(home, "bin");
    const markerFile = path.join(home, "openshell-calls");
    const sshMarkerFile = path.join(home, "ssh-calls");
    fs.mkdirSync(localBin, { recursive: true });
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'if [ "$1" = "sandbox" ] && [ "$2" = "exec" ] && [ "$3" = "--name" ] && [ "$4" = "alpha" ]; then',
        '  cmd="$8"',
        '  if [[ "$cmd" == *"curl -sf"* ]]; then echo "__VYREX_SANDBOX_EXEC_STARTED__"; echo RUNNING; exit 0; fi',
        '  if [[ "$cmd" == *"OPENCLAW="* ]]; then echo "__VYREX_SANDBOX_EXEC_STARTED__"; echo UNEXPECTED_RECOVERY; exit 1; fi',
        "fi",
        "exit 0",
      ].join("\n"),
      { mode: 0o755 },
  ...
  it("connect --probe-only does not retry a failed sandbox exec recovery over SSH", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-connect-probe-no-ssh-"));
    const localBin = path.join(home, "bin");
    const markerFile = path.join(home, "openshell-calls");
    const sshMarkerFile = path.join(home, "ssh-calls");
    fs.mkdirSync(localBin, { recursive: true });
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'if [ "$1" = "sandbox" ] && [ "$2" = "exec" ] && [ "$3" = "--name" ] && [ "$4" = "alpha" ]; then',
        '  cmd="$8"',
        '  if [[ "$cmd" == *"OPENCLAW="* ]]; then echo "__VYREX_SANDBOX_EXEC_STARTED__"; echo RECOVERY_FAILED >&2; exit 42; fi',
        '  if [[ "$cmd" == *"curl -sf"* ]]; then echo "__VYREX_SANDBOX_EXEC_STARTED__"; echo STOPPED; exit 0; fi',
        "fi",
  ...
  it("connect --probe-only falls back to SSH when sandbox exec never starts", () => {
    const home = fs.mkdtempSync(
      path.join(os.tmpdir(), "vyrex-cli-connect-probe-exec-fallback-"),
    );
    const localBin = path.join(home, "bin");
  ...
  it("connect --probe-only falls back to SSH when sandbox exec times out after starting", () => {
    const home = fs.mkdtempSync(
      path.join(os.tmpdir(), "vyrex-cli-connect-probe-exec-timeout-"),
    );
    const localBin = path.join(home, "bin");
    const openshellCalls = path.join(home, "openshell-calls");
    const sshCalls = path.join(home, "ssh-calls");
    ...
    const r = runWithEnv("alpha connect --probe-only", {
      HOME: home,
      PATH: `${localBin}:${process.env.PATH || ""}`,
      VYREX_SANDBOX_EXEC_TIMEOUT_MS: "50",
    });
  ...
  it("recovers non-OpenClaw agents over SSH instead of root sandbox exec", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-connect-probe-agent-"));
    const localBin = path.join(home, "bin");
    const openshellCalls = path.join(home, "openshell-calls");
    const sshCalls = path.join(home, "ssh-calls");
    const stateFile = path.join(home, "probe-state");
    fs.mkdirSync(localBin, { recursive: true });
    writeSandboxRegistry(home, { agent: "lumyn" });
    fs.writeFileSync(stateFile, "stopped");
    fs.writeFileSync(
      path.join(localBin, "openshell"),
      [
        ...
        "    exit 0",
        "  fi",
        '  if [[ "$cmd" == *"LUMYN_HOME=/sandbox/.lumyn"* || "$cmd" == *"AGENT_BIN="* ]]; then',
        "    echo '__VYREX_SANDBOX_EXEC_STARTED__'",
    ...
    fs.writeFileSync(
      ...
      [
        ...
        'printf \'ARGS %s\\n\' "$*" >> "$calls"',
        'printf \'CMD %s\\n\' "$cmd" >> "$calls"',
        'if [[ "$cmd" == *"AGENT_BIN=\'/usr/local/bin/lumyn\'"* ]]; then',
        '  echo recovered > "$state_file"',
        "  echo 'GATEWAY_PID=789'",
        "  exit 0",
        "fi",
    ...
    expect(r.code).toBe(0);
    expect(r.out).toContain("Probe complete: recovered Lumyn Agent gateway");
    const openshellLog = fs.readFileSync(openshellCalls, "utf8");
    const sshLog = fs.readFileSync(sshCalls, "utf8");
    expect(openshellLog).toContain("sandbox exec --name alpha -- sh -c");
    expect(openshellLog).toContain("sandbox ssh-config alpha");
    expect(openshellLog).not.toContain("LUMYN_HOME=/sandbox/.lumyn");
    expect(openshellLog).not.toContain("AGENT_BIN=");
    expect(openshellLog).not.toContain("sandbox connect");
    expect(sshLog).toContain("LUMYN_HOME=/sandbox/.lumyn");
    expect(sshLog).toContain("AGENT_BIN='/usr/local/bin/lumyn'");
    expect(sshLog).not.toMatch(/(^|\s)-tt?(\s|$)/);
  });

  it("waits for sandbox readiness before connecting", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-connect-wait-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    const markerFile = path.join(home, "openshell-calls");
  ...
  it("prints recovery guidance when readiness polling hits a terminal sandbox state", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-connect-failed-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    const markerFile = path.join(home, "openshell-calls");
    fs.mkdirSync(localBin, { recursive: true });
    ...
    expect(r.out.includes("vyrex alpha logs --follow")).toBeTruthy();
    expect(r.out.includes("vyrex alpha status")).toBeTruthy();
    const calls = fs.readFileSync(markerFile, "utf8").trim().split("\n").filter(Boolean);
    expect(calls).toContain("sandbox get alpha");
  ...
  it("removes stale registry entries when connect targets a missing live sandbox", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-stale-connect-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    fs.mkdirSync(localBin, { recursive: true });
    fs.mkdirSync(registryDir, { recursive: true });
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "  exit 1",
        "fi",
        // Simulate a healthy, active `vyrex` named gateway so the
  ...
  it("recovers a missing registry entry from the last onboard session during list", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-list-session-recover-"));
    const localBin = path.join(home, "bin");
    const vyrexDir = path.join(home, ".vyrex");
    fs.mkdirSync(localBin, { recursive: true });
    fs.mkdirSync(vyrexDir, { recursive: true });
    fs.writeFileSync(
      path.join(vyrexDir, "sandboxes.json"),
      JSON.stringify({
        sandboxes: {
          gamma: {
            name: "gamma",
    ...
    fs.writeFileSync(
      path.join(vyrexDir, "onboard-session.json"),
      JSON.stringify(
        {
          version: 1,
          sessionId: "session-1",
    ...
    const saved = JSON.parse(fs.readFileSync(path.join(vyrexDir, "sandboxes.json"), "utf8"));
  ...
  it("imports additional live sandboxes into the registry during list recovery", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-list-live-recover-"));
    const localBin = path.join(home, "bin");
    const vyrexDir = path.join(home, ".vyrex");
    fs.mkdirSync(localBin, { recursive: true });
    fs.mkdirSync(vyrexDir, { recursive: true });
  ...
  it("skips invalid recovered sandbox names during list recovery", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-list-invalid-recover-"));
    const localBin = path.join(home, "bin");
    const vyrexDir = path.join(home, ".vyrex");
    fs.mkdirSync(localBin, { recursive: true });
    fs.mkdirSync(vyrexDir, { recursive: true });
  ...
  it("connect recovers a named sandbox from the last onboard session when the registry is empty", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-connect-recover-session-"));
    const localBin = path.join(home, "bin");
    const vyrexDir = path.join(home, ".vyrex");
  ...
  it("connect surfaces sandbox-not-found when recovery cannot find the requested sandbox (#2164)", () => {
    const home = fs.mkdtempSync(
      path.join(os.tmpdir(), "vyrex-cli-connect-unknown-after-recovery-"),
    );
    const localBin = path.join(home, "bin");
    const vyrexDir = path.join(home, ".vyrex");
    fs.mkdirSync(localBin, { recursive: true });
  ...
  it("preserves SIGINT exit semantics for logs --follow", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-logs-sigint-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    fs.mkdirSync(localBin, { recursive: true });
    fs.mkdirSync(registryDir, { recursive: true });
  ...
  it(
    ...
    () => {
      const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-gateway-error-"));
      const localBin = path.join(home, "bin");
      const registryDir = path.join(home, ".vyrex");
      fs.mkdirSync(localBin, { recursive: true });
      fs.mkdirSync(registryDir, { recursive: true });
  ...
  it(
    ...
    () => {
      const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-status-timeout-"));
      const localBin = path.join(home, "bin");
      const registryDir = path.join(home, ".vyrex");
      fs.mkdirSync(localBin, { recursive: true });
      fs.mkdirSync(registryDir, { recursive: true });
      ...
      const r = runWithEnv(
        "alpha status",
        {
          HOME: home,
          PATH: `${localBin}:${process.env.PATH || ""}`,
          VYREX_STATUS_PROBE_TIMEOUT_MS: "100",
        },
        10000,
      );
  ...
  it("recovers status after gateway runtime is reattached", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-recover-status-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "  exit 0",
        "fi",
        'if [ "$1" = "gateway" ] && [ "$2" = "info" ] && [ "$3" = "-g" ] && [ "$4" = "vyrex" ]; then',
        "  echo 'Gateway Info'",
        "  echo",
        "  echo '  Gateway: vyrex'",
        "  exit 0",
    ...
    expect(r.code).toBe(0);
    expect(r.out.includes("Recovered Vyrex gateway runtime")).toBeTruthy();
    expect(r.out.includes("Sandbox: alpha")).toBeTruthy();
  });

  it("shows a clear local inference warning when Ollama is down", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-local-inference-down-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    fs.mkdirSync(localBin, { recursive: true });
    fs.mkdirSync(registryDir, { recursive: true });
  ...
  it(
    "does not treat a different connected gateway as a healthy vyrex gateway",
    () => {
      const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-mixed-gateway-"));
      const localBin = path.join(home, "bin");
      const registryDir = path.join(home, ".vyrex");
      fs.mkdirSync(localBin, { recursive: true });
      fs.mkdirSync(registryDir, { recursive: true });
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "  exit 0",
          "fi",
          'if [ "$1" = "gateway" ] && [ "$2" = "select" ] && [ "$3" = "vyrex" ]; then',
          "  exit 0",
          "fi",
          'if [ "$1" = "gateway" ] && [ "$2" = "start" ] && [ "$3" = "--name" ] && [ "$4" = "vyrex" ]; then',
          "  exit 0",
          "fi",
          'if [ "$1" = "inference" ] && [ "$2" = "get" ]; then',
          "  exit 0",
      ...
      expect(r.code).toBe(0);
      expect(r.out.includes("Recovered Vyrex gateway runtime")).toBeFalsy();
      expect(r.out.includes("Could not verify sandbox 'alpha'")).toBeTruthy();
  ...
  it(
    ...
    () => {
      const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-ansi-transport-hint-"));
      const localBin = path.join(home, "bin");
      const registryDir = path.join(home, ".vyrex");
      fs.mkdirSync(localBin, { recursive: true });
      fs.mkdirSync(registryDir, { recursive: true });
  ...
  it(
    ...
    () => {
      const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-ansi-auth-hint-"));
      const localBin = path.join(home, "bin");
      const registryDir = path.join(home, ".vyrex");
      fs.mkdirSync(localBin, { recursive: true });
      fs.mkdirSync(registryDir, { recursive: true });
  ...
  it("explains unrecoverable gateway trust rotation after restart", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-identity-drift-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    fs.mkdirSync(localBin, { recursive: true });
    fs.mkdirSync(registryDir, { recursive: true });
  ...
  it(
    ...
    () => {
      const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-gateway-unreachable-"));
      const localBin = path.join(home, "bin");
      const registryDir = path.join(home, ".vyrex");
      fs.mkdirSync(localBin, { recursive: true });
      fs.mkdirSync(registryDir, { recursive: true });
      ...
      expect(
        statusResult.out.includes("Retry `openshell gateway start --name vyrex`"),
      ).toBeTruthy();
  ...
  it(
    ...
    () => {
      const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-gateway-missing-"));
      const localBin = path.join(home, "bin");
      const registryDir = path.join(home, ".vyrex");
      fs.mkdirSync(localBin, { recursive: true });
      fs.mkdirSync(registryDir, { recursive: true });
...
describe("list shows live gateway inference", () => {
  it("shows live gateway inference for the default sandbox (#2369)", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-list-live-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    fs.mkdirSync(localBin, { recursive: true });
    fs.mkdirSync(registryDir, { recursive: true });
  ...
  it("falls back to registry values when openshell inference get fails", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-list-fallback-"));
    const localBin = path.join(home, "bin");
    const registryDir = path.join(home, ".vyrex");
    fs.mkdirSync(localBin, { recursive: true });
    fs.mkdirSync(registryDir, { recursive: true });
  ...
  // ── Issue #1904: sandbox not upgraded after Vyrex upgrade ───
  // Original report: user upgrades Vyrex from v0.0.11→v0.0.15 via
  ...
  it(
    "upgrade-sandboxes --check detects a stale sandbox after Vyrex upgrade (#1904)",
    testTimeoutOptions(),
    () => {
      const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-upgrade-sandboxes-"));
      const localBin = path.join(home, "bin");
      const vyrexDir = path.join(home, ".vyrex");
      fs.mkdirSync(localBin, { recursive: true });
      fs.mkdirSync(vyrexDir, { recursive: true });
  ...
  it(
    ...
    testTimeoutOptions(),
    () => {
      const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-upgrade-current-"));
      const localBin = path.join(home, "bin");
      const vyrexDir = path.join(home, ".vyrex");
      fs.mkdirSync(localBin, { recursive: true });
      fs.mkdirSync(vyrexDir, { recursive: true });
  ...
  it("share with no subcommand prints usage help", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-share-"));
    writeSandboxRegistry(home);

    const r = runWithEnv("alpha share", { HOME: home });

    expect(r.code).toBe(1);
    expect(r.out).toContain("Usage: vyrex <name> share");
  ...
  it("share is recognized as a valid sandbox action (not 'Unknown action')", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-share-action-"));
    writeSandboxRegistry(home);
