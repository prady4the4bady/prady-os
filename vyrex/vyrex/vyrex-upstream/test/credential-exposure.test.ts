...
import { buildSubprocessEnv as buildPluginSubprocessEnv } from "../vyrex/src/lib/subprocess-env";
...
const RUNNER_TS = path.join(import.meta.dirname, "..", "vyrex", "src", "blueprint", "runner.ts");
...
describe("credential exposure in process arguments", () => {
  ...
  it("subprocess env builder does not spread full process.env into subprocesses", () => {
    ...
    try {
      ...
      process.env.PATH = `/tmp/vyrex-fake-bin:${process.env.PATH || ""}`;
      const env = buildCliSubprocessEnv();
      expect(env.NVIDIA_API_KEY).toBeUndefined();
      expect(env.PATH).toContain("/tmp/vyrex-fake-bin");
    } finally {
