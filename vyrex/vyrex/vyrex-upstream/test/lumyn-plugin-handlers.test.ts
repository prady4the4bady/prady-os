...
const PLUGIN_PATH = path.join(import.meta.dirname, "..", "agents", "lumyn", "plugin", "__init__.py");
...
describe("Lumyn Vyrex plugin handlers", () => {
  it("accepts Lumyn dispatch kwargs for status, info, and reload handlers", () => {
...
spec = importlib.util.spec_from_file_location("lumyn_plugin", plugin_path)
...
`);
    ...
    expect(result.status).toContain("Vyrex Sandbox Status (Lumyn)");
