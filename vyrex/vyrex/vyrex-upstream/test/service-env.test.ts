...
const VYREX_START_SCRIPT = join(import.meta.dirname, "../scripts/vyrex-start.sh");

function extractRuntimeShellEnvSnippet() {
  const src = readFileSync(VYREX_START_SCRIPT, "utf-8");
  const start = src.indexOf("write_runtime_shell_env() {");
  const end = src.indexOf("# cleanup_on_signal", start);
  if (start === -1 || end === -1 || end <= start) {
    throw new Error(
      "Failed to extract write_runtime_shell_env from scripts/vyrex-start.sh — " +
...
function extractRuntimeShellEnvShimSnippet() {
  ...
  if (start === -1 || end === -1 || end <= start) {
    throw new Error(
      "Failed to extract ensure_runtime_shell_env_shim from scripts/vyrex-start.sh — " +
...
describe("service environment", () => {
  describe("start-services behavior", () => {
    ...
    it("starts without messaging-related warnings", { timeout: 30000 }, () => {
      const workspace = mkdtempSync(join(tmpdir(), "vyrex-services-no-key-"));
  ...
  describe("SANDBOX_NAME defaulting", () => {
    it("start-services.sh preserves existing SANDBOX_NAME", () => {
      const result = execSync(
        'bash -c \'SANDBOX_NAME="${VYREX_SANDBOX:-${SANDBOX_NAME:-default}}"; export SANDBOX_NAME; bash -c "echo \\$SANDBOX_NAME"\'',
        {
          encoding: "utf-8",
          env: { ...process.env, VYREX_SANDBOX: "", SANDBOX_NAME: "my-box" },
        },
      ).trim();
      expect(result).toBe("my-box");
    });

    it("start-services.sh uses VYREX_SANDBOX over SANDBOX_NAME", () => {
      const result = execSync(
        'bash -c \'SANDBOX_NAME="${VYREX_SANDBOX:-${SANDBOX_NAME:-default}}"; export SANDBOX_NAME; bash -c "echo \\$SANDBOX_NAME"\'',
        {
          encoding: "utf-8",
          env: { ...process.env, VYREX_SANDBOX: "from-env", SANDBOX_NAME: "old" },
        },
      ).trim();
    ...
    it("start-services.sh falls back to default when both unset", () => {
      const result = execSync(
        ...
        {
          encoding: "utf-8",
          env: { ...process.env, VYREX_SANDBOX: "", SANDBOX_NAME: "" },
        },
      ).trim();
  ...
  describe("GIT_SSL_CAINFO for proxy CA trust (issue #2270)", () => {
    ...
    it("entrypoint exports GIT_SSL_CAINFO when SSL_CERT_FILE points to a real file", () => {
      const scriptPath = join(import.meta.dirname, "../scripts/vyrex-start.sh");
      ...
      const fakeDir = mkdtempSync(join(tmpdir(), "vyrex-git-ssl-entrypoint-"));
      const fakeCaBundle = join(fakeDir, "ca-bundle.pem");
      const tmpFile = join(tmpdir(), `vyrex-git-ssl-entrypoint-${process.pid}.sh`);
      try {
        writeFileSync(
          fakeCaBundle,
          "-----BEGIN CERTIFICATE-----\nfake\n-----END CERTIFICATE-----\n",
    ...
    it("proxy-env.sh includes GIT_SSL_CAINFO when set", () => {
      const fakeDataDir = join(tmpdir(), `vyrex-git-ssl-test-${process.pid}`);
      const fakeCaBundle = join(fakeDataDir, "ca-bundle.pem");
      execFileSync("mkdir", ["-p", fakeDataDir]);
      const tmpFile = join(tmpdir(), `vyrex-git-ssl-env-${process.pid}.sh`);
      try {
        ...
        const wrapper = [
          ...
          persistBlock
            .trimEnd()
            .replaceAll("/tmp/vyrex-proxy-env.sh", `${fakeDataDir}/proxy-env.sh`),
        ].join("\n");
    ...
    it("proxy-env.sh omits GIT_SSL_CAINFO when not set", () => {
      const fakeDataDir = join(tmpdir(), `vyrex-git-ssl-noop-test-${process.pid}`);
      execFileSync("mkdir", ["-p", fakeDataDir]);
      const tmpFile = join(tmpdir(), `vyrex-git-ssl-noop-env-${process.pid}.sh`);
      try {
        const persistBlock = extractRuntimeShellEnvSnippet();
        const wrapper = [
          "#!/usr/bin/env bash",
  ...
  describe("XDG and tool cache redirects (issue #804)", () => {
    it("entrypoint pre-creates redirected dirs and restricts GNUPGHOME permissions", () => {
      ...
      const fakeTmp = mkdtempSync(join(tmpdir(), "vyrex-tool-redirects-"));
      const block = src.slice(start, end).replaceAll("/tmp/", `${fakeTmp}/`);
      const tmpFile = join(tmpdir(), `vyrex-tool-redirects-${process.pid}.sh`);
      try {
        writeFileSync(
          tmpFile,
          [
  ...
  describe("proxy environment variables (issue #626)", () => {
    ...
    function extractToolRedirects() {
      ...
      if (!block.trim()) {
        throw new Error(
          "Failed to extract _TOOL_REDIRECTS from scripts/vyrex-start.sh — " +
    ...
    function extractProxyVars(env = {}) {
      ...
      if (!proxyBlock.trim()) {
        throw new Error(
          "Failed to extract proxy configuration from scripts/vyrex-start.sh — " +
      ...
      const tmpFile = join(tmpdir(), `vyrex-proxy-test-${process.pid}.sh`);
      try {
        writeFileSync(tmpFile, wrapper, { mode: 0o700 });
    ...
    it("VYREX_PROXY_HOST overrides default gateway IP", () => {
      const vars = extractProxyVars({ VYREX_PROXY_HOST: "192.168.64.1" });
      expect(vars.HTTP_PROXY).toBe("http://192.168.64.1:3128");
      expect(vars.HTTPS_PROXY).toBe("http://192.168.64.1:3128");
    });

    it("VYREX_PROXY_PORT overrides default proxy port", () => {
      const vars = extractProxyVars({ VYREX_PROXY_PORT: "8080" });
    ...
    it("entrypoint writes proxy-env.sh to writable data dir", () => {
      const fakeDataDir = join(tmpdir(), `vyrex-data-test-${process.pid}`);
      execFileSync("mkdir", ["-p", fakeDataDir]);
      const tmpFile = join(tmpdir(), `vyrex-proxyenv-write-test-${process.pid}.sh`);
      try {
        const persistBlock = extractRuntimeShellEnvSnippet();
    ...
    it("backfills proxy-env.sh source shims into stale rc files", () => {
      const fakeHome = join(tmpdir(), `vyrex-rc-shim-test-${process.pid}`);
      const proxyEnvPath = join(fakeHome, "proxy-env.sh");
      const tmpFile = join(tmpdir(), `vyrex-rc-shim-write-test-${process.pid}.sh`);
    ...
    it("entrypoint overwrites proxy-env.sh cleanly on repeated invocations", () => {
      const fakeDataDir = join(tmpdir(), `vyrex-idempotent-test-${process.pid}`);
      execFileSync("mkdir", ["-p", fakeDataDir]);
      const tmpFile = join(tmpdir(), `vyrex-idempotent-write-test-${process.pid}.sh`);
      try {
        const persistBlock = extractRuntimeShellEnvSnippet();
        const toolRedirects = extractToolRedirects();
        const wrapper = [
    ...
    it("entrypoint replaces stale proxy values on restart", () => {
      const fakeDataDir = join(tmpdir(), `vyrex-replace-test-${process.pid}`);
      execFileSync("mkdir", ["-p", fakeDataDir]);
      const tmpFile = join(tmpdir(), `vyrex-replace-write-test-${process.pid}.sh`);
      try {
        const persistBlock = extractRuntimeShellEnvSnippet();
        const toolRedirects = extractToolRedirects();
    ...
    it("emit_sandbox_sourced_file prevents symlink-following attack on proxy-env.sh", () => {
      const fakeDataDir = join(tmpdir(), `vyrex-symlink-test-${process.pid}`);
      execFileSync("mkdir", ["-p", fakeDataDir]);
      const tmpFile = join(tmpdir(), `vyrex-symlink-write-test-${process.pid}.sh`);
      try {
        const persistBlock = extractRuntimeShellEnvSnippet();
        ...
        const wrapper = [
          ...
          '_PROXY_URL="http://${PROXY_HOST}:${PROXY_PORT}"',
          '_NO_PROXY_VAL="localhost,127.0.0.1,::1,${PROXY_HOST}"',
          persistBlock.trimEnd().replaceAll("/tmp/vyrex-proxy-env.sh", proxyEnvPath),
        ].join("\n");
        writeFileSync(tmpFile, wrapper, { mode: 0o700 });
        execFileSync("bash", [tmpFile], { encoding: "utf-8" });
    ...
    it("[simulation] sourcing proxy-env.sh overrides narrow NO_PROXY and no_proxy", () => {
      const fakeDataDir = join(tmpdir(), `vyrex-bashi-test-${process.pid}`);
      execFileSync("mkdir", ["-p", fakeDataDir]);
      try {
        const envContent = [
    ...
    it("regression #2109: proxy-env.sh includes NODE_OPTIONS --require when NODE_USE_ENV_PROXY=1", () => {
      const fakeDataDir = join(tmpdir(), `vyrex-http-fix-test-${process.pid}`);
      execFileSync("mkdir", ["-p", fakeDataDir]);
      const tmpFile = join(tmpdir(), `vyrex-http-fix-env-${process.pid}.sh`);
      const fakeFixPath = "/tmp/vyrex-http-proxy-fix.js";
      try {
        const persistBlock = extractRuntimeShellEnvSnippet();
        const wrapper = [
          "#!/usr/bin/env bash",
        ...
        // vyrex-start.sh and written to /tmp at boot.
    ...
    it("regression #2109: proxy-env.sh does NOT include NODE_OPTIONS when NODE_USE_ENV_PROXY is unset", () => {
      const fakeDataDir = join(tmpdir(), `vyrex-http-noop-test-${process.pid}`);
      execFileSync("mkdir", ["-p", fakeDataDir]);
      const tmpFile = join(tmpdir(), `vyrex-http-noop-env-${process.pid}.sh`);
      try {
        const persistBlock = extractRuntimeShellEnvSnippet();
        const wrapper = [
          "#!/usr/bin/env bash",
    ...
    it("Vyrex#1570: proxy-env.sh includes ws-proxy-fix NODE_OPTIONS when fix script exists", () => {
      const fakeDataDir = join(tmpdir(), `vyrex-ws-fix-test-${process.pid}`);
      const fakeWsFixScript = join(fakeDataDir, "ws-proxy-fix.js");
      execFileSync("mkdir", ["-p", fakeDataDir]);
      const tmpFile = join(tmpdir(), `vyrex-ws-fix-env-${process.pid}.sh`);
      try {
        const persistBlock = extractRuntimeShellEnvSnippet();
        const wrapper = [
          "#!/usr/bin/env bash",
    ...
    it("Vyrex#1570: proxy-env.sh omits ws-proxy-fix when script does not exist", () => {
      const fakeDataDir = join(tmpdir(), `vyrex-ws-noop-test-${process.pid}`);
      execFileSync("mkdir", ["-p", fakeDataDir]);
      const tmpFile = join(tmpdir(), `vyrex-ws-noop-env-${process.pid}.sh`);
      try {
        const persistBlock = extractRuntimeShellEnvSnippet();
        const wrapper = [
          "#!/usr/bin/env bash",
  ...
  describe("ws-proxy-fix preload (issue #1570)", () => {
    const wsFixPath = join(import.meta.dirname, "../vyrex-blueprint/scripts/ws-proxy-fix.js");
