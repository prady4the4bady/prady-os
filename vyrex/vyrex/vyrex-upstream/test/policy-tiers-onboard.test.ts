...
function runScript(scriptBody: string): SpawnSyncReturns<string> {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-tier-onboard-"));
 ...
 *
 * Sets VYREX_POLICY_TIER, VYREX_POLICY_MODE, and VYREX_POLICY_PRESETS
...
process.env.VYREX_POLICY_TIER = ${JSON.stringify(tierEnv)};
process.env.VYREX_POLICY_MODE = ${JSON.stringify(policyMode)};
process.env.VYREX_POLICY_PRESETS = ${JSON.stringify(policyPresets)};
...
`;
  ...
  // #2429: an unrecognised VYREX_POLICY_MODE used to hard-exit at step 8/8,
  ...
  // that the user may have meant VYREX_POLICY_TIER when the value looks like
  // a tier name.
  it("falls back to tier suggestions when VYREX_POLICY_MODE is unknown (#2429)", () => {
    const policiesPath = JSON.stringify(path.join(repoRoot, "dist", "lib", "policies.js"));
    const script =
      buildPreamble({
        tierEnv: "balanced",
...
`;
    ...
    assert.match(result.stderr, /Unsupported VYREX_POLICY_MODE: restricted/);
    assert.match(result.stderr, /VYREX_POLICY_TIER=restricted/);
...
`;
    ...
    const payload = JSON.parse(result.stdout.trim());
    assert.ok(!payload.error, `unexpected error: ${payload.error}`);
    assert.match(result.stderr, /Unsupported VYREX_POLICY_MODE: garbage/);
    assert.ok(
      !/did you mean VYREX_POLICY_TIER/.test(result.stderr),
...
`;
  ...
  function run(body: string): SpawnSyncReturns<string> {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-presets-"));
    const scriptPath = path.join(tmpDir, "script.js");
    fs.writeFileSync(scriptPath, buildPresetsScript(body));
    const result = spawnSync(process.execPath, [scriptPath], {
      cwd: repoRoot,
      encoding: "utf-8",
      env: { ...process.env, HOME: tmpDir, VYREX_NON_INTERACTIVE: "1" },
      timeout: 10000,
    });
    fs.rmSync(tmpDir, { recursive: true, force: true });
    return result;
