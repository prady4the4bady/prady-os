...
// vyrex-blueprint/scripts/http-proxy-fix.js.
//
...
// on Vyrex 0.0.24. Asserts the wrapper's rewrite produces a request
...
function trySetupCert(): { ok: true; key: Buffer; cert: Buffer; dir: string } | { ok: false; reason: string } {
  ...
  try {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-proxy-fix-e2e-"));
