...
// See: https://github.com/NVIDIA/Vyrex/issues/1711
...
// Status output with a foreign (non-vyrex) gateway name
...
describe("hasStaleGateway", () => {
  ...
  it("returns false when gateway name does not match", () => {
    const other = GW_INFO_NAMED.replace("vyrex", "other-gw");
...
describe("getReportedGatewayName", () => {
  it("extracts gateway name from status output", () => {
    expect(getReportedGatewayName(STATUS_CONNECTED)).toBe("vyrex");
  });

  it("extracts gateway name from gateway info output", () => {
    expect(getReportedGatewayName(GW_INFO_NAMED)).toBe("vyrex");
  });
...
describe("isGatewayHealthy", () => {
  ...
  it("returns false when gateway name does not match", () => {
    const wrongName = GW_INFO_ACTIVE.replace("vyrex", "other-gw");
...
describe("getGatewayReuseState", () => {
  ...
  it("returns 'stale' when named gateway exists but no active endpoint", () => {
    // gwInfo has "Gateway: vyrex" but activeGatewayInfo is empty — no live endpoint
