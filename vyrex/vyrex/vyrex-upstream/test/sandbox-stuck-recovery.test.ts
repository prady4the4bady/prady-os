...
function setupFixture(sandboxName: string, phase: string) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-stuck-"));
...
function runCli(
  ...
  return spawnSync(
    process.execPath,
    [path.join(repoRoot, "bin", "vyrex.js"), sandboxName, subcommand],
    {
      cwd: repoRoot,
      encoding: "utf-8",
      env: {
...
describe("sandbox stuck in non-Ready phase (#2016)", () => {
  it(
    ...
    () => {
      ...
      // waits until VYREX_CONNECT_TIMEOUT elapses.
      const result = runCli(tmpDir, sandboxName, "connect", {
        VYREX_CONNECT_TIMEOUT: "3",
      });
      ...
      expect(combined).toContain("VYREX_CONNECT_TIMEOUT");
    },
  );
  ...
  it(
    ...
    () => {
      ...
      expect(combined).toContain(`vyrex ${sandboxName} logs --follow`);
      expect(combined).toContain(`vyrex ${sandboxName} status`);
    },
  );
  ...
  it(
    ...
    () => {
      ...
      expect(combined).toContain(`vyrex ${sandboxName} rebuild --yes`);
    },
  );
