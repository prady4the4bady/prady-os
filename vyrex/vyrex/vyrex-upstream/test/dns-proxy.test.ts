...
describe("setup-dns-proxy.sh", () => {
  ...
  it("exits with usage when no sandbox name provided", () => {
    const result = spawnSync("bash", [SETUP_DNS_PROXY, "vyrex"], {
      encoding: /** @type {const} */ "utf-8",
      env: { ...process.env },
    });
  ...
  it("configures DNS proxy through kubectl and verifies sandbox DNS end to end", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-dns-proxy-"));
...
`,
    ...
    try {
      const result = spawnSync("bash", [SETUP_DNS_PROXY, "vyrex", "box[1]"], {
        encoding: "utf-8",
        env: {
          ...process.env,
...
describe("fix-coredns.sh", () => {
  ...
  it("patches CoreDNS on a Podman-style Docker host using a resolved upstream", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-fix-coredns-"));
    const fakeBin = path.join(tmp, "bin");
    const dockerLog = path.join(tmp, "docker.log");
...
set -euo pipefail
printf '%s\n' "$*" >> ${JSON.stringify(dockerLog)}
if [ "\${1:-}" = "ps" ]; then echo "openshell-cluster-vyrex"; exit 0; fi
...
`,
    ...
    try {
      const result = spawnSync("bash", [FIX_COREDNS, "vyrex"], {
        encoding: "utf-8",
        env: {
          ...process.env,
  ...
  it("rejects invalid resolved upstream values before patching CoreDNS", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-fix-coredns-bad-"));
    const fakeBin = path.join(tmp, "bin");
    fs.mkdirSync(fakeBin);
    fs.writeFileSync(
      path.join(fakeBin, "docker"),
