...
const SHIM_MARKER = "# Vyrex dev-shim - managed by scripts/npm-link-or-shim.sh";

function setupFakeRepo(): { tmpDir: string; repoDir: string; homeDir: string; fakeBin: string } {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-link-shim-"));
  ...
  fs.writeFileSync(
    path.join(repoDir, "bin", "vyrex.js"),
    "#!/usr/bin/env node\nconsole.log('vyrex stub ok');\n",
    { mode: 0o755 },
  );
...
describe("npm-link-or-shim.sh", () => {
  it("falls back to a wrapper script on npm link failure, preserving the Node directory", () => {
    ...
    const shimPath = path.join(homeDir, ".local", "bin", "vyrex");
    ...
    expect(contents).toContain(`exec "${path.join(repoDir, "bin", "vyrex.js")}"`);
  ...
  it("does not create a shim when npm link succeeds", () => {
    ...
    expect(result.status).toBe(0);
    expect(fs.existsSync(path.join(homeDir, ".local", "bin", "vyrex"))).toBe(false);
    expect(result.stderr).toBe("");
  });

  it("is a no-op when VYREX_INSTALLING is already set (avoids prepare-script recursion)", () => {
    const { repoDir, homeDir, fakeBin } = setupFakeRepo();
    writeFakeNpm(fakeBin, "fail");
  ...
  it("refuses to overwrite a foreign file at the shim path and exits non-zero", () => {
    ...
    expect(result.stderr).toContain("not managed by Vyrex");
  });

  it("refreshes an existing Vyrex-managed shim (idempotent re-run)", () => {
    const { repoDir, homeDir, fakeBin } = setupFakeRepo();
    writeFakeNpm(fakeBin, "fail");
    ...
    expect(refreshed).toContain(`exec "${path.join(repoDir, "bin", "vyrex.js")}"`);
