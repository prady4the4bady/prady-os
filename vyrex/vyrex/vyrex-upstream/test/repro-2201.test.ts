 ...
 *   `vyrex rebuild` builds the wrong sandbox type because it picks up
 ...
 *   2. User onboards "lumyn"  (agent="lumyn")
 *      → session now has agent="lumyn", sandboxName="lumyn"
 *   3. User runs `vyrex openclaw rebuild`
 *      → rebuild calls onboard --resume, which reads session.agent="lumyn"
 *      → builds lumyn Dockerfile instead of openclaw  ← BUG
 *
 * This test runs the real CLI (`vyrex <name> rebuild --yes`) with fake
 ...
 *   - We then run `vyrex <rebuildTarget> rebuild --yes`
 *
...
function createFixture({
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-2201-"));
  tmpFixtures.push(tmpDir);
  const vyrexDir = path.join(tmpDir, ".vyrex");
  fs.mkdirSync(vyrexDir, { recursive: true, mode: 0o700 });
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      metadata: { gatewayName: "vyrex", fromDockerfile: fromDockerfile },
      steps: {
...
`,
  ...
  return { tmpDir, vyrexDir, sandboxName };
}

function runRebuild(fixture: ReturnType<typeof createFixture>) {
  return spawnSync(
    process.execPath,
    [path.join(REPO_ROOT, "bin", "vyrex.js"), fixture.sandboxName, "rebuild", "--yes"],
    {
      cwd: REPO_ROOT,
      encoding: "utf-8",
      env: {
...
function readSession(fixture: ReturnType<typeof createFixture>): SessionFixture {
  const p = path.join(fixture.vyrexDir, "onboard-session.json");
...
describe("Issue #2201: rebuild syncs agent from registry, not stale session", () => {
  it(
    "rebuild openclaw after lumyn was onboarded last (reporter scenario)",
    { timeout: 60_000 },
    () => {
      // Exact scenario from the bug report: user has openclaw + lumyn,
      // lumyn was onboarded last, then runs `vyrex openclaw rebuild`.
      const f = createFixture({
        rebuildTarget: { name: "openclaw", agent: null },
        lastOnboarded: { name: "lumyn", agent: "lumyn" },
      });
      runRebuild(f);
      // With fix: session.agent = null (synced from openclaw registry entry)
      // Without fix: session.agent stays "lumyn" (from lumyn onboard)
      expect(readSessionAgent(f)).toBeNull();
    },
  );

  it(
    "rebuild lumyn after openclaw was onboarded last (reverse scenario)",
    { timeout: 60_000 },
    () => {
      const f = createFixture({
        rebuildTarget: { name: "lumyn", agent: "lumyn" },
        lastOnboarded: { name: "openclaw", agent: null },
      });
      runRebuild(f);
      // With fix: session.agent = "lumyn" (synced from lumyn registry entry)
      // Without fix: session.agent stays null (from openclaw onboard)
      expect(readSessionAgent(f)).toBe("lumyn");
    },
  );
});
