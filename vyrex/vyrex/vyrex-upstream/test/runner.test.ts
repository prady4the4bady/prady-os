...
describe("runner env merging", () => {
  it("preserves process env when opts.env is provided to runCapture", () => {
    ...
    try {
      ...
      expect(output).toBe("vyrex sk-test-secret");
    } finally {
...
describe("regression guards", () => {
  ...
  it("vyrex.ts does not use execSync", () => {
    const src = fs.readFileSync(
      path.join(import.meta.dirname, "..", "src", "vyrex.ts"),
      "utf-8",
    );
    const lines = src.split("\n");
    for (let i = 0; i < lines.length; i += 1) {
      if (lines[i].includes("execSync") && !lines[i].includes("execFileSync")) {
        expect.unreachable(`src/vyrex.ts:${i + 1} uses execSync — use execFileSync instead`);
      }
    }
  });
  ...
  it("CLI rejects malicious sandbox names before shell commands (e2e)", () => {
    const canaryDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-canary-"));
    const canary = path.join(canaryDir, "executed");
    try {
      const result = spawnSync(
        "node",
        [
          path.join(import.meta.dirname, "..", "bin", "vyrex.js"),
          `test; touch ${canary}`,
          "connect",
        ],
        {
  ...
  describe("curl-pipe-to-shell guards (#574, #583)", () => {
    ...
    it("brev e2e suite includes a deploy-cli mode", () => {
      ...
      expect(src).toContain('VYREX_DEPLOY_NO_CONNECT: "1"');
    });
