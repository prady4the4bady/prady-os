...
describe("onboard helpers", () => {
  it("uses Lumyn-oriented sandbox defaults when VyrexLumyn selects Lumyn", () => {
    const previousSandboxName = process.env.VYREX_SANDBOX_NAME;
    try {
      delete process.env.VYREX_SANDBOX_NAME;
      const lumyn = loadAgent("lumyn");
      ...
      expect(getDefaultSandboxNameForAgent(lumyn)).toBe("lumyn");
      expect(getSandboxPromptDefault(lumyn)).toBe("lumyn");

      process.env.VYREX_SANDBOX_NAME = "custom-lumyn";
      expect(getSandboxPromptDefault(lumyn)).toBe("lumyn");
    } finally {
      if (previousSandboxName === undefined) {
        delete process.env.VYREX_SANDBOX_NAME;
      } else {
        process.env.VYREX_SANDBOX_NAME = previousSandboxName;
      }
    }
  });
  ...
  it("classifies sandbox create timeout failures and tracks upload progress", () => {
    ...
    expect(
      classifySandboxCreateFailure(
        [
          '  Pushing image openshell/sandbox-from:123 into gateway "vyrex"',
  ...
  it("builds a sandbox sync script that does not rewrite OpenClaw config content", () => {
    ...
    assert.match(script, /cat > ~\/\.vyrex\/config\.json/);
  ...
  it("patches the staged Dockerfile with the selected model and chat UI URL", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-dockerfile-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
        "ARG VYREX_MODEL=nvidia/nemotron-3-super-120b-a12b",
        "ARG VYREX_PROVIDER_KEY=nvidia",
        "ARG VYREX_PRIMARY_MODEL_REF=nvidia/nemotron-3-super-120b-a12b",
        "ARG CHAT_UI_URL=http://127.0.0.1:18789",
        "ARG VYREX_INFERENCE_COMPAT_B64=e30=",
        "ARG VYREX_WEB_SEARCH_ENABLED=0",
    ...
    try {
      ...
      assert.match(patched, /^ARG VYREX_MODEL=gpt-5\.4$/m);
      assert.match(patched, /^ARG VYREX_PROVIDER_KEY=openai$/m);
      assert.match(patched, /^ARG VYREX_PRIMARY_MODEL_REF=openai\/gpt-5\.4$/m);
      assert.match(patched, /^ARG CHAT_UI_URL=http:\/\/127\.0\.0\.1:19999$/m);
      assert.match(patched, /^ARG VYREX_BUILD_ID=build-123$/m);
    } finally {
  ...
  it("patches the staged Dockerfile with Discord guild config for server workspaces", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-dockerfile-discord-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
        ...
        "ARG VYREX_DISCORD_GUILDS_B64=e30=",
        "ARG VYREX_BUILD_ID=default",
      ].join("\n"),
    );
    ...
    try {
      ...
      const patched = fs.readFileSync(dockerfilePath, "utf8");
      assert.match(patched, /^ARG VYREX_MESSAGING_CHANNELS_B64=/m);
      const guildLine = patched
        .split("\n")
        .find((line) => line.startsWith("ARG VYREX_DISCORD_GUILDS_B64="));
  ...
  it("patches the staged Dockerfile with Discord guild config that allows all server members", () => {
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "vyrex-onboard-dockerfile-discord-open-"),
    );
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
  ...
  it("#1737: patches the staged Dockerfile with Telegram mention-only config", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-dockerfile-tg-mention-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
        ...
        "ARG VYREX_MESSAGING_ALLOWED_IDS_B64=e30=",
        "ARG VYREX_DISCORD_GUILDS_B64=e30=",
        "ARG VYREX_TELEGRAM_CONFIG_B64=e30=",
        "ARG VYREX_BUILD_ID=default",
      ].join("\n"),
    );
    ...
    try {
      ...
      const line = patched
        .split("\n")
        .find((l) => l.startsWith("ARG VYREX_TELEGRAM_CONFIG_B64="));
  ...
  it("#1737: patches the staged Dockerfile with Telegram open-group config when requireMention=false", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-dockerfile-tg-open-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
  ...
  it("#1737: preserves default Telegram group-open behavior when telegramConfig is empty", () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-dockerfile-tg-empty-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
    ...
    try {
      ...
      const patched = fs.readFileSync(dockerfilePath, "utf8");
      assert.match(patched, /^ARG VYREX_TELEGRAM_CONFIG_B64=e30=$/m);
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  it("uses the default port as-is when VYREX_DASHBOARD_PORT is not overridden", () => {
    const chain = buildChain({
  ...
  it("patches the staged Dockerfile for Anthropic with anthropic-messages routing", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-dockerfile-anthropic-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
        ...
        "ARG VYREX_INFERENCE_API=openai-completions",
        "ARG VYREX_INFERENCE_COMPAT_B64=e30=",
        "ARG VYREX_WEB_SEARCH_ENABLED=0",
        "ARG VYREX_BUILD_ID=default",
      ].join("\n"),
    ...
    try {
      ...
      const patched = fs.readFileSync(dockerfilePath, "utf8");
      assert.match(patched, /^ARG VYREX_MODEL=claude-sonnet-4-5$/m);
      assert.match(patched, /^ARG VYREX_PROVIDER_KEY=anthropic$/m);
      assert.match(patched, /^ARG VYREX_PRIMARY_MODEL_REF=anthropic\/claude-sonnet-4-5$/m);
      assert.match(patched, /^ARG VYREX_INFERENCE_BASE_URL=https:\/\/inference\.local$/m);
      assert.match(patched, /^ARG VYREX_INFERENCE_API=anthropic-messages$/m);
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });

  it("regression #1409: bakes VYREX_PROXY_HOST/PORT env into the staged Dockerfile", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-dockerfile-proxy-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
        ...
        "ARG VYREX_WEB_SEARCH_ENABLED=0",
        "ARG VYREX_BUILD_ID=default",
        "ARG VYREX_PROXY_HOST=10.200.0.1",
        "ARG VYREX_PROXY_PORT=3128",
      ].join("\n"),
    );

    const priorHost = process.env.VYREX_PROXY_HOST;
    const priorPort = process.env.VYREX_PROXY_PORT;
    process.env.VYREX_PROXY_HOST = "1.2.3.4";
    process.env.VYREX_PROXY_PORT = "9999";
    try {
      patchStagedDockerfile(
        dockerfilePath,
        "gpt-5.4",
      ...
      const patched = fs.readFileSync(dockerfilePath, "utf8");
      assert.match(patched, /^ARG VYREX_PROXY_HOST=1\.2\.3\.4$/m);
      assert.match(patched, /^ARG VYREX_PROXY_PORT=9999$/m);
    } finally {
      if (priorHost === undefined) {
        delete process.env.VYREX_PROXY_HOST;
      } else {
        process.env.VYREX_PROXY_HOST = priorHost;
      }
      if (priorPort === undefined) {
        delete process.env.VYREX_PROXY_PORT;
      } else {
        process.env.VYREX_PROXY_PORT = priorPort;
      }
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  it("regression #1409: leaves Dockerfile defaults when proxy env is unset", () => {
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "vyrex-onboard-dockerfile-proxy-default-"),
    );
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
    ...
    try {
      ...
      assert.match(patched, /^ARG VYREX_PROXY_HOST=10\.200\.0\.1$/m);
      assert.match(patched, /^ARG VYREX_PROXY_PORT=3128$/m);
    } finally {
      if (priorHost !== undefined) process.env.VYREX_PROXY_HOST = priorHost;
      if (priorPort !== undefined) process.env.VYREX_PROXY_PORT = priorPort;
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });

  it("regression #2421: bakes VYREX_INFERENCE_INPUTS into the staged Dockerfile when env is set", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-dockerfile-inputs-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
        ...
        "ARG VYREX_WEB_SEARCH_ENABLED=0",
        "ARG VYREX_BUILD_ID=default",
        "ARG VYREX_INFERENCE_INPUTS=text",
      ].join("\n"),
    );

    const prior = process.env.VYREX_INFERENCE_INPUTS;
    process.env.VYREX_INFERENCE_INPUTS = "text,image";
    try {
      patchStagedDockerfile(
        dockerfilePath,
        "gpt-5.4",
      ...
      const patched = fs.readFileSync(dockerfilePath, "utf8");
      assert.match(patched, /^ARG VYREX_INFERENCE_INPUTS=text,image$/m);
    } finally {
      if (prior === undefined) {
        delete process.env.VYREX_INFERENCE_INPUTS;
      } else {
        process.env.VYREX_INFERENCE_INPUTS = prior;
      }
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });

  it("regression #2421: rejects malformed VYREX_INFERENCE_INPUTS and keeps default", () => {
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "vyrex-onboard-dockerfile-inputs-bad-"),
    );
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    ...
    try {
      ...
      for (const [index, value] of rejectCases.entries()) {
        ...
        if (value === undefined) {
          ...
          process.env.VYREX_INFERENCE_INPUTS = value;
        }
        patchStagedDockerfile(
          dockerfilePath,
          "gpt-5.4",
        ...
        assert.match(
          ...
          /^ARG VYREX_INFERENCE_INPUTS=text$/m,
  ...
  it("regression #1409: rejects malformed VYREX_PROXY_HOST/PORT and keeps defaults", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-dockerfile-proxy-bad-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
    ...
    process.env.VYREX_PROXY_HOST = "1.2.3.4\nRUN rm -rf /";
    process.env.VYREX_PROXY_PORT = "abcd";
    try {
      patchStagedDockerfile(
        dockerfilePath,
        "gpt-5.4",
  ...
  it("#2281: bakes VYREX_AGENT_TIMEOUT env into the staged Dockerfile", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-dockerfile-timeout-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
        ...
        "ARG VYREX_WEB_SEARCH_ENABLED=0",
        "ARG VYREX_BUILD_ID=default",
        "ARG VYREX_AGENT_TIMEOUT=600",
      ].join("\n"),
    );

    const priorTimeout = process.env.VYREX_AGENT_TIMEOUT;
    process.env.VYREX_AGENT_TIMEOUT = "1800";
    try {
      patchStagedDockerfile(
        dockerfilePath,
        "gpt-5.4",
      ...
      const patched = fs.readFileSync(dockerfilePath, "utf8");
      assert.match(patched, /^ARG VYREX_AGENT_TIMEOUT=1800$/m);
    } finally {
      if (priorTimeout === undefined) {
        delete process.env.VYREX_AGENT_TIMEOUT;
      } else {
        process.env.VYREX_AGENT_TIMEOUT = priorTimeout;
      }
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  it("patches the staged Dockerfile with Brave Search config when enabled", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-dockerfile-web-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
    ...
    try {
      ...
      const patched = fs.readFileSync(dockerfilePath, "utf8");
      assert.match(patched, /^ARG VYREX_WEB_SEARCH_ENABLED=1$/m);
      // Regression guard: the old secret-bearing build arg must not reappear.
      assert.doesNotMatch(patched, /VYREX_WEB_CONFIG_B64/);
    } finally {
  ...
  it("#2433: agentSupportsWebSearch detects whether agent Dockerfile declares the web search ARG", () => {
    // OpenClaw Dockerfile has ARG VYREX_WEB_SEARCH_ENABLED → supported.
    // Lumyn Dockerfile does not → not supported.
    ...
    expect(agentSupportsWebSearch(loadAgent("lumyn"))).toBe(false);
  });

  it("#2433: agentSupportsWebSearch honors the effective custom Dockerfile for Brave-capable agents", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-web-search-custom-"));
    ...
    try {
      ...
      expect(agentSupportsWebSearch(loadAgent("lumyn"), withArg)).toBe(false);
  ...
  it("#2433: configureWebSearch skips unsupported Lumyn instead of prompting for Brave", async () => {
    const priorBraveKey = process.env.BRAVE_API_KEY;
    process.env.BRAVE_API_KEY = "brv-test-key";
    try {
      await expect(configureWebSearch(null, loadAgent("lumyn"))).resolves.toBeNull();
    } finally {
      if (priorBraveKey === undefined) {
        delete process.env.BRAVE_API_KEY;
      } else {
  ...
  it("#2433: configureWebSearch does not call the prompt helper for unsupported Lumyn", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-web-search-prompt-"));
...
(async () => {
  const result = await configureWebSearch(null, loadAgent("lumyn"));
...
`;
  ...
  it("regression #1317: getBlueprintMinOpenshellVersion reads min_openshell_version from blueprint.yaml", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-blueprint-min-version-"));
    const blueprintDir = path.join(tmpDir, "vyrex-blueprint");
  ...
  it("regression #1317: getBlueprintMinOpenshellVersion returns null on missing or unparseable blueprint", () => {
    ...
    const missingDir = path.join(
      os.tmpdir(),
      "vyrex-blueprint-missing-" + Date.now().toString(),
    );
    expect(getBlueprintMinOpenshellVersion(missingDir)).toBe(null);

    // Present file, missing field — must NOT block onboard
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-blueprint-no-field-"));
    const blueprintDir = path.join(tmpDir, "vyrex-blueprint");
    fs.mkdirSync(blueprintDir, { recursive: true });
    ...
    const badDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-blueprint-bad-yaml-"));
    const badBlueprintDir = path.join(badDir, "vyrex-blueprint");
    ...
    const wrongTypeDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-blueprint-wrong-type-"));
    const wrongTypeBlueprintDir = path.join(wrongTypeDir, "vyrex-blueprint");
    ...
    const badShapeDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-blueprint-bad-shape-"));
    const badShapeBlueprintDir = path.join(badShapeDir, "vyrex-blueprint");
  ...
  it("getBlueprintMaxOpenshellVersion reads max_openshell_version from blueprint.yaml", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-blueprint-max-version-"));
    const blueprintDir = path.join(tmpDir, "vyrex-blueprint");
    fs.mkdirSync(blueprintDir, { recursive: true });
    fs.writeFileSync(
      path.join(blueprintDir, "blueprint.yaml"),
  ...
  it("getBlueprintMaxOpenshellVersion returns null on missing or unparseable blueprint", () => {
    ...
    const missingDir = path.join(
      os.tmpdir(),
      "vyrex-blueprint-max-missing-" + Date.now().toString(),
    );
    expect(getBlueprintMaxOpenshellVersion(missingDir)).toBe(null);

    // Present file, missing field — must NOT block onboard
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-blueprint-no-max-field-"));
    const blueprintDir = path.join(tmpDir, "vyrex-blueprint");
    fs.mkdirSync(blueprintDir, { recursive: true });
    fs.writeFileSync(path.join(blueprintDir, "blueprint.yaml"), 'version: "0.1.0"\n');
    try {
  ...
  it("treats the gateway as healthy only when vyrex is running and connected", () => {
    expect(
      isGatewayHealthy(
    ...
    expect(
      isGatewayHealthy(
        ...
        "Error:   × No gateway metadata found for 'vyrex'.",
        "Gateway Info\n\n  Gateway: openshell\n  Gateway endpoint: https://127.0.0.1:8080",
      ),
    ).toBe(false);
    expect(
    ...
    expect(isGatewayHealthy("Gateway status: Disconnected", "Gateway: vyrex")).toBe(false);
  ...
  it("allows slow sandbox create recovery to wait beyond 60 seconds", () => {
    ...
    assert.match(source, /VYREX_SANDBOX_READY_TIMEOUT", 180/);
  ...
  it("prints doctor logs automatically when gateway fails to start (#1605)", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-gateway-diag-"));
...
if [[ "$*" == *"doctor"*"logs"* ]]; then
  ...
  printf "  Container vyrex_k3s ran out of memory\\r\\n"
  printf "  Gateway auth token: nvapi-fakecredential-9999\\r\\n"
  exit 0
fi
if [[ "$*" == *"gateway"*"start"* ]]; then
  printf "\\033[33mDeploying\\033[0m gateway vyrex...\\r\\n"
...
`,
    ...
    // 10 s + 30 s minTimeout delays, and VYREX_HEALTH_POLL_COUNT=0 skips the
...
`;
    ...
    const result = spawnSync(nodeExec, [scriptPath], {
      ...
      env: {
        ...
        VYREX_HEALTH_POLL_COUNT: "0",
        VYREX_NON_INTERACTIVE: "1",
      },
    });
  ...
  it("filters local-only artifacts out of the sandbox build context", () => {
    expect(
      shouldIncludeBuildContextPath(
        "/repo/vyrex-blueprint",
        "/repo/vyrex-blueprint/orchestrator/main.py",
      ),
    ).toBe(true);
    expect(
      shouldIncludeBuildContextPath(
        "/repo/vyrex-blueprint",
        "/repo/vyrex-blueprint/.venv/bin/python",
      ),
    ).toBe(false);
    expect(
      shouldIncludeBuildContextPath(
        "/repo/vyrex-blueprint",
        "/repo/vyrex-blueprint/.ruff_cache/cache",
      ),
    ).toBe(false);
    expect(
      shouldIncludeBuildContextPath(
        "/repo/vyrex-blueprint",
        "/repo/vyrex-blueprint/._pyvenv.cfg",
      ),
    ).toBe(false);
  });

  it("normalizes sandbox name hints from the environment", () => {
    const previous = process.env.VYREX_SANDBOX_NAME;
    process.env.VYREX_SANDBOX_NAME = "  My-Assistant  ";
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.VYREX_SANDBOX_NAME = previous;
      }
    }
  });

  it("prefers the explicit --name option over VYREX_SANDBOX_NAME", () => {
    const previous = process.env.VYREX_SANDBOX_NAME;
    process.env.VYREX_SANDBOX_NAME = "from-env";
    try {
  ...
  it("does not fire a resume conflict from VYREX_SANDBOX_NAME alone", () => {
    ...
    // happens to export VYREX_SANDBOX_NAME in their shell rc.
    const previous = process.env.VYREX_SANDBOX_NAME;
    process.env.VYREX_SANDBOX_NAME = "other-sandbox";
    try {
  ...
  it("returns provider and model hints only for non-interactive runs", () => {
    const previousProvider = process.env.VYREX_PROVIDER;
    const previousModel = process.env.VYREX_MODEL;
    process.env.VYREX_PROVIDER = "cloud";
    process.env.VYREX_MODEL = "nvidia/test-model";
    try {
      ...
      if (previousProvider === undefined) {
        delete process.env.VYREX_PROVIDER;
      } else {
        process.env.VYREX_PROVIDER = previousProvider;
      }
      if (previousModel === undefined) {
        delete process.env.VYREX_MODEL;
      } else {
        process.env.VYREX_MODEL = previousModel;
      }
    }
  });
  ...
  it("detects resume conflicts for explicit provider and model changes", () => {
    ...
    const previousModel = process.env.VYREX_MODEL;
    process.env.VYREX_PROVIDER = "cloud";
    process.env.VYREX_MODEL = "nvidia/other-model";
    try {
  ...
  it("writes sandbox sync scripts to a temp file for stdin redirection", () => {
    ...
    try {
      expect(scriptFile).toMatch(/vyrex-sync.*\.sh$/);
      ...
      expect(parentDir).toContain("vyrex-sync");
      ...
      const parentDir = path.dirname(scriptFile);
      if (parentDir !== os.tmpdir() && path.basename(parentDir).startsWith("vyrex-sync-")) {
  ...
  it("stages only the files required to build the sandbox image", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-build-context-"));
    ...
    try {
      ...
      expect(fs.existsSync(path.join(buildCtx, "vyrex", "package-lock.json"))).toBe(true);
      expect(fs.existsSync(path.join(buildCtx, "vyrex", "src"))).toBe(true);
      expect(fs.existsSync(path.join(buildCtx, "vyrex-blueprint", ".venv"))).toBe(false);
      expect(fs.existsSync(path.join(buildCtx, "scripts", "vyrex-start.sh"))).toBe(true);
      expect(fs.existsSync(path.join(buildCtx, "scripts", "setup.sh"))).toBe(false);
      expect(fs.existsSync(path.join(buildCtx, "vyrex", "node_modules"))).toBe(false);
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  it("passes credential names to openshell without embedding secret values in argv", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-inference-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
    ...
    assert.match(commands[0].command, /gateway select vyrex/);
  ...
  it("does not delete saved OpenAI credentials when configuring local vLLM", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-local-vllm-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
    ...
    assert.match(providerCommand.command, /--credential VYREX_VLLM_LOCAL_TOKEN/);
    assert.doesNotMatch(providerCommand.command, /--credential OPENAI_API_KEY/);
    assert.equal(providerCommand.env?.VYREX_VLLM_LOCAL_TOKEN, "dummy");
  ...
  it("detects when the live inference route already matches the requested provider and model", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-inference-ready-"));
...
`,
  ...
  it("detects when OpenClaw is already configured inside the sandbox", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-openclaw-ready-"));
    const fakeOpenshell = path.join(tmpDir, "openshell");
...
`,
  ...
  it("detects when recorded policy presets are already applied", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-policy-ready-"));
    const registryDir = path.join(tmpDir, ".vyrex");
...
`,
  ...
  it("uses native Anthropic provider creation without embedding the secret in argv", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-anthropic-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("updates OpenAI-compatible providers without passing an unsupported --type flag", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-openai-update-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("re-prompts for credentials when openshell inference set fails with authorization errors", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-apply-auth-retry-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("returns control to provider selection when inference apply recovery chooses back", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-apply-back-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("uses named sandbox exec for dashboard and web-search probes", () => {
    ...
    assert.match(source, /"sandbox",\s*"exec",\s*"-n",\s*sandboxName,\s*"--",\s*"lumyn"/);
    ...
    assert.doesNotMatch(source, /\["sandbox",\s*"exec",\s*sandboxName,\s*"lumyn"/);
  ...
  it("migrates a legacy credentials.json into env so setupInference can register the provider", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-resume-cred-"));
    const fakeBin = path.join(tmpDir, "bin");
    ...
    const legacyDir = path.join(tmpDir, ".vyrex");
...
`;
  ...
  it("drops stale local sandbox registry entries when the live sandbox is gone", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-stale-sandbox-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-create-sandbox-"));
      const fakeBin = path.join(tmpDir, "bin");
...
(async () => {
  process.env.OPENSHELL_GATEWAY = "vyrex";
...
`;
      ...
      assert.match(createCommand.command, /vyrex-start/);
  ...
  it("binds the dashboard forward to 0.0.0.0 when CHAT_UI_URL points to a remote host", async () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-remote-forward-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("injects VYREX_DASHBOARD_PORT into sandbox create envArgs when set (#1925)", async () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-dashboard-port-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
    ...
    assert.ok(createCommand, "expected sandbox create command");
    // Part 1 of fix (#1925): VYREX_DASHBOARD_PORT must be in envArgs so
    // vyrex-start.sh can unconditionally override CHAT_UI_URL at runtime,
    // overriding whatever value the Docker image had baked in.
    assert.match(createCommand.command, /VYREX_DASHBOARD_PORT=19000/);
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "vyrex-onboard-messaging-providers-"),
      );
      const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("aborts onboard when a messaging provider upsert fails", { timeout: 60_000 }, async () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-provider-fail-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-reuse-providers-"));
      const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "vyrex-onboard-noninteractive-notready-"),
      );
      const fakeBin = path.join(tmpDir, "bin");
...
`;
      ...
      delete env["VYREX_RECREATE_SANDBOX"];
      const result = spawnSync(process.execPath, [scriptPath], {
        cwd: repoRoot,
        encoding: "utf-8",
        env,
      ...
      assert.ok(
        output.includes("--recreate-sandbox") || output.includes("VYREX_RECREATE_SANDBOX"),
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-recreate-flag-"));
      const fakeBin = path.join(tmpDir, "bin");
...
(async () => {
  process.env.OPENSHELL_GATEWAY = "vyrex";
  process.env.VYREX_RECREATE_SANDBOX = "1";
  const sandboxName = await createSandbox(null, "gpt-5.4", "nvidia-prod", null, "my-assistant");
  console.log(JSON.stringify({ sandboxName, commands }));
})().catch((error) => {
  console.error(error);
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-recreate-preserves-"));
      const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-interactive-reuse-"));
      const fakeBin = path.join(tmpDir, "bin");
...
`;
      ...
      // Run WITHOUT VYREX_NON_INTERACTIVE to exercise interactive path
      const env: Record<string, string | undefined> = {
        ...process.env,
        HOME: tmpDir,
        PATH: `${fakeBin}:${process.env.PATH || ""}`,
      };
      delete env["VYREX_NON_INTERACTIVE"];
      delete env["VYREX_RECREATE_SANDBOX"];
      const result = spawnSync(process.execPath, [scriptPath], {
        cwd: repoRoot,
        encoding: "utf-8",
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "vyrex-onboard-interactive-decline-"),
      );
      const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "vyrex-onboard-interactive-notready-"),
      );
      const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("upsertProvider creates a new provider and returns ok on success", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-upsert-provider-create-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("upsertProvider does not add its own log line on top of runner output (#1506)", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-upsert-no-dup-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("upsertProvider updates existing provider instead of creating (#1155)", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-upsert-provider-update-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("upsertProvider returns error details when create or update fails", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-upsert-provider-fail-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("providerExistsInGateway returns true when provider exists", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-provider-exists-true-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("hydrateCredentialEnv writes stored credentials into process.env for host-side bridges", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-hydrate-cred-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("providerExistsInGateway returns false when provider is missing", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-provider-exists-false-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-create-ready-"));
      const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("restores the dashboard forward when onboarding reuses an existing ready sandbox", async () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-reuse-forward-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("prints resume guidance when sandbox image upload times out", () => {
    ...
    assert.match(joined, /Recovery: vyrex onboard --resume/);
    assert.match(
      joined,
  ...
  it("accepts gateway inference when system inference is separately not configured", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-inference-get-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("accepts gateway inference output that omits the Route line", () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-inference-route-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "vyrex-onboard-enabled-channels-filter-"),
      );
      const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "vyrex-onboard-enabled-channels-empty-"),
      );
      const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "vyrex-onboard-messaging-noninteractive-"),
      );
      const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "vyrex-onboard-messaging-no-tokens-"),
      );
      const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "vyrex-onboard-slack-format-reject-"),
      );
      const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it(
    ...
    async () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(
        path.join(os.tmpdir(), "vyrex-onboard-slack-app-format-reject-"),
      );
      const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("uses the custom Dockerfile parent directory as build context when --from is given", async () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-from-dockerfile-"));
    const fakeBin = path.join(tmpDir, "bin");
    ...
    fs.writeFileSync(
      ...
      [
        "FROM ubuntu:22.04",
        "ARG VYREX_MODEL=nvidia/nemotron-super-49b-v1",
        "ARG VYREX_PROVIDER_KEY=nvidia",
        "ARG VYREX_PRIMARY_MODEL_REF=nvidia/nemotron-super-49b-v1",
        "ARG CHAT_UI_URL=http://127.0.0.1:18789",
        "ARG VYREX_INFERENCE_BASE_URL=https://inference.local/v1",
        "ARG VYREX_INFERENCE_API=openai-completions",
        "ARG VYREX_INFERENCE_COMPAT_B64=e30=",
...
`;
  ...
  it("exits with an error when the --from Dockerfile path does not exist", async () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-from-missing-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("exits with an error when the --from Dockerfile path is a directory", async () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-from-dir-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("exits clearly when the --from Dockerfile is inside an ignored context path", async () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-from-ignored-"));
    const fakeBin = path.join(tmpDir, "bin");
...
`;
  ...
  it("cleans up the custom build context when staging fails", async () => {
    const repoRoot = path.join(import.meta.dirname, "..");
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-from-cleanup-"));
    const fakeBin = path.join(tmpDir, "bin");
...
fs.mkdtempSync = (prefix, ...rest) => {
  ...
  if (String(prefix).includes("vyrex-build-")) {
...
`;
  ...
  it("patchStagedDockerfile rewrites ARG BASE_IMAGE when baseImageRef is provided", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-base-image-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
        "ARG BASE_IMAGE=ghcr.io/nvidia/vyrex/sandbox-base:latest",
        "ARG VYREX_MODEL=nvidia/nemotron-3-super-120b-a12b",
        "ARG VYREX_PROVIDER_KEY=nvidia",
        "ARG VYREX_PRIMARY_MODEL_REF=nvidia/nemotron-3-super-120b-a12b",
        "ARG CHAT_UI_URL=http://127.0.0.1:18789",
    ...
    const fakeRef =
      "ghcr.io/nvidia/vyrex/sandbox-base@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa";
    try {
      patchStagedDockerfile(
        dockerfilePath,
        "gpt-5.4",
      ...
      assert.match(
        patched,
        /^ARG BASE_IMAGE=ghcr\.io\/nvidia\/vyrex\/sandbox-base@sha256:a{64}$/m,
      );
  ...
  it("patchStagedDockerfile preserves ARG BASE_IMAGE when baseImageRef is null", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-base-image-null-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
    ...
    try {
      ...
      assert.match(
        patched,
        /^ARG BASE_IMAGE=ghcr\.io\/nvidia\/vyrex\/sandbox-base:latest$/m,
  ...
  it("patchStagedDockerfile is safe when Dockerfile has no ARG BASE_IMAGE line", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-no-base-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
    ...
    const fakeRef =
      "ghcr.io/nvidia/vyrex/sandbox-base@sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
    try {
      patchStagedDockerfile(
        dockerfilePath,
        "gpt-5.4",
  ...
  it("regression #1904: BASE_IMAGE must reference sandbox-base, not openshell-community", () => {
    ...
    // and applied it to vyrex/sandbox-base (different registry).
    // Verify that patchStagedDockerfile only writes refs to sandbox-base.
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-regression-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    fs.writeFileSync(
      dockerfilePath,
      [
    ...
    const correctRef =
      "ghcr.io/nvidia/vyrex/sandbox-base@sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc";
    try {
      patchStagedDockerfile(
        dockerfilePath,
        "gpt-5.4",
      ...
      assert.ok(
        baseLine.includes("vyrex/sandbox-base"),
        `BASE_IMAGE must reference vyrex/sandbox-base, got: ${baseLine}`,
      );
      assert.ok(
  ...
  it("patchStagedDockerfile does NOT overwrite custom --from BASE_IMAGE that differs from sandbox-base", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-onboard-custom-base-"));
    const dockerfilePath = path.join(tmpDir, "Dockerfile");
    ...
    const sandboxRef =
      "ghcr.io/nvidia/vyrex/sandbox-base@sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd";
    try {
      patchStagedDockerfile(
        dockerfilePath,
        "gpt-5.4",
  ...
  it("regression #1904: pullAndResolveBaseImageDigest uses sandbox-base registry", () => {
    ...
    assert.ok(
      SANDBOX_BASE_IMAGE.includes("vyrex/sandbox-base"),
      `SANDBOX_BASE_IMAGE must reference vyrex/sandbox-base, got: ${SANDBOX_BASE_IMAGE}`,
    );
    assert.ok(
