...
SANDBOX_NAME="${SANDBOX_NAME:-${VYREX_SANDBOX_NAME:-e2e-cloud-experimental}}"
...
# ── 6: CAN write to .vyrex/state (Landlock read_write via parent) ─
info "6. Can write to .vyrex/state (Landlock read_write)"
OUT=$(sandbox_exec "touch /sandbox/.vyrex/state/landlock-test && echo OK || echo FAILED" || true)
if echo "$OUT" | grep -q "OK"; then
  pass ".vyrex/state is writable under Landlock"
else
  fail_test ".vyrex/state is NOT writable under Landlock: $OUT"
fi
...
sandbox_exec "sed -i '/^# test$/d' /sandbox/.bashrc /sandbox/.profile 2>/dev/null || true; rm -f /sandbox/landlock-test /sandbox/.openclaw/landlock-test /sandbox/.vyrex/state/landlock-test /usr/landlock-test /etc/landlock-test /tmp/landlock-test 2>/dev/null" || true
