...
#
# Port 8080 conflict during vyrex onboard (VDR3 #5)
#
...
#
# Expects a working Vyrex/OpenShell install from a prior onboard (gateway may
# hold 8080). Destroys the vyrex gateway, binds a dummy listener on 8080, runs
# vyrex onboard --non-interactive, asserts preflight fails with
...
#
# Environment (typical):
#   VYREX_SANDBOX_NAME                    — default: e2e-cloud-experimental
#   VYREX_NON_INTERACTIVE                — should be 1 (onboard non-interactive)
#   VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1  — required for non-interactive onboard/re-onboard
#   NVIDIA_API_KEY                          — required if onboard reaches cloud inference (restore path)
#
# Usage:
#   VYREX_NON_INTERACTIVE=1 VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 NVIDIA_API_KEY=nvapi-... bash test/e2e/e2e-cloud-experimental/test-port8080-conflict.sh
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-cloud-experimental}"
...
INFO "Stopping vyrex gateway so we can bind a non-OpenShell process on 8080..."
...
P4_LOG="$(mktemp)"
INFO "Running vyrex onboard --non-interactive (expect preflight to fail on port 8080)..."
set +e
VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 vyrex onboard --non-interactive >"$P4_LOG" 2>&1
p4_exit=$?
set -euo pipefail
p4_out="$(cat "$P4_LOG")"
rm -f "$P4_LOG"
...
if [ "$p4_exit" -eq 0 ]; then
  FAIL "Expected vyrex onboard to exit non-zero when 8080 is taken (got 0)"
  exit 1
fi
PASS "vyrex onboard exited non-zero (${p4_exit}) with 8080 blocked"
...
INFO "Restoring vyrex gateway for subsequent phases..."
if ! openshell gateway start --name vyrex 2>&1; then
  FAIL "openshell gateway start --name vyrex failed after port test"
  exit 1
fi
gw_ok=0
...
if openshell sandbox get "$SANDBOX_NAME" >/dev/null 2>&1; then
  ...
  INFO "Sandbox missing after gateway destroy/recreate — re-onboarding with VYREX_RECREATE_SANDBOX=1..."
  P4R_LOG="$(mktemp)"
  set +e
  VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 VYREX_RECREATE_SANDBOX=1 vyrex onboard --non-interactive >"$P4R_LOG" 2>&1
  p4r_exit=$?
  set -euo pipefail
