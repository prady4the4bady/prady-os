...
#   OPENCLAW_AGENT_PREFIX — default "vyrex-start" (run before openclaw agent, same as telegram-bridge)

set -euo pipefail

SANDBOX_NAME="${SANDBOX_NAME:-${VYREX_SANDBOX_NAME:-}}"
SKILL_ID="${SKILL_ID:-skill-smoke-fixture}"
VERIFY_TOKEN="${VERIFY_TOKEN:-SKILL_SMOKE_VERIFY_K9X2}"
OPENCLAW_AGENT_PREFIX="${OPENCLAW_AGENT_PREFIX:-vyrex-start}"
AGENT_LAUNCHER=""
...
[ -n "$SANDBOX_NAME" ] || die "set SANDBOX_NAME (or VYREX_SANDBOX_NAME)"
...
# regression in Vyrex #2490 was masked by exactly this antipattern in
...
# OpenClaw stores sessions under /sandbox/.openclaw in Vyrex sandboxes.
_lock_rm=""
...
# an SSRF block, transport reset, or gateway error. See Vyrex #2490.
