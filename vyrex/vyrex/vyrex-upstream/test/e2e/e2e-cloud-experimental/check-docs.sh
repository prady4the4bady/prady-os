...
#   2) CLI parity — `vyrex --help` vs ### `vyrex …` in docs/reference/commands.md.
#
...
usage() {
  cat <<'EOF'
Documentation checks: Markdown links + vyrex --help vs commands reference.
...
run_cli_check() {
  local CLI_JS="$REPO_ROOT/bin/vyrex.js"
  ...
  log "[cli] comparing: $NODE bin/vyrex.js --dump-commands"
  # shellcheck disable=SC2016
  # log text: backticks are documentation markers, not command substitution
  log '[cli]        vs: docs/reference/commands.md (### `vyrex …` headings only)'
  ...
  # shellcheck disable=SC2016
  # log text: backticks are documentation markers, not command substitution
  log '[cli] phase 2/2: extract ### `vyrex …` headings from commands reference'
  # Allow optional MyST suffix on the same line, e.g. ### `vyrex onboard` {#anchor}
  # Strip argument placeholders (<arg>, [optional]) to match canonical usage signatures.
  grep -E '^### `vyrex ' "$COMMANDS_MD" | LC_ALL=C perl -CS -ne '
    if (/^### `([^`]+)`\s*(?:\{[^}]+\})?\s*$/) {
      my $c = $1;
      while ($c =~ s/\s*\[[^\]]*\]\s*$//) {}
      while ($c =~ s/\s+<[^>]+>\s*$//) {}
  ...
  if cmp -s "$_tmp/help.txt" "$_tmp/doc.txt"; then
    log "[cli] parity OK (${_n_help} vyrex command(s))"
  ...
  echo "" >&2
  echo "Only in commands.md (add to help() in bin/vyrex.js or fix heading):" >&2
...
check_remote_url() {
  ...
  if ! "$CURL" -fsS -L -o /dev/null \
    ...
    -A 'Vyrex-doc-link-check/1.0 (+https://github.com/NVIDIA/Vyrex)' \
    "$url" 2>/dev/null; then
...
check_docs_default_ignored_urls() {
  printf '%s\n' \
    'https://github.com/NVIDIA/Vyrex/commits/main' \
    'https://github.com/NVIDIA/Vyrex/pulls?q=is%3Apr+is%3Amerged' \
    'https://github.com/NVIDIA/Vyrex/pulls?q=is:pr+is:merged' \
