...
#   3. `vyrex onboard --resume --non-interactive` skips cached preflight,
...
export VYREX_E2E_DEFAULT_TIMEOUT=600
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-resume}"
...
env -u NVIDIA_API_KEY \
  ...
  node "$REPO/bin/vyrex.js" onboard --resume --non-interactive >"$RESUME_LOG" 2>&1
resume_exit=$?
...
[[ "${VYREX_E2E_KEEP_SANDBOX:-}" = "1" ]] || run_vyrex "$SANDBOX_NAME" destroy 2>/dev/null || true
openshell sandbox delete "$SANDBOX_NAME" 2>/dev/null || true
openshell forward stop 18789 2>/dev/null || true
openshell gateway destroy -g vyrex 2>/dev/null || true
rm -f "$SESSION_FILE"
