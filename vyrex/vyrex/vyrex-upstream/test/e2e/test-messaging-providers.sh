...
# Prerequisites:
#   - Docker running
#   - Vyrex installed (install.sh or brev-setup.sh already ran)
#   - NVIDIA_API_KEY set
#   - openshell on PATH
#
...
#   VYREX_SANDBOX_NAME                  — sandbox name (default: e2e-msg-provider)
...
#   VYREX_E2E_STRICT_DISCORD_GATEWAY    — fail instead of skip on known Discord gateway blockers
#
# Usage:
...
#
# See: https://github.com/NVIDIA/Vyrex/pull/1081
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-msg-provider}"
...
STRICT_DISCORD_GATEWAY="${VYREX_E2E_STRICT_DISCORD_GATEWAY:-0}"

# ══════════════════════════════════════════════════════════════════
# Phase 1: Install Vyrex (non-interactive mode)
# ══════════════════════════════════════════════════════════════════
section "Phase 1: Install Vyrex with messaging tokens"
...
# Ref: #2340
BASE_POLICY="$REPO/vyrex-blueprint/policies/openclaw-sandbox.yaml"
SLACK_PRESET="$REPO/vyrex-blueprint/policies/presets/slack.yaml"
...
# Vyrex, and runs onboard. Messaging tokens are already exported so
# the onboard step creates providers and attaches them to the sandbox.
info "Running install.sh --non-interactive..."
info "This installs Node.js, openshell, Vyrex, and runs onboard with messaging providers."
...
# real token. The placeholder is what vyrex-slack-token-rewriter.js
...
if echo "$sandbox_node_opts" | grep -q "vyrex-slack-token-rewriter.js"; then
...
" 2>/dev/null || true)
  if [ "$slack_configured" = "yes" ]; then
    ...
    guard_exists=$(openshell sandbox exec --name "$SANDBOX_NAME" -- ls -la /tmp/vyrex-slack-channel-guard.js 2>/dev/null || echo "EXEC_FAILED")
...
const msg = 'Vyrex E2E test ' + new Date().toISOString();
...
if echo "$gw_port" | grep -q "OPEN"; then
  ...
  start_log=$(openshell sandbox exec --name "$SANDBOX_NAME" -- cat /tmp/vyrex-start.log 2>/dev/null || true)
  if [ -n "$start_log" ]; then
    info "Entrypoint log (last 40 lines of /tmp/vyrex-start.log):"
...
if [ -z "$gw_log" ]; then
  ...
  gw_log=$(vyrex "$SANDBOX_NAME" logs 2>&1 | tail -200 || true)
fi
