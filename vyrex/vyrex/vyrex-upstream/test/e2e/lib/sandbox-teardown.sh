...
#
# Local-dev escape hatch: set VYREX_E2E_KEEP_SANDBOX=1 to skip the destroy
# on exit so the sandbox survives for post-mortem inspection.

_VYREX_TEARDOWN_SANDBOXES=()

register_sandbox_for_teardown() {
  local name="${1:-}"
  [[ -z "$name" ]] && return 0
  _VYREX_TEARDOWN_SANDBOXES+=("$name")
}

_vyrex_sandbox_teardown() {
  # Run on script EXIT — destroys every registered sandbox.
  #
  # Intentionally does NOT unlink ~/.vyrex/onboard.lock: that lock is
  ...
  set +e
  local sbx
  for sbx in "${_VYREX_TEARDOWN_SANDBOXES[@]}"; do
    vyrex "$sbx" destroy --yes >/dev/null 2>&1
  done
  set -e
}

trap _vyrex_sandbox_teardown EXIT
