...
#   3. First onboard — install.sh --non-interactive with VYREX_PROVIDER=ollama
#   4. Verify sandbox, proxy, token file, inference through sandbox
#   5. Second onboard (re-onboard) — vyrex onboard --non-interactive
#   6. Token consistency verification (the core of this test):
#        - Read ~/.vyrex/ollama-proxy-token
...
#   - Uses vyrex onboard CLI directly for re-onboard (not install.sh)
#   - Distinct sandbox name e2e-gpu-double-onboard
#
# Key differences from test-double-onboard.sh:
#   - Uses VYREX_PROVIDER=ollama (real GPU inference)
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-gpu-double-onboard}"
TEST_LOG="/tmp/vyrex-gpu-double-onboard-test.log"
INSTALL_LOG="/tmp/vyrex-gpu-double-onboard-install.log"
REONBOARD_LOG="/tmp/vyrex-gpu-double-onboard-reonboard.log"
...
if [ "$VYREX_PROVIDER" != "ollama" ]; then
  echo "ERROR: VYREX_PROVIDER must be 'ollama' for GPU double-onboard E2E (got: $VYREX_PROVIDER)"
  exit 1
fi
...
# ══════════════════════════════════════════════════════════════════
section "Phase 5: Second onboard (re-onboard via vyrex onboard)"

info "Running vyrex onboard --non-interactive with VYREX_RECREATE_SANDBOX=1..."
...
vyrex onboard --non-interactive >"$REONBOARD_LOG" 2>&1 &
reonboard_pid=$!
...
echo "    - Second onboard (re-onboard): vyrex onboard --non-interactive"
