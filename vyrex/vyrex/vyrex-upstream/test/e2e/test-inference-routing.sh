...
# Vyrex Inference Routing E2E Tests
#
...
#   TC-INF-09: Custom OpenAI-compatible endpoint (requires VYREX_ENDPOINT_URL + COMPATIBLE_API_KEY)
#
...
# ── Overall timeout ──────────────────────────────────────────────────────────
export VYREX_E2E_DEFAULT_TIMEOUT=1200
...
# Execute a command inside the sandbox via vyrex connect.
sandbox_exec() {
  local cmd="$1"
  local ssh_cfg
  ssh_cfg="$(mktemp)"
...
test_inf_05_credential_isolation() {
  ...
  # Unconditional destroy catches not-ready sandboxes that `vyrex list` misses.
...
test_inf_06_invalid_api_key() {
  ...
  output=$(NVIDIA_API_KEY="nvapi-INTENTIONALLY-INVALID-KEY-FOR-E2E-TEST" \
    VYREX_NON_INTERACTIVE=1 \
    VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 \
    VYREX_SANDBOX_NAME="e2e-invalid-key" \
    run_with_timeout 120 vyrex onboard --non-interactive --yes-i-accept-third-party-software \
    2>&1) || exit_code=$?
  ...
  if vyrex "e2e-invalid-key" status 2>/dev/null | grep -qiE "running|ready"; then
    fail "TC-INF-06: Sandbox cleanup" "Sandbox 'e2e-invalid-key' is still running after failed onboard"
    vyrex "e2e-invalid-key" destroy --yes 2>/dev/null || true
  else
...
test_inf_07_unreachable_endpoint() {
  ...
  output=$(NVIDIA_API_KEY="nvapi-valid-format-but-fake-key-1234567890" \
    VYREX_NON_INTERACTIVE=1 \
    VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 \
    VYREX_SANDBOX_NAME="e2e-unreachable" \
    VYREX_PROVIDER="custom" \
    VYREX_ENDPOINT_URL="https://vyrex-e2e.invalid/v1" \
  ...
  #    The product may transiently create then roll back the sandbox during
  #    onboard; the important invariant is that no active sandbox remains.
  if vyrex "e2e-unreachable" status 2>/dev/null | grep -qiE "running|ready"; then
    fail "TC-INF-07: Sandbox cleanup" "Sandbox 'e2e-unreachable' is still running after failed onboard"
    vyrex "e2e-unreachable" destroy --yes 2>/dev/null || true
  else
...
test_inf_02_openai() {
  ...
  local model="${VYREX_OPENAI_MODEL:-gpt-4o-mini}"
  rm -f "$HOME/.vyrex/onboard.lock" 2>/dev/null || true

  log "  Preflight: destroying any existing '$sbx_name' sandbox..."
  vyrex "$sbx_name" destroy --yes 2>/dev/null || true

  log "  Onboarding with OpenAI provider, model: $model"
  local onboard_exit=0
  VYREX_SANDBOX_NAME="$sbx_name" \
    VYREX_NON_INTERACTIVE=1 \
    VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 \
    VYREX_POLICY_TIER="open" \
    VYREX_PROVIDER="openai" \
    VYREX_MODEL="$model" \
    OPENAI_API_KEY="$api_key" \
    run_with_timeout 300 vyrex onboard --non-interactive --yes-i-accept-third-party-software \
...
test_inf_03_anthropic() {
  ...
  local model="${VYREX_ANTHROPIC_MODEL:-claude-sonnet-4-6}"
  rm -f "$HOME/.vyrex/onboard.lock" 2>/dev/null || true
...
test_inf_09_compatible_endpoint() {
  ...
  local endpoint_url="${VYREX_ENDPOINT_URL:-}"
  local endpoint_model="${VYREX_COMPAT_MODEL:-}"
  local endpoint_key="${COMPATIBLE_API_KEY:-}"

  if [[ -z "$endpoint_url" || -z "$endpoint_model" || -z "$endpoint_key" ]]; then
    skip "TC-INF-09" "Missing VYREX_ENDPOINT_URL, VYREX_COMPAT_MODEL, or COMPATIBLE_API_KEY"
    return
  fi
  ...
  VYREX_SANDBOX_NAME="$sbx_name" \
    ...
    VYREX_POLICY_TIER="open" \
    VYREX_PROVIDER="custom" \
    VYREX_ENDPOINT_URL="$endpoint_url" \
    VYREX_MODEL="$endpoint_model" \
...
teardown() {
  ...
  set +e
  vyrex "$SANDBOX_NAME" destroy --yes 2>/dev/null || true
  vyrex "e2e-openai" destroy --yes 2>/dev/null || true
  vyrex "e2e-anthropic" destroy --yes 2>/dev/null || true
  vyrex "e2e-invalid-key" destroy --yes 2>/dev/null || true
  vyrex "e2e-unreachable" destroy --yes 2>/dev/null || true
  vyrex "e2e-compat-ep" destroy --yes 2>/dev/null || true
  set -e
}
...
summary() {
  echo ""
  echo "============================================================"
  echo "  Vyrex Inference Routing E2E Results"
  echo "============================================================"
...
main() {
  echo ""
  echo "============================================================"
  echo "  Vyrex Inference Routing E2E Tests"
  echo "  $(date)"
  echo "============================================================"
  echo ""
