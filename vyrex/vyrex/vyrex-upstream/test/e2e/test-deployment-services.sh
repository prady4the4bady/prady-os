...
# Vyrex Deployment & Services E2E Tests
#
# Covers:
#   TC-STATE-02: backup-workspace.sh backup → destroy → recreate → restore
#   TC-DEPLOY-01a: vyrex tunnel start (cloudflared tunnel)
#   TC-DEPLOY-01b: tunnel URL serves the OpenClaw dashboard
#   TC-DEPLOY-01c: vyrex tunnel stop removes URL from status
#   TC-DEPLOY-03: vyrex uninstall --keep-openshell --yes
#
# Prerequisites:
#   - Docker running
#   - NVIDIA_API_KEY set
...
#
# TC-DEPLOY-03 is DESTRUCTIVE — it uninstalls Vyrex. Runs last.
# =============================================================================

set -euo pipefail

# ── Overall timeout ──────────────────────────────────────────────────────────
export VYREX_E2E_DEFAULT_TIMEOUT=3600
...
# ── Config ───────────────────────────────────────────────────────────────────
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-deploy-svc}"
...
install_vyrex() {
  ...
  if ! command -v vyrex >/dev/null 2>&1; then
    log "ERROR: install.sh failed — vyrex not found"
    exit 1
  fi
}
...
onboard_sandbox() {
  ...
  VYREX_SANDBOX_NAME="$name" \
    VYREX_NON_INTERACTIVE=1 \
    VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 \
    VYREX_POLICY_TIER="open" \
    run_with_timeout 1800 vyrex onboard --non-interactive --yes-i-accept-third-party-software \
    2>&1 | tee -a "$LOG_FILE" || {
...
test_state_02_backup_restore() {
  ...
  local backup_dir
  backup_dir=$(find "$HOME/.vyrex/backups" -mindepth 1 -maxdepth 1 -type d 2>/dev/null | sort -r | head -1)
  ...
  vyrex "$SANDBOX_NAME" destroy --yes 2>&1 | tee -a "$LOG_FILE" || true

  if vyrex list 2>/dev/null | grep -q "$SANDBOX_NAME"; then
...
# TC-DEPLOY-01a: vyrex tunnel start (cloudflared tunnel)
# TC-DEPLOY-01b: tunnel URL serves the OpenClaw dashboard
# TC-DEPLOY-01c: vyrex tunnel stop removes tunnel URL from status
# =============================================================================
test_deploy_01_start_stop() {
  ...
  if ! vyrex list 2>/dev/null | grep -Fq -- "$SANDBOX_NAME"; then
  ...
  log "  Step 1: Running vyrex tunnel start..."
  local start_output start_rc=0
  start_output=$(vyrex tunnel start 2>&1) || start_rc=$?
  log "  Start output: ${start_output}"
  if [[ $start_rc -ne 0 ]]; then
    fail "TC-DEPLOY-01a: Start" "vyrex tunnel start failed (exit $start_rc)"
    return
  fi

  log "  Step 2: Reading vyrex status (polling for tunnel URL)..."
  local status_output tunnel_url
  for i in $(seq 1 15); do
    status_output=$(vyrex status 2>&1) || true
  ...
  if [[ -n "$tunnel_url" ]]; then
    ...
    vyrex tunnel stop 2>/dev/null || true
    return
  fi
  ...
  log "  Step 4: Running vyrex tunnel stop..."
  local stop_output stop_rc=0
  stop_output=$(vyrex tunnel stop 2>&1) || stop_rc=$?
  log "  Tunnel stop output:     ${stop_output//$'\n'/$'\n'    }"
  if [[ $stop_rc -ne 0 ]]; then
    fail "TC-DEPLOY-01c: Stop command" "vyrex tunnel stop failed (exit $stop_rc)"
    return
  fi
  ...
  if [[ -z "$tunnel_url" ]]; then
    ...
    for i in $(seq 1 10); do
      status_rc=0
      post_status=$(vyrex status 2>&1) || status_rc=$?
      if [[ $status_rc -ne 0 ]]; then
        log "  [$i] vyrex status failed (exit $status_rc), retrying in 1s..."
        sleep 1
        continue
      fi
      status_ok=1
    ...
    if [[ $status_ok -eq 0 ]]; then
      fail "TC-DEPLOY-01c: Stop" "Could not read vyrex status after stop"
...
test_deploy_03_uninstall_keep_openshell() {
  ...
  if [[ -f "$REPO_ROOT/uninstall.sh" ]]; then
    ...
    uninstall_output=$(vyrex uninstall --keep-openshell --yes 2>&1) || true
  fi
  ...
  log "  Step 4: Verifying vyrex removed..."
  if ! command -v vyrex >/dev/null 2>&1; then
    pass "TC-DEPLOY-03: vyrex removed after uninstall"
  else
    local vyrex_path
    vyrex_path=$(command -v vyrex)
    if [[ "$vyrex_path" == "$REPO_ROOT"* ]]; then
      pass "TC-DEPLOY-03: uninstall completed (vyrex in source tree is expected)"
    else
      fail "TC-DEPLOY-03: vyrex" "vyrex still found at $vyrex_path after uninstall"
    fi
  fi
}
...
main() {
  echo ""
  echo "============================================================"
  echo "  Vyrex Deployment & Services E2E Tests"
  echo "  $(date)"
  echo "============================================================"
  echo ""
