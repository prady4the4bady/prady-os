...
#
#   1. A pre-existing plaintext ~/.vyrex/credentials.json from an earlier
...
#      exists under ~/.vyrex/.
#
#   4. `vyrex credentials list` reports providers from the OpenShell
#      gateway, not from disk.
#
#   5. If ~/.vyrex/credentials.json exists as a symlink to an unrelated
...
# to ~/.vyrex/ — set VYREX_E2E_KEEP_SANDBOX=1 to skip the teardown
# and inspect post-mortem.
#
# Prerequisites:
#   - Docker running
#   - openshell + vyrex on PATH
...
export VYREX_E2E_DEFAULT_TIMEOUT=2400
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-cred-migration}"
...
if ! command -v openshell >/dev/null 2>&1; then
  ...
  bash "$REPO/install.sh" --yes-i-accept-third-party-software \
    >/tmp/vyrex-e2e-install.log 2>&1 || {
    fail "install.sh failed; see /tmp/vyrex-e2e-install.log"
    exit 1
  }
fi
...
command -v vyrex >/dev/null 2>&1 || {
  fail "vyrex still missing after install"
  exit 1
}
pass "openshell + vyrex on PATH"

REAL_API_KEY="$NVIDIA_API_KEY"
VYREX_DIR="$HOME/.vyrex"
LEGACY_FILE="$VYREX_DIR/credentials.json"
...
# Start from a clean ~/.vyrex to avoid interference from prior runs.
rm -rf "$VYREX_DIR"
mkdir -p "$VYREX_DIR"
chmod 700 "$VYREX_DIR"
...
if [ "$ONBOARD_EXIT" -eq 0 ]; then
  pass "vyrex onboard succeeded with only the legacy file as the credential source"
else
  fail "vyrex onboard failed (exit $ONBOARD_EXIT); see log below"
...
if ! PROVIDERS_OUT=$(openshell -g vyrex provider list --names 2>&1); then
  fail "openshell -g vyrex provider list --names failed"
  printf '%s\n' "$PROVIDERS_OUT" | indent
  exit 1
fi
info "Providers in vyrex gateway:"
printf '%s\n' "$PROVIDERS_OUT" | indent
...
# spawning another vyrex process is to verify they are NOT registered
...
# ══════════════════════════════════════════════════════════════════
# Phase 3: vyrex credentials list reads from the gateway, not disk
# ══════════════════════════════════════════════════════════════════
section "Phase 3: vyrex credentials list"

if ! CREDS_LIST_OUT=$(vyrex credentials list 2>&1); then
  fail "vyrex credentials list failed"
