...
#   VYREX_SANDBOX_NAME                  — sandbox name (default: e2e-shields)
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-shields}"
...
AUDIT_FILE="$HOME/.vyrex/state/shields-audit.jsonl"
...
INSTALL_LOG="/tmp/vyrex-e2e-shields-install.log"
...
command -v vyrex >/dev/null 2>&1 || {
  fail "vyrex not on PATH"
  exit 1
}
...
pass "Vyrex installed (sandbox: $SANDBOX_NAME)"
...
STATUS_DEFAULT=$(vyrex "${SANDBOX_NAME}" shields status 2>&1)
echo "$STATUS_DEFAULT"
...
SHIELDS_UP_OUTPUT=$(vyrex "${SANDBOX_NAME}" shields up 2>&1)
...
CONFIG_GET_OUTPUT=$(vyrex "${SANDBOX_NAME}" config get 2>&1)
...
DOTPATH_OUTPUT=$(vyrex "${SANDBOX_NAME}" config get --key inference 2>&1 || true)
...
STATUS_OUTPUT=$(vyrex "${SANDBOX_NAME}" shields status 2>&1)
echo "$STATUS_OUTPUT"
...
SHIELDS_DOWN_OUTPUT=$(vyrex "${SANDBOX_NAME}" shields down \
...
STATUS_DOWN=$(vyrex "${SANDBOX_NAME}" shields status 2>&1)
echo "$STATUS_DOWN"
...
if RESTORE_UP_OUTPUT=$(vyrex "${SANDBOX_NAME}" shields up 2>&1); then
...
vyrex "${SANDBOX_NAME}" shields down --timeout 10s --reason "Auto-restore timer E2E" 2>&1

# Verify shields are down
STATUS_TIMER=$(vyrex "${SANDBOX_NAME}" shields status 2>&1)
...
for _poll in $(seq 1 12); do
  sleep 5
  STATUS_AFTER_TIMER=$(vyrex "${SANDBOX_NAME}" shields status 2>&1)
...
vyrex "${SANDBOX_NAME}" shields up 2>&1
DOUBLE_UP=$(vyrex "${SANDBOX_NAME}" shields up 2>&1 || true)
...
vyrex "${SANDBOX_NAME}" shields down --timeout 5m --reason "Cleanup" 2>&1
...
DOUBLE_DOWN=$(vyrex "${SANDBOX_NAME}" shields down --timeout 5m --reason "Should fail" 2>&1 || true)
