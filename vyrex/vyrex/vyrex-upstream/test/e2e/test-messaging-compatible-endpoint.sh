...
#   - onboards with VYREX_PROVIDER=custom and Telegram enabled
...
# Prerequisites:
#   - Docker running
#   - Vyrex installed or a source checkout that install.sh can install
#
# Environment:
#   VYREX_SANDBOX_NAME        — sandbox name (default: e2e-msg-compat)
#   VYREX_COMPAT_MOCK_PORT   — mock endpoint port (default: 18089)
#   VYREX_COMPAT_MODEL       — model id for the compatible endpoint mock
#   VYREX_COMPAT_MOCK_API_KEY — optional; defaults to a fake hermetic key
...
cli_command_available_from_source() {
  [ -f "$REPO/dist/vyrex.js" ] && command -v node >/dev/null 2>&1 && command -v openshell >/dev/null 2>&1
}

run_cli() {
  if cli_command_available_from_source; then
    node "$REPO/bin/vyrex.js" "$@"
  else
    vyrex "$@"
  fi
}
...
destroy_sandbox_best_effort() {
  ...
  set +e
  if cli_command_available_from_source; then
    run_with_timeout 120 node "$REPO/bin/vyrex.js" "$SANDBOX_NAME" destroy --yes >/dev/null 2>&1
  elif command -v vyrex >/dev/null 2>&1; then
    run_with_timeout 120 vyrex "$SANDBOX_NAME" destroy --yes >/dev/null 2>&1
  fi
...
run_compatible_onboard() {
  ...
  export VYREX_SKIP_TELEGRAM_REACHABILITY=1
  export VYREX_PROVIDER=custom
  export VYREX_ENDPOINT_URL="$COMPAT_ENDPOINT_URL"
  export VYREX_MODEL="$COMPAT_MODEL"
  export VYREX_PREFERRED_API=openai-completions
  export VYREX_POLICY_MODE=custom
  export VYREX_POLICY_PRESETS=telegram
  ...
  if cli_command_available_from_source; then
    ...
    info "Using source-built CLI at $REPO/bin/vyrex.js"
    destroy_sandbox_best_effort
    run_with_timeout 1200 node "$REPO/bin/vyrex.js" onboard --fresh --non-interactive --yes-i-accept-third-party-software \
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-msg-compat}"
COMPAT_MOCK_PORT="${VYREX_COMPAT_MOCK_PORT:-18089}"
COMPAT_MODEL="${VYREX_COMPAT_MODEL:-mock/deepseek-compatible}"
COMPATIBLE_KEY="${VYREX_COMPAT_MOCK_API_KEY:-fake-compatible-key-e2e}"
...
COMPAT_MOCK_PID=""
ONBOARD_LOG="/tmp/vyrex-e2e-messaging-compatible-endpoint-install.log"
...
if [ -n "${TELEGRAM_BOT_TOKEN_REAL:-}" ] \
  ...
  && [ -n "${VYREX_ENDPOINT_URL:-}" ] \
  && [ -n "${VYREX_COMPAT_MODEL:-}" ]; then
