...
#     detection)
#
# Uses two distinct fake tokens per provider. The test validates that Vyrex
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-token-rotation}"
...
# ── Phase 0: Install Vyrex with token A ────────────────────────

section "Phase 0: Install Vyrex and first onboard with token A"
...
" "$REGISTRY" "$SANDBOX_NAME" 2>/dev/null; then
  ...
  export SLACK_BOT_TOKEN="$SLACK_BOT_TOKEN_A"
  export SLACK_APP_TOKEN="$SLACK_APP_TOKEN_A"
  unset VYREX_RECREATE_SANDBOX

  ONBOARD_OUTPUT=$(vyrex onboard --non-interactive 2>&1)
  onboard_exit=$?
