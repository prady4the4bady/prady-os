...
# Vyrex Network Policy E2E Tests
#
# Covers:
...
# Prerequisites:
#   - Docker running
#   - Vyrex installed (or install.sh available)
...
apply_preset() {
  ...
  local exit_code=0
  vyrex "$SANDBOX_NAME" policy-add "$preset_name" --yes 2>&1 | tee -a "$LOG_FILE" || exit_code=$?
  sleep 3
  return "$exit_code"
}
...
apply_preset_interactive() {
  ...
  preset_list=$(VYREX_NON_INTERACTIVE='' vyrex "$SANDBOX_NAME" policy-add </dev/null 2>&1) || true
  ...
  local exit_code=0
  set +e
  VYREX_NON_INTERACTIVE='' expect <<EOF 2>&1 | tee -a "$LOG_FILE"
set timeout 30
spawn env VYREX_NON_INTERACTIVE= vyrex $SANDBOX_NAME policy-add
...
setup_sandbox() {
  ...
  # Unconditional destroy — `vyrex list` does not always surface sandboxes
  # stuck in a not-ready state, and a not-ready sandbox blocks onboard with
  # "already exists but is not ready" before VYREX_RECREATE_SANDBOX=1 kicks in.
  ...
  VYREX_SANDBOX_NAME="$SANDBOX_NAME" \
    ...
    run_with_timeout 600 vyrex onboard --non-interactive --yes-i-accept-third-party-software \
    2>&1 | tee -a "$LOG_FILE" || {
...
\"" 2>&1) || true
  ...
  dry_output=$(vyrex "$SANDBOX_NAME" policy-add jira --dry-run 2>&1) || dry_rc=$?
...
test_net_06_permissive_mode() {
  ...
  local permissive_path="$REPO_ROOT/vyrex-blueprint/policies/openclaw-sandbox-permissive.yaml"
...
test_net_09_ssrf_validation() {
  ...
  local result
  result=$(node -e "
const { isPrivateIp } = require('$REPO_ROOT/vyrex/dist/blueprint/ssrf');
...
# ── Teardown ─────────────────────────────────────────────────────────────────
teardown() {
  # Do not unlink ~/.vyrex/onboard.lock: that lock is global and PID-
...
main() {
  echo ""
  echo "============================================================"
  echo "  Vyrex Network Policy E2E Tests"
  echo "  $(date)"
  echo "============================================================"
  echo ""
