...
#   2. Run the Vyrex installer with VYREX_PROVIDER=ollama
...
#   VYREX_SANDBOX_NAME                  — sandbox name (default: e2e-gpu-ollama)
#   VYREX_RECREATE_SANDBOX=1            — recreate sandbox if it exists
#   VYREX_MODEL                         — model for onboard (default: auto-selected by onboard)
#   SKIP_UNINSTALL                         — set to 1 to skip uninstall (debugging)
#
# Usage:
#   VYREX_NON_INTERACTIVE=1 VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 bash test/e2e/test-gpu-e2e.sh
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-gpu-ollama}"
TEST_LOG="/tmp/vyrex-gpu-e2e-test.log"
INSTALL_LOG="/tmp/vyrex-gpu-e2e-install.log"

# Enforce Ollama provider — this script only tests local GPU inference.
export VYREX_PROVIDER="${VYREX_PROVIDER:-ollama}"
if [ "$VYREX_PROVIDER" != "ollama" ]; then
  echo "ERROR: VYREX_PROVIDER must be 'ollama' for GPU E2E (got: $VYREX_PROVIDER)"
  exit 1
fi
...
# The vyrex onboard flow handles startup and model pull itself.
...
# ══════════════════════════════════════════════════════════════════
# Phase 3: Install Vyrex and onboard with Ollama
# ══════════════════════════════════════════════════════════════════
section "Phase 3: Install Vyrex and onboard with Ollama"
...
info "Running install.sh --non-interactive with VYREX_PROVIDER=ollama..."
...
# 4b: Status ok
if vyrex "$SANDBOX_NAME" status >/dev/null 2>&1; then
  pass "vyrex ${SANDBOX_NAME} status exits 0"
else
  fail "vyrex ${SANDBOX_NAME} status failed"
fi
...
PROXY_PORT="${VYREX_OLLAMA_PROXY_PORT:-11435}"
TOKEN_FILE="$HOME/.vyrex/ollama-proxy-token"
...
# Determine the model to test. Prefer VYREX_MODEL (set by workflow), then
# fall back to querying Ollama's /api/tags (handles auto-selection by onboard).
CONFIGURED_MODEL="${VYREX_MODEL:-}"
if [ -n "$CONFIGURED_MODEL" ]; then
  ...
  if curl -sf http://127.0.0.1:11434/api/tags 2>/dev/null \
    ...
    info "Using VYREX_MODEL: $CONFIGURED_MODEL (confirmed in Ollama)"
  else
    info "VYREX_MODEL=$CONFIGURED_MODEL not found in Ollama tags — querying available models"
    CONFIGURED_MODEL=""
  fi
fi
...
vyrex "$SANDBOX_NAME" destroy --yes 2>&1 | tail -5 || true
...
if [ "${SKIP_UNINSTALL:-}" = "1" ]; then
  ...
  if [ -d "$HOME/.vyrex" ]; then
    fail "$HOME/.vyrex directory still exists after uninstall"
  else
    pass "$HOME/.vyrex removed"
  fi
fi
...
echo "    - install.sh --non-interactive with VYREX_PROVIDER=ollama"
