...
# Prerequisites:
#   - Docker running
#   - Vyrex installed and sandbox running (test-full-e2e.sh Phase 0-3)
#   - NVIDIA_API_KEY set
#   - openshell on PATH
#
# Environment variables:
#   VYREX_SANDBOX_NAME  — sandbox name (default: e2e-test)
#   NVIDIA_API_KEY         — required
#
# Usage:
#   VYREX_NON_INTERACTIVE=1 VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 NVIDIA_API_KEY=nvapi-... bash test/e2e/test-credential-sanitization.sh
#
# See: https://github.com/NVIDIA/Vyrex/pull/156
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-test}"
...
MOCK_DIR=$(mktemp -d /tmp/vyrex-cred-test-XXXXXX)
...
c7_nvapi=$(sandbox_exec "grep -r 'nvapi-' /sandbox/.openclaw/ /sandbox/.vyrex/ 2>/dev/null | grep -v 'STRIPPED' | grep -v '/policies/' | grep -v '/plugin-runtime-deps/' | head -5" || true)
c7_ghp=$(sandbox_exec "grep -r 'ghp_' /sandbox/.openclaw/ /sandbox/.vyrex/ 2>/dev/null | grep -v 'STRIPPED' | grep -v '/policies/' | grep -v '/plugin-runtime-deps/' | head -5" || true)
c7_npm=$(sandbox_exec "grep -r 'npm_' /sandbox/.openclaw/ /sandbox/.vyrex/ 2>/dev/null | grep -v 'STRIPPED' | grep -v '/policies/' | grep -v '/plugin-runtime-deps/' | head -5" || true)
...
SYMLINK_DIR=$(mktemp -d /tmp/vyrex-symlink-test-XXXXXX)
...
BLUEPRINT_FILE="$REPO/vyrex-blueprint/blueprint.yaml"
