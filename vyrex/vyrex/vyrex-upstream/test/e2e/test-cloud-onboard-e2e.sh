...
#
# Tests the public installer flow (curl nvidia.com/vyrex.sh | bash),
...
#
# Environment:
#   VYREX_NON_INTERACTIVE=1                         — required for non-interactive install
#   VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1             — required for non-interactive install
#   VYREX_SANDBOX_NAME                              — sandbox name (default: e2e-cloud-onboard)
#   VYREX_RECREATE_SANDBOX=1                        — recreate if exists
#   VYREX_POLICY_MODE=custom                        — custom policy mode
#   VYREX_POLICY_PRESETS=npm,pypi                   — policy presets
#   RUN_E2E_CLOUD_ONBOARD_INTERACTIVE_INSTALL=0        — set 0 for non-interactive (default), 1 for expect
#   VYREX_INSTALL_SCRIPT_URL                        — override public installer URL
#   VYREX_INSTALL_REF                               — Git ref cloned by public installer
#   VYREX_PUBLIC_INSTALL_CWD                        — override temp cwd for public install
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-cloud-onboard}"
CLOUD_MODEL="${VYREX_CLOUD_EXPERIMENTAL_MODEL:-nvidia/nemotron-3-super-120b-a12b}"
INSTALL_LOG="${E2E_CLOUD_ONBOARD_INSTALL_LOG:-/tmp/vyrex-e2e-cloud-onboard-install.log}"
INTERACTIVE_INSTALL="${RUN_E2E_CLOUD_ONBOARD_INTERACTIVE_INSTALL:-0}"
PUBLIC_INSTALL_CWD="${VYREX_PUBLIC_INSTALL_CWD:-}"
...
if [ "$INTERACTIVE_INSTALL" != "1" ]; then
  if [ "${VYREX_NON_INTERACTIVE:-}" != "1" ]; then
    fail "VYREX_NON_INTERACTIVE=1 is required for non-interactive install"
    exit 1
  fi
...
export VYREX_EXPERIMENTAL=1
export VYREX_PROVIDER=cloud
export VYREX_MODEL="$CLOUD_MODEL"
export VYREX_POLICY_MODE="${VYREX_POLICY_MODE:-custom}"
export VYREX_POLICY_PRESETS="${VYREX_POLICY_PRESETS:-npm,pypi}"

PUBLIC_INSTALL_REF="${VYREX_PUBLIC_INSTALL_REF:-${GITHUB_SHA:-}}"
if [ -n "$PUBLIC_INSTALL_REF" ]; then
  export VYREX_INSTALL_REF="$PUBLIC_INSTALL_REF"
  export VYREX_INSTALL_TAG="$PUBLIC_INSTALL_REF"
fi
if [ -z "${VYREX_INSTALL_SCRIPT_URL:-}" ] && [ -n "$PUBLIC_INSTALL_REF" ]; then
  VYREX_INSTALL_SCRIPT_URL="https://raw.githubusercontent.com/NVIDIA/Vyrex/${PUBLIC_INSTALL_REF}/install.sh"
else
  VYREX_INSTALL_SCRIPT_URL="${VYREX_INSTALL_SCRIPT_URL:-https://www.nvidia.com/vyrex.sh}"
fi
export VYREX_INSTALL_SCRIPT_URL

info "Model: ${CLOUD_MODEL}, Policy: ${VYREX_POLICY_MODE} ${VYREX_POLICY_PRESETS}"
if [ -n "${VYREX_INSTALL_REF:-}" ]; then
  info "Public installer will clone Vyrex ref: ${VYREX_INSTALL_REF}"
else
  info "Public installer will clone Vyrex ref: latest"
fi
...
if [ "$INTERACTIVE_INSTALL" = "1" ]; then
  ...
  if [ -z "$PUBLIC_INSTALL_CWD" ]; then
    PUBLIC_INSTALL_CWD="$(mktemp -d "${TMPDIR:-/tmp}/vyrex-public-install.XXXXXX")"
  else
    mkdir -p "$PUBLIC_INSTALL_CWD"
  fi
  info "Installing (non-interactive): curl -fsSL ${VYREX_INSTALL_SCRIPT_URL} | bash"
  info "Public install cwd: ${PUBLIC_INSTALL_CWD}"
  (
    cd "$PUBLIC_INSTALL_CWD" || exit 1
    curl -fsSL "$VYREX_INSTALL_SCRIPT_URL" | bash
  ) >"$INSTALL_LOG" 2>&1 &
  install_pid=$!
...
if grep -q "Vyrex package.json found in the selected source checkout" "$INSTALL_LOG"; then
...
if grep -q "Installing Vyrex from GitHub" "$INSTALL_LOG" \
  && grep -q "Resolved install ref:" "$INSTALL_LOG" \
  && grep -q "Cloning Vyrex source" "$INSTALL_LOG"; then
...
if [ "${VYREX_E2E_KEEP_SANDBOX:-}" = "1" ]; then
  skip "Cleanup skipped (VYREX_E2E_KEEP_SANDBOX=1)"
else
