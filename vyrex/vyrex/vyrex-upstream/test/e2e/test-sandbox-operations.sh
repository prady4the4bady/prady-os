...
# Vyrex Sandbox Operations E2E Test Suite
#
# Covers: TC-SBX-01 through TC-SBX-11
# Assumes: Vyrex is installed, no sandbox is currently onboarded
#
# Test ordering:
...
export VYREX_E2E_DEFAULT_TIMEOUT=1800
...
require_sandbox() {
  if ! vyrex list 2>/dev/null | grep -q "$1"; then
...
is_onboard_import_stream_reset() {
  ...
  grep -q "Connection reset by peer (os error 104)" "$output_file" \
    && grep -Eq "The image appears to have reached the gateway before the stream failed|Recovery: vyrex onboard --resume" "$output_file"
}
...
resume_onboard_after_import_stream_reset() {
  ...
  log "  [onboard] Image reached gateway but import stream reset; retrying with vyrex onboard --resume..."

  local attempt delay resume_exit resume_output
  for attempt in 1 2 3; do
    rm -f "$HOME/.vyrex/onboard.lock" 2>/dev/null || true
    resume_exit=0
    resume_output="$(mktemp)"
    log "  [onboard] Resume attempt ${attempt}/3..."
    VYREX_SANDBOX_NAME="$name" \
      VYREX_NON_INTERACTIVE=1 \
      VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 \
      vyrex onboard --resume --non-interactive --yes-i-accept-third-party-software \
    ...
    log "  [onboard] vyrex onboard --resume attempt ${attempt}/3 exited with code $resume_exit"
...
# Onboard a sandbox by name. Removes stale locks, runs vyrex onboard in
# non-interactive mode, and returns 0 if the sandbox appears in vyrex list.
onboard_sandbox() {
  local name="$1"
  ...
  if ! vyrex list 2>/dev/null | grep -q "$name"; then
    log "  [onboard_sandbox] Sandbox '$name' not found in vyrex list after onboard"
    return 1
  fi
  return 0
}
...
# ── Install Vyrex if not present ──────────────────────────────────────────
# Matches the pattern from test-sandbox-survival.sh and test-full-e2e.sh:
# each E2E test installs Vyrex from source so it runs on a fresh CI runner.
install_vyrex() {
  if command -v vyrex &>/dev/null; then
    log "vyrex already installed: $(vyrex --version 2>/dev/null || echo 'unknown')"
    return 0
  fi

  log "=== Installing Vyrex via install.sh ==="
  ...
  if [[ $install_exit -ne 0 ]]; then
    ...
    install_sandbox="${VYREX_SANDBOX_NAME:-my-assistant}"
  ...
  if ! command -v vyrex &>/dev/null; then
    echo -e "${RED}FATAL: vyrex not found on PATH after install${NC}"
    exit 1
  fi

  log "vyrex installed: $(vyrex --version 2>/dev/null || echo 'unknown')"

  # Destroy the sandbox that install.sh created (we create our own)
  local install_sandbox
  install_sandbox="${VYREX_SANDBOX_NAME:-my-assistant}"
  if vyrex list 2>/dev/null | grep -q "$install_sandbox"; then
    log "Destroying install sandbox '$install_sandbox'..."
    vyrex "$install_sandbox" destroy --yes 2>/dev/null || true
  fi
}

# ── Pre-flight ───────────────────────────────────────────────────────────────
# Verify prerequisites (Docker, API key), install Vyrex if needed, and
...
preflight() {
  ...
  log "vyrex: $(vyrex --version 2>/dev/null || echo 'unknown')"
  ...
  if [[ -f "$HOME/.vyrex/onboard.lock" ]]; then
    log "Removing stale onboard lock"
    rm -f "$HOME/.vyrex/onboard.lock"
  fi

  for sb in "$SANDBOX_A" "$SANDBOX_B"; do
    if vyrex list 2>/dev/null | grep -q "$sb"; then
      log "Cleaning up leftover sandbox: $sb"
      vyrex "$sb" destroy --yes 2>/dev/null || true
    fi
  done
...
test_sbx_01_list_sandboxes() {
  ...
  local output
  output=$(vyrex list 2>&1)

  if echo "$output" | grep -q "$SANDBOX_A"; then
    pass "TC-SBX-01: vyrex list shows '$SANDBOX_A'"
  else
    fail "TC-SBX-01: List Sandboxes" "'$SANDBOX_A' not found in vyrex list output"
  fi
}
...
test_sbx_03_status_fields() {
  ...
  local output
  output=$(vyrex "$SANDBOX_A" status 2>&1)
...
test_sbx_04_log_streaming() {
  ...
  output=$(run_with_timeout 10 vyrex "$SANDBOX_A" logs 2>&1) || logs_exit=$?

  if [[ $logs_exit -ne 0 ]]; then
    fail "TC-SBX-04: Log Streaming" "vyrex logs exited with code $logs_exit"
  elif [[ -n "$output" ]]; then
    pass "TC-SBX-04: Log streaming produced output ($(echo "$output" | wc -l | tr -d ' ') lines)"
  else
    fail "TC-SBX-04: Log Streaming" "vyrex logs succeeded but produced no output"
  fi

  run_with_timeout 5 vyrex "$SANDBOX_A" logs --follow &>/dev/null &
  local pid=$!
  sleep 3
...
test_sbx_07_registry_rebuild() {
  ...
  local registry="$HOME/.vyrex/sandboxes.json"
  if [[ ! -f "$registry" ]]; then
  ...
  local output
  output=$(run_with_timeout 60 vyrex list 2>&1) || true
...
test_sbx_08_process_recovery() {
  ...
  log "  Running vyrex status (expect process recovery)..."
  local status_output status_exit=0
  status_output=$(run_with_timeout 120 vyrex "$SANDBOX_A" status 2>&1) || status_exit=$?

  if [[ $status_exit -ne 0 ]]; then
    fail "TC-SBX-08: Process Recovery (status)" "vyrex status exited with code $status_exit"
...
test_sbx_05_destroy_cleanup() {
  ...
  if ! vyrex list 2>/dev/null | grep -q "$target"; then
  ...
  vyrex "$target" destroy --yes 2>&1 | tee -a "$LOG_FILE" || destroy_exit=$?

  if [[ $destroy_exit -ne 0 ]]; then
    fail "TC-SBX-05: Destroy ($target)" "vyrex destroy exited with code $destroy_exit"
  fi

  if vyrex list 2>/dev/null | grep -q "$target"; then
    fail "TC-SBX-05: Destroy ($target)" "Still in vyrex list after destroy (exit $destroy_exit)"
  else
    pass "TC-SBX-05: '$target' removed from vyrex list"
  fi
...
test_sbx_06_gateway_recovery() {
  ...
  local container="openshell-cluster-vyrex"
  ...
  log "  Running vyrex status in background..."
  vyrex "$SANDBOX_A" status >"$status_output" 2>&1 &
  local status_pid=$!
  ...
  for i in $(seq 1 40); do
    ...
    if ! kill -0 "$status_pid" 2>/dev/null; then
      ...
      log "  vyrex status exited with code $exit_code after $((i * 15))s"
  ...
  if kill -0 "$status_pid" 2>/dev/null; then
    log "  vyrex status still running after 10 min — killing"
  ...
  if $recovered; then
    ...
    fail "TC-SBX-06: Gateway Recovery" "vyrex status did not recover the gateway"
  fi
}
...
test_sbx_10_multi_sandbox_metadata() {
  ...
  if $found_a && $found_b; then
    pass "TC-SBX-10: Both sandboxes visible in vyrex list"
  else
...
teardown() {
  ...
  # Do not unlink ~/.vyrex/onboard.lock: see rationale in
...
main() {
  echo ""
  echo "============================================================"
  echo "  Vyrex Sandbox Operations E2E Test Suite"
  echo "  $(date)"
  echo "============================================================"
  echo ""
