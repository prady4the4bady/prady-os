...
# E2E: Docker 26+ overlayfs nested-mount auto-fix (Vyrex#2481)
#
# Validates that Vyrex transparently builds a fuse-overlayfs cluster
...
# validates the negative path: with VYREX_DISABLE_OVERLAY_FIX=1 the
...
#   6. Negative — onboard with VYREX_DISABLE_OVERLAY_FIX=1, expect
...
#   VYREX_SANDBOX_NAME                     — sandbox name (default: e2e-overlayfs)
#   VYREX_E2E_TIMEOUT_SECONDS              — overall timeout (default: 1500)
#   VYREX_OVERLAYFS_E2E_NEGATIVE_TIMEOUT   — negative-phase k3s wait (default: 300)
#
# Usage:
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-overlayfs}"
NEGATIVE_TIMEOUT="${VYREX_OVERLAYFS_E2E_NEGATIVE_TIMEOUT:-300}"
GATEWAY_CONTAINER="openshell-cluster-vyrex"
...
# symlink at /tmp/vyrex-e2e-daemon.json.bak could redirect the
...
STATE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/vyrex-overlayfs-e2e.XXXXXX")"
DAEMON_JSON_BACKUP="${STATE_DIR}/daemon.json.bak"
DAEMON_JSON_ABSENT_MARKER="${STATE_DIR}/daemon.json.absent"
INSTALL_LOG="${VYREX_E2E_INSTALL_LOG:-/tmp/vyrex-e2e-install.log}"
ONBOARD_LOG_POSITIVE="/tmp/vyrex-e2e-onboard-positive.log"
ONBOARD_LOG_NEGATIVE="/tmp/vyrex-e2e-onboard-negative.log"
...
patched_images=$(docker image ls --format '{{.Repository}}:{{.Tag}}' | grep -E '^vyrex-cluster:' || true)
...
# that already exports VYREX_DISABLE_OVERLAY_FIX=1 or
# VYREX_OVERLAY_SNAPSHOTTER=native can't silently change the path the
# positive phase is asserting on (lines 325-345 below).
env -u VYREX_DISABLE_OVERLAY_FIX -u VYREX_OVERLAY_SNAPSHOTTER \
  VYREX_NON_INTERACTIVE=1 \
  VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 \
...
patched_tag=$(docker image ls --format '{{.Repository}}:{{.Tag}}' | grep -E '^vyrex-cluster:.*-fuse-overlayfs-[0-9a-f]{8}$' | head -1)
if [ -n "$patched_tag" ]; then
  pass "Patched cluster image present: $patched_tag"
else
  fail "No vyrex-cluster:*-fuse-overlayfs-* image found after onboard"
fi
...
if [ -z "$patched_tag" ]; then
  ...
  # `vyrex-cluster:<openshell-version>-<snapshotter>-<sha8>`).
  openshell_version=$(printf '%s\n' "$patched_tag" | sed -E 's|^vyrex-cluster:([^-]+)-.*|\1|')
...
# ══════════════════════════════════════════════════════════════════
section "Phase 5: Negative path (VYREX_DISABLE_OVERLAY_FIX=1)"
...
if [ $negative_exit -ne 0 ]; then
  ...
  fail "Onboard unexpectedly succeeded with VYREX_DISABLE_OVERLAY_FIX=1"
fi
