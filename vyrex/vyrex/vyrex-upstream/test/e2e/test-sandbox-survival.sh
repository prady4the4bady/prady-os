...
#
# Validates EVERY complaint from NVIDIA/Vyrex#486, #888, #859, #1086:
...
#   5. No re-onboard required (vyrex <name> status/connect work)
#   6. Live inference works end-to-end after restart
#   7. Vyrex registry retains sandbox entry
#   8. Gateway stop/start is non-destructive
#
# This test uses Vyrex's own install.sh to set up everything including
...
#   VYREX_SANDBOX_NAME                  — sandbox name (default: e2e-survival)
#   VYREX_E2E_TIMEOUT_SECONDS           — overall timeout (default: 900)
#
# Usage:
...
export VYREX_E2E_DEFAULT_TIMEOUT=900
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-survival}"

# shellcheck source=test/e2e/lib/sandbox-teardown.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib/sandbox-teardown.sh"
register_sandbox_for_teardown "$SANDBOX_NAME"

REGISTRY="$HOME/.vyrex/sandboxes.json"
...
if [ "${VYREX_ACCEPT_THIRD_PARTY_SOFTWARE:-}" != "1" ]; then
  fail "VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 is required"
  exit 1
fi
...
# ══════════════════════════════════════════════════════════════════
# Phase 2: Install Vyrex (which installs OpenShell)
# ══════════════════════════════════════════════════════════════════
section "Phase 2: Install Vyrex via install.sh"

info "Running install.sh --non-interactive (installs Node.js, OpenShell, Vyrex, runs onboard)..."
...
env \
  VYREX_NON_INTERACTIVE=1 \
  VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 \
  VYREX_SANDBOX_NAME="$SANDBOX_NAME" \
...
# 3a: Vyrex registry has it
if [ -f "$REGISTRY" ] && grep -Fq "\"${SANDBOX_NAME}\"" "$REGISTRY"; then
  pass "Vyrex registry contains '$SANDBOX_NAME'"
else
  fail "Vyrex registry missing '$SANDBOX_NAME' — onboard may have failed"
  exit 1
fi

# 3b: vyrex list shows it
if list_output=$(vyrex list 2>&1) && grep -Fq "$SANDBOX_NAME" <<<"$list_output"; then
  pass "vyrex list shows '$SANDBOX_NAME'"
else
  fail "vyrex list doesn't show '$SANDBOX_NAME': ${list_output:0:200}"
  exit 1
fi
...
MARKER_VALUE="vyrex-survival-$(date +%s)"
...
if openshell gateway stop -g vyrex 2>/dev/null; then
...
CONTAINER_NAME="openshell-cluster-vyrex"
...
if openshell gateway start --name vyrex 2>&1; then
...
for attempt in $(seq 1 60); do
  ...
  if echo "$gw_status" | grep -qi "Connected" && echo "$gw_status" | grep -qi "vyrex"; then
    HEALTHY=1
    break
  fi
  sleep 5
...
# 7c: Vyrex registry still has it — #486 "No sandboxes registered"
if [ -f "$REGISTRY" ] && grep -Fq "\"${SANDBOX_NAME}\"" "$REGISTRY"; then
  pass "Vyrex registry still contains '$SANDBOX_NAME' after restart"
else
  fail "Vyrex registry lost '$SANDBOX_NAME' after restart (#486)"
fi

# 7d: vyrex list shows it — the actual user-facing command
if list_output=$(vyrex list 2>&1) && grep -Fq "$SANDBOX_NAME" <<<"$list_output"; then
  pass "vyrex list shows '$SANDBOX_NAME' after restart"
else
  fail "vyrex list doesn't show '$SANDBOX_NAME' after restart: ${list_output:0:200}"
fi

# 7e: vyrex status works — #859 "unclear CLI behavior"
# No special intervention should be required after gateway restart.
# If vyrex status hangs, that IS the bug — use timeout to detect it.
# Write to a temp file instead of $() to avoid pipe FD inheritance:
# vyrex's SSH recovery can spawn background processes that hold the
# pipe open, preventing $() from returning even after timeout kills vyrex.
STATUS_TMP="$(mktemp)"
TIMEOUT_STATUS=""
command -v timeout >/dev/null 2>&1 && TIMEOUT_STATUS="timeout 120"
command -v gtimeout >/dev/null 2>&1 && TIMEOUT_STATUS="gtimeout 120"
$TIMEOUT_STATUS vyrex "$SANDBOX_NAME" status >"$STATUS_TMP" 2>&1
status_exit=$?
status_output=$(cat "$STATUS_TMP")
rm -f "$STATUS_TMP"
if [ "$status_exit" -eq 0 ]; then
  pass "vyrex $SANDBOX_NAME status exits 0 after restart (no re-onboard needed)"
elif [ "$status_exit" -eq 124 ]; then
  fail "vyrex $SANDBOX_NAME status TIMED OUT after restart (port forward or SSH recovery hung)"
else
  fail "vyrex $SANDBOX_NAME status failed after restart (exit $status_exit): ${status_output:0:200}"
fi
...
if [ "$SSH_OK" -eq 1 ]; then
  ...
  vyrex "$SANDBOX_NAME" logs 2>&1 | grep -i "handshake" | head -5 || true
fi
