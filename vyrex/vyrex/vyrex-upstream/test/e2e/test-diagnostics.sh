...
# =============================================================================
# test-diagnostics.sh
# Vyrex Diagnostics & Credential E2E Tests
#
# Covers:
#   TC-DIAG-04: vyrex --version (semver output, exit 0)
#   TC-DIAG-02: vyrex debug --quick (fast, non-empty archive)
#   TC-DIAG-01: vyrex debug --output (tarball, no credentials in archive)
#   TC-DIAG-05: /vyrex status inside sandbox (model + provider)
...
# ── Overall timeout ──────────────────────────────────────────────────────────
if [ -z "${VYREX_E2E_NO_TIMEOUT:-}" ]; then
  export VYREX_E2E_NO_TIMEOUT=1
  TIMEOUT_SECONDS="${VYREX_E2E_TIMEOUT_SECONDS:-3600}"
...
onboard_sandbox() {
  ...
  VYREX_SANDBOX_NAME="$name" \
    ...
    ${TIMEOUT_CMD:+$TIMEOUT_CMD 600} vyrex onboard --non-interactive --yes-i-accept-third-party-software \
    2>&1 | tee -a "$LOG_FILE" || {
...
# =============================================================================
# TC-DIAG-04: vyrex --version
# =============================================================================
test_diag_04_version() {
  log "=== TC-DIAG-04: vyrex --version ==="

  local version_output version_rc=0
  version_output=$(vyrex --version 2>&1) || version_rc=$?

  log "  Output: $version_output (exit $version_rc)"

  if [[ $version_rc -ne 0 ]]; then
    fail "TC-DIAG-04: Exit code" "vyrex --version exited with $version_rc"
    return
  fi
...
# =============================================================================
# TC-DIAG-02: vyrex debug --quick
# =============================================================================
test_diag_02_debug_quick() {
  log "=== TC-DIAG-02: vyrex debug --quick ==="
  ...
  debug_output=$(${TIMEOUT_CMD:+$TIMEOUT_CMD 30} vyrex debug --quick --output "$output_file" 2>&1) || debug_rc=$?
...
# =============================================================================
# TC-DIAG-01: vyrex debug --output (full tarball + credential sanitization)
# =============================================================================
test_diag_01_debug_tarball() {
  ...
  local debug_output debug_rc=0
  debug_output=$(vyrex debug --output "$output_file" 2>&1) || debug_rc=$?
...
test_diag_05_sandbox_config() {
  ...
  log "  Checking vyrex status from host..."
  local status_output
  status_output=$(vyrex "$SANDBOX_NAME" status 2>&1) || true
  if echo "$status_output" | grep -qiE "Model.*nemotron\|Model.*nvidia\|Model.*llama"; then
    pass "TC-DIAG-05: vyrex status shows model info"
  elif echo "$status_output" | grep -qi "Model"; then
    pass "TC-DIAG-05: vyrex status shows Model field"
  else
    fail "TC-DIAG-05: Status" "No model info in vyrex status output"
  fi
}
...
test_diag_03_credentials() {
  ...
  list_output=$(vyrex credentials list 2>&1) || list_rc=$?
  ...
  reset_output=$(vyrex credentials reset NVIDIA_API_KEY --yes 2>&1) || reset_rc=$?
  ...
  local post_list
  post_list=$(vyrex credentials list 2>&1) || true
...
main() {
  echo ""
  echo "============================================================"
  echo "  Vyrex Diagnostics E2E Tests"
  echo "  $(date)"
  echo "============================================================"
  echo ""
