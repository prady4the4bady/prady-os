...
#
# What this tests:
#   1. Run brev-launchable-ci-cpu.sh with VYREX_REF=current branch
#   2. Verify installation artifacts (vyrex, openshell, Node.js ≥22, Docker, sentinel)
#   3. vyrex onboard --non-interactive with NVIDIA_API_KEY (cloud provider)
#   4. Sandbox health: vyrex list, status, gateway running
...
#   VYREX_REF              — git ref for brev-launchable-ci-cpu.sh (default: current branch)
#   VYREX_SANDBOX_NAME     — sandbox name (default: e2e-launchable)
#   VYREX_RECREATE_SANDBOX — set to 1 to recreate if exists
...
#
# Usage:
#   VYREX_NON_INTERACTIVE=1 VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 \
#     NVIDIA_API_KEY=nvapi-... bash test/e2e/test-launchable-smoke.sh
#
# See: https://github.com/NVIDIA/Vyrex/issues/2599
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-launchable}"
INSTALL_LOG="/tmp/vyrex-launchable-install.log"
TEST_LOG="/tmp/vyrex-launchable-test.log"

# The launchable script clones into ~/Vyrex by default. For CI, use
# a unique directory so we don't collide with the checkout.
VYREX_CLONE_DIR="${VYREX_CLONE_DIR:-${HOME}/Vyrex-launchable}"
export VYREX_CLONE_DIR

# The launchable script clones from github.com/NVIDIA/Vyrex using
# VYREX_REF as the branch. To test the CURRENT code (not main HEAD),
...
sudo rm -rf "$VYREX_CLONE_DIR" 2>/dev/null || rm -rf "$VYREX_CLONE_DIR" || true
...
# $VYREX_CLONE_DIR/.git and runs the refresh path (fetch+checkout)
# instead of a fresh clone from NVIDIA/Vyrex. We create a "main" branch
# at the current commit so VYREX_REF=main resolves locally.
info "Pre-seeding $VYREX_CLONE_DIR from checkout at $REPO..."
git clone --local --no-hardlinks "$REPO" "$VYREX_CLONE_DIR"
...
git -C "$VYREX_CLONE_DIR" checkout -B main HEAD 2>/dev/null || true
git -C "$VYREX_CLONE_DIR" remote set-url origin "$VYREX_CLONE_DIR"
...
info "VYREX_REF=$VYREX_REF"
info "VYREX_CLONE_DIR=$VYREX_CLONE_DIR"
...
info "This installs Docker, Node.js 22, OpenShell, clones Vyrex, builds CLI+plugin."
...
# 3a: vyrex on PATH and --help works
if command -v vyrex >/dev/null 2>&1; then
  pass "vyrex on PATH: $(command -v vyrex)"
else
  fail "vyrex not found on PATH after launchable install"
fi
...
# 3e: Sentinel file
SENTINEL="/var/run/vyrex-launchable-ready"
if [ -f "$SENTINEL" ]; then
...
if [ -d "$VYREX_CLONE_DIR/.git" ]; then
  pass "Vyrex cloned at $VYREX_CLONE_DIR"
else
  fail "Vyrex clone directory missing: $VYREX_CLONE_DIR"
fi

if [ -d "$VYREX_CLONE_DIR/dist" ]; then
  pass "CLI built (dist/ exists)"
else
  fail "CLI not built (dist/ missing)"
fi

if [ -d "$VYREX_CLONE_DIR/vyrex/dist" ]; then
  pass "Plugin built (vyrex/dist/ exists)"
else
  fail "Plugin not built (vyrex/dist/ missing)"
fi
...
# community path: the user's Vyrex is in ~/Vyrex, not a CI checkout.
cd "$VYREX_CLONE_DIR" || {
  fail "Could not cd to $VYREX_CLONE_DIR"
  exit 1
}

info "Running vyrex onboard --non-interactive..."
info "Provider: NVIDIA Endpoints (cloud)"
info "Sandbox name: $SANDBOX_NAME"

ONBOARD_LOG="/tmp/vyrex-launchable-onboard.log"
export VYREX_SANDBOX_NAME="$SANDBOX_NAME"
export VYREX_RECREATE_SANDBOX="${VYREX_RECREATE_SANDBOX:-1}"

vyrex onboard --non-interactive >"$ONBOARD_LOG" 2>&1 &
onboard_pid=$!
...
if [ $onboard_exit -eq 0 ]; then
  pass "vyrex onboard completed (exit 0)"
else
  fail "vyrex onboard failed (exit $onboard_exit)"
...
# 5d: Gateway running
if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "vyrex\|openshell"; then
...
echo "    - brev-launchable-ci-cpu.sh bootstrap (Docker, Node.js, OpenShell, Vyrex)"
echo "    - Installation artifacts (binaries on PATH, sentinel file, built outputs)"
echo "    - Onboard via launchable-installed Vyrex (cloud provider)"
