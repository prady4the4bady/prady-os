...
#
# Runs check-docs.sh to verify vyrex --help matches commands.md
...
# check-docs.sh needs vyrex on PATH for CLI parity check.
...
if command -v vyrex >/dev/null 2>&1; then
  ...
  if command -v vyrex >/dev/null 2>&1; then
    pass "vyrex on PATH (after sourcing nvm)"
  else
    fail "vyrex not on PATH — install Vyrex first"
    exit 1
  fi
fi
...
info "Running check-docs.sh --only-cli (vyrex --help vs commands.md)..."
set +e
