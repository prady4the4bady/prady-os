...
/**
 * Branch Validation E2E — installs Vyrex FROM SOURCE on a fresh Brev instance.
 *
...
const CLI_PATH = path.join(REPO_DIR, "bin", "vyrex.js");
...
const LAUNCHABLE_SENTINEL = "/var/run/vyrex-launchable-ready";
...
/** Run a command on the remote VM with env vars set for Vyrex. */
function sshEnv(
  cmd: string,
  ...
  const envParts = [
    ...
    `export VYREX_SANDBOX_NAME=e2e-test`,
  ];
...
function runLocalDeploy(instanceName: string): void {
  const env = {
    ...
    VYREX_SANDBOX_NAME: "e2e-test",
    VYREX_PROVIDER: process.env.VYREX_PROVIDER || "build",
    VYREX_DEPLOY_NO_CONNECT: "1",
    VYREX_DEPLOY_NO_START_SERVICES: "1",
  };
...
function bootstrapLaunchable(elapsed: () => string): { remoteDir: string; needsOnboard: boolean } {
  // The launchable clones Vyrex to ~/Vyrex
  const remoteHome = ssh("echo $HOME");
  const resolvedRemoteDir = `${remoteHome}/Vyrex`;
  ...
  // without this step bin/vyrex.js would `require("../dist/vyrex")`
  ...
  ssh(
    `source ~/.nvm/nvm.sh 2>/dev/null || true && cd ${resolvedRemoteDir}/vyrex && npm install && npm run build`,
    {
      timeout: 120_000,
      stream: true,
    },
  ...
  // Expose the vyrex CLI on PATH. The launchable setup script already
  // creates /usr/local/bin/vyrex → $VYREX_CLONE_DIR/bin/vyrex.js
  ...
  console.log(`[${elapsed()}] Linking vyrex CLI (direct symlink)...`);
  ssh(
    `sudo ln -sf ${resolvedRemoteDir}/bin/vyrex.js /usr/local/bin/vyrex && sudo chmod +x ${resolvedRemoteDir}/bin/vyrex.js`,
    {
      timeout: 30_000,
      stream: true,
    },
  );
  console.log(`[${elapsed()}] vyrex CLI linked`);

  return { remoteDir: resolvedRemoteDir, needsOnboard: true };
}

/**
 * Launch vyrex onboard in background and poll until the sandbox is Ready.
 *
 * The `vyrex onboard` process hangs after sandbox creation because
...
function pollForSandboxReady(elapsed: () => string): void {
  ...
  console.log(`[${elapsed()}] Starting vyrex onboard in background...`);
  ...
  try {
    sshEnv(
      [
        `source ~/.nvm/nvm.sh 2>/dev/null || true`,
        `cd ${remoteDir}`,
        `nohup vyrex onboard --non-interactive </dev/null >/tmp/vyrex-onboard.log 2>&1 & disown`,
        `sleep 2`,
    ...
    try {
      const check = ssh("test -f /tmp/vyrex-onboard.log && echo OK || echo MISSING", {
        timeout: 10_000,
      });
  ...
  while (Date.now() - onboardStart < maxOnboardWaitMs) {
    try {
      ...
      try {
        const tail = ssh("tail -2 /tmp/vyrex-onboard.log 2>/dev/null || echo '(no log yet)'", {
          timeout: 10_000,
        });
    ...
    try {
      const session = ssh("cat ~/.vyrex/onboard-session.json 2>/dev/null || echo '{}'", {
        timeout: 10_000,
      });
      const parsed = JSON.parse(session);
      if (parsed.status === "failed") {
        const failLog = ssh("cat /tmp/vyrex-onboard.log 2>/dev/null || echo 'no log'", {
          timeout: 10_000,
        });
...
function writeManualRegistry(elapsed: () => string): void {
  ...
  try {
    ssh(
      `pkill -f "vyrex onboard" 2>/dev/null; pkill -f "openshell sandbox create" 2>/dev/null; sleep 1; true`,
      { timeout: 15_000 },
    );
  } catch {
  ...
  ssh(
    `mkdir -p ~/.vyrex && printf '%s' '${shellEscape(registryJson)}' > ~/.vyrex/sandboxes.json`,
    { timeout: 15_000 },
  );
...
describe.runIf(hasRequiredVars && hasAuthenticatedBrev)("Brev E2E", () => {
  beforeAll(() => {
    ...
    if (TEST_SUITE === "deploy-cli") {
      console.log(`[${elapsed()}] Running vyrex deploy end to end...`);
      instanceCreated = true;
      runLocalDeploy(requireInstanceName());
      refreshAndWaitForSsh(elapsed);
      const remoteHome = ssh("echo $HOME");
      remoteDir = `${remoteHome}/vyrex`;
    } else {
    ...
    if (TEST_SUITE !== "full") {
      ...
      const registry = JSON.parse(ssh(`cat ~/.vyrex/sandboxes.json`, { timeout: 10_000 }));
