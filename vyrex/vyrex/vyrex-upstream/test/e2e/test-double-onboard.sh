...
#   - prove repeat onboard reuses the healthy shared Vyrex gateway
...
# CI job timeout.
export VYREX_E2E_DEFAULT_TIMEOUT=4800
...
PHASE_TIMEOUT="${VYREX_E2E_PHASE_TIMEOUT:-1200}"
...
FAKE_HOST="127.0.0.1"
FAKE_PORT="${VYREX_FAKE_PORT:-18080}"
FAKE_BASE_URL="http://${FAKE_HOST}:${FAKE_PORT}/v1"
FAKE_LOG="$(mktemp)"
FAKE_PID=""

if command -v node >/dev/null 2>&1 && [ -f "$REPO_ROOT/bin/vyrex.js" ]; then
  VYREX_CMD=(node "$REPO_ROOT/bin/vyrex.js")
else
  VYREX_CMD=(vyrex)
fi
...
run_onboard() {
  ...
  local -a env_args=(
    ...
    "VYREX_ENDPOINT_URL=${FAKE_BASE_URL}"
    "VYREX_MODEL=test-model"
    "VYREX_SANDBOX_NAME=${sandbox_name}"
    "VYREX_POLICY_MODE=skip"
    "VYREX_DASHBOARD_PORT="
    "CHAT_UI_URL="
  )
  if [ "$recreate" = "1" ]; then
    env_args+=("VYREX_RECREATE_SANDBOX=1")
  fi

  run_with_timeout "$PHASE_TIMEOUT" env "${env_args[@]}" "${VYREX_CMD[@]}" onboard --non-interactive >"$log_file" 2>&1
  RUN_ONBOARD_EXIT=$?
...
if [ -x "$REPO_ROOT/bin/vyrex.js" ] || command -v vyrex >/dev/null 2>&1; then
  run_vyrex "$SANDBOX_A" destroy --yes 2>/dev/null || true
  run_vyrex "$SANDBOX_B" destroy --yes 2>/dev/null || true
fi
...
if [ -x "$REPO_ROOT/bin/vyrex.js" ] || command -v vyrex >/dev/null 2>&1; then
  pass "vyrex CLI available"
else
  fail "vyrex CLI not found — cannot continue"
  exit 1
fi
...
if openshell gateway info -g vyrex 2>/dev/null | grep -q "vyrex"; then
...
info "Running vyrex onboard with VYREX_RECREATE_SANDBOX=1..."

GATEWAY_ID_BEFORE=$(docker ps -qf "name=openshell-cluster-vyrex" | head -1)
...
GATEWAY_ID_AFTER=$(docker ps -qf "name=openshell-cluster-vyrex" | head -1)
...
info "Running vyrex onboard with new sandbox name..."

GATEWAY_ID_BEFORE3=$(docker ps -qf "name=openshell-cluster-vyrex" | head -1)
...
GATEWAY_ID_AFTER3=$(docker ps -qf "name=openshell-cluster-vyrex" | head -1)
...
# surface it in vyrex list, and not collide with A's dashboard.
...
LIST_LOG="$(mktemp)"
run_vyrex list >"$LIST_LOG" 2>&1 || true
...
if [ -n "$port_a" ] && [ -n "$port_b" ]; then
  pass "vyrex list shows dashboard ports for both test sandboxes (#2174)"
else
  fail "vyrex list did not show dashboard ports for both test sandboxes (a=${port_a:-missing} b=${port_b:-missing})"
  info "Observed vyrex list output:"
  printf '%s\n' "$list_output" | sed 's/^/    /'
fi

if [ -n "$port_a" ] && [ -n "$port_b" ] && [ "$port_a" != "$port_b" ]; then
  pass "vyrex list shows distinct dashboard ports for test sandboxes (#2174)"
else
...
info "Deleting '$SANDBOX_A' directly in OpenShell to leave a stale Vyrex registry entry..."
...
STATUS_LOG="$(mktemp)"
run_vyrex "$SANDBOX_A" status >"$STATUS_LOG" 2>&1
status_exit=$?
...
info "Stopping the Vyrex gateway runtime to verify current lifecycle behavior..."

openshell forward stop 18789 2>/dev/null || true
openshell gateway stop -g vyrex 2>/dev/null || true

GATEWAY_LOG="$(mktemp)"
run_vyrex "$SANDBOX_B" status >"$GATEWAY_LOG" 2>&1
gateway_status_exit=$?
...
if grep -qE \
  "Recovered Vyrex gateway runtime|gateway is no longer configured after restart/rebuild|gateway is still refusing connections after restart|gateway trust material rotated after restart" \
...
# (stopped in Phase 6), `vyrex destroy` may delete the sandbox from
...
run_vyrex "$SANDBOX_A" status 2>/dev/null || true
run_vyrex "$SANDBOX_B" status 2>/dev/null || true
