...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-repair}"
OTHER_SANDBOX_NAME="${VYREX_OTHER_SANDBOX_NAME:-e2e-other}"

# Shim so the teardown helper's trap can call `vyrex destroy` even when
# this repo-local test run has no globally-installed `vyrex` on PATH (it
# drives the CLI via `node "$REPO/bin/vyrex.js"` via run_vyrex).
if ! command -v vyrex >/dev/null 2>&1; then
  vyrex() { node "$REPO/bin/vyrex.js" "$@"; }
fi
...
run_vyrex "$SANDBOX_NAME" destroy 2>/dev/null || true
run_vyrex "$OTHER_SANDBOX_NAME" destroy 2>/dev/null || true
...
# Use VYREX_POLICY_MODE=custom without VYREX_POLICY_PRESETS — this is a
# real validation path that exits 1 at the policy step after the sandbox is
# already created, leaving resumable session state.
#
# Note: the previous approach (VYREX_POLICY_MODE=invalid) stopped working
...
VYREX_NON_INTERACTIVE=1 \
  ...
  VYREX_POLICY_MODE=custom \
  VYREX_POLICY_PRESETS="" \
  node "$REPO/bin/vyrex.js" onboard --non-interactive >"$FIRST_LOG" 2>&1
first_exit=$?
...
if echo "$first_output" | grep -q "VYREX_POLICY_PRESETS is required when VYREX_POLICY_MODE=custom"; then
...
env -u NVIDIA_API_KEY \
  ...
  node "$REPO/bin/vyrex.js" onboard --resume --non-interactive >"$REPAIR_LOG" 2>&1
repair_exit=$?
...
if run_vyrex "$SANDBOX_NAME" status >/dev/null 2>&1; then
...
VYREX_NON_INTERACTIVE=1 \
  ...
  VYREX_POLICY_MODE=custom \
  VYREX_POLICY_PRESETS="" \
  node "$REPO/bin/vyrex.js" onboard --non-interactive >"$REINJECT_LOG" 2>&1 || true
rm -f "$REINJECT_LOG"
...
env -u NVIDIA_API_KEY \
  VYREX_NON_INTERACTIVE=1 \
  VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 \
  VYREX_SANDBOX_NAME="$OTHER_SANDBOX_NAME" \
  VYREX_POLICY_MODE=skip \
  node "$REPO/bin/vyrex.js" onboard --resume --non-interactive >"$SANDBOX_CONFLICT_LOG" 2>&1
...
env -u NVIDIA_API_KEY \
  ...
  VYREX_MODEL=gpt-5.4 \
  VYREX_POLICY_MODE=skip \
  node "$REPO/bin/vyrex.js" onboard --resume --non-interactive >"$PROVIDER_CONFLICT_LOG" 2>&1
