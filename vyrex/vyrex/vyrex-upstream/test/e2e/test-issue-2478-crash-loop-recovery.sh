...
# Long-running e2e regression for NVIDIA/Vyrex#2478 — gateway crash-loop
...
#   /tmp/vyrex-proxy-env.sh at sandbox-start and reach the gateway via
#   ~/.bashrc on the FIRST start. Before #2478 the gateway recovery path
#   (laptop sleep, health-monitor restart, manual `vyrex <name> connect`)
...
#   VYREX_SANDBOX_NAME                  — sandbox name (default: e2e-2478)
#   VYREX_E2E_TIMEOUT_SECONDS           — overall timeout (default: 1500)
#   VYREX_E2E_CRASH_CYCLES              — crash-recover cycles (default: 5)
#   VYREX_E2E_SOAK_SECONDS              — idle soak window (default: 300)
#
# Usage:
...
export VYREX_E2E_DEFAULT_TIMEOUT=1500
...
SANDBOX_NAME="${VYREX_SANDBOX_NAME:-e2e-2478}"
CRASH_CYCLES="${VYREX_E2E_CRASH_CYCLES:-5}"
SOAK_SECONDS="${VYREX_E2E_SOAK_SECONDS:-300}"
...
# Read /tmp/vyrex-proxy-env.sh — the single source of truth for the
...
proxy_env_contents() {
  sandbox_exec sh -c "cat /tmp/vyrex-proxy-env.sh 2>/dev/null"
}
...
gateway_guards_active() {
  ...
  if ! echo "$env_contents" | grep -q 'vyrex-sandbox-safety-net'; then
    echo "  [guards] proxy-env.sh missing safety-net export"
    return 1
  fi
  if ! echo "$env_contents" | grep -q 'vyrex-ciao-network-guard'; then
...
# as a process. A Vyrex user reported on #2478 that pre-fix the ciao
...
gateway_diagnostics() {
  ...
  echo "  [vyrex status]"
  vyrex "$SANDBOX_NAME" status 2>&1 | head -30 | sed 's/^/    /'
...
run_probe_only_or_fail() {
  ...
  local probe_out
  probe_out="$(mktemp)"
  if ! timeout 60 vyrex "$SANDBOX_NAME" connect --probe-only >"$probe_out" 2>&1; then
...
if [ "${VYREX_NON_INTERACTIVE:-}" != "1" ] || [ "${VYREX_ACCEPT_THIRD_PARTY_SOFTWARE:-}" != "1" ]; then
  fail "VYREX_NON_INTERACTIVE=1 and VYREX_ACCEPT_THIRD_PARTY_SOFTWARE=1 are required"
  exit 1
fi
...
if ! command -v vyrex >/dev/null 2>&1; then
  fail "vyrex not on PATH after install"
  exit 1
fi
pass "vyrex on PATH"
...
for cycle in $(seq 1 "$CRASH_CYCLES"); do
  ...
  # `vyrex <name> connect --probe-only` calls
...
SNAPSHOT_B64="$(sandbox_exec sh -c 'base64 < /tmp/vyrex-proxy-env.sh' | tr -d '[:space:]')"
SNAPSHOT_SIZE="$(sandbox_exec sh -c 'wc -c < /tmp/vyrex-proxy-env.sh' | tr -d '[:space:]')"
...
# can't silently respawn the gateway before vyrex status runs the
# recovery script (which is the only path that emits the warning).
sandbox_exec sh -c 'rm -f /tmp/vyrex-proxy-env.sh' >/dev/null
...
sandbox_exec sh -c "echo '$SNAPSHOT_B64' | base64 -d > /tmp/vyrex-proxy-env.sh && chmod 444 /tmp/vyrex-proxy-env.sh" >/dev/null

# Verify restore is byte-identical to the snapshot.
restored_size="$(sandbox_exec sh -c 'wc -c < /tmp/vyrex-proxy-env.sh' | tr -d '[:space:]')"
...
[[ "${VYREX_E2E_KEEP_SANDBOX:-}" = "1" ]] || vyrex "$SANDBOX_NAME" destroy --yes >/dev/null 2>&1 || true
