...
const CLI_PATH = JSON.stringify(path.join(REPO_ROOT, "dist", "vyrex.js"));
...
function runPolicyAdd(
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-policy-add-"));
...
process.argv = ["node", "vyrex.js", "test-sandbox", "policy-add", ...${JSON.stringify(extraArgs)}];
...
const items = JSON.parse(process.env.VYREX_TEST_ITEMS);
const options = JSON.parse(process.env.VYREX_TEST_OPTIONS || "{}");
...
`;
  ...
  return spawnSync(process.execPath, ["-e", script], {
    ...
    env: {
      ...process.env,
      VYREX_TEST_ITEMS: JSON.stringify(SELECT_FROM_LIST_ITEMS),
      VYREX_TEST_OPTIONS: JSON.stringify({ applied }),
    },
  });
}
...
describe("policies", () => {
  ...
  describe("buildPolicySetCommand", () => {
    ...
    it("uses the resolved openshell binary when provided by the installer path", () => {
      process.env.VYREX_OPENSHELL_BIN = "/tmp/fake path/openshell";
      try {
        const cmd = policies.buildPolicySetCommand("/tmp/policy.yaml", "my-assistant");
        expect(cmd).toEqual([
        ...
        delete process.env.VYREX_OPENSHELL_BIN;
      }
    });
  });
...
`;
  ...
  describe("policy-add confirmation", () => {
    ...
    it("honors non-interactive mode when a preset name is provided", () => {
      const result = runPolicyAdd("n", ["pypi"], { VYREX_NON_INTERACTIVE: "1" });
    ...
    it("fails fast in non-interactive mode without a preset name", () => {
      const result = runPolicyAdd("y", [], { VYREX_NON_INTERACTIVE: "1" });
  ...
  describe("policy-remove confirmation", () => {
    function runPolicyRemove(
      ...
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-policy-remove-"));
...
process.argv = ["node", "vyrex.js", "test-sandbox", "policy-remove", ...${JSON.stringify(extraArgs)}];
Promise.resolve(require(${CLI_PATH}).mainPromise).finally(() => {
  process.stdout.write("\n__CALLS__" + JSON.stringify(calls));
});
`;
    ...
    it("honors non-interactive mode when removing an explicit preset", () => {
      const result = runPolicyRemove("n", ["pypi"], { VYREX_NON_INTERACTIVE: "1" });
    ...
    it("fails fast in non-interactive mode without a preset name", () => {
      const result = runPolicyRemove("y", [], { VYREX_NON_INTERACTIVE: "1" });
  ...
  describe("policy-remove custom presets", () => {
    function runPolicyRemoveCustom(
      ...
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-policy-remove-custom-"));
...
process.argv = ["node", "vyrex.js", "test-sandbox", "policy-remove", ${JSON.stringify(presetName)}, ...${JSON.stringify(extraArgs)}];
Promise.resolve(require(${CLI_PATH}).mainPromise).finally(() => {
  process.stdout.write("\n__CALLS__" + JSON.stringify(calls));
});
`;
  ...
  describe("loadPresetFromFile", () => {
    function writeTmp(body: string, ext = "yaml") {
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-custom-preset-"));
  ...
  describe("policy-add --from-file / --from-dir", () => {
    function runPolicyAddExternal(
      ...
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-policy-external-"));
...
`;
    ...
    it("applies a custom preset when --from-file and --yes are provided", () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-from-file-"));
    ...
    it("exits non-zero when --from-file points to an unreadable preset", () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-from-file-bad-"));
    ...
    it("does not apply and does not prompt under --from-file --dry-run", () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-from-file-dry-"));
      const file = path.join(tmp, "custom-rule.yaml");
      fs.writeFileSync(file, "preset:\n  name: custom-rule\nnetwork_policies: {}\n");
    ...
    it("skips the confirmation prompt when VYREX_NON_INTERACTIVE=1", () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-from-file-env-"));
      const file = path.join(tmp, "custom-rule.yaml");
      fs.writeFileSync(file, "preset:\n  name: custom-rule\nnetwork_policies: {}\n");
      const result = runPolicyAddExternal(["--from-file", file], { VYREX_NON_INTERACTIVE: "1" });
      expect(result.status).toBe(0);
      const calls = JSON.parse(result.stdout.split("__CALLS__")[1].trim()) as PolicyCall[];
      expect(calls.some((c) => c.type === "prompt")).toBeFalsy();
      expect(calls).toContainEqual({
    ...
    it("does not apply an external preset when the confirmation prompt is declined", () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-from-file-no-"));
      const file = path.join(tmp, "custom-rule.yaml");
      fs.writeFileSync(file, "preset:\n  name: custom-rule\nnetwork_policies: {}\n");
    ...
    it("applies every preset in --from-dir in sorted order and aborts on the first failure", () => {
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-from-dir-"));
      fs.writeFileSync(
    ...
    it("--from-dir skips hidden dotfile yaml presets", () => {
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-from-dir-hidden-"));
    ...
    it("--from-dir skips sub-directories whose names end in .yaml/.yml", () => {
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-from-dir-skipdir-"));
