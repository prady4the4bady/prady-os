...
//
// Regression test for #2177 — when a user re-runs `vyrex onboard` on an
...
function runScript(scriptBody: string): SpawnSyncReturns<string> {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-preset-diff-"));
...
describe("setupPoliciesWithSelection preset-diff (issue #2177)", () => {
  ...
  // defaults (applies 5 presets), second with VYREX_POLICY_PRESETS=npm —
