...
describe("vyrex CLI runtime recovery", () => {
  it(
    "recovers sandbox status when openshell is only available via the resolved fallback path",
    testTimeoutOptions(),
    () => {
      const repoRoot = path.join(import.meta.dirname, "..");
      const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-cli-recovery-"));
...
`,
      ...
      const result = spawnSync(
        process.execPath,
        [path.join(repoRoot, "bin", "vyrex.js"), "my-assistant", "status"],
        {
          cwd: repoRoot,
          encoding: "utf-8",
          env: {
      ...
      assert.match(result.stdout, /Recovered Vyrex gateway runtime via (start|select)/);
