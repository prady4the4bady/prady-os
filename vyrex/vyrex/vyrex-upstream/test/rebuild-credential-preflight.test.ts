 ...
 *    ~/.vyrex/credentials.json when process.env is empty.
...
function createFixture(opts: {
  ...
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "vyrex-2273-"));
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      metadata: { gatewayName: "vyrex", fromDockerfile: null },
      steps: {
        preflight: {
          status: "complete",
          startedAt: null,
  ...
  if (savedCredential) {
    fs.writeFileSync(
      path.join(vyrexDir, "credentials.json"),
...
if (a[0]==="gateway" && a[1]==="info")       { process.stdout.write("vyrex\\n"); process.exit(0); }
...
`,
  ...
  return { tmpDir, vyrexDir, sandboxName, fakeRoot };
}
...
function runRebuild(
  ...
  return spawnSync(
    process.execPath,
    [
      path.join(REPO_ROOT, "bin", "vyrex.js"),
      fixture.sandboxName,
      "rebuild",
      "--yes",
    ],
...
function registryHasSandbox(fixture: ReturnType<typeof createFixture>): boolean {
  const regPath = path.join(fixture.vyrexDir, "sandboxes.json");
  ...
  describe("Layer 2: preflight credential check", () => {
    ...
    it(
      ...
      () => {
        ...
        const sessionPath = path.join(f.vyrexDir, "onboard-session.json");
