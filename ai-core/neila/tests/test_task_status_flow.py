...
def test_schedule_task_creates_requested_status_file(tmp_path):
    from neila.tools.control import _schedule_task
    from neila.task_results import STATUS_REQUESTED
...
def test_get_task_result_returns_full_completed_output(tmp_path):
    ...
    from neila.tools.control import _get_task_result
...
def test_wait_for_task_reports_rejected_duplicate(tmp_path):
    from neila.task_results import STATUS_REJECTED_DUPLICATE, write_task_result
    from neila.tools.control import _wait_for_task
...
def test_handle_schedule_task_duplicate_writes_rejected_status(tmp_path, monkeypatch):
    ...
    from neila.task_results import STATUS_REJECTED_DUPLICATE
...
def test_handle_text_response_keeps_full_reasoning_note():
    from neila.loop import _handle_text_response
...
def test_request_restart_latches_reason_until_task_end(tmp_path, monkeypatch):
    from neila.tools import control as control_module
