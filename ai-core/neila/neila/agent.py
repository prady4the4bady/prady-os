"""
NEILA agent core — thin orchestrator.
...
from neila.agent_task_pipeline import (
    build_trace_summary, emit_task_results, build_review_context,
)
from neila.task_results import STATUS_RUNNING, write_task_result
...
class NEILAAgent:
    ...
    def handle_task(self, task: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            from neila.config import load_settings, apply_settings_to_env
        ...
        try:
            ...
            if task_type_str == "deep_self_review":
                ...
                try:
                    from neila.deep_self_review import run_deep_self_review, is_review_available
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    try:
                        from neila.task_continuation import capture_review_continuation_from_state
...
def make_agent(repo_dir: str, drive_root: str, event_queue: Any = None) -> NEILAAgent:
    env = Env(repo_dir=pathlib.Path(repo_dir), drive_root=pathlib.Path(drive_root))
    return NEILAAgent(env, event_queue=event_queue)
