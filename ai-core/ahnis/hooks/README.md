# AHNIS Hooks — Auto-Save for Terminal AI Tools

These hook scripts make AHNIS save automatically. No manual "save" commands needed.
...
- **`STATE_DIR`** — Where hook state is stored (defaults to `~/.AHNIS/hook_state/`)
...
- **`AHNIS_PYTHON`** — Optional env var. Python interpreter with AHNIS + chromadb installed. Auto-detects: `AHNIS_PYTHON` env var → repo `venv/bin/python3` → system `python3`. Set this if your venv is in a non-standard location.
...
```bash
AHNIS mine <dir>               # Mine all files in a directory
AHNIS mine <dir> --mode convos # Mine conversation transcripts only
```
...
**Hooks require session restart after install.** Claude Code loads hooks from `settings.json` at session start only. If you run `AHNIS init` or manually edit hook config mid-session, the hooks won't fire until you restart Claude Code. This is a Claude Code limitation.
...
export MEMPAL_PYTHON="$HOME/.venvs/AHNIS/bin/python"  # or your venv
```

Resolution priority: `$MEMPAL_PYTHON` (if set and executable) → `$(command -v python3)` → bare `python3`. The interpreter only needs `json` and `sys` from the standard library — `AHNIS` itself does not need to be installed in it.

Note: the `AHNIS mine` auto-ingest runs via the `AHNIS` CLI, so that command also needs to be on the hook's `PATH`. Installing with `pipx install AHNIS` or `uv tool install AHNIS` puts it on a stable global location; otherwise extend the hook environment's `PATH` to include your venv's `bin/`.
...
```bash
AHNIS mine ~/.codex/sessions/ --mode convos
```
