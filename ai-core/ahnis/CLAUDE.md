...
python -m pytest tests/ -v --ignore=tests/benchmarks --cov=AHNIS --cov-report=term-missing
...
- **Adding an MCP tool**: `AHNIS/mcp_server.py` — add handler function + TOOLS dict entry
- **Changing search**: `AHNIS/searcher.py`
- **Modifying mining**: `AHNIS/miner.py` (project files) or `AHNIS/convo_miner.py` (transcripts)
- **Adding a storage backend**: subclass `AHNIS/backends/base.py`, register in `backends/__init__.py`
- **Input validation**: `AHNIS/config.py` — `sanitize_name()` / `sanitize_content()`
