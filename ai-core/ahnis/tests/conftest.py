"""
conftest.py — Shared fixtures for AHNIS tests.
...
AHNIS imports — so that module-level initialisations (e.g.
...
# ── Isolate HOME before any AHNIS imports ──────────────────────────
_original_env = {}
_session_tmp = tempfile.mkdtemp(prefix="AHNIS_session_")
...
# Now it is safe to import AHNIS modules that trigger initialisation.
import chromadb  # noqa: E402
import pytest  # noqa: E402

from AHNIS.config import AHNISConfig  # noqa: E402
from AHNIS.knowledge_graph import KnowledgeGraph  # noqa: E402
...
def tmp_dir():
    ...
    d = tempfile.mkdtemp(prefix="AHNIS_test_")
    yield d
...
def config(tmp_dir, palace_path):
    """A AHNISConfig pointing at the temp palace."""
    ...
    return AHNISConfig(config_dir=cfg_dir)
...
def collection(palace_path):
    ...
    col = client.get_or_create_collection("AHNIS_drawers", metadata={"hnsw:space": "cosine"})
    yield col
    client.delete_collection("AHNIS_drawers")
    del client
