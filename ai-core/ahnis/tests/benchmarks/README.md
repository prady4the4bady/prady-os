# AHNIS Scale Benchmark Suite

106 tests that benchmark AHNIS at scale to validate real-world performance limits.

## Why

AHNIS has strong academic scores (96.6% R@5 on LongMemEval) but no empirical data on how it behaves at scale. Key unknowns:
...
- **`generate_project_tree()`** — writes real files + `AHNIS.yaml` for `mine()` to ingest
