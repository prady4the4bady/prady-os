...
class TestSaveHookAutoMines:
    ...
    def test_hook_mines_transcript_path(self):
        ...
        assert "AHNIS mine" in src, "hook must invoke `AHNIS mine`"
        assert (
    ...
    def test_mempal_dir_default_not_empty(self):
        ...
        if has_empty_default:
            ...
            has_alternative = (
                src.count("AHNIS mine") > 1
                or "TRANSCRIPT_PATH" in src.split("AHNIS mine")[0]
            )
...
class TestShellHookTranscriptValidation:
    ...
    Mirrors AHNIS.hooks_cli._validate_transcript_path so unsafe paths
