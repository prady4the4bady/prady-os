...
import pytest
from AHNIS.config import AHNISConfig, normalize_wing_name, sanitize_kg_value, sanitize_name


def test_default_config():
    cfg = AHNISConfig(config_dir=tempfile.mkdtemp())
    assert "palace" in cfg.palace_path
    assert cfg.collection_name == "AHNIS_drawers"
...
def test_config_from_file():
    ...
    cfg = AHNISConfig(config_dir=tmpdir)
    assert cfg.palace_path == "/custom/palace"


def test_embedding_device_defaults_to_auto(monkeypatch):
    monkeypatch.delenv("AHNIS_EMBEDDING_DEVICE", raising=False)
    cfg = AHNISConfig(config_dir=tempfile.mkdtemp())
...
def test_embedding_device_env_overrides_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AHNIS_EMBEDDING_DEVICE", "  CoreML  ")
...
def test_env_override():
    raw = "/env/palace"
    os.environ["AHNIS_PALACE_PATH"] = raw
    try:
        cfg = AHNISConfig(config_dir=tempfile.mkdtemp())
        ...
        del os.environ["AHNIS_PALACE_PATH"]
...
def test_env_path_expanduser():
    ...
    raw = os.path.join("~", "AHNIS-test")
    os.environ["AHNIS_PALACE_PATH"] = raw
    try:
        cfg = AHNISConfig(config_dir=tempfile.mkdtemp())
        assert cfg.palace_path == os.path.abspath(os.path.expanduser(raw))
        assert cfg.palace_path.endswith("AHNIS-test")
    finally:
        del os.environ["AHNIS_PALACE_PATH"]
...
def test_env_path_abspath_collapses_traversal():
    ...
    raw = os.path.join(tempfile.gettempdir(), "palace", "..", "AHNIS-test")
...
def test_env_path_legacy_alias_normalized():
    ...
    # AHNIS_PALACE_PATH. We don't assert "~" is absent from the final
    ...
    os.environ.pop("AHNIS_PALACE_PATH", None)
    raw = os.path.join("~", "legacy-alias", "..", "AHNIS-test")
