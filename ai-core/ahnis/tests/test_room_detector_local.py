"""Tests for AHNIS.room_detector_local."""

from unittest.mock import MagicMock, patch

from AHNIS.room_detector_local import (
    FOLDER_ROOM_MAP,
...
def test_save_config_creates_yaml(tmp_path):
    ...
    config_file = tmp_path / "AHNIS.yaml"
...
def test_detect_rooms_local_yes_mode(tmp_path):
    ...
    assert (tmp_path / "AHNIS.yaml").exists()
...
def test_detect_rooms_local_interactive(tmp_path):
    ...
    with (
        patch.dict("sys.modules", {"AHNIS.miner": mock_miner}),
        patch(
            "AHNIS.room_detector_local.get_user_approval",
