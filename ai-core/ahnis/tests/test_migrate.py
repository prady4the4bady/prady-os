"""Tests for destructive-operation safety in AHNIS.migrate."""

import os
from types import SimpleNamespace
from unittest.mock import MagicMock, patch

from AHNIS.migrate import _restore_stale_palace, migrate
...
def test_migrate_aborts_without_confirmation(tmp_path, capsys):
    ...
    with (
        ...
        patch("AHNIS.migrate.detect_chromadb_version", return_value="0.5.x"),
        patch(
            "AHNIS.migrate.extract_drawers_from_sqlite",
            return_value=[{"id": "id1", "document": "doc", "metadata": {"wing": "w", "room": "r"}}],
        ),
        patch("builtins.input", return_value="n"),
        patch("AHNIS.migrate.shutil.copytree") as mock_copytree,
        patch("AHNIS.migrate.shutil.rmtree") as mock_rmtree,
    ):
        result = migrate(str(palace_dir))
...
def test_restore_stale_palace_logs_and_swallows_on_failure(tmp_path, capsys):
    ...
    with patch("AHNIS.migrate.os.replace", side_effect=OSError("boom")):
        _restore_stale_palace(str(palace_path), str(stale_path))
