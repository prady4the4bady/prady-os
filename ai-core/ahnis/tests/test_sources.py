...
from AHNIS.sources.transforms import (
...
def test_resolve_adapter_priority_order():
    ...
    # Default is filesystem (preserves existing ``AHNIS mine <path>`` behavior).
...
def test_palace_context_drawer_id_is_sha256_prefix_not_sha1():
    ...
    from AHNIS.sources.context import _build_drawer_id
...
def test_entry_point_group_exists_and_returns_zero_or_more_adapters():
    ...
    # but the ``AHNIS.sources`` entry-point group is declared so third-
