"""Tests for explicit tunnel helpers in AHNIS.palace_graph."""
...
with patch.dict("sys.modules", {"chromadb": MagicMock()}):
    import AHNIS.palace_graph as palace_graph
...
class TestHyphenatedWingNormalization:
    ...
    ``AHNIS-public`` resolves to ``AHNIS_public`` and matches the
    ...
    def test_list_tunnels_filters_hyphenated_wing(self, tmp_path, monkeypatch):
        ...
        palace_graph.create_tunnel("AHNIS_public", "auth", "wing_people", "users")

        assert len(palace_graph.list_tunnels("AHNIS-public")) == 1
        assert len(palace_graph.list_tunnels("AHNIS_public")) == 1
    ...
    def test_follow_tunnels_matches_hyphenated_wing(self, tmp_path, monkeypatch):
        ...
        by_hyphen = palace_graph.follow_tunnels("AHNIS-public", "auth")
        by_under = palace_graph.follow_tunnels("AHNIS_public", "auth")
    ...
    def test_find_tunnels_warns_on_empty_result(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level("WARNING", logger="AHNIS_graph"):
