...
def _tools_dict_keys() -> list:
    ...
    src = _read(AHNIS_PKG / "mcp_server.py")
    return re.findall(r'"(AHNIS_\w+)":\s*\{', src)
...
def _doc_tool_names() -> list:
    ...
    is introduced by a level-3 heading `### \\`AHNIS_xxx\\``.
    """
    doc = _read(MCP_TOOLS_DOC_PATH)
    return re.findall(r"^###\s+`(AHNIS_\w+)`", doc, re.MULTILINE)
...
class TestReadmeToolsExistInCode:
    ...
    def test_every_readme_tool_exists_in_tools_dict(self):
        ...
        lists tools like AHNIS_get_aaak_spec. Each one must actually be
        registered in the TOOLS dict in AHNIS/mcp_server.py.
...
class TestClosetsExist:
    ...
    def test_closets_importable(self):
        """get_closets_collection should be importable from AHNIS.palace."""
        from AHNIS.palace import get_closets_collection
...
class TestClosetFirstSearch:
    ...
    def test_closet_boost_search_exists(self):
        ...
        src = _read(AHNIS_PKG / "searcher.py")
...
class TestEntityMetadataExtraction:
    ...
    def test_entity_extraction_in_palace_or_miner(self):
        ...
        miner_src = _read(AHNIS_PKG / "miner.py")
        palace_src = _read(AHNIS_PKG / "palace.py")
...
class TestStripNoise:
    ...
    def test_strip_noise_exists(self):
        ...
        src = _read(AHNIS_PKG / "normalize.py")
    ...
    def test_strip_noise_importable(self):
        """strip_noise should be importable from AHNIS.normalize."""
        from AHNIS.normalize import strip_noise
...
class TestDiaryIngest:
    ...
    def test_diary_ingest_module_exists(self):
        ...
        File must exist at AHNIS/diary_ingest.py."""
        path = AHNIS_PKG / "diary_ingest.py"
        assert path.is_file(), (
            "AHNIS/diary_ingest.py does not exist. "
    ...
    def test_diary_ingest_importable(self):
        ...
        try:
            importlib.import_module("AHNIS.diary_ingest")
        except ImportError:
            pytest.fail(
                "AHNIS.diary_ingest is not importable. Module must exist and import cleanly."
            )
...
class TestFactChecker:
    ...
    def test_fact_checker_module_exists(self):
        ...
        fact_checker.py must exist at AHNIS/fact_checker.py."""
        path = AHNIS_PKG / "fact_checker.py"
        assert path.is_file(), (
            "AHNIS/fact_checker.py does not exist. "
    ...
    def test_fact_checker_importable(self):
        ...
        try:
            importlib.import_module("AHNIS.fact_checker")
        except ImportError:
            pytest.fail(
                "AHNIS.fact_checker is not importable. Module must exist and import cleanly."
            )
...
class TestTunnelFunctions:
    ...
    def test_find_tunnels_exists(self):
        ...
        src = _read(AHNIS_PKG / "palace_graph.py")
    ...
    def test_tunnel_functions_importable(self):
        ...
        from AHNIS.palace_graph import find_tunnels, traverse, graph_stats
...
class TestClosetLLM:
    ...
    def test_closet_llm_module_exists(self):
        ...
        closet_llm.py must exist at AHNIS/closet_llm.py."""
        path = AHNIS_PKG / "closet_llm.py"
        assert path.is_file(), (
            "AHNIS/closet_llm.py does not exist. "
    ...
    def test_closet_llm_importable(self):
        ...
        try:
            importlib.import_module("AHNIS.closet_llm")
        except ImportError:
            pytest.fail(
                "AHNIS.closet_llm is not importable. Module must exist and import cleanly."
            )
...
class TestMineLock:
    ...
    def test_mine_lock_importable(self):
        """mine_lock should be importable from AHNIS.palace."""
        from AHNIS.palace import mine_lock
...
class TestVersionConsistency:
    ...
    def test_version_py_matches_pyproject(self):
        ...
        version_src = _read(AHNIS_PKG / "version.py")
...
class TestDialectNotLossless:
    ...
    def test_dialect_docstring_says_not_lossless(self):
        ...
        src = _read(AHNIS_PKG / "dialect.py")
...
class TestHallMetadata:
    ...
    def test_miners_write_hall_metadata(self):
        ...
        miner_src = _read(AHNIS_PKG / "miner.py")
        convo_miner_src = _read(AHNIS_PKG / "convo_miner.py")
...
class TestBackendAbstraction:
    ...
    def test_backends_base_exists(self):
        ...
        path = AHNIS_PKG / "backends" / "base.py"
        assert (
            path.is_file()
        ), "AHNIS/backends/base.py does not exist. Backend abstraction layer is missing."
        src = _read(path)
        assert (
    ...
    def test_backends_chroma_exists(self):
        ...
        path = AHNIS_PKG / "backends" / "chroma.py"
        assert path.is_file(), "AHNIS/backends/chroma.py does not exist."
        src = _read(path)
        assert (
    ...
    def test_backends_importable(self):
        ...
        from AHNIS.backends.base import BaseCollection
        from AHNIS.backends.chroma import ChromaBackend
...
class TestI18n:
    ...
    def test_i18n_directory_exists(self):
        ...
        path = AHNIS_PKG / "i18n"
        assert path.is_dir(), "AHNIS/i18n/ directory does not exist."
    ...
    def test_english_baseline_exists(self):
        ...
        path = AHNIS_PKG / "i18n" / "en.json"
        assert (
            path.is_file()
        ), "AHNIS/i18n/en.json does not exist. English baseline is required."
...
class TestWakeUpTokenCost:
    ...
    def test_readme_wakeup_cost_matches_layers(self):
        ...
        readme = _readme()
        layers_src = _read(AHNIS_PKG / "layers.py")
