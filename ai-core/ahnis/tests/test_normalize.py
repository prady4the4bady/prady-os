...
def test_format_tool_use_unknown_tool():
    block = {
        "type": "tool_use",
        "id": "t1",
        "name": "mcp__AHNIS__search",
        "input": {"query": "firmware probe", "limit": 5},
    }
    result = _format_tool_use(block)
    assert result.startswith("[mcp__AHNIS__search]")
...
def test_format_tool_result_unknown_short():
    ...
    result = _format_tool_result("some output", "mcp__AHNIS__search")
...
def test_messages_to_transcript_basic():
    ...
    with patch("AHNIS.normalize.spellcheck_user_text", side_effect=lambda x: x, create=True):
...
def test_normalize_rejects_large_file():
    ...
    with patch("AHNIS.normalize.os.path.getsize", return_value=600 * 1024 * 1024):
        try:
