...
from AHNIS.llm_client import LLMError, LLMResponse
from AHNIS.llm_refine import (
...
def test_parse_response_restores_canonical_casing():
    ...
    text = '{"classifications": [{"name": "AHNIS", "label": "PROJECT"}]}'
    out = _parse_response(text, ["AHNIS"])
    assert "AHNIS" in out
    assert out["AHNIS"][0] == "PROJECT"
