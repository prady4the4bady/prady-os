"""Tests for AHNIS.repair — scan, prune, and rebuild HNSW index."""
...
from AHNIS import repair


# ── _get_palace_path ──────────────────────────────────────────────────


@patch("AHNIS.repair.AHNISConfig", create=True)
...
def test_get_palace_path_fallback():
    with patch("AHNIS.repair._get_palace_path") as mock_get:
        mock_get.return_value = os.path.join(os.path.expanduser("~"), ".AHNIS", "palace")
        result = mock_get()
        assert ".AHNIS" in result
...
@patch("AHNIS.repair.ChromaBackend")
...
def test_rebuild_index_success(mock_backend_cls, mock_shutil, tmp_path):
    ...
    mock_backend.delete_collection.assert_called_once_with(str(tmp_path), "AHNIS_drawers")
    mock_backend.create_collection.assert_called_once_with(str(tmp_path), "AHNIS_drawers")
...
def test_check_extraction_safety_passes_when_counts_match(tmp_path):
    ...
    with patch("AHNIS.repair.sqlite_drawer_count", return_value=500):
...
def test_check_extraction_safety_passes_when_sqlite_unreadable_and_under_cap(tmp_path):
    ...
    with patch("AHNIS.repair.sqlite_drawer_count", return_value=None):
...
def test_check_extraction_safety_aborts_when_sqlite_higher(tmp_path):
    ...
    with patch("AHNIS.repair.sqlite_drawer_count", return_value=67_580):
        try:
...
def test_check_extraction_safety_override_skips_check(tmp_path):
    ...
    with patch("AHNIS.repair.sqlite_drawer_count", return_value=99_999):
