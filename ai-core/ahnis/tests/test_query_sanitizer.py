...
from AHNIS.query_sanitizer import (
    MAX_QUERY_LENGTH,
    MIN_QUERY_LENGTH,
    SAFE_QUERY_LENGTH,
    sanitize_query,
...
class TestQuestionExtraction:
    ...
    def test_question_in_system_prompt_ignored_when_real_question_exists(self):
        ...
        real_query = "What databases does AHNIS support?"
        query = system_prompt + real_query
        result = sanitize_query(query)
        assert result["was_sanitized"] is True
        assert "AHNIS" in result["clean_query"] or "database" in result["clean_query"].lower()
...
class TestTailSentence:
    ...
    def test_keyword_style_query(self):
        ...
        query = system_prompt + "\nAHNIS ChromaDB integration setup"
        result = sanitize_query(query)
        assert result["was_sanitized"] is True
        assert "AHNIS" in result["clean_query"] or "ChromaDB" in result["clean_query"]
...
class TestRealWorldScenarios:
    ...
    def test_AHNIS_wakeup_prepended(self):
        """Simulates AHNIS wake-up output prepended to a query."""
        wakeup = (
            "AHNIS loaded. Wings: technical, emotions, identity. "
    ...
    def test_2000_char_system_prompt_with_question(self):
        ...
        real_query = "What is the status of the AHNIS project?"
        query = system_prompt + real_query
        result = sanitize_query(query)
        assert result["was_sanitized"] is True
