"""Tests for AHNIS.layers — Layer0, Layer1, Layer2, Layer3, MemoryStack."""

import os
from unittest.mock import MagicMock, patch

from AHNIS.layers import Layer0, Layer1, Layer2, Layer3, MemoryStack
...
def test_layer0_default_path():
    layer = Layer0()
    expected = os.path.expanduser("~/.AHNIS/identity.txt")
...
def test_layer1_no_palace():
    ...
    with patch("AHNIS.layers.AHNISConfig") as mock_cfg:
...
def test_layer1_generates_essential_story():
    ...
    with (
        patch("AHNIS.layers.AHNISConfig") as mock_cfg,
        patch("AHNIS.layers._get_collection", return_value=mock_col),
    ):
