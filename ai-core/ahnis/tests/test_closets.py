...
from AHNIS.palace_graph import (
    create_tunnel,
    delete_tunnel,
    follow_tunnels,
    list_tunnels,
)
from AHNIS.searcher import (
    _bm25_scores,
...
def _lock_worker(target: str, name: str, hold_seconds: float, log_path: str) -> None:
    ...
    from AHNIS.palace import mine_lock as _mine_lock
...
class TestMineLock:
    def test_lock_acquires_and_releases(self, tmp_path):
        ...
        with mine_lock(target):
            lock_dir = os.path.expanduser("~/.AHNIS/locks")
...
class TestDiaryIngest:
    def test_ingest_creates_drawers_and_closets(self, tmp_path):
        ...
        from AHNIS.diary_ingest import ingest_diaries
    ...
    def test_state_file_lives_outside_diary_dir(self, tmp_path):
        ...
        from AHNIS.diary_ingest import _state_file_for, ingest_diaries
        ...
        # State file does exist under ~/.AHNIS/state/.
        ...
        # separator string that would fail on Windows (``\.AHNIS\state\``).
        assert state_path.parent.name == "state"
        assert state_path.parent.parent.name == ".AHNIS"
    ...
    def test_wing_prefixed_drawer_id_prevents_cross_diary_collision(self, tmp_path):
        ...
        from AHNIS.diary_ingest import _diary_drawer_id, ingest_diaries
