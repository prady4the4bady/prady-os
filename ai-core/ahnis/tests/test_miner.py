...
from AHNIS.miner import load_config, mine, scan_project, status
from AHNIS.palace import NORMALIZE_VERSION, file_already_mined
...
def test_project_mining():
    ...
    try:
        ...
        with open(project_root / "AHNIS.yaml", "w") as f:
            yaml.dump(
                {
                    "wing": "test_project",
                    "rooms": [
        ...
        col = client.get_collection("AHNIS_drawers")
        assert col.count() > 0
    finally:
...
def test_scan_project_skips_AHNIS_generated_files():
    with tempfile.TemporaryDirectory() as tmpdir:
        project_root = Path(tmpdir).resolve()
        write_file(project_root / "entities.json", '{"people": [], "projects": []}')
        write_file(project_root / "AHNIS.yaml", "wing: test\nrooms: []\n")
...
def test_entity_metadata_finds_cyrillic_names(monkeypatch):
    ...
    import AHNIS.palace as palace_mod
    from AHNIS.miner import _extract_entities_for_metadata
    ...
    monkeypatch.setattr(
        "AHNIS.config.AHNISConfig.entity_languages",
...
def test_file_already_mined_check_mtime():
    ...
    try:
        ...
        col = client.get_or_create_collection(
            "AHNIS_drawers", metadata={"hnsw:space": "cosine"}
        )
...
def test_status_handles_none_metadata_without_crash(tmp_path, capsys):
    ...
    with patch("AHNIS.miner.get_collection", return_value=FakeCol()):
        status(str(tmp_path))
...
def test_file_already_mined_returns_false_for_stale_normalize_version():
    ...
    try:
        ...
        os.makedirs(palace_path)
        client = chromadb.PersistentClient(path=palace_path)
        col = client.get_or_create_collection("AHNIS_drawers")
...
def test_add_drawer_stamps_normalize_version(tmp_path):
    ...
    from AHNIS.miner import add_drawer
...
def test_mine_creates_topic_tunnels_for_shared_topics(tmp_path, monkeypatch):
    ...
    Issue #1180.
    """
    from AHNIS import miner, palace_graph

    # Redirect both the registry and tunnel-storage paths into tmp_path
    # so we never touch the developer's real ~/.AHNIS directory.
...
def test_mine_no_tunnel_when_threshold_blocks_overlap(tmp_path, monkeypatch):
    """Bumping ``AHNIS_TOPIC_TUNNEL_MIN_COUNT`` above the actual overlap
    ...
    tunnels_file = tmp_path / "tunnels.json"
    monkeypatch.setattr(palace_graph, "_TUNNEL_FILE", str(tunnels_file))
    monkeypatch.setenv("AHNIS_TOPIC_TUNNEL_MIN_COUNT", "2")
...
def test_mine_keyboard_interrupt_prints_summary_and_exits_130(tmp_path, capsys):
    ...
    with patch("AHNIS.miner.process_file", side_effect=fake_process_file):
...
def test_mine_keyboard_interrupt_quotes_path_with_spaces_in_resume_hint(tmp_path, capsys):
    ...
    spaces / metacharacters yields a copy-paste-safe `AHNIS mine ...`
    ...
    assert f"AHNIS mine {shlex.quote(str(project_root))}" in out
...
def test_mine_cleans_up_pid_file_on_interrupt(tmp_path):
    ...
    with (
        patch("AHNIS.hooks_cli._MINE_PID_FILE", pid_file),
        patch("AHNIS.miner.process_file", side_effect=fake_process_file),
    ):
        with pytest.raises(SystemExit):
            mine(str(project_root), str(palace_path))
