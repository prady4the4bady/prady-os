...
the Python where ``AHNIS`` is installed, so the shell path needs to:
  ...
  3. *never* crash the hook when the resolved interpreter can't import
     AHNIS — log and skip the auto-ingest instead, so Claude Code
...
# and simulate a missing AHNIS install when configured that way.
MARKER_FILE="{marker}"
if [ -n "$MARKER_FILE" ]; then
    echo "{path.name}" >> "$MARKER_FILE"
fi
CAN_IMPORT={"1" if can_import_AHNIS else "0"}
# Simulate the "AHNIS is not installed in this interpreter" case.
if [ "$CAN_IMPORT" = "0" ]; then
    if [ "$1" = "-c" ] && echo "$2" | grep -q "import AHNIS"; then
        exit 1
    fi
    if [ "$1" = "-m" ] && [ "$2" = "AHNIS" ]; then
        exit 1
