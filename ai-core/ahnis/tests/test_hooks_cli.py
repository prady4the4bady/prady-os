...
def test_AHNIS_python_returns_string():
    result = _AHNIS_python()
    assert isinstance(result, str)
    assert "python" in result


def test_AHNIS_python_finds_venv():
...
def _capture_hook_output(hook_fn, data, harness="claude-code", state_dir=None):
    ...
    buf = io.StringIO()
    patches = [patch("AHNIS.hooks_cli._output", side_effect=lambda d: buf.write(json.dumps(d)))]
    if state_dir:
        patches.append(patch("AHNIS.hooks_cli.STATE_DIR", state_dir))
    # Mock AHNISConfig so tests don't depend on user's ~/.AHNIS/config.json
    ...
    patches.append(patch("AHNIS.config.AHNISConfig", return_value=mock_config))
...
def test_stop_hook_passthrough_when_active(tmp_path):
    with patch("AHNIS.hooks_cli.STATE_DIR", tmp_path):
...
def test_stop_hook_saves_silently_at_interval(tmp_path):
    ...
    with patch("AHNIS.hooks_cli._save_diary_direct", return_value=save_result) as mock_save:
        result = _capture_hook_output(
            hook_stop,
            {"session_id": "test", "stop_hook_active": False, "transcript_path": str(transcript)},
            state_dir=tmp_path,
...
def test_stop_hook_tracks_save_point(tmp_path):
    ...
    with patch("AHNIS.hooks_cli._save_diary_direct", return_value=save_result):
    ...
    with patch("AHNIS.hooks_cli._save_diary_direct") as mock_save:
        result = _capture_hook_output(hook_stop, data, state_dir=tmp_path)
    assert result == {}
...
def test_wing_from_transcript_path_non_projects_layout():
    ...
    path = "/home/igor/.claude/projects/-home-igor-dev-AHNIS-AHNIS/session.jsonl"
    assert _wing_from_transcript_path(path) == "wing_AHNIS"
...
def test_output_writes_to_real_stdout_fd_when_mcp_server_loaded():
    ...
    fake_module = types.ModuleType("AHNIS.mcp_server")

    read_fd, write_fd = os.pipe()
    try:
        fake_module._REAL_STDOUT_FD = write_fd
        with patch.dict("sys.modules", {"AHNIS.mcp_server": fake_module}):
            from AHNIS.hooks_cli import _output
...
def test_log_oserror_is_silenced(tmp_path):
    ...
    with patch("AHNIS.hooks_cli.STATE_DIR", Path("/nonexistent/deeply/nested/dir")):
        # Should not raise
...
def test_maybe_auto_ingest_with_env(tmp_path):
    ...
    with patch.dict("os.environ", {"MEMPAL_DIR": str(mempal_dir)}):
        with patch("AHNIS.hooks_cli.STATE_DIR", tmp_path):
            with patch("AHNIS.hooks_cli._MINE_PID_FILE", tmp_path / "mine.pid"):
                with patch("AHNIS.hooks_cli.subprocess.Popen") as mock_popen:
                    _maybe_auto_ingest()
...
def test_maybe_auto_ingest_uses_AHNIS_python(tmp_path):
    """Spawned mine command uses _AHNIS_python(), not bare sys.executable.
    ...
    under (typically the venv that owns AHNIS).
    """
    mempal_dir = tmp_path / "project"
    mempal_dir.mkdir()
    with patch.dict("os.environ", {"MEMPAL_DIR": str(mempal_dir)}):
        with patch("AHNIS.hooks_cli.STATE_DIR", tmp_path):
            with patch("AHNIS.hooks_cli._MINE_PID_FILE", tmp_path / "mine.pid"):
                with patch(
                    "AHNIS.hooks_cli._AHNIS_python", return_value="/fake/venv/python"
                ):
                    with patch("AHNIS.hooks_cli.subprocess.Popen") as mock_popen:
                        _maybe_auto_ingest()
                        cmd = mock_popen.call_args[0][0]
...
def test_mine_sync_with_env_uses_projects_mode(tmp_path):
    ...
    with patch.dict("os.environ", {"MEMPAL_DIR": str(mempal_dir)}):
        with patch("AHNIS.hooks_cli.STATE_DIR", tmp_path):
            with patch("AHNIS.hooks_cli.subprocess.run") as mock_run:
                _mine_sync()
...
def test_mine_sync_uses_AHNIS_python(tmp_path):
    """Sync mine command uses _AHNIS_python(), not bare sys.executable."""
    mempal_dir = tmp_path / "project"
    mempal_dir.mkdir()
    with patch.dict("os.environ", {"MEMPAL_DIR": str(mempal_dir)}):
        with patch("AHNIS.hooks_cli.STATE_DIR", tmp_path):
            with patch("AHNIS.hooks_cli._AHNIS_python", return_value="/fake/venv/python"):
                with patch("AHNIS.hooks_cli.subprocess.run") as mock_run:
                    _mine_sync()
                    cmd = mock_run.call_args[0][0]
                    assert cmd[0] == "/fake/venv/python"
...
def test_maybe_auto_ingest_oserror(tmp_path):
    ...
    with patch.dict("os.environ", {"MEMPAL_DIR": str(mempal_dir)}):
        with patch("AHNIS.hooks_cli.STATE_DIR", tmp_path):
            with patch("AHNIS.hooks_cli._MINE_PID_FILE", tmp_path / "mine.pid"):
                with patch("AHNIS.hooks_cli.subprocess.Popen", side_effect=OSError("fail")):
                    _maybe_auto_ingest()  # should not raise
...
def test_maybe_auto_ingest_skips_when_mine_running(tmp_path):
    ...
    with patch.dict("os.environ", {"MEMPAL_DIR": str(mempal_dir)}):
        with patch("AHNIS.hooks_cli.STATE_DIR", tmp_path):
            with patch("AHNIS.hooks_cli._mine_already_running", return_value=True):
                with patch("AHNIS.hooks_cli.subprocess.Popen") as mock_popen:
                    _maybe_auto_ingest()
                    mock_popen.assert_not_called()
...
def test_mine_already_running_dead_pid(tmp_path):
    ...
    with patch("AHNIS.hooks_cli._MINE_PID_FILE", pid_file):
        assert _mine_already_running() is False
...
def test_precompact_with_mempal_dir_oserror(tmp_path):
    ...
    mempal_dir.mkdir()
    with patch.dict("os.environ", {"MEMPAL_DIR": str(mempal_dir)}):
        with patch("AHNIS.hooks_cli.subprocess.run", side_effect=OSError("fail")):
            result = _capture_hook_output(
                hook_precompact,
                {"session_id": "test"},
                state_dir=tmp_path,
...
def test_precompact_with_timeout(tmp_path):
    ...
    with patch.dict("os.environ", {"MEMPAL_DIR": str(mempal_dir)}):
        with patch(
            "AHNIS.hooks_cli.subprocess.run",
...
def test_run_hook_dispatches_session_start(tmp_path):
    ...
    with patch("sys.stdin", io.StringIO(stdin_data)):
        with patch("AHNIS.hooks_cli.STATE_DIR", tmp_path):
            with patch("AHNIS.hooks_cli._output") as mock_output:
...
def test_count_logs_warning_on_rejected_path(tmp_path):
    ...
    with patch("AHNIS.hooks_cli.STATE_DIR", tmp_path):
        with patch("AHNIS.hooks_cli._log") as mock_log:
...
def test_stop_hook_rejects_injected_stop_hook_active(tmp_path):
    ...
    with patch(
        "AHNIS.hooks_cli._save_diary_direct", return_value={"count": 1, "themes": []}
    ) as mock_save:
        _capture_hook_output(
            hook_stop,
            {
