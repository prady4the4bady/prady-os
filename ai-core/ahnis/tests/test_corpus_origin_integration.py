...
def test_baseline_v333_misclassifies_persona_names_as_people(ai_dialogue_corpus: Path):
    ...
    asserts the fix.
    """
    from AHNIS.entity_detector import detect_entities, scan_for_detection
...
def test_discover_entities_threads_corpus_origin_through(
    ...
    """
    from AHNIS.project_scanner import discover_entities
...
def _stub_cfg(palace_dir: Path):
    """Build a AHNISConfig stub whose palace_path points at tmp space.

    Used by Pass 0 tests so the origin.json write is captured in tmp_path
    instead of hitting the real ~/.AHNIS location.
    """
    cfg = MagicMock()
...
def test_init_pass_zero_writes_origin_json_to_palace(ai_dialogue_corpus: Path, tmp_path: Path):
    ...
    and persist the result to ``<palace>/.AHNIS/origin.json`` in the
    documented schema_version=1 wrapper.
    """
    from AHNIS.cli import cmd_init
    ...
    with (
        patch("AHNIS.cli.AHNISConfig", return_value=_stub_cfg(palace)),
        patch("AHNIS.cli._maybe_run_mine_after_init"),
        patch("AHNIS.room_detector_local.detect_rooms_local"),
    ):
        cmd_init(args)

    origin_path = palace / ".AHNIS" / "origin.json"
...
def test_init_pass_zero_passes_corpus_origin_to_discover_entities(
    ...
    with (
        patch("AHNIS.cli.AHNISConfig", return_value=_stub_cfg(palace)),
        patch("AHNIS.project_scanner.discover_entities", side_effect=fake_discover),
        patch("AHNIS.cli._maybe_run_mine_after_init"),
        patch("AHNIS.room_detector_local.detect_rooms_local"),
    ):
        cmd_init(args)
...
def test_llm_refine_includes_corpus_origin_context_in_prompt(
    ...
    from AHNIS.llm_refine import refine_entities
...
def test_llm_refine_no_origin_keeps_v333_prompt_shape(monkeypatch):
    ...
    from AHNIS.llm_refine import SYSTEM_PROMPT, refine_entities
...
# ── AHNIS mine --redetect-origin flag ───────────────────────────────
...
def test_mine_default_does_not_redetect_origin(ai_dialogue_corpus: Path, tmp_path: Path):
    """Default `AHNIS mine` (no --redetect-origin flag) must NOT run
    corpus_origin detection — the flag is opt-in.
    """
    from AHNIS.cli import cmd_mine
    ...
    with (
        patch("AHNIS.cli.AHNISConfig", return_value=_stub_cfg(palace)),
        patch("AHNIS.cli._run_pass_zero") as mock_pass_zero,
        patch("AHNIS.miner.mine"),
    ):
        cmd_mine(args)

    mock_pass_zero.assert_not_called()
    assert not (palace / ".AHNIS" / "origin.json").exists()


def test_mine_with_redetect_origin_flag_writes_origin_json(
    ai_dialogue_corpus: Path, tmp_path: Path
):
    """`AHNIS mine --redetect-origin` re-runs corpus_origin detection
    on the project and persists the result to <palace>/.AHNIS/origin.json.
    """
    from AHNIS.cli import cmd_mine
    ...
    origin_path = palace / ".AHNIS" / "origin.json"
    assert origin_path.exists(), "--redetect-origin must write <palace>/.AHNIS/origin.json"
    data = json.loads(origin_path.read_text())
...
def test_mine_redetect_uses_full_content_not_sampled(tmp_path: Path):
    ...
    data = json.loads((palace / ".AHNIS" / "origin.json").read_text())
    assert data["result"]["likely_ai_dialogue"] is True, (
...
def test_init_default_attempts_llm_provider(ai_dialogue_corpus: Path, tmp_path: Path):
    """``AHNIS init`` (no flags) MUST try to acquire an LLM
    ...
    with (
        patch("AHNIS.cli.AHNISConfig", return_value=_stub_cfg(palace)),
        patch("AHNIS.cli.get_provider", return_value=fake_provider) as mock_get,
        patch("AHNIS.cli._maybe_run_mine_after_init"),
        patch("AHNIS.room_detector_local.detect_rooms_local"),
    ):
        cmd_init(args)
    ...
    (
        ...
        (
            "Default `AHNIS init` did not attempt LLM provider acquisition. "
...
def test_init_no_llm_skips_provider_acquisition(ai_dialogue_corpus: Path, tmp_path: Path):
    """``AHNIS init --no-llm`` is the explicit opt-out path. No
    ...
    with (
        patch("AHNIS.cli.AHNISConfig", return_value=_stub_cfg(palace)),
        patch("AHNIS.cli.get_provider") as mock_get,
        patch("AHNIS.cli._maybe_run_mine_after_init"),
        patch("AHNIS.room_detector_local.detect_rooms_local"),
    ):
        cmd_init(args)
...
def test_init_graceful_fallback_when_provider_unavailable(
    ...
    with (
        patch("AHNIS.cli.AHNISConfig", return_value=_stub_cfg(palace)),
        patch("AHNIS.cli.get_provider", return_value=fake_provider),
        patch("AHNIS.cli._maybe_run_mine_after_init"),
        patch("AHNIS.room_detector_local.detect_rooms_local"),
    ):
...
def test_init_graceful_fallback_on_provider_construction_error(
    ...
    """
    from AHNIS.cli import cmd_init
    from AHNIS.llm_client import LLMError
    ...
    with (
        patch("AHNIS.cli.AHNISConfig", return_value=_stub_cfg(palace)),
        patch("AHNIS.cli.get_provider", side_effect=LLMError("no api key")),
        patch("AHNIS.cli._maybe_run_mine_after_init"),
        patch("AHNIS.room_detector_local.detect_rooms_local"),
    ):
        cmd_init(args)  # MUST NOT raise
...
def test_init_legacy_llm_flag_compatible(ai_dialogue_corpus: Path, tmp_path: Path):
    """Backwards compatibility: `AHNIS init --llm` still works as
...
def test_end_to_end_init_with_llm_separates_personas(ai_dialogue_corpus: Path, tmp_path: Path):
    """End-to-end through `AHNIS init` on the DEFAULT path (LLM enabled).
    ...
    """
    from AHNIS.cli import cmd_init
    from AHNIS.corpus_origin import CorpusOriginResult
    ...
    origin_data = json.loads((palace / ".AHNIS" / "origin.json").read_text())
...
def test_no_llm_path_matches_v333_classification(ai_dialogue_corpus: Path, tmp_path: Path):
    ...
    origin = json.loads((palace / ".AHNIS" / "origin.json").read_text())
...
def test_re_init_idempotent(ai_dialogue_corpus: Path, tmp_path: Path):
    """Running `AHNIS init` twice on the same project produces the
    ...
    with (
        ...
        cmd_init(args)
        first = json.loads((palace / ".AHNIS" / "origin.json").read_text())
        cmd_init(args)
        second = json.loads((palace / ".AHNIS" / "origin.json").read_text())
...
def test_persona_user_name_collision_user_kept_in_people(
    ...
    """
    from AHNIS.entity_detector import detect_entities
    ...
    from AHNIS.entity_detector import scan_for_detection
...
def test_integration_cmd_init_runs_pass_zero_to_pass_four_in_order(
    ...
    real_run_pass_zero = __import__("AHNIS.cli", fromlist=["_run_pass_zero"])._run_pass_zero
    ...
    with (
        patch("AHNIS.cli.AHNISConfig", return_value=_stub_cfg(palace)),
        patch("AHNIS.cli._run_pass_zero", side_effect=trace_pass_zero),
        patch("AHNIS.project_scanner.discover_entities", side_effect=trace_discover),
        patch("AHNIS.room_detector_local.detect_rooms_local", side_effect=trace_rooms),
        patch("AHNIS.cli._ensure_AHNIS_files_gitignored", side_effect=trace_gitignore),
        patch("AHNIS.cli._maybe_run_mine_after_init", side_effect=trace_mine_prompt),
    ):
        cmd_init(args)
...
def test_integration_entities_json_includes_topics_excludes_personas(
    ...
    with (
        patch("AHNIS.cli.AHNISConfig", return_value=_stub_cfg(palace)),
        patch("AHNIS.cli.get_provider", return_value=fake_provider),
        patch("AHNIS.cli.detect_origin_llm", return_value=fake_origin),
        patch("AHNIS.cli._maybe_run_mine_after_init"),
        patch("AHNIS.room_detector_local.detect_rooms_local"),
    ):
        cmd_init(args)
...
def test_integration_add_to_known_entities_called_with_wing(
    ...
    with (
        ...
        patch("AHNIS.cli._maybe_run_mine_after_init"),
        patch("AHNIS.room_detector_local.detect_rooms_local"),
        patch("AHNIS.miner.add_to_known_entities") as mock_add,
    ):
        cmd_init(args)
...
def test_integration_llm_refine_corpus_origin_preamble_does_not_break_topic_label(
    ...
    from AHNIS.llm_refine import VALID_LABELS, refine_entities
...
# instead.
#
# This test asserts nothing in `AHNIS/` or `tests/` contains those
# markers. If a future commit re-introduces "Phase 1" or "Igor's review §"
# anywhere, this test goes RED and blocks the merge.
#
# Pre-existing exception: the `AHNIS/sources/` and `AHNIS/backends/`
...
def test_no_internal_coordination_jargon_in_source_or_tests():
    ...
    leaks: list = []
    for pattern_dir in ("AHNIS", "tests"):
...
def test_merge_tier_fields_heuristic_yes_llm_no_keeps_heuristic_bool():
    ...
    from AHNIS.cli import _run_pass_zero
    from AHNIS.corpus_origin import CorpusOriginResult
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("AHNIS.cli.detect_origin_llm", return_value=llm_wrong_result):
    ...
    from AHNIS.corpus_origin import detect_origin_heuristic
...
def test_merge_tier_fields_heuristic_no_no_personas_leak():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("AHNIS.cli.detect_origin_llm", return_value=llm_agreeing_result):
            wrapped = _run_pass_zero(
                project_dir=str(project_dir),
                palace_dir=str(palace_dir),
                llm_provider=fake_provider,
...
def test_merge_tier_fields_confidence_matches_heuristic_call():
    ...
    from AHNIS.cli import _run_pass_zero
    from AHNIS.corpus_origin import CorpusOriginResult, detect_origin_heuristic
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("AHNIS.cli.detect_origin_llm", return_value=llm_distinct_result):
            wrapped = _run_pass_zero(
                project_dir=str(project_dir),
                palace_dir=str(palace_dir),
                llm_provider=fake_provider,
...
#
# When AHNIS init resolves an LLM provider whose endpoint will send
# user content off the local machine/network, init MUST print a clear
# warning naming the provider, stating that AHNIS doesn't control
...
def test_init_prints_privacy_warning_when_provider_is_external(
    ...
    # The warning should also tell users AHNIS isn't responsible
    ...
    assert (
        ...
        f"Privacy warning must clarify AHNIS doesn't control how the "
