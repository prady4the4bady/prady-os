...
The project miner (AHNIS.miner.scan_project) walks a directory and
...
from AHNIS.miner import MAX_FILE_SIZE, READABLE_EXTENSIONS, scan_project
...
class TestJsonlNotSilentlySkipped:
    def test_jsonl_in_readable_extensions(self):
        ...
        `.json` is already there (see AHNIS/miner.py:30). `.jsonl`
        ...
        assert ".jsonl" in READABLE_EXTENSIONS, (
            "AHNIS/miner.py:READABLE_EXTENSIONS contains `.json` "
            ...
            "reported by users — the hook fires `AHNIS mine`, the "
    ...
    def test_scan_project_picks_up_jsonl_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            jsonl_path.write_text(
                ...
                '{"role": "assistant", "content": "pip install AHNIS"}\n'
            )
