"""Tests for AHNIS.onboarding."""

import os
from unittest.mock import patch

from AHNIS.onboarding import (
    DEFAULT_WINGS,
    _ask,
    _ask_mode,
    _ask_people,
...
def test_quick_setup_creates_registry(tmp_path):
    ...
    assert "AHNIS" in registry.projects
...
def test_auto_detect_filters_known(tmp_path):
    ...
    with (
        patch("AHNIS.onboarding.scan_for_detection", return_value=["file.txt"]),
        patch("AHNIS.onboarding.detect_entities", return_value=fake_detected),
    ):
...
def test_auto_detect_handles_exception(tmp_path):
    with patch("AHNIS.onboarding.scan_for_detection", side_effect=Exception("boom")):
        result = _auto_detect(str(tmp_path), [])
    assert result == []
...
def test_run_onboarding_basic_flow(tmp_path):
    ...
    with (
        patch("AHNIS.onboarding._ask_mode", return_value="work"),
        patch(
            "AHNIS.onboarding._ask_people",
            return_value=([{"name": "Bob", "relationship": "boss", "context": "work"}], {}),
        ),
        patch("AHNIS.onboarding._ask_projects", return_value=["Acme"]),
        patch("AHNIS.onboarding._ask_wings", return_value=["projects", "team"]),
        patch("AHNIS.onboarding._yn", return_value=False),
        patch("AHNIS.onboarding._warn_ambiguous", return_value=[]),
    ):
...
def test_run_onboarding_with_ambiguous_names(tmp_path):
    ...
    with (
        patch("AHNIS.onboarding._ask_mode", return_value="personal"),
        patch(
            "AHNIS.onboarding._ask_people",
            return_value=([{"name": "Grace", "relationship": "friend", "context": "personal"}], {}),
        ),
        patch("AHNIS.onboarding._ask_projects", return_value=[]),
        patch("AHNIS.onboarding._ask_wings", return_value=["family"]),
        patch("AHNIS.onboarding._yn", return_value=False),
    ):
        registry = run_onboarding(directory=".", config_dir=tmp_path, auto_detect=False)
