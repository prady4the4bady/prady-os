...
def _get_collection(palace_path, create=False):
    ...
    if create:
        return (
            client,
            client.get_or_create_collection("AHNIS_drawers", metadata={"hnsw:space": "cosine"}),
        )
    return client, client.get_collection("AHNIS_drawers")
...
class TestHandleRequest:
    def test_initialize(self):
        from AHNIS.mcp_server import handle_request

        resp = handle_request({"method": "initialize", "id": 1, "params": {}})
        assert resp["result"]["serverInfo"]["name"] == "AHNIS"
        assert resp["id"] == 1
    ...
    def test_initialize_unknown_version_falls_back_to_latest(self):
        ...
        from AHNIS.mcp_server import SUPPORTED_PROTOCOL_VERSIONS

        assert resp["result"]["protocolVersion"] == SUPPORTED_PROTOCOL_VERSIONS[0]

    def test_initialize_missing_version_uses_oldest(self):
        from AHNIS.mcp_server import handle_request, SUPPORTED_PROTOCOL_VERSIONS
    ...
    def test_tools_list(self):
        ...
        assert "AHNIS_status" in names
        assert "AHNIS_search" in names
        assert "AHNIS_add_drawer" in names
        assert "AHNIS_kg_add" in names
        ...
        resp = handle_request(
            {
                "method": "tools/call",
                "id": 10,
                "params": {"name": "AHNIS_status", "arguments": None},
            }
        )
        ...
        resp = handle_request(
            {
                "method": "tools/call",
                "id": 5,
                "params": {"name": "AHNIS_status", "arguments": {}},
            }
        )
...
class TestReadTools:
    def test_status_cold_start_no_collection(self, monkeypatch, config, palace_path, kg):
        ...
        After `AHNIS init`, chroma.sqlite3 exists but the AHNIS_drawers
        ...
        client = chromadb.PersistentClient(path=palace_path)
        del client
        from AHNIS.mcp_server import tool_status
    ...
    def test_status_handles_none_metadata_without_partial(
        ...
        with _patch("AHNIS.mcp_server._get_collection") as mock_get_col:
            fake_col = type("C", (), {"count": lambda self: 2})()
            mock_get_col.return_value = fake_col
            with _patch(
                "AHNIS.mcp_server._get_cached_metadata",
    ...
    def test_list_wings(self, monkeypatch, config, palace_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from AHNIS.mcp_server import tool_list_wings
    ...
    def test_list_rooms_all(self, monkeypatch, config, palace_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from AHNIS.mcp_server import tool_list_rooms
    ...
    def test_get_taxonomy(self, monkeypatch, config, palace_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from AHNIS.mcp_server import tool_get_taxonomy
...
class TestSearchTool:
    def test_search_basic(self, monkeypatch, config, palace_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from AHNIS.mcp_server import tool_search
...
class TestWriteTools:
    def test_add_drawer(self, monkeypatch, config, palace_path, kg):
        ...
        _client, _col = _get_collection(palace_path, create=True)
        del _client
        from AHNIS.mcp_server import tool_add_drawer
    ...
    def test_delete_drawer(self, monkeypatch, config, palace_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from AHNIS.mcp_server import tool_delete_drawer
    ...
    def test_check_duplicate(self, monkeypatch, config, palace_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from AHNIS.mcp_server import tool_check_duplicate
    ...
    def test_get_drawer(self, monkeypatch, config, palace_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from AHNIS.mcp_server import tool_get_drawer
    ...
    def test_list_drawers(self, monkeypatch, config, palace_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from AHNIS.mcp_server import tool_list_drawers
    ...
    def test_update_drawer_content(self, monkeypatch, config, palace_path, seeded_collection, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from AHNIS.mcp_server import tool_update_drawer, tool_get_drawer
    ...
    def test_update_drawer_wing_and_room(
        ...
        _patch_mcp_server(monkeypatch, config, kg)
        from AHNIS.mcp_server import tool_update_drawer
...
class TestKGTools:
    def test_kg_add(self, monkeypatch, config, palace_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        from AHNIS.mcp_server import tool_kg_add
    ...
    def test_kg_query(self, monkeypatch, config, palace_path, seeded_kg):
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from AHNIS.mcp_server import tool_kg_query
    ...
    def test_kg_invalidate(self, monkeypatch, config, palace_path, seeded_kg):
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from AHNIS.mcp_server import tool_kg_invalidate
    ...
    def test_kg_timeline(self, monkeypatch, config, palace_path, seeded_kg):
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from AHNIS.mcp_server import tool_kg_timeline
    ...
    def test_kg_stats(self, monkeypatch, config, palace_path, seeded_kg):
        _patch_mcp_server(monkeypatch, config, seeded_kg)
        from AHNIS.mcp_server import tool_kg_stats
...
class TestDiaryTools:
    def test_diary_write_and_read(self, monkeypatch, config, palace_path, kg):
        ...
        _client, _col = _get_collection(palace_path, create=True)
        del _client
        from AHNIS.mcp_server import tool_diary_write, tool_diary_read
    ...
    def test_diary_read_empty(self, monkeypatch, config, palace_path, kg):
        ...
        _client, _col = _get_collection(palace_path, create=True)
        del _client
        from AHNIS.mcp_server import tool_diary_read
    ...
    def test_diary_write_same_second_shared_prefix_no_collision(
        ...
        from AHNIS.mcp_server import tool_diary_read, tool_diary_write
