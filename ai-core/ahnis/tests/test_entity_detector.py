"""Tests for AHNIS.entity_detector."""
...
from AHNIS.entity_detector import (
    PROSE_EXTENSIONS,
    STOPWORDS,
    _print_entity_list,
    classify_entity,
...
def _temp_locale(locale_code: str, entity_section: dict):
    """Context manager that drops a locale JSON into AHNIS/i18n/ for the test body.
    ...
    Note: writes into the real AHNIS/i18n/ directory. If a test process is
    ...
    Recover with `rm AHNIS/i18n/zz-test-*.json`.
    """
    from AHNIS import i18n
    from AHNIS import entity_detector
...
def test_get_entity_patterns_unknown_lang_falls_back_to_english():
    ...
    from AHNIS.i18n import get_entity_patterns
...
def test_build_patterns_cache_is_keyed_by_language():
    ...
    from AHNIS.entity_detector import _build_patterns
...
def test_normalize_langs_handles_string_input():
    ...
    from AHNIS.entity_detector import _normalize_langs
...
def test_config_entity_languages_defaults_to_english(tmp_path, monkeypatch):
    """AHNISConfig.entity_languages defaults to ['en'] with no config file."""
    from AHNIS.config import AHNISConfig

    monkeypatch.delenv("AHNIS_ENTITY_LANGUAGES", raising=False)
    monkeypatch.delenv("MEMPAL_ENTITY_LANGUAGES", raising=False)
    cfg = AHNISConfig(config_dir=str(tmp_path))
...
def test_config_entity_languages_from_env(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("AHNIS_ENTITY_LANGUAGES", "en,pt-br,ru")
    cfg = AHNISConfig(config_dir=str(tmp_path))
...
def test_config_set_entity_languages_persists(tmp_path, monkeypatch):
    ...
    cfg2 = AHNISConfig(config_dir=str(tmp_path))
