"""Tests for AHNIS.entity_registry."""

from unittest.mock import patch

from AHNIS.entity_registry import (
...
def test_save_and_load_roundtrip(tmp_path):
    ...
    assert "AHNIS" in loaded.projects
...
def test_lookup_known_project(tmp_path):
    registry = EntityRegistry.load(config_dir=tmp_path)
    registry.seed(mode="work", people=[], projects=["AHNIS"])
    result = registry.lookup("AHNIS")
...
def test_research_local_only_by_default(tmp_path):
    ...
    with patch(
        "AHNIS.entity_registry._wikipedia_lookup",
...
def test_wikipedia_404_returns_unknown(tmp_path):
    ...
    with patch("AHNIS.entity_registry._wikipedia_lookup", return_value=mock_result):
