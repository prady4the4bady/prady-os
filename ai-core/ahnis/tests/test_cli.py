"""Tests for AHNIS.cli — the main CLI dispatcher."""
...
@patch("AHNIS.cli.AHNISConfig")
def test_cmd_status_default_palace(mock_config_cls):
    mock_config_cls.return_value.palace_path = "/fake/palace"
    args = argparse.Namespace(palace=None)
    mock_miner = MagicMock()
    with patch.dict("sys.modules", {"AHNIS.miner": mock_miner}):
        cmd_status(args)
...
def test_cmd_search_calls_search(mock_config_cls):
    ...
    with patch("AHNIS.searcher.search") as mock_search:
        cmd_search(args)
...
def test_cmd_search_error_exits(mock_config_cls):
    ...
    from AHNIS.searcher import SearchError

    with patch("AHNIS.searcher.search", side_effect=SearchError("fail")):
...
def test_cmd_instructions_calls_run_instructions():
    ...
    with patch("AHNIS.instructions_cli.run_instructions") as mock_run:
...
def test_cmd_hook_calls_run_hook():
    ...
    with patch("AHNIS.hooks_cli.run_hook") as mock_run:
        cmd_hook(args)
...
def test_cmd_init_no_entities(mock_config_cls, tmp_path):
    ...
    with (
        patch("AHNIS.entity_detector.scan_for_detection", return_value=[]),
        patch("AHNIS.room_detector_local.detect_rooms_local") as mock_rooms,
...
def test_cmd_init_with_entities(mock_config_cls, tmp_path):
    ...
    args = argparse.Namespace(dir=str(tmp_path), yes=True)
    with (
        patch("AHNIS.entity_detector.scan_for_detection", return_value=fake_files),
        patch("AHNIS.entity_detector.detect_entities", return_value=detected),
        patch("AHNIS.entity_detector.confirm_entities", return_value=confirmed),
        ...
        patch("AHNIS.cli._run_pass_zero", return_value=None),
...
def test_cmd_init_normalizes_wing_name_for_topics_registry(mock_config_cls, tmp_path):
    ...
    same slug ``AHNIS.yaml`` uses, otherwise ``topics_by_wing`` keys
    ...
    with (
        ...
        patch("AHNIS.entity_detector.detect_entities", return_value=detected),
        patch("AHNIS.entity_detector.confirm_entities", return_value=confirmed),
        patch("AHNIS.miner.add_to_known_entities") as mock_register,
        patch("AHNIS.room_detector_local.detect_rooms_local"),
        patch("builtins.open", MagicMock()),
        patch("AHNIS.cli._maybe_run_mine_after_init"),
        # Pass-zero corpus-origin detection runs unconditionally inside
        # cmd_init now (#1221 / #1223). It accesses AHNISConfig fields
...
def test_maybe_run_mine_prompt_accepted_runs_mine(tmp_path):
    ...
    from AHNIS.cli import _maybe_run_mine_after_init

    args = _init_args(tmp_path, yes=False, auto_mine=False)
    cfg = _fake_cfg(tmp_path)
    scanned = _fake_scanned(tmp_path, n=3)
    with (
        patch("AHNIS.miner.mine") as mock_mine,
        patch("AHNIS.miner.scan_project", return_value=scanned),
...
def test_maybe_run_mine_prompt_yes_accepted_runs_mine(tmp_path):
    ...
    with (
        patch("AHNIS.miner.mine") as mock_mine,
        patch("AHNIS.miner.scan_project", return_value=[]),
...
def test_maybe_run_mine_prompt_declined_prints_hint(tmp_path, capsys):
    ...
    assert f"AHNIS mine {shlex.quote(str(tmp_path))}" in out
...
def test_maybe_run_mine_decline_quotes_path_with_spaces(tmp_path, capsys):
    ...
    assert f"AHNIS mine {shlex.quote(str(spaced_dir))}" in out
    # Bare unquoted form must NOT appear — that's the bug we're guarding.
    assert f"AHNIS mine {spaced_dir} " not in out
    assert f"AHNIS mine {spaced_dir}`" not in out
...
def test_maybe_run_mine_failure_surfaces_via_exit(tmp_path, capsys):
    ...
    cfg = _fake_cfg(tmp_path)
    with (
        patch("AHNIS.miner.mine", side_effect=RuntimeError("boom")),
        patch("AHNIS.miner.scan_project", return_value=[]),
    ):
        with pytest.raises(SystemExit) as exc_info:
            _maybe_run_mine_after_init(args, cfg)
...
def test_cmd_mine_projects_mode(mock_config_cls):
    ...
    with patch("AHNIS.miner.mine") as mock_mine:
        cmd_mine(args)
        mock_mine.assert_called_once_with(
            project_dir="/src",
            palace_path="/fake/palace",
...
def test_cmd_mine_convos_mode(mock_config_cls):
    ...
    with patch("AHNIS.convo_miner.mine_convos") as mock_mine:
        cmd_mine(args)
        mock_mine.assert_called_once_with(
            convo_dir="/chats",
            palace_path="/fake/palace",
...
def test_cmd_wakeup(mock_config_cls, capsys):
    ...
    with patch("AHNIS.layers.MemoryStack", return_value=mock_stack):
        cmd_wakeup(args)
    out = capsys.readouterr().out
...
def test_cmd_split_basic():
    ...
    with patch("AHNIS.split_mega_files.main") as mock_main:
        cmd_split(args)
...
def test_main_status_dispatches():
    with (
        patch("sys.argv", ["AHNIS", "status"]),
        patch("AHNIS.cli.cmd_status") as mock_cmd,
    ):
        main()
...
def test_main_search_dispatches():
    with (
        patch("sys.argv", ["AHNIS", "search", "my query"]),
        patch("AHNIS.cli.cmd_search") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_init_dispatches():
    with (
        patch("sys.argv", ["AHNIS", "init", "/some/dir"]),
        patch("AHNIS.cli.cmd_init") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_mine_dispatches():
    with (
        patch("sys.argv", ["AHNIS", "mine", "/some/dir"]),
        patch("AHNIS.cli.cmd_mine") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_wakeup_dispatches():
    with (
        patch("sys.argv", ["AHNIS", "wake-up"]),
        patch("AHNIS.cli.cmd_wakeup") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_split_dispatches():
    with (
        patch("sys.argv", ["AHNIS", "split", "/chats"]),
        patch("AHNIS.cli.cmd_split") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_mcp_command_prints_setup_guidance(monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", ["AHNIS", "mcp"])

    main()

    captured = capsys.readouterr()
    assert "AHNIS MCP quick setup:" in captured.out
    assert "claude mcp add AHNIS -- AHNIS-mcp" in captured.out
    assert "\nOptional custom palace:\n" in captured.out
    assert "AHNIS-mcp --palace /path/to/palace" in captured.out
...
def test_mcp_command_uses_custom_palace_path_when_provided(monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", ["AHNIS", "--palace", "~/tmp/my palace", "mcp"])
    ...
    assert "AHNIS-mcp --palace" in captured.out
...
def test_main_hook_no_subcommand_prints_help(capsys):
    with patch("sys.argv", ["AHNIS", "hook"]):
        main()
    out = capsys.readouterr().out
...
def test_main_hook_run_dispatches():
    with (
        patch(
            "sys.argv",
            ["AHNIS", "hook", "run", "--hook", "session-start", "--harness", "claude-code"],
        ),
        patch("AHNIS.cli.cmd_hook") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_instructions_no_subcommand_prints_help(capsys):
    with patch("sys.argv", ["AHNIS", "instructions"]):
        main()
    out = capsys.readouterr().out
...
def test_main_instructions_dispatches():
    with (
        patch("sys.argv", ["AHNIS", "instructions", "help"]),
        patch("AHNIS.cli.cmd_instructions") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_repair_dispatches():
    with (
        patch("sys.argv", ["AHNIS", "repair"]),
        patch("AHNIS.cli.cmd_repair") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_main_compress_dispatches():
    with (
        patch("sys.argv", ["AHNIS", "compress"]),
        patch("AHNIS.cli.cmd_compress") as mock_cmd,
    ):
        main()
        mock_cmd.assert_called_once()
...
def test_cmd_repair_no_palace(mock_config_cls, tmp_path, capsys):
    ...
    args = argparse.Namespace(palace=None)
    with patch("AHNIS.backends.chroma.ChromaBackend"):
        cmd_repair(args)
    out = capsys.readouterr().out
...
def test_cmd_repair_error_reading(mock_config_cls, tmp_path, capsys):
    ...
    with patch("AHNIS.backends.chroma.ChromaBackend", return_value=mock_backend):
        cmd_repair(args)
    out = capsys.readouterr().out
...
def test_cmd_repair_aborts_without_confirmation(mock_config_cls, tmp_path, capsys):
    ...
    mock_backend = _mock_backend_for(col=mock_col)
    with (
        patch("AHNIS.backends.chroma.ChromaBackend", return_value=mock_backend),
        patch("builtins.input", return_value="n"),
    ):
        cmd_repair(args)
    out = capsys.readouterr().out
...
def test_cmd_compress_dry_run(mock_config_cls, capsys):
    ...
    with (
        patch("AHNIS.backends.chroma.ChromaBackend", return_value=mock_backend),
        patch.dict("sys.modules", {"AHNIS.dialect": mock_dialect_mod}),
    ):
        cmd_compress(args)
    out = capsys.readouterr().out
...
def test_cmd_compress_stores_results(mock_config_cls, capsys):
    """Non-dry-run compress stores to AHNIS_compressed collection."""
    mock_config_cls.return_value.palace_path = "/fake/palace"
    args = argparse.Namespace(palace=None, wing=None, dry_run=False, config=None)
    mock_col = MagicMock()
    mock_col.get.side_effect = [
...
def test_cmd_repair_trailing_slash_does_not_recurse():
    ...
    with patch("AHNIS.cli.os.path.isdir", return_value=False):
        cmd_repair(args)
