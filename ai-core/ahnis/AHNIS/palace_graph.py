"""
palace_graph.py — Graph traversal layer for AHNIS
======================================================
...
logger = logging.getLogger("AHNIS_graph")
...
def _normalize_wing(wing: str | None) -> str | None:
    ...
    (e.g. ``AHNIS_public``).  Callers that pass the raw directory name
    (``AHNIS-public``) would silently miss.  This helper aligns the lookup
...
#
# Stored as a JSON file at ~/.AHNIS/tunnels.json so they persist
# across palace rebuilds (not in ChromaDB which can be recreated).


_TUNNEL_FILE = os.path.join(os.path.expanduser("~"), ".AHNIS", "tunnels.json")
...
# ``~/.AHNIS/known_entities.json`` under ``topics_by_wing``.
#
