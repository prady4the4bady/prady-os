...
Usage (standalone):
    python -m AHNIS.repair status
    python -m AHNIS.repair scan [--wing X]
    python -m AHNIS.repair prune --confirm
    python -m AHNIS.repair rebuild

Usage (from CLI):
    AHNIS repair
    AHNIS repair-scan [--wing X]
    AHNIS repair-prune --confirm
"""
...
def _get_palace_path():
    ...
    try:
        ...
        return AHNISConfig().palace_path
    except Exception:
        default = os.path.join(os.path.expanduser("~"), ".AHNIS", "palace")
        return default
...
def rebuild_index(palace_path=None, confirm_truncation_ok: bool = False):
    ...
    print(f"\n{'=' * 55}")
    print("  AHNIS Repair — Index Rebuild")
    print(f"{'=' * 55}\n")
...
def status(palace_path=None) -> dict:
    ...
    chromadb tries to load the undersized HNSW. Running ``AHNIS
    ...
    directly via :func:`AHNIS.backends.chroma.hnsw_capacity_status`.
    ...
    drawers = hnsw_capacity_status(palace_path, "AHNIS_drawers")
    closets = hnsw_capacity_status(palace_path, "AHNIS_closets")
    ...
    if drawers["diverged"] or closets["diverged"]:
        print("\n  Recommended: run `AHNIS repair` to rebuild the index.")
    print()
...
def repair_max_seq_id(
    ...
    *,
    ...
    print(f"\n{'=' * 55}")
    print("  AHNIS Repair — max_seq_id Un-poison")
    print(f"{'=' * 55}\n")
    print(f"  Palace:  {palace_path}")
    if segment:
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="AHNIS repair tools")
