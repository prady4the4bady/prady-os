"""ChromaDB-backed AHNIS storage backend (RFC 001 reference implementation)."""
...
def quarantine_stale_hnsw(palace_path: str, stale_seconds: float = 300.0) -> list[str]:
    ...
    2. **Integrity gate** (``_segment_appears_healthy``). Even when the
       ...
       from vector recall until the operator runs ``AHNIS repair
...
def hnsw_capacity_status(palace_path: str, collection_name: str = "AHNIS_drawers") -> dict:
    ...
    try:
        ...
        if hnsw_count is None:
            ...
            if sqlite_count > _HNSW_DIVERGENCE_ABSOLUTE:
                ...
                out["message"] = (
                    ...
                    "until the segment is rebuilt. Run `AHNIS repair`."
                )
            else:
        ...
        if divergence > threshold:
            ...
            out["message"] = (
                ...
                "are invisible to vector search. Run `AHNIS repair` to rebuild."
            )
        else:
            out["status"] = "ok"
            out["message"] = (
...
def _sqlite_embedding_count(palace_path: str, collection_name: str) -> Optional[int]:
    ...
    Mirrors :func:`AHNIS.repair.sqlite_drawer_count` but kept in this
...
class ChromaBackend(BaseBackend):
    """AHNIS's default ChromaDB backend.
