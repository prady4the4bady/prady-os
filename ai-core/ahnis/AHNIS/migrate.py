#!/usr/bin/env python3
"""
AHNIS migrate — Recover a palace created with a different ChromaDB version.
...
Since AHNIS 3.2.0 (chromadb>=1.5.4), chromadb automatically migrates
...
older AHNIS release) or if automatic migration fails.

Usage:
    AHNIS migrate                          # migrate default palace
    AHNIS migrate --palace /path/to/palace  # migrate specific palace
    AHNIS migrate --dry-run                # show what would be migrated
"""
...
def contains_palace_database(path: str) -> bool:
    """Return True when path looks like a AHNIS ChromaDB directory."""
...
def migrate(palace_path: str, dry_run: bool = False, confirm: bool = False):
    ...
    try:
        col = ChromaBackend().get_collection(palace_path, "AHNIS_drawers")
        count = col.count()
    ...
    temp_palace = tempfile.mkdtemp(prefix="AHNIS_migrate_")
    print(f"  Creating fresh palace in {temp_palace}...")
    fresh_backend = ChromaBackend()
    col = fresh_backend.get_or_create_collection(temp_palace, "AHNIS_drawers")
