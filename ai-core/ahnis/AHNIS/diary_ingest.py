...
- Only new entries are processed by default (tracks entry count in a state
  file under ``~/.AHNIS/state/`` — never inside the user's diary dir).
...
Usage:
    python -m AHNIS.diary_ingest --dir ~/daily_summaries --palace ~/.AHNIS/palace
    python -m AHNIS.diary_ingest --dir ~/daily_summaries --palace ~/.AHNIS/palace --force
"""
...
def _state_file_for(palace_path: str, diary_dir: Path) -> Path:
    """Return the per-(palace, diary-dir) state-file path under ~/.AHNIS/state.
    ...
    """
    state_root = Path(os.path.expanduser("~")) / ".AHNIS" / "state"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--palace", default=os.path.expanduser("~/.AHNIS/palace"))
