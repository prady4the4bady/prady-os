...
Used by AHNIS init before mining begins.
...
Multi-language support:
    ...
    the ``entity`` section of ``AHNIS/i18n/<lang>.json``. Every public
...
Usage:
    from AHNIS.entity_detector import detect_entities, confirm_entities
...
# ``AHNIS/i18n/en.json``. Callers that need multi-language behavior should
...
def detect_entities(
    ...
    Args:
        ...
        corpus_origin: Optional corpus-origin context (the dict produced
            by ``AHNIS.corpus_origin`` and persisted to
            ``<palace>/.AHNIS/origin.json`` by ``AHNIS init``).
