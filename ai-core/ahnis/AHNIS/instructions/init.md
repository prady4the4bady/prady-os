...
Guide the user through a complete AHNIS setup. Follow each step in order,
...
## Step 2: Check if AHNIS is already installed

Run `pip show AHNIS` to see if the package is already present. If it is,
report the installed version and skip to Step 4.

## Step 3: Install AHNIS

Run `pip install AHNIS`.

### Error handling -- pip failures

If `pip install AHNIS` fails, try these fallbacks in order:

1. Try `pip3 install AHNIS`
2. Try `python -m pip install AHNIS` (or `python3 -m pip install AHNIS`)
...
Ask the user which project directory they want to initialize with AHNIS.
Offer the current working directory as the default. Wait for their response
before continuing.

## Step 5: Initialize the palace

Run `AHNIS init --yes <dir>` where `<dir>` is the directory from Step 4.

If this fails, report the error and stop.

## Step 6: Configure MCP server

Run the following command to register the AHNIS MCP server with Claude:
...
Run `AHNIS status` and confirm the output shows a healthy palace.
...
- Use /AHNIS:mine to start adding data to their palace
- Use /AHNIS:search to query their palace and retrieve stored knowledge
