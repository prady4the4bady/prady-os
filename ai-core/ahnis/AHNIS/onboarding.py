#!/usr/bin/env python3
"""
onboarding.py — AHNIS first-run setup.

Asks the user:
  1. How they're using AHNIS (work / personal / combo)
...
Seeds the entity_registry with confirmed data so AHNIS knows your world
from minute one — before a single session is indexed.

Usage:
    python3 -m AHNIS.onboarding
    or: AHNIS init
"""

from pathlib import Path
from AHNIS.entity_registry import EntityRegistry
...
def _ask_mode() -> str:
    _header("Welcome to AHNIS")
    print("""
  AHNIS is a personal memory system. To work well, it needs to know
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help AHNIS distinguish project
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from AHNIS.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_aaak_bootstrap(
    ...
    """
    AHNIS_dir = Path(config_dir) if config_dir else Path.home() / ".AHNIS"
    AHNIS_dir.mkdir(parents=True, exist_ok=True)
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by AHNIS init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    (AHNIS_dir / "aaak_entities.md").write_text("\n".join(registry_lines), encoding="utf-8")
    ...
    (AHNIS_dir / "critical_facts.md").write_text("\n".join(facts_lines), encoding="utf-8")
...
def run_onboarding(
  ...
  AHNIS will check the context before treating them as person names.
...
""")
    ...
    print("\n  AAAK entity registry: ~/.AHNIS/aaak_entities.md")
    print("  Critical facts bootstrap: ~/.AHNIS/critical_facts.md")
