#!/usr/bin/env python3
"""
entity_registry.py — Persistent personal entity registry for AHNIS.
...
def _wikipedia_lookup(word: str) -> dict:
    ...
    **Privacy warning:** This function makes an outbound HTTPS request to
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "AHNIS/1.0"})
...
class EntityRegistry:
    ...
    Stored at ~/.AHNIS/entity_registry.json
    Schema:
    {
    ...
    DEFAULT_PATH = Path.home() / ".AHNIS" / "entity_registry.json"
    ...
    def learn_from_text(self, text: str, min_confidence: float = 0.75, languages=("en",)) -> list:
        ...
        configured ``AHNISConfig().entity_languages`` to match the
        locales used at ``AHNIS init`` time.
        """
        from AHNIS.entity_detector import extract_candidates, score_entity, classify_entity
