...
def _warn_if_legacy_metric(col) -> None:
    ...
    need ``AHNIS repair`` to rebuild with the correct metric.
    ...
    print(
        ...
        "          Run `AHNIS repair` to rebuild the index with the correct metric.",
        file=_sys.stderr,
    )
    ...
    for h in hits:
        ...
        if len(expanded) > MAX_HYDRATION_CHARS:
            expanded = (
                ...
                "Use AHNIS_get_drawer for full content.]"
            )
        h["text"] = expanded
