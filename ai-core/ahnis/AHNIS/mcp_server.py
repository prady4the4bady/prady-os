#!/usr/bin/env python3
"""
AHNIS MCP Server — read/write palace access for Claude Code
================================================================
Install: claude mcp add AHNIS -- AHNIS-mcp [--palace /path/to/palace]

Tools (read):
  AHNIS_status          — total drawers, wing/room breakdown
  ...
  AHNIS_search          — semantic search, optional wing/room filter
  AHNIS_check_duplicate — check if content already exists before filing

Tools (write):
  AHNIS_add_drawer      — file verbatim content into a wing/room
  AHNIS_delete_drawer   — remove a drawer by ID

Tools (maintenance):
  AHNIS_reconnect       — force cache invalidation and reconnect after external writes
"""
...
logger = logging.getLogger("AHNIS_mcp")


def _parse_args():
    parser = argparse.ArgumentParser(description="AHNIS MCP Server")
    parser.add_argument(
        "--palace",
        metavar="PATH",
...
if _args.palace:
    os.environ["AHNIS_PALACE_PATH"] = os.path.abspath(_args.palace)

_config = AHNISConfig()
# Only override KG path when --palace is explicitly provided; otherwise use
# KnowledgeGraph's default (~/.AHNIS/knowledge_graph.sqlite3).
if _args.palace:
...
# :func:`AHNIS.searcher._bm25_only_via_sqlite`. Cleared after a
...
def _refresh_vector_disabled_flag() -> None:
    ...
    try:
        info = hnsw_capacity_status(_config.palace_path, "AHNIS_drawers")
    except Exception:
    ...
    if info.get("diverged"):
        if not _vector_disabled:
            logger.warning(
                ...
                "BM25-only sqlite fallback. Run `AHNIS repair` to restore "
                "vector search.",
...
_WAL_DIR = Path(os.path.expanduser("~/.AHNIS/wal"))
...
def _no_palace():
    return {
        ...
        "hint": "Run: AHNIS init <dir> && AHNIS mine <dir>",
    }
...
# Also available via AHNIS_get_aaak_spec tool.

PALACE_PROTOCOL = """IMPORTANT — AHNIS Memory Protocol:
1. ON WAKE-UP: Call AHNIS_status to load palace overview + AAAK spec.
2. BEFORE RESPONDING about any person, project, or past event: call AHNIS_kg_query or AHNIS_search FIRST. Never guess — verify.
3. IF UNSURE about a fact (name, gender, age, relationship): say "let me check" and query the palace. Wrong is worse than slow.
4. AFTER EACH SESSION: call AHNIS_diary_write to record what happened, what you learned, what matters.
5. WHEN FACTS CHANGE: call AHNIS_kg_invalidate on the old fact, AHNIS_kg_add for the new one.

This protocol ensures the AI KNOWS before it speaks. Storage is not memory — but storage + this protocol = memory."""

AAAK_SPEC = """AAAK is a compressed memory dialect that AHNIS uses for efficient storage.
...
def tool_check_duplicate(content: str, threshold: float = 0.9):
    ...
    if _vector_disabled:
        ...
        return {
            ...
            "vector_disabled_reason": _vector_disabled_reason,
            "hint": (
                "duplicate detection requires vector search; run " "`AHNIS repair` to restore"
            ),
        }
    try:
        results = col.query(
...
def tool_memories_filed_away():
    ...
    state_dir = Path.home() / ".AHNIS" / "hook_state"
...
TOOLS = {
    ...
    "AHNIS_get_aaak_spec": {
        "description": "Get the AAAK dialect specification — the compressed memory format AHNIS uses. Call this if you need to read or write AAAK-compressed memories.",
        "input_schema": {"type": "object", "properties": {}},
    ...
    if method == "initialize":
        ...
        return {
            ...
            "result": {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "AHNIS", "version": __version__},
            },
        }
    elif method == "ping":
...
def main():
    _restore_stdout()
    logger.info("AHNIS MCP Server starting...")
