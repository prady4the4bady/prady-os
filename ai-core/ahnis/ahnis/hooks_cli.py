"""
Hook logic for AHNIS — Python implementation of session-start, stop, and precompact hooks.
...
def _AHNIS_python() -> str:
    """Return the python interpreter that has AHNIS installed.
    ...
    1. AHNIS_PYTHON env var (explicit override)
    2. Venv python from package install path
    3. Editable install: venv/ sibling to AHNIS/
    4. sys.executable fallback
    """
    # Honor explicit override (used by shell hook wrappers)
    env_python = os.environ.get("AHNIS_PYTHON", "")
    if env_python and os.path.isfile(env_python) and os.access(env_python, os.X_OK):
        return env_python
    # This file lives at <venv>/lib/pythonX.Y/site-packages/AHNIS/hooks_cli.py
    # or <project>/AHNIS/hooks_cli.py (editable install).
    venv_bin = Path(__file__).resolve().parents[3] / "bin" / "python"
    if venv_bin.is_file():
        return str(venv_bin)
    # Editable install: assumes project root has a venv/ sibling to AHNIS/
...
STOP_BLOCK_REASON = (
    "AUTO-SAVE checkpoint (AHNIS). Save this session's key content:\n"
    "1. AHNIS_diary_write — session summary (what was discussed, "
    "key decisions, current state of work)\n"
    "2. AHNIS_add_drawer — verbatim quotes, decisions, code snippets "
    "(place in appropriate wing and room)\n"
    "3. AHNIS_kg_add — entity relationships (optional)\n"
    "For THIS save, use AHNIS MCP tools only (not auto-memory .md files). "
    "Use verbatim quotes where possible. Continue conversation after saving."
)

PRECOMPACT_BLOCK_REASON = (
    "COMPACTION IMMINENT (AHNIS). Save ALL session content before context is lost:\n"
    "1. AHNIS_diary_write — thorough session summary\n"
    "2. AHNIS_add_drawer — ALL verbatim quotes, decisions, code, context "
    "(place each in appropriate wing and room)\n"
    "3. AHNIS_kg_add — entity relationships (optional)\n"
    "For THIS save, use AHNIS MCP tools only (not auto-memory .md files). "
    "Be thorough — after compaction this is all that survives. "
    "Save everything to AHNIS, then allow compaction to proceed."
)
...
def _output(data: dict):
    ...
    If AHNIS.mcp_server is already loaded, reuse its saved real stdout fd.
    ...
    mcp_mod = sys.modules.get("AHNIS.mcp_server") or sys.modules.get(
...
def _maybe_auto_ingest():
    ...
    for mine_dir, mode in targets:
        try:
            _spawn_mine([_AHNIS_python(), "-m", "AHNIS", "mine", mine_dir, "--mode", mode])
        except OSError:
            pass
...
def _desktop_toast(body: str, title: str = "AHNIS"):
    """Send a desktop notification via notify-send. Fails silently."""
    try:
        subprocess.Popen(
            ["notify-send", "--app-name=AHNIS", "--icon=brain", title, body],
...
def _ingest_transcript(transcript_path: str):
    ...
    try:
        AHNISConfig()  # validate config loads
    except Exception:
        return
...
def hook_stop(data: dict, harness: str):
    ...
    if str(stop_hook_active).lower() in ("true", "1", "yes"):
        ...
        try:
            ...
            _log(
                f"WARNING: could not import AHNISConfig for stop guard: {exc}; defaulting to silent mode"
            )
        else:
            try:
                silent_guard = AHNISConfig().hook_silent_save
