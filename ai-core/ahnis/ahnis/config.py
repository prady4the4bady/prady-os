"""
AHNIS configuration system.

Priority: env vars > config file (~/.AHNIS/config.json) > defaults
"""
...
def normalize_wing_name(name: str) -> str:
    ...
    and when writing `AHNIS.yaml`, so the miner's lookup matches at
...
DEFAULT_PALACE_PATH = os.path.expanduser("~/.AHNIS/palace")
DEFAULT_COLLECTION_NAME = "AHNIS_drawers"
...
class AHNISConfig:
    """Configuration manager for AHNIS.
    ...
    def __init__(self, config_dir=None):
        ...
        """
        self._config_dir = (
            Path(config_dir) if config_dir else Path(os.path.expanduser("~/.AHNIS"))
        )
    ...
    def palace_path(self):
        ...
        env_val = os.environ.get("AHNIS_PALACE_PATH") or os.environ.get("MEMPAL_PALACE_PATH")
        if env_val:
    ...
    def entity_languages(self):
        ...
        Reads from env var ``AHNIS_ENTITY_LANGUAGES`` (comma-separated)
        first, then the ``entity_languages`` field in ``config.json``,
        defaulting to ``["en"]``.
        """
        env_val = os.environ.get("AHNIS_ENTITY_LANGUAGES") or os.environ.get(
    ...
    def embedding_device(self):
        ...
        ``"dml"``. Read from env ``AHNIS_EMBEDDING_DEVICE`` first, then
        ``embedding_device`` in ``config.json``, then ``"auto"``.

        ``auto`` resolves to the first available accelerator at runtime via
        :mod:`AHNIS.embedding`; requesting an unavailable accelerator
        logs a warning and falls back to CPU.
        """
        env_val = os.environ.get("AHNIS_EMBEDDING_DEVICE")
        if env_val:
    ...
    def topic_tunnel_min_count(self):
        ...
        link. Reads ``AHNIS_TOPIC_TUNNEL_MIN_COUNT`` env first, then the
        config-file value, then ``1``.
        """
        env_val = os.environ.get("AHNIS_TOPIC_TUNNEL_MIN_COUNT")
        if env_val:
            try:
                parsed = int(env_val)
                if parsed >= 1:
