...
Supported devices (env ``AHNIS_EMBEDDING_DEVICE`` or ``embedding_device``
in ``~/.AHNIS/config.json``):

* ``auto`` — prefer CUDA ▸ CoreML ▸ DirectML, fall back to CPU
* ``cpu`` — force CPU (the historical default)
* ``cuda`` — NVIDIA GPU via ``onnxruntime-gpu`` (``pip install AHNIS[gpu]``)
...
def _resolve_providers(device: str) -> tuple[list, str]:
    ...
    if preferred not in available:
        if device not in _WARNED:
            extra = _DEVICE_EXTRA.get(device, "the matching AHNIS extra for your device")
            logger.warning(
...
def _build_ef_class():
    ...
    class _AHNISONNX(ONNXMiniLM_L6_V2):
        @staticmethod
        def name() -> str:
            return "default"
...
def get_embedding_function(device: Optional[str] = None):
    ...
    ``device=None`` reads from :class:`AHNISConfig.embedding_device`.
    ...
    if device is None:
        ...
        device = AHNISConfig().embedding_device
