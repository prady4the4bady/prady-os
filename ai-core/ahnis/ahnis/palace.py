...
def get_collection(
    palace_path: str,
    collection_name: str = "AHNIS_drawers",
    create: bool = True,
):
...
def get_closets_collection(palace_path: str, create: bool = True):
    ...
    return get_collection(palace_path, collection_name="AHNIS_closets", create=create)
...
def _candidate_entity_words(text: str) -> list:
    ...
    if _CANDIDATE_RX_CACHE is None:
        ...
        patterns = get_entity_patterns(AHNISConfig().entity_languages)
        rxs = []
...
def mine_lock(source_file: str):
    ...
    """
    lock_dir = os.path.join(os.path.expanduser("~"), ".AHNIS", "locks")
...
class MineAlreadyRunning(RuntimeError):
    """Raised when another `AHNIS mine` already holds the per-palace lock."""
...
def mine_palace_lock(palace_path: str):
    ...
    single source; it does not prevent N copies of `AHNIS mine <dir>`
        ...
        if os.name == "nt":
            ...
            try:
                ...
                raise MineAlreadyRunning(
                    f"another `AHNIS mine` is already running against {resolved}"
                ) from exc
        else:
            import fcntl
