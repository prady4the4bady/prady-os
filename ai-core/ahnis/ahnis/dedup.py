...
Usage (standalone):
    python -m AHNIS.dedup                          # dedup all
    python -m AHNIS.dedup --dry-run                # preview only
    python -m AHNIS.dedup --threshold 0.10         # stricter (near-identical only)
    python -m AHNIS.dedup --threshold 0.35         # looser (catches paraphrased content)
    python -m AHNIS.dedup --wing my_project        # scope to one wing
    python -m AHNIS.dedup --stats                  # stats only
    python -m AHNIS.dedup --source "my_project"    # filter by source

Usage (from CLI):
    AHNIS dedup [--dry-run] [--threshold 0.15] [--stats]
"""
...
def _get_palace_path():
    ...
    try:
        ...
        return os.path.join(os.path.expanduser("~"), ".AHNIS", "palace")
