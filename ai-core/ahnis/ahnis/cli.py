#!/usr/bin/env python3
"""
AHNIS — Give your AI a memory. No API key required.

Two ways to ingest:
  Projects:      AHNIS mine ~/projects/my_app          (code, docs, notes)
  Conversations: AHNIS mine <convo-dir> --mode convos     (Claude Code, Claude.ai, ChatGPT, Slack exports)

Same palace. Same search. Different ingest strategies.

Commands:
    AHNIS init <dir>                  Detect rooms from folder structure
    AHNIS split <dir>                 Split concatenated mega-files into per-session files
    AHNIS mine <dir>                  Mine project files (default)
    AHNIS mine <dir> --mode convos    Mine conversation exports
    AHNIS search "query"              Find anything, exact words
    AHNIS mcp                         Show MCP setup command
    AHNIS wake-up                     Show L0 + L1 wake-up context
    AHNIS wake-up --wing my_app       Wake-up for a specific project
    AHNIS status                      Show what's been filed

Examples:
    AHNIS init ~/projects/my_app
    AHNIS mine ~/projects/my_app
    AHNIS mine ~/.claude/projects/-Users-you-Projects-my_app --mode convos --wing my_app
    AHNIS search "why did we switch to GraphQL"
    AHNIS search "pricing discussion" --wing my_app --room costs
"""
...
def _gather_origin_samples(project_dir) -> list:
    ...
    Skips AHNIS's own per-project artifacts (``entities.json``,
    ``AHNIS.yaml``) so a re-run of ``AHNIS init`` produces the
    ...
    for filepath in files:
        if filepath.name in _AHNIS_PROJECT_FILES:
            continue
...
def _run_pass_zero(project_dir, palace_dir, llm_provider) -> dict:
    ...
    result to ``<palace>/.AHNIS/origin.json``.
    ...
    origin_path = Path(palace_dir).expanduser() / ".AHNIS" / "origin.json"
    try:
...
def _ensure_AHNIS_files_gitignored(project_dir) -> bool:
    """If project_dir is a git repo, ensure AHNIS's per-project files
    ...
    `AHNIS init` writes AHNIS.yaml + entities.json into the
    ...
    missing = [p for p in _AHNIS_PROJECT_FILES if p not in existing_lines]
    if not missing:
        return False
    prefix = "" if not existing or existing.endswith("\n") else "\n"
    block = prefix + "\n# AHNIS per-project files (issue #185)\n" + "\n".join(missing) + "\n"
...
def cmd_init(args):
            ...
            if ok:
                ...
                if candidate.is_external_service:
                    print(
                        ...
                        f"AHNIS does not control how the provider logs, "
    ...
    # <palace>/.AHNIS/origin.json and supplies corpus context to the
    ...
    if total > 0:
        ...
        if confirmed["people"] or confirmed["projects"] or confirmed.get("topics"):
            ...
            # ``AHNIS.yaml`` so the miner's tunnel lookup hits the
    ...
    _ensure_AHNIS_files_gitignored(args.dir)
...
def _maybe_run_mine_after_init(args, cfg) -> None:
    ...
    if not auto_mine:
        ...
        if answer not in ("", "y", "yes"):
            print(f"\n  Skipped. Run `AHNIS mine {shlex.quote(project_dir)}` when ready.")
            return
...
def cmd_mine(args):
    palace_path = os.path.expanduser(args.palace) if args.palace else AHNISConfig().palace_path
    include_ignored = []
    for raw in args.include_ignored or []:
        include_ignored.extend(part.strip() for part in raw.split(",") if part.strip())

    # --redetect-origin re-runs corpus_origin on the current corpus state
    # and overwrites <palace>/.AHNIS/origin.json before mining proceeds.
    # Heuristic-only by design — full LLM detection lives on `AHNIS init`.
...
def cmd_repair(args):
    ...
    palace_path = os.path.abspath(
        os.path.expanduser(args.palace) if args.palace else AHNISConfig().palace_path
    )
    ...
    try:
        col = backend.get_collection(palace_path, "AHNIS_drawers")
        total = col.count()
    ...
    backend.delete_collection(palace_path, "AHNIS_drawers")
    new_col = backend.create_collection(palace_path, "AHNIS_drawers")
...
def cmd_mcp(args):
    """Show how to wire AHNIS into MCP-capable hosts."""
    base_server_cmd = "AHNIS-mcp"
    ...
    print("AHNIS MCP quick setup:")
    print(f"  claude mcp add AHNIS -- {server_cmd}")
    print("\nRun the server directly:")
    print(f"  {server_cmd}")

    if not args.palace:
        print("\nOptional custom palace:")
        print(f"  claude mcp add AHNIS -- {base_server_cmd} --palace /path/to/palace")
...
def cmd_compress(args):
    ...
    try:
        ...
        print(f"\n  No palace found at {palace_path}")
        print("  Run: AHNIS init <dir> then AHNIS mine <dir>")
        sys.exit(1)
    ...
    if not args.dry_run:
        try:
            comp_col = backend.get_or_create_collection(palace_path, "AHNIS_compressed")
            ...
            print(
                f"  Stored {len(compressed_entries)} compressed drawers in 'AHNIS_compressed' collection."
            )
        except Exception as e:
...
def main():
    version_label = f"AHNIS {__version__}"
    parser = argparse.ArgumentParser(
        description="AHNIS — Give your AI a memory. No API key required.",
    ...
    parser.add_argument(
        "--palace",
        default=None,
        help="Where the palace lives (default: from ~/.AHNIS/config.json or ~/.AHNIS/palace)",
    )
    ...
    p_init.add_argument(
        ...
        help=(
            ...
            "(AHNIS_ENTITY_LANGUAGES env var or config.json), or 'en'. "
    ...
    p_mine.add_argument(
        "--agent",
        default="AHNIS",
        help="Your name — recorded on every drawer (default: AHNIS)",
    )
    ...
    p_mine.add_argument(
        ...
        help=(
            ...
            "<palace>/.AHNIS/origin.json. Useful when the corpus has grown "
            "since `AHNIS init` and the stored origin may be stale. "
            "Heuristic-only (no LLM call) — re-run `AHNIS init --llm` for "
            "Tier 2 refinement."
        ),
    )
    p_mine.add_argument(
    ...
    sub.add_parser(
        "mcp",
        help="Show MCP setup command for connecting AHNIS to your AI client",
    )
