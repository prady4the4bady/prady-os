...
Reads AHNIS.yaml from the project directory to know the wing + rooms.
...
def load_config(project_dir: str) -> dict:
    """Load AHNIS.yaml from project directory (falls back to mempal.yaml)."""
    import yaml

    resolved_project_dir = Path(project_dir).expanduser().resolve()
    config_path = resolved_project_dir / "AHNIS.yaml"
...
_ENTITY_REGISTRY_PATH = os.path.join(os.path.expanduser("~"), ".AHNIS", "known_entities.json")
...
def _refresh_known_entities_cache() -> None:
    """Reload ``~/.AHNIS/known_entities.json`` into the module cache if
...
def add_to_known_entities(entities_by_category: dict, wing: str = None) -> str:
    """Union ``entities_by_category`` into ``~/.AHNIS/known_entities.json``.

    Accepts ``{category: [names]}`` shape as produced by ``AHNIS init``
...
def detect_hall(content: str) -> str:
    ...
    if _HALL_KEYWORDS_CACHE is None:
        ...
        _HALL_KEYWORDS_CACHE = AHNISConfig().hall_keywords
...
def mine(
    ...
    try:
        ...
        print(
            f"AHNIS: another `mine` is already running against "
...
def _mine_impl(
    ...
    try:
        ...
        print('\n  Next: AHNIS search "what you\'re looking for"')
        print(f"{'=' * 55}\n")
    except KeyboardInterrupt:
        ...
        print(
            f"\n  Re-run `AHNIS mine {shlex.quote(project_dir)}` to resume — "
...
def _cleanup_mine_pid_file() -> None:
    ...
    The PID file (``~/.AHNIS/hook_state/mine.pid``, written by the
    hook in :func:`AHNIS.hooks_cli._spawn_mine`) tracks the PID of
...
def status(palace_path: str):
    ...
    print(f"\n{'=' * 55}")
    print(f"  AHNIS Status — {total} drawers")
    print(f"{'=' * 55}\n")
