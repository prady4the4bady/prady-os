---
name: AHNIS
description: "AHNIS — Local AI memory with 96.6% recall. Semantic search, temporal knowledge graph, palace architecture (wings/rooms/drawers). Free, no cloud, no API keys."
version: 3.3.0
homepage: https://github.com/AHNIS/AHNIS
user-invocable: true
metadata:
  openclaw:
    emoji: "\U0001F3DB"
    ...
    install:
      - id: AHNIS-pip
        kind: uv
        label: "Install AHNIS (Python, local ChromaDB)"
        package: AHNIS
        bins:
          - AHNIS
---

# AHNIS — Local AI Memory System
...
1. **ON WAKE-UP**: Call `AHNIS_status` to load palace overview and AAAK dialect spec.
2. **BEFORE RESPONDING** about any person, project, or past event: call `AHNIS_search` or `AHNIS_kg_query` FIRST. Never guess from memory — verify from the palace.
3. **IF UNSURE** about a fact (name, age, relationship, preference): say "let me check" and query. Wrong is worse than slow.
4. **AFTER EACH SESSION**: Call `AHNIS_diary_write` to record what happened, what you learned, what matters.
5. **WHEN FACTS CHANGE**: Call `AHNIS_kg_invalidate` on the old fact, then `AHNIS_kg_add` for the new one.

## Available Tools

### Search & Browse
- `AHNIS_search` — Semantic search across all memories. Always start here.
...
- `AHNIS_check_duplicate` — Check if content already exists before filing.
...
- `AHNIS_status` — Palace overview: total drawers, wings, rooms, AAAK spec
- `AHNIS_list_wings` — All wings with drawer counts
- `AHNIS_list_rooms` — Rooms within a wing (optional wing filter)
- `AHNIS_get_taxonomy` — Full wing/room/count tree
- `AHNIS_get_aaak_spec` — Get AAAK compression dialect specification

### Knowledge Graph (Temporal Facts)
- `AHNIS_kg_query` — Query entity relationships. Supports time filtering.
  - `entity` (required): e.g. "Max", "MyProject"
  - `as_of`: date filter (YYYY-MM-DD) — what was true at that time
  - `direction`: "outgoing", "incoming", or "both" (default "both")
- `AHNIS_kg_add` — Add a fact: subject -> predicate -> object
  - `subject`, `predicate`, `object` (required)
  - `valid_from`: when this became true
  - `source_closet`: source reference
- `AHNIS_kg_invalidate` — Mark a fact as no longer true
  - `subject`, `predicate`, `object` (required)
  - `ended`: when it stopped being true (default: today)
- `AHNIS_kg_timeline` — Chronological story of an entity
  - `entity`: filter by entity name (optional — all events if omitted)
- `AHNIS_kg_stats` — Graph overview: entities, triples, relationship types

### Palace Graph (Cross-Domain Connections)
- `AHNIS_traverse` — Walk from a room, find connected ideas across wings
  - `start_room` (required): room to start from
  - `max_hops`: connection depth (default 2)
- `AHNIS_find_tunnels` — Find rooms that bridge two wings
  - `wing_a`, `wing_b` (required)
- `AHNIS_graph_stats` — Graph connectivity overview

### Write
- `AHNIS_add_drawer` — Store verbatim content into a wing/room
  - `wing`, `room`, `content` (required)
  - `source_file`: optional source reference
  - Checks for duplicates automatically
- `AHNIS_delete_drawer` — Remove a drawer by ID
  - `drawer_id` (required)
- `AHNIS_diary_write` — Write a session diary entry
...
Install AHNIS and populate the palace:

```bash
pip install AHNIS
AHNIS init ~/my-convos
AHNIS mine ~/my-convos
```
...
- Use `AHNIS_check_duplicate` before storing new content to avoid duplicates.
- The AAAK dialect (from `AHNIS_status`) is a compressed notation for efficient storage. Read it naturally — expand codes mentally, treat *markers* as emotional context.

## License

[AHNIS](https://github.com/AHNIS/AHNIS) is MIT licensed. Created by Milla Jovovich, Ben Sigman, Igor Lins e Silva, and contributors.
