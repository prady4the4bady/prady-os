...
- **Status:** Draft
- **Tracking issue:** [#989](https://github.com/AHNIS/AHNIS/issues/989)
- **Related:** [#274](https://github.com/AHNIS/AHNIS/issues/274), [#23](https://github.com/AHNIS/AHNIS/pull/23), [#169](https://github.com/AHNIS/AHNIS/pull/169), [#232](https://github.com/AHNIS/AHNIS/pull/232), [#567](https://github.com/AHNIS/AHNIS/pull/567), [#98](https://github.com/AHNIS/AHNIS/pull/98), [#591](https://github.com/AHNIS/AHNIS/pull/591), [#592](https://github.com/AHNIS/AHNIS/pull/592), [#702](https://github.com/AHNIS/AHNIS/pull/702), [#981](https://github.com/AHNIS/AHNIS/issues/981), [#244](https://github.com/AHNIS/AHNIS/pull/244), [#419](https://github.com/AHNIS/AHNIS/pull/419), [#300](https://github.com/AHNIS/AHNIS/pull/300), [#952](https://github.com/AHNIS/AHNIS/pull/952), [#389](https://github.com/AHNIS/AHNIS/pull/389), [#434](https://github.com/AHNIS/AHNIS/pull/434)
- **Sibling spec:** [RFC 001 — Storage Backend Plugin Specification](001-storage-backend-plugin-spec.md)
- **Spec version:** `1.0`

## Summary

A formal contract for AHNIS source adapters so third parties can ship `pip install AHNIS-source-<name>` packages (Cursor, OpenCode, git, Slack, Notion, email, calendar, Whisper transcripts, …) that drop into `AHNIS mine` without patching core. The spec defines the adapter interface, record shape, metadata schema contract, privacy class, entry-point registration, incremental-ingest semantics, closet integration, a declared-transformation model that replaces the informal "verbatim" promise with a verifiable one, conformance tests, and the refactor of the existing file and conversation miners into first-party adapters on the same contract.

RFC 001 formalized the write side (where drawers are stored). This RFC formalizes the read side (where content comes from). Both are required for AHNIS to function as a durable daemon managing heterogeneous palaces across many source types.
...
|---|---|---|
| [#274](https://github.com/AHNIS/AHNIS/issues/274) | Cursor | `workspaceStorage/*.vscdb` SQLite extraction |
| [#23](https://github.com/AHNIS/AHNIS/pull/23) | OpenCode | SQLite session database |
| [#169](https://github.com/AHNIS/AHNIS/pull/169) | Pi agent | JSONL session normalizer |
| [#232](https://github.com/AHNIS/AHNIS/pull/232) | Cursor (JSONL variant) | JSONL normalizer |
| [#567](https://github.com/AHNIS/AHNIS/pull/567), [#98](https://github.com/AHNIS/AHNIS/pull/98) | Git | `git log` + `gh pr view` with structured diff summary |
| [#591](https://github.com/AHNIS/AHNIS/pull/591), [#592](https://github.com/AHNIS/AHNIS/pull/592) | Delphi Oracle | Real-time intelligence signals |
| [#702](https://github.com/AHNIS/AHNIS/pull/702) | Cursor + factory.ai | Combined session miners |

Plus three ingesters already grafted into core:

- `AHNIS/miner.py` — filesystem project miner, fixed char-window chunking, keyword hall routing
- `AHNIS/convo_miner.py` — chat transcript miner with exchange-pair chunking
- `AHNIS/normalize.py` — format detection for four chat-export shapes (Claude Code JSONL, Codex JSONL, Claude.ai / ChatGPT / Slack JSON)

Plus one open proposal for a different ingest semantic:

- [#981](https://github.com/AHNIS/AHNIS/issues/981) — path-level descriptions: mine metadata-as-content instead of raw bytes for matched paths. This is a legitimate third ingest mode (alongside chunked-content and whole-record) that the current architecture has no home for.
...
1. A source adapter ships as a standalone Python package; `pip install AHNIS-source-<name>` is sufficient to use it.
2. `AHNIS mine` and the MCP mine tool are source-agnostic — all extraction goes through registered adapters. No `if source_type == 'foo'` branches in core.
...
def source_summary(self, *, source: SourceRef) -> SourceSummary:
    """Describe a source without extracting (e.g., 'git repo AHNIS,
...
An adapter instance is long-lived and serves many mine operations. Adapters MUST be thread-safe for concurrent `ingest` calls across different `SourceRef` values. AHNIS core serializes calls within a single `SourceRef` unless an adapter advertises `supports_parallel_ingest` (not in v1 — reserved for v1.1).

### 2.5 Routing

Routing is the adapter's responsibility. The filesystem adapter reads `AHNIS.yaml` (hall keywords, rooms list) via `AHNISConfig()` and returns `RouteHint(wing=..., room=..., hall=...)` on each drawer. This relocates `detect_room()` and `detect_hall()` (currently in `miner.py` and `convo_miner.py`) into their respective adapters.

Order of precedence for routing:
1. Explicit `--wing` / `--room` CLI flags → passed through `SourceRef.options` → adapter honors verbatim.
2. Palace config match (`AHNIS.yaml` hall keywords, room keywords) → adapter computes.
3. Adapter-internal fallback (e.g., filesystem adapter falls back to `"general"` room).

Adapters advertising `adapter_owns_routing` return the final answer; core uses it verbatim. Adapters not advertising it return None and core applies a generic fallback router (writing to wing `default`, room `general`, hall `general`). Absent any adapter, this is how `AHNIS mine` behaves today.
...
```toml
# pyproject.toml of AHNIS-source-cursor
[project.entry-points."AHNIS.sources"]
cursor = "AHNIS_source_cursor:CursorAdapter"
```

AHNIS discovers adapters at process start via `importlib.metadata.entry_points(group="AHNIS.sources")`.

### 3.2 In-tree registry (secondary)

```python
from AHNIS.sources.registry import register
...
```bash
AHNIS mine --source cursor ~/                      # explicit adapter
AHNIS mine --source git /path/to/repo              # explicit adapter
AHNIS mine --source filesystem /path/to/project    # explicit adapter
AHNIS mine /path/to/project                        # implicit: filesystem (default)
```

The default when no `--source` is given is `filesystem`, preserving current `AHNIS mine <path>` behavior.
...
```json
{
  "sources": {
    ...
    "my-git": {
      "type": "git",
      "repos": ["/projects/AHNIS", "/projects/site"]
    }
  },
  "palaces": {
    "work": {
...
Single-user local mode: config is optional. `AHNIS mine <path>` with no config uses the `filesystem` adapter and defaults.

### 4.2 Environment variables

- `AHNIS_SOURCE_<NAME>_*` — per-adapter secrets and connection info. Examples: `AHNIS_SOURCE_SLACK_TOKEN`, `AHNIS_SOURCE_NOTION_API_KEY`, `AHNIS_SOURCE_GIT_GITHUB_TOKEN`.
...
```python
metadata["entity_hints_json"] = '[{"type":"person","name":"Milla Jovovich","confidence":0.95,"offset":120},{"type":"project","name":"AHNIS","confidence":1.0,"offset":0}]'
```

The value is a JSON-encoded string (type `json_string` in the adapter schema). Core parses on read and feeds into `AHNIS/entity_detector.py` as a prior: hints with `confidence >= 0.9` bypass the heuristic detector; lower-confidence hints feed into it as candidates.
...
Adapters with `supports_kg_triples` write directly to the SQLite knowledge graph via `AHNIS/knowledge_graph.py` — **not** to drawer metadata. Chroma cannot store structured triples; the KG already exists for this purpose.

The adapter calls the existing `KnowledgeGraph.add_triple()` (signature verified against `AHNIS/knowledge_graph.py:130`):
...
- **Default floor: none** — v1 accepts all levels unless the palace explicitly configures a floor. This keeps the single-user local default low-friction (users who run `AHNIS mine` on a git repo expect `secrets_possible` drawers to land). Enterprise deployments MUST set a floor; docs for regulated-domain setup will recommend starting strict and relaxing as needed.
...
Privacy class is how a regulated-domain deployment (medical, legal, financial) can use AHNIS safely. Without it, flexible ingest becomes a liability; with it, ingest is scoped by policy.
...
AHNIS ships `AHNIS.sources.testing.AbstractSourceAdapterContractSuite` — a pytest mixin. Every adapter package ships a concrete subclass:

```python
from AHNIS.sources.testing import AbstractSourceAdapterContractSuite
...
For transformations not in the reserved list (§1.4) — adapter-custom names — the adapter MUST provide a reference implementation callable under `AHNIS.sources.transforms.<adapter_name>_<transform_name>`. The conformance suite imports and applies it. Undiscoverable custom transforms fail the test.
...
- AHNIS refuses to load an adapter declaring a different major spec version.
...
- `AHNIS/cli.py` subcommand `mine` routes through the `AHNIS.sources` registry. `--mode {projects,convos}` becomes a deprecated alias for `--source {filesystem,conversations}`.
- `AHNIS/mcp_server.py` `AHNIS_mine` tool accepts a `source` parameter.
- `AHNIS/palace.py` exposes `PalaceContext` — a per-mine-invocation facade that bundles the drawer collection, closet collection, knowledge graph, palace config, and progress hooks. Adapters receive this; they do not import `palace.py` directly.
...
|---|---|
| [#274](https://github.com/AHNIS/AHNIS/issues/274) Cursor SQLite | Becomes `AHNIS-source-cursor` third-party package. Author has a working prototype on Windows; needs `describe_schema()`, `declared_transformations`, and the conformance suite. Prior #287 (closed unmerged) is predecessor work. |
| [#23](https://github.com/AHNIS/AHNIS/pull/23) OpenCode SQLite | Becomes `AHNIS-source-opencode`. Same shape as Cursor. |
| [#169](https://github.com/AHNIS/AHNIS/pull/169) Pi agent | Becomes `AHNIS-source-pi` or a format plugin under the conversations adapter (depending on format similarity). |
| [#232](https://github.com/AHNIS/AHNIS/pull/232) Cursor JSONL | Deprecated in favor of #274's SQLite path; or a second mode of `AHNIS-source-cursor`. |
| [#567](https://github.com/AHNIS/AHNIS/pull/567), [#98](https://github.com/AHNIS/AHNIS/pull/98) git-mine | Closest existing work to what the spec envisions. Becomes first-party `AHNIS/sources/git.py`. Exercises `whole_record` mode, `supports_structured_metadata`, `supports_closet_hints` (decision-signal quotes), `supports_kg_triples` (commit authorship, PR review relationships). |
| [#591](https://github.com/AHNIS/AHNIS/pull/591), [#592](https://github.com/AHNIS/AHNIS/pull/592) Delphi Oracle | Deferred. The live-stream pattern is out of scope for v1 (§Non-goals). A v1.1 addition will specify webhook/stream adapters. |
| [#702](https://github.com/AHNIS/AHNIS/pull/702) Cursor + factory.ai | Splits into two adapter packages. |
| [#981](https://github.com/AHNIS/AHNIS/issues/981) path-level descriptions | Absorbed by §1.5 `metadata_only` mode + §5.1 `ingest_mode`. A new first-party `descriptions` adapter or a second mode on `filesystem`. |
| [#244](https://github.com/AHNIS/AHNIS/pull/244) Cursor memory-first MCP workflow docs | Points at `AHNIS-source-cursor` once the adapter lands. |
| [#419](https://github.com/AHNIS/AHNIS/pull/419), [#300](https://github.com/AHNIS/AHNIS/pull/300), [#952](https://github.com/AHNIS/AHNIS/pull/952) language-extension additions to `READABLE_EXTENSIONS` | Becomes per-language config on the filesystem adapter. Contributors can publish domain-specific adapters without touching core. |
| [#389](https://github.com/AHNIS/AHNIS/pull/389) sensitive content scanner | Expected enforcement mechanism for the `secrets_possible` privacy class (§6.2). Not a blocker for this spec, but a natural consumer. |
| [#434](https://github.com/AHNIS/AHNIS/pull/434) auto-populate KG from drawers | Complementary: post-hoc derivation of KG triples from drawer content. Adapters with `supports_kg_triples` provide the up-front path; #434 handles everything else. |
...
1. Land the cleanup PR (§9): introduce `AHNIS/sources/`, refactor `miner.py` → filesystem adapter, `convo_miner.py` → conversations adapter, route CLI and MCP through the sources registry. Behavior preserved end-to-end. Closets get built for conversation drawers as a side effect.
2. Land this spec as-is. Add `AbstractSourceAdapterContractSuite`, entry-point discovery, `AdapterSchema` validation, privacy-class enforcement (floor-gated writes), declared-transformation reference implementations in `AHNIS/sources/transforms.py`.
3. Land `AHNIS/sources/git.py` as the first-party adapter absorbing #567. Exercises `whole_record`, `supports_structured_metadata`, `supports_closet_hints`, `supports_kg_triples` together.
4. Encourage the Cursor (#274), OpenCode (#23), and Pi (#169) authors to publish as third-party packages under `AHNIS-source-*`. Offer review help against the spec.
5. Publish adapter-authoring docs at [AHNISofficial.com/guide/authoring-sources](https://AHNISofficial.com/guide/authoring-sources.html).
