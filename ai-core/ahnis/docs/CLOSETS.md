...
Closets are created during `AHNIS mine`. For each file mined:
...
Closets are stored in the `AHNIS_closets` ChromaDB collection alongside `AHNIS_drawers`. If you delete and rebuild the palace, closets are recreated during the next `AHNIS mine`.

## How search uses closets

```
Query → search AHNIS_closets (fast, small documents)
         ↓
    top closet hits → parse `→drawer_id_a,drawer_id_b` pointers
         ↓
    fetch exactly those drawers from AHNIS_drawers (verbatim content)
         ↓
...
Closet functions live in `AHNIS/palace.py`:
...
The closet-first search path lives in `AHNIS/searcher.py`:
