...
This guide explains how to set up AHNIS as a permanent memory for the [Gemini CLI](https://github.com/google/gemini-cli).
...
On many Linux systems, installing Python packages globally is restricted. We recommend using a local virtual environment within the AHNIS directory.
...
# Install dependencies and AHNIS in editable mode
...
You can manually define who you are and what projects you work on by creating/editing these files in `~/.AHNIS/`:

- **`~/.AHNIS/identity.txt`**: A plain text file describing your role and focus.
- **`~/.AHNIS/wing_config.json`**: A JSON file mapping projects and name variants to "Wings".

## 3. Connect to Gemini CLI (MCP)

Register AHNIS as an MCP server so Gemini CLI can use its tools.

```bash
gemini mcp add AHNIS /absolute/path/to/AHNIS/.venv/bin/python3 -m AHNIS.mcp_server --scope user
```
