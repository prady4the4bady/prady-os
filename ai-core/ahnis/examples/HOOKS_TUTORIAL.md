# How to Use AHNIS Hooks (Auto-Save)

AHNIS hooks act as an "Auto-Save" feature. They help your AI keep a permanent memory without you needing to run manual commands.
...
**Note:** Replace `/absolute/path/to/hooks/` with the actual path where you cloned the AHNIS repository (e.g., `~/projects/AHNIS/hooks/`).
...
- **`AHNIS_PYTHON`** — Python interpreter with AHNIS + chromadb. Auto-detects: env var → repo venv → system python3
- **`MEMPAL_DIR`** — Optional directory for auto-ingest via `AHNIS mine`
