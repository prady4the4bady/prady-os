...
-----
    ...
    AHNIS_EMBEDDING_DEVICE=cuda python benchmarks/mine_bench.py

Flags
-----

    --device cpu|cuda|coreml|dml|auto   Override AHNIS_EMBEDDING_DEVICE
...
def mine_once(project_dir: str, palace_path: str, batched: bool) -> tuple[int, float]:
    ...
    from AHNIS import miner
    from AHNIS.miner import load_config, scan_project
    from AHNIS.palace import get_closets_collection, get_collection
...
def _reset_backend_caches() -> None:
    ...
    from AHNIS.palace import _DEFAULT_BACKEND
...
def main() -> None:
    ...
    parser.add_argument(
        "--device",
        default=None,
        help="Override AHNIS_EMBEDDING_DEVICE (cpu|cuda|coreml|dml|auto)",
    )
    parser.add_argument(
        "--scenarios",
    ...
    if args.device:
        os.environ["AHNIS_EMBEDDING_DEVICE"] = args.device

    from AHNIS.embedding import describe_device, get_embedding_function
