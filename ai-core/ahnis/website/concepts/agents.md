...
AHNIS currently supports **agent diaries** through MCP tools. The practical model is simple: give an agent a stable name, and write/read diary entries under that agent's wing.

::: warning Current Scope
This page documents the diary workflow that exists today. AHNIS does **not** currently ship an agent registry, `~/.AHNIS/agents/*.json`, or a `AHNIS_list_agents` tool.
:::
...
| Tool | Description |
|------|-------------|
| `AHNIS_diary_write` | Write an AAAK diary entry |
