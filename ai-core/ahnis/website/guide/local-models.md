...
AHNIS works with any local LLM — Llama, Mistral, or any offline model. Since local models generally don't speak MCP yet, there are two approaches.
...
```bash
AHNIS wake-up --wing driftwood > context.txt
```
...
```bash
AHNIS search "auth decisions" > results.txt
