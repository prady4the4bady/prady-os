...
AHNIS uses ChromaDB's semantic vector search to find relevant memories. When you search, you get **verbatim text** — the exact words, never summaries.
...
AHNIS search "database decision" --wing myapp

# Filter by room (topic)
AHNIS search "auth decisions" --room auth-migration

# Filter by both
AHNIS search "pricing" --wing driftwood --room costs

# More results
AHNIS search "deploy process" --results 10
```
...
The AI calls `AHNIS_search` behind the scenes. You never type a search command.
