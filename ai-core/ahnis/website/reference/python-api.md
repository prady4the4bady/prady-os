...
High-level overview of the key Python interfaces you'd use to integrate AHNIS into your application.
...
```python
from AHNIS.searcher import search_memories
...
```python
from AHNIS.layers import MemoryStack

stack = MemoryStack()  # uses default paths from AHNISConfig
...
```python
from AHNIS.knowledge_graph import KnowledgeGraph

kg = KnowledgeGraph()  # uses default path: ~/.AHNIS/knowledge_graph.sqlite3
...
```python
from AHNIS.palace_graph import build_graph, traverse, find_tunnels, graph_stats
...
```python
from AHNIS.dialect import Dialect
...
config = AHNISConfig()
print(config.palace_path)       # ~/.AHNIS/palace
print(config.collection_name)   # AHNIS_drawers
```
