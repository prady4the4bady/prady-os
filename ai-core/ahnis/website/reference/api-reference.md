...
Identity layer (~50 tokens). Reads from `~/.AHNIS/identity.txt`.
...
## `AHNIS.knowledge_graph`

### `class KnowledgeGraph(db_path=None)`

Default path: `~/.AHNIS/knowledge_graph.sqlite3`
...
Reads from `~/.AHNIS/config.json` and environment variables.

| Property | Type | Default | Description |
|----------|------|---------|-------------|
| `palace_path` | `str` | `~/.AHNIS/palace` | ChromaDB storage path |
| `collection_name` | `str` | `AHNIS_drawers` | ChromaDB collection name |
