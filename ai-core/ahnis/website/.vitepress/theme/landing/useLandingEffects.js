...
onMounted(() => {
  ...
  document.body.classList.add('AHNIS-active')

  /* ---------- Waitlist submission ---------- */
  ;(function initWaitlist(){
    const ENDPOINT = 'https://br.staging.AHNISofficial.com/waitlist'
    const forms = document.querySelectorAll('.AHNIS-landing .waitlist')
    const emailRe = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
  ...
  ;(function(){
    ...
    const items = document.querySelectorAll('.AHNIS-landing .stratum, .AHNIS-landing .mech, .AHNIS-landing .slab')
    items.forEach(el => {
      el.style.opacity = '0'
...
onBeforeUnmount(() => {
  if (typeof document === 'undefined') return
  document.body.classList.remove('AHNIS-active')
